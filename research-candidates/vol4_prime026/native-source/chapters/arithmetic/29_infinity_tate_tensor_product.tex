...
\Cref{v4-arith:thm:F2-reduction} reduced the conjecture of global
\(E_{2}\)-enhancement to a single \(\infty\)-operadic statement (F2.c):
the restricted tensor product of \(E_{n}\)-monoidal stable presentable
\(\infty\)-categories with distinguished units inherits an
\(E_{n}\)-monoidal structure. The proof uses Lurie~HA~4.8~+~5.1,
Fresse's treatment of the \(E_{n}\) operad, and Dunn additivity; no
arithmetic input is required, and F2.c is proved as pure
\(\infty\)-categorical assembly. The
mixed-characteristic Iwahori \(E_{2}\) local input F2.b remains open;
F2 itself is therefore conditional.
...
gives the global transport step in \Cref{v4-arith:thm:F2-reduction}
conditional on the local Iwahori \(E_{2}\) input.

\subsection{Source Disjointness}

The proof sources of \Cref{v4-arith:tate:thm:F2c-main} are
Lurie~HA~4.8 (tensor products of presentable \(\infty\)-categories) and
Fresse (integral \(E_{n}\) operad). The independent comparison sources
are Dunn additivity (HA~5.1.2.4; Dunn~1988), Ayala--Francis
factorization homology, and Ginot's account of factorization algebras
of \(\infty\)-categories (\S\ref{v4-arith:tate:verification}). The
derivation and comparison sources are disjoint: Lurie~HA~4.8 does
not invoke factorization homology, and Ayala--Francis / Ginot do not
prove the colimit-preservation lemmas of HA~4.8 internally.

\section{Obstructions and Their Resolutions}
\label{v4-arith:tate:table}

Six potential obstructions to \Cref{v4-arith:tate:thm:F2c-main} were
identified and resolved; each resolution either cleans the hypothesis,
strengthens the statement, or is absorbed into a subsidiary lemma.

\subsection{Centrality of the Spherical Unit}

\begin{obstruction}[unital inclusion]
...
\begin{correction}[unit centrality]
Lurie~HA~5.1.2.26 shows that the unit object \(\mathbb{1}_{v}\)
...
(D2) includes the datum needed for insertion: \(u_{v}\) is a
...
\subsection{\(\mathcal{O}\)-Algebra Structure under Filtered Colimits}

\begin{obstruction}[filtered colimit of \(\mathcal{O}\)-algebras]
...
\begin{correction}[colimit preservation]
...
\subsection{BD Factorization Axiom on Discrete \(V\)}

\begin{obstruction}[factorization axiom]
...
\begin{correction}[discreteness of \(V\)]
...
\subsection{Dunn Additivity and the Restricted Tensor Product}

\begin{obstruction}[Dunn additivity compatibility]
...
\begin{correction}[operad-variable functoriality]
Additivity is a statement about the \(\infty\)-operad
...
\subsection{Cofinality Invariance of the \(\mathcal{O}\)-Structure}

\begin{obstruction}[uniqueness modulo cofinal filtration]
...
\begin{correction}[cofinality invariance]
...
\subsection{Ramified Places without a Spherical Unit}

\begin{obstruction}[ramified archimedean places]
...
\begin{correction}[cofinite spherical locus]
The setup requires a spherical unit only on a cofinite subset
...
archimedean factor \(\mathcal{C}_{\infty}\) admits the \(K_{\infty}\)-finite
spherical vector \(|0\rangle_{\infty}\in\mathcal{C}_{\infty}\) (the
\(O(2)\)-fixed vector of \(GL_{2}(\mathbb{R})\)-representations on the
Iwahori-level Hecke module); consequently \(V^{\mathrm{sph}}=V\) for
the target application. Detailed in
...
The proof of \Cref{v4-arith:tate:thm:F2c-main} draws on three
ingredients from Lurie~HA and Fresse, recorded here with their
precise references. Numbering follows Lurie HA, second edition (2017
draft).
...
\begin{theorem}[Fresse~2017 \emph{Homotopy of Operads} I.4.1; Lurie HA~5.1.0.4]
...
\Cref{v4-arith:tate:thm:unit-central} is the foundation of
Lemma~\ref{v4-arith:tate:lem:unital-incl-is-monoidal}.
...
This is the mechanism of colimit-preservation
(\S\ref{v4-arith:tate:colim-preservation}).

\section{Unital Inclusion Functors}
\label{v4-arith:tate:unital-inclusion}

Inserting a spherical unit at a new place gives an
\(\mathcal{O}\)-monoidal functor; the following lemma makes this precise.
...
HA~3.2.4.5 (tensor products of \(\mathcal{O}\)-algebras). The spherical
data \((\eta_{i},u_{i})\) for \(i>k\) are maps
\(\eta_{i}\colon\mathrm{Sp}\to\mathcal{C}_{i}\) of pointed
\(\mathcal{O}\)-algebras by Setup~\ref{v4-arith:tate:setup:data}(D2).
When \(u_{i}=\mathbb{1}_{\mathcal{C}_{i}}\) is the monoidal unit,
\Cref{v4-arith:tate:thm:unit-central} (HA~5.1.2.26) gives canonical
\(\mathcal{O}\)-centrality. When \(u_{i}\) is not the unit, (D2) supplies
central idempotence as part of the datum. In either case, each
\(u_{i}\) is \(\mathcal{O}\)-central in \(\mathcal{C}_{i}\); the
idempotent-algebra hypothesis of HA~4.8.2.9 is satisfied, so
\(u_{i}\) defines an \(\mathcal{O}\)-algebra map
\(\mathrm{Sp}\to\mathcal{C}_{i}\) in \(\mathrm{Alg}_{\mathcal{O}}(\mathrm{Pr}^{\mathrm{L}}_{\mathrm{st}})\).

By HA~5.1.2.22 the universal property of the tensor product in
\(\mathrm{Alg}_{\mathcal{O}}(\mathrm{Pr}^{\mathrm{L}}_{\mathrm{st}})\)
gives a canonical \(\mathcal{O}\)-monoidal map
\[
\bigotimes_{i=1}^{k}\mathcal{C}_{i}\;\simeq\;\bigotimes_{i=1}^{k}\mathcal{C}_{i}\otimes\mathrm{Sp}^{\otimes(m-k)}\;\xrightarrow{\;\mathrm{id}^{\otimes k}\otimes(u_{k+1},\ldots,u_{m})\;}\;\bigotimes_{i=1}^{m}\mathcal{C}_{i}.
\]
This is the functor~\eqref{v4-arith:tate:eq:unital-incl}. To verify
\(\mathcal{O}\)-monoidality: every \(\mathcal{O}\)-operation
...
\end{tikzcd}
\end{equation}
commutes up to canonical homotopy: each factor at position \(i>k\)
contributes \(u_{i}\otimes\cdots\otimes u_{i}\) (\(r\) times), and the
\(\mathcal{O}\)-operation on this object is controlled by unit centrality
(HA~5.1.2.26) or the central idempotent algebra structure of (D2).
Both constructions around~\eqref{v4-arith:tate:eq:unital-incl-commute} differ
by a canonically contractible space of homotopies.
\end{proof}
...
Fix \(v\in V\). Either \(V\setminus\{v\}\subseteq V^{\mathrm{sph}}\),
in which case \(\{v\}\in\mathrm{Fin}^{\mathrm{sph}}(V)\) and (M2)
follows from Step~3 with \(S=\{v\}\); or \(v\in V\setminus V^{\mathrm{sph}}\)
(so \(v\) is ramified), in which case every \(S\in\mathrm{Fin}^{\mathrm{sph}}(V)\)
already contains \(v\) (since \(S\) must contain \(V\setminus V^{\mathrm{sph}}\)).
In the second case, take any \(S\in\mathrm{Fin}^{\mathrm{sph}}(V)\)
(hence \(v\in S\)); the composition
\[
\mathcal{C}_{v}\;\longrightarrow\;\bigotimes_{v'\in S}\mathcal{C}_{v'}\;\longrightarrow\;\bigotimes'_{v'\in V}(\mathcal{C}_{v'},u_{v'})
\]
in which the first map inserts spherical units at \(S\setminus\{v\}\)
(Lemma~\ref{v4-arith:tate:lem:unital-incl-is-monoidal}) and the
second is \(\iota_{S}\) (Step~3) is \(\mathcal{O}\)-monoidal. The
composite is independent of \(S\): any two choices \(S_{1},S_{2}\)
are both contained in \(S_{1}\cup S_{2}\in\mathrm{Fin}^{\mathrm{sph}}(V)\),
and the composites agree modulo canonical equivalence of
\(\mathcal{O}\)-monoidal functors by \Cref{v4-arith:tate:cor:iota-restriction}.
This is (M2).

\medskip\noindent\textbf{Step 5 (property (M3): uniqueness up to contractible space of choices).}
By Step~2, \(\varinjlim_{S}\Phi(S)\) is the colimit of \(\Phi\) in
\(\mathrm{Alg}_{\mathcal{O}}(\mathrm{Pr}^{\mathrm{L}}_{\mathrm{st}})\),
which is initial among co-cones under \(\Phi\)
(\Cref{v4-arith:tate:prop:colim-preserves-O-alg}; HTT~4.1.1.8 for
cofinality invariance). Any \(\mathcal{O}\)-monoidal structure on
\(\bigotimes'_{v}(\mathcal{C}_{v},u_{v})\) compatible with
\((\iota_{S})\) defines exactly a co-cone under \(\Phi\) in
\(\mathrm{Alg}_{\mathcal{O}}(\mathrm{Pr}^{\mathrm{L}}_{\mathrm{st}})\)
with apex \(\bigotimes'_{v}(\mathcal{C}_{v},u_{v})\). Since
\(\varinjlim_{S}\Phi(S)\) is initial, the space of \(\mathcal{O}\)-algebra
maps from \(\varinjlim_{S}\Phi(S)\) to any fixed co-cone apex is
contractible; taking the apex to be \(\varinjlim_{S}\Phi(S)\) itself
(with the colimit \(\mathcal{O}\)-structure), the space of
\(\mathcal{O}\)-monoidal structures compatible with \((\iota_{S})\)
on the underlying object \(\bigotimes'_{v}(\mathcal{C}_{v},u_{v})\)
is contractible. This is (M3).

Steps~1--5 together prove \Cref{v4-arith:tate:thm:F2c-main}.
\end{proof}
...
\Cref{v4-arith:tate:def:restricted-tensor} is a degenerate instance
of a Beilinson--Drinfeld (BD, Chiral Algebras~\S3.4) factorization
algebra, reduced to discrete \(V\): the BD factorization axiom
degenerates to a tautological tensor-product decomposition over
...
product formulation of \Cref{v4-arith:tate:def:restricted-tensor} is the correct generalization
to discrete bases; the BD content that genuinely concentrates at
the archimedean place is handled separately
(\Cref{v4-arith:thm:CG-conclusion}).
\end{remark}

\subsection{Compatibility with Dunn Additivity}
\label{v4-arith:tate:subsec:dunn-compat}

The restricted tensor product commutes with Dunn additivity
(\Cref{v4-arith:tate:thm:dunn-additivity}) in the following precise sense.
...
\subsection{The Corollary: F2.c Closed, F2 Still Conditional}

\begin{corollary}[restricted-tensor \(E_{2}\) transfer]
...
for \(v=\infty\); the Iwahori-double-coset class at \(v=p\)). If each
local category already has the required mixed-characteristic
Iwahori \(E_{2}\)-monoidal structure, then the
...
contractible choice. This closes only the restricted-tensor
transport part F2.c. It does not prove the local
mixed-characteristic Iwahori \(E_{2}\) input F2.b.
\end{corollary}
...
\Cref{v4-arith:thm:F2-reduction}. The proof assumes the local
\(E_{2}\)-structure as input; it therefore does not close F2 until
the mixed-characteristic Iwahori enhancement F2.b is proved.
\end{proof}
...
contractible. Taking \(\mathcal{D}=\bigotimes'_{v}(\mathcal{C}_{v},u_{v})\),
which by \Cref{v4-arith:tate:cor:rtp-as-colimit-in-alg} is
\(\varinjlim\Phi\) as an \(\mathcal{O}\)-monoidal \(\infty\)-category,
gives contractibility of the space of \(\mathcal{O}\)-monoidal structures
on \(\varinjlim\Phi\) compatible with the inclusions: the space of
\(\mathcal{O}\)-algebra endomorphisms of \(\varinjlim\Phi\) respecting
the co-cone is contractible by the universal property of the initial
object \(\varinjlim\Phi\).
\end{proof}
...
\(\{\mathcal{C}_{v}\}\) and covariant under base change.

\begin{proposition}[functoriality in the family]
\label{v4-arith:tate:prop:functoriality-family}
\ClaimStatusProvedHere. Given two families of data
...
cases the \(E_{\infty}\)-structure is induced from that of \(\mathrm{Mod}\).
\end{example}
...
factorization supplies an additional \(E_{2}\)-enhancement absorbed into
\Cref{v4-arith:tate:thm:F2c-main}.
\end{example}
...
context; the arithmetic version makes contact with the
...
\section{Factorization-Homology Comparison}
\label{v4-arith:tate:verification}

Factorization homology (Ayala--Francis) provides a comparison path
for \Cref{v4-arith:tate:thm:F2c-main} independent of the
colimit-preservation argument: the Ayala--Francis account does not
invoke Lurie~HA~4.8 directly, proceeding instead via
cobordism-hypothesis-style arguments for algebras on disjoint unions
of framed manifolds.
...
\begin{theorem}[factorization-homology comparison for F2.c]
...
\begin{proof}
Ayala--Francis prove (Thm~3.24; Ayala--Francis
...
\begin{remark}[Ginot's Ext comparison via factorization algebras]
\label{v4-arith:tate:rem:ginot-ext-verification}
A further independent comparison comes from Ginot,
...
with both of the previous two constructions. Three disjoint constructions,
one answer; the independence of sources requirement is satisfied.
\end{remark}
...
\section{Completeness of F2.c and Residual Local Input}
\label{v4-arith:tate:completeness-F2}

\begin{theorem}[F2.c assembly for local Iwahori data]
\label{v4-arith:tate:thm:F2-closure}
\ClaimStatusProvedHere. F2.c supplies the following assembly step.
Given local
mixed-characteristic Iwahori Hecke categories
\(\mathrm{Hk}^{\mathrm{Iw}}_{GL_{2},v}\) equipped with
the required \(E_{2}\)-monoidal structures and spherical units
\(|0\rangle_{v}\), their Tate restricted tensor product over
\(\overline{C}\) inherits a canonical \(E_{2}\)-monoidal structure,
unique up to contractible choice.
\end{theorem}

\begin{proof}
This is exactly \Cref{v4-arith:tate:cor:F2-closed}: apply
\Cref{v4-arith:tate:thm:F2c-main} with \(\mathcal{O}=E_{2}\) and
the family of local Iwahori Hecke categories. The theorem assumes
the local \(E_{2}\) structures; it proves their restricted-tensor
assembly.
\end{proof}

\begin{remark}[epistemic form of F2]
\label{v4-arith:tate:rem:F2-form-final}
\Cref{v4-arith:tate:thm:F2-closure} proves the restricted-tensor
assembly part F2.c. F2 itself remains conditional: the local
mixed-characteristic Iwahori \(E_{2}\) enhancement F2.b is not yet
available, and later uses of F2 remain conditional on it.
\end{remark}

\begin{corollary}[F3 reduction after F2.c]
\label{v4-arith:tate:cor:F3-reduces-with-F2-closed}
\ClaimStatusNeedsVerification. After F2.c, the F2 hypothesis in
F3 is reduced to the local mixed-characteristic Iwahori \(E_{2}\)
input F2.b. Thus F3 still requires F1, F2.b, and
(F3.BZN\(^{\mathrm{Ar}}\)).
\end{corollary}
...
(F3.BZN\(^{\mathrm{Ar}}\)). By
\Cref{v4-arith:tate:thm:F2-closure}, only the restricted-tensor
part of F2 is discharged. The local Iwahori \(E_{2}\) input remains
in the hypotheses.
\end{proof}

\section{Tate Tensor Boundary}
...
\(u_{v}=|0\rangle_{v}\) supplies the F2.c restricted-tensor assembly
step (\Cref{v4-arith:tate:cor:F2-closed}); the local Iwahori
\(E_{2}\)-input for full F2 is separate.
...
\(E_{2}\)-application.
\item Independent comparison via Ayala--Francis factorization
...
\(\mathcal{O}\)-monoidal structure through a second, derivation-disjoint
construction; the derivation and comparison sources do not share a common
sub-argument.
\item The restricted-tensor part F2.c is proved as a formal theorem
under the pointed-unit \(E_{2}\) hypotheses stated above. F2 itself
remains conditional on the mixed-characteristic Iwahori \(E_{2}\)
local input. Any simplification of F3 that assumes F2 remains
conditional on that local input.
\end{enumerate}

F2.c is a pure \(\infty\)-categorical statement; the only arithmetic
content is the choice of the place-set \(V\) and the family
\(\{\mathrm{Hk}^{\mathrm{Iw}}_{GL_{2},v}\}\). The proof draws
exclusively on Lurie HA~4.8~+~5.1 and central-unit properties; F2
itself remains conditional, conditional on the local Iwahori
\(E_{2}\)-monoidal input.
...
formalism of Lurie HA, and it remains open
