\chapter{Archimedean Tate Kernel via Gaussian Test Functions: Scalar Reduction and Density Limits}
\label{v4-arith:w7-d:chapter}

\emph{Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction} confined the
negative archimedean term in the archimedean Tate positivity
inequality to \([-t_{\ast},t_{\ast}]\), with
\(t_{\ast}\approx 0.8507\), and kept the residual functional \(R[f]\)
in the full Weil inequality.
Chapter~\ref{v4-arith:w5-i:chapter} stated the closed-form
evaluation of the Weil distribution at Gaussian test functions. The
two inputs combine on Gaussians
\(f_{\lambda}(u)=e^{-\pi\lambda u^{2}}\) are self-dual under Fourier
(with the Poisson-adapted normalisation of
Chapter~\ref{v4-arith:w5-i:chapter}), and their convolutions remain
Gaussian: \(f_{\lambda}*\widetilde{f_{\lambda}}=\sqrt{1/(2\lambda)}\,f_{\lambda/2}\).
\(W_{\infty}\)-positivity restricted to the Gaussian sub-class therefore collapses to a
single scalar inequality \(I(\lambda)\geq 0\)
parameterised by \(\lambda>0\), where
\(I(\lambda)=\int_{\mathbb{R}}e^{-2\pi t^{2}/\lambda}g(t)\,dt\).
This scalar quantity is computed
explicitly as a convergent series of error-function integrals
indexed by the Hurwitz terms of the digamma kernel; an
asymptotic analysis determines the sign of \(I(\lambda)\) in
the two limiting regimes \(\lambda\to 0\) and \(\lambda\to\infty\), and
identifies a unique critical \(\lambda_{c}>0\) at which
\(I(\lambda_{c})=0\). \(W_{\infty}\)-positivity on the Gaussian
sub-class holds unconditionally for \(\lambda\geq\lambda_{c}\). No
density passage from the one-parameter centred Gaussian family to the
full Paley--Wiener class is used. Linear density would in any case not
preserve a quadratic positivity cone. The resulting assertions are
\(W_{\infty}\)-positivity on the Gaussian sub-class for
\(\lambda\geq\lambda_{c}\), a scalar-valued characterisation of
the admissible \(\lambda\)-range, and the remaining
\(R[f]\)-coupled inequality for full A-TP.}

\section{Inputs and Reduction Strategy}
\label{v4-arith:w7-d:context}

Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}
isolated the negative archimedean part of A-TP on
\([-t_{\ast},t_{\ast}]\), with \(t_{\ast}\approx 0.8507\). Full A-TP is
the inequality \(B[f]\leq A[f]+2\pi R[f]\), where
\(g(t)=\mathrm{Re}\,\psi(1/4+it/2)+\log\pi\) governs \(A\) and \(B\).
Chapter~\ref{v4-arith:w5-i:chapter}
evaluated the Weil distribution on the Gaussian family
\(f_{\lambda}(u)=e^{-\pi\lambda u^{2}}\) in closed form via Hurwitz
series plus Gaussian self-duality.

The Gaussian test family lies in the Schwartz distributional extension
of the Weil formula, not in the strict Paley--Wiener class with compact
Fourier support. It collapses the full \(L^{2}\)-weighted positivity
problem for \(W_{\infty}\) to a scalar equation in the single parameter
\(\lambda\). This scalar result is not a density theorem. The admissible
\(\lambda\)-range is explicit. Linear approximation does not preserve a
quadratic positive cone.

\subsection{Normalisation Convention}
\label{v4-arith:w7-d:normalization}

We fix a Fourier normalisation consistent with the earlier chapters.
...
The proof uses four classical sources. None enters the Weil 1952 derivation
(used earlier) or the Rankin--Selberg integral of
...
Theory of Fourier Integrals} (2nd edn, Oxford 1948). Fourier-transform
normalisations and continuity of classical integral functionals.
...
The four inputs (Erd\'elyi 1953 special
functions; Gradshteyn--Ryzhik 1965 tables; Titchmarsh 1948 Fourier
integrals; Rudin 1973 functional analysis) are analytic-tool lists
that have not been cited as proof sources in
Chapters~\ref{v4-arith:closure}, \ref{v4-arith:kp-compat},
\ref{v4-arith:kms-gns-full}, \ref{v4-arith:w5-a:chapter},
\ref{v4-arith:w5-i:chapter}, or \ref{v4-arith:w6-a:ATP-direct-obstruction}.
...
\(W_{\infty}\)-positivity on the Gaussian sub-class (the requirement
...
\begin{remark}[one-parameter reduction via Gaussian convolution]
...
From Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}
...
\Cref{v4-arith:w7-d:cor:scalar-reduction}, \(W_{\infty}\)-positivity on the Gaussian
...
The critical \(\lambda_{c}\approx 2.52\) corresponds to
a Gaussian \(f_{\lambda_{c}}\) with Fourier transform
\(\widehat{f}_{\lambda_{c}}=\lambda_{c}^{-1/2}e^{-\pi t^{2}/\lambda_{c}}\)
whose standard deviation is \(\sqrt{\lambda_{c}/(2\pi)}\approx 0.633\),
strictly below the sign-change \(t_{\ast}\approx 0.8507\) of
\(g\): the Fourier transform of \(f_{\lambda_{c}}\) concentrates its
\(L^{2}\)-mass inside \([-t_{\ast},t_{\ast}]\) where
\(g<0\). For \(\lambda>\lambda_{c}\) the Gaussian widens to
sample the positive region of \(g\) at high frequencies.
\end{proof}

\section{\(W_{\infty}\)-Positivity on the Gaussian Sub-Class for
\(\lambda\geq\lambda_{c}\)}
\label{v4-arith:w7-d:gauss-ATP}

\begin{theorem}[Gaussian \(W_{\infty}\)-positivity above \(\lambda_{c}\)]
...
\end{equation}
with equality if and only if \(\lambda=\lambda_{c}\). Full A-TP follows
only on the sublocus where
\(R[f_{\lambda}]\geq W_{\infty}(f_{\lambda}*\widetilde{f_{\lambda}})\).
\end{theorem}
...
The final sentence is the bounded-interval reduction
\Cref{v4-arith:w6-a:thm:bounded-reduction}; the present scalar
calculation controls only \(W_{\infty}\).
\end{proof}

\begin{corollary}[\(W_{\infty}\)-positive Gaussian range is exactly
\([\lambda_{c},\infty)\)]
...
\begin{remark}[sign structure of \(W_{\infty}\) on the Gaussian sub-class]
...
characterisation of the Gaussian \(W_{\infty}\)-positive set:
\(\Lambda_{\infty}^{+}=[\lambda_{c},\infty)\)
with \(\lambda_{c}\approx 2.52\). This is strictly stronger than the
trivial Gaussian-convolution vanishing of
\Cref{v4-arith:w7-d:thm:gauss-ATP} (which asserts
\(W(f_{\lambda}*f_{\lambda})=0\) via the Weil formula): the present
result separates \(W_{\mathrm{fin}}\), \(W_{\infty}\), and the polar
terms, and establishes the sign of each piece individually.
\end{remark}

\section{No Gaussian Density Extension: Boundary of the Positive Cone}
\label{v4-arith:w7-d:density-extension}

The one-parameter centred Gaussian family is not used as a dense
system in \(\mathrm{PW}\). The only valid extension of the scalar
Gaussian computation is the positive cone where the quadratic form has
the same sign structure. Outside this cone, linear approximation does
...
\begin{corollary}[no centred-Gaussian density theorem is used]
\label{v4-arith:w7-d:cor:gauss-dense}
\ClaimStatusProvedHere. The one-parameter centred Gaussian family
\(\{f_{\lambda}\}_{\lambda>0}\) is not asserted to be dense in
\(\mathcal{S}_{w}\) or in \(\mathrm{PW}\). The Gaussian argument is
confined to the scalar family and to the positive cone generated from
that family.
\end{corollary}

\begin{proof}
Every \(f_{\lambda}\) is even, hence every finite linear combination
of such functions is even. It cannot approximate an odd Schwartz
function in any Hausdorff locally convex topology stronger than
distributional convergence. The argument is even, but no cited theorem
gives density in the even sector. No density step is
therefore part of the proof.
\end{proof}

\subsection{Boundary of the Density Extension}
\label{v4-arith:w7-d:density:breaks}

\begin{proposition}[boundary of Gaussian density extension of \(W_{\infty}\)-positivity]
\label{v4-arith:w7-d:prop:density-breaks}
\ClaimStatusProvedHere. The Gaussian \(W_{\infty}\)-positivity of
\Cref{v4-arith:w7-d:thm:gauss-ATP} (unconditional for
\(\lambda\geq\lambda_{c}\)) does \emph{not} extend by linear
continuity to arbitrary \(f\in\mathrm{PW}(\mathbb{R})\). Continuity of
\(W_{\infty}\) is linear in \(\phi=f*\widetilde f\); positivity is a
quadratic cone condition in \(f\). A linear approximation theorem,
even if supplied from a larger translated-Gaussian system, would not
preserve this cone.
\end{proposition}

\begin{proof}
The \(W_{\infty}\)-inequality is quadratic in \(f\). It involves
\(f*\widetilde{f}\), not \(f\) itself. The quadratic structure
\(|\widehat{f}(t)|^{2}g(t)\geq-\|g\|_{L^{\infty}[-t_{\ast},t_{\ast}]}\cdot|\widehat{f}(t)|^{2}\)
for \(|t|\leq t_{\ast}\) means that the approximation
\(\sum_{i}c_{i}f_{\lambda_{i}}\) produces cross-terms
\(\sum_{i,j}c_{i}\bar{c}_{j}\widehat{f_{\lambda_{i}}}(t)\overline{\widehat{f_{\lambda_{j}}}(t)}\)
in \(|\widehat{f}(t)|^{2}\),
which contain both \(\lambda_{i},\lambda_{j}\geq\lambda_{c}\) pairs
and \(\lambda_{i}<\lambda_{c}\) or \(\lambda_{j}<\lambda_{c}\) pairs depending on
the \(\lambda_{i}\)'s. The cross-term structure is indefinite in general;
density-by-continuity preserves positivity only along the positive cone.

Take \(f=\alpha f_{\lambda_{1}}+\beta f_{\lambda_{2}}\)
with \(\lambda_{1}<\lambda_{c}<\lambda_{2}\). Then
\(|\widehat{f}(t)|^{2}=\alpha^{2}\lambda_{1}^{-1}e^{-2\pi t^{2}/\lambda_{1}}
+2\alpha\beta(\lambda_{1}\lambda_{2})^{-1/2}e^{-\pi t^{2}(1/\lambda_{1}+1/\lambda_{2})}
+\beta^{2}\lambda_{2}^{-1}e^{-2\pi t^{2}/\lambda_{2}}\),
so the Weil archimedean contribution is
\begin{equation*}
W_{\infty}(f*\widetilde{f})
=\frac{1}{2\pi}\!\left[\frac{\alpha^{2}}{\lambda_{1}}I(\lambda_{1})
+\frac{2\alpha\beta}{\sqrt{\lambda_{1}\lambda_{2}}}I(\mu_{12})
+\frac{\beta^{2}}{\lambda_{2}}I(\lambda_{2})\right],
\end{equation*}
where \(\mu_{12}:=2\lambda_{1}\lambda_{2}/(\lambda_{1}+\lambda_{2})\)
is the harmonic mean. This is a quadratic form in \((\alpha,\beta)\).
For \(\lambda_{2}>\lambda_{c}\) and \(\mu_{12}>\lambda_{c}\) but
\(\lambda_{1}<\lambda_{c}\), the diagonal term proportional to
\(I(\lambda_{1})<0\), so choosing \(\alpha\) large and
\(\beta\) small gives \(W_{\infty}(f*\widetilde{f})<0\).
\end{proof}

\begin{remark}[quadratic obstruction to density extension]
\label{v4-arith:w7-d:rem:density-failure}
\Cref{v4-arith:w7-d:prop:density-breaks} identifies the mechanism:
the \(W_{\infty}\)-inequality is quadratic in \(f\),
and quadratic-form positivity lifts from a positive
cone of generators (Gaussians with \(\lambda\geq\lambda_{c}\)) only
along conic combinations. This is a standard phenomenon in
...
\begin{theorem}[\(W_{\infty}\)-positivity on the Gaussian positive cone]
...
\(\lambda\geq\lambda_{c}\). Then \(W_{\infty}\ge0\) holds unconditionally on the
...
Indeed, for \(x,y\geq\lambda_{c}\), the harmonic mean \(H=2xy/(x+y)\)
satisfies \(H\geq\lambda_{c}\): the equivalence
\(H\geq\lambda_{c}
\Leftrightarrow 1/H\leq 1/\lambda_{c}\Leftrightarrow
(1/x+1/y)/2\leq 1/\lambda_{c}\)
holds because \(x\geq\lambda_{c}\) gives \(1/x\leq 1/\lambda_{c}\),
and \(y\geq\lambda_{c}\) gives \(1/y\leq 1/\lambda_{c}\), so
\((1/x+1/y)/2\leq 1/\lambda_{c}\).
Hence \(\mu\geq\lambda_{c}\).
...
Gaussian convolution cone in the Schwartz extension. It contains only
...
\(W_{\infty}\ge0\) holds unconditionally on
\(\mathcal{C}_{\mathrm{Gauss}}^{+}\), which strictly enlarges the
high-frequency dominance class
\(\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\) of
Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction} on the Gaussian
sub-sector.
\end{remark}

\subsection{Comparison with the bounded-interval reduction HFD Sub-Class}
...
gives a strict enlargement of either component alone where
\(W_{\infty}\ge0\) holds unconditionally. Full A-TP on this union
still requires \(R[f]\geq W_{\infty}(f*\widetilde f)\). Neither enlargement changes the
fundamental obstruction: A-TP on the \emph{complement}
...
\begin{theorem}[Gaussian reduction refinement of the \(W_{\infty}\) obstruction]
\label{v4-arith:w7-d:thm:wave7-d-irreducible}
\ClaimStatusProvedHere. The archimedean inequality \(W_{\infty}\ge0\)
closes unconditionally on the union
\begin{equation}
...
\end{equation}
Full A-TP on this union follows only on the sublocus
...
is the remaining \(W_{\infty}\) residual; it contains test functions whose
...
part of the \(R[f]\)-coupled A-TP obstruction, which is RH-equivalent
by \Cref{v4-arith:w5-a:thm:ATP-equals-weil-positivity}. By
...
\begin{proof}
Combine:
\Cref{v4-arith:w6-a:thm:HFD-ATP} (HFD \(W_{\infty}\)-closure),
...
and
\Cref{v4-arith:w6-a:thm:irreducible-ATP} (the full bounded-interval
inequality remains coupled to \(R[f]\)).
\end{proof}

\begin{remark}[three consequences of the Gaussian reduction]
\label{v4-arith:w7-d:rem:consequence}
The Gaussian reduction produces three consequences.

\begin{description}
\item[\textbf{(a) Gaussian sub-class closure.}] \(W_{\infty}\ge0\) holds
...
\item[\textbf{(b) Scalar characterisation.}] The Gaussian \(W_{\infty}\)-positive
...
\item[\textbf{(c) Density extension analysis.}] A centred-Gaussian
density theorem is not used, and linear approximation would not extend
full A-TP to PW
...
density. \(W_{\infty}\)-positivity extends to the positive Gaussian cone
...
\section{The Gaussian \(W_{\infty}\)-Positivity Theorem: Results}
\label{v4-arith:w7-d:summary}

\begin{enumerate}
\item Gaussian self-duality and convolution closure collapse \(W_{\infty}\)-positivity on
...
\item \(W_{\infty}\)-positivity on the Gaussian sub-class holds unconditionally for
\(\lambda\geq\lambda_{c}\)
(\Cref{v4-arith:w7-d:thm:gauss-ATP},
...
\(W_{\infty}\)-positivity extends unconditionally to the positive Gaussian cone
\(\mathcal{C}_{\mathrm{Gauss}}^{+}\)
(\Cref{v4-arith:w7-d:thm:positive-cone}).

\item The combined \(W_{\infty}\)-closed class
\(\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\cup\mathcal{C}_{\mathrm{Gauss}}^{+}\)
strictly enlarges either the HFD sub-class of
Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction} or the Gaussian
positive cone \(\mathcal{C}_{\mathrm{Gauss}}^{+}\) alone. Full A-TP
still retains \(R[f]\), and the remaining full obstruction is
RH-equivalent
...
The Gaussian reduction obeys the Beilinson principle:
\(W_{\infty}\ge0\) holds on
...
The elementary ceiling is reached: further change requires either
...
Integrals} (2nd edn, Oxford). Fourier normalisations and continuity of
tempered-distribution functionals.
...
for the digamma function (cross-reference to the earlier analysis of
Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}).
...
Used as the Weil-explicit-formula input.
\end{description}

\noindent\emph{Disjointness comparison.} The analytic lists
(Erd\'elyi 1953 for erfc integral identities;
...
are classical analytic-tool lists that do not appear as derivation
sources in Chapters~\ref{v4-arith:closure}, \ref{v4-arith:kp-compat},
\ref{v4-arith:kms-gns-full}, \ref{v4-arith:w5-a:chapter},
\ref{v4-arith:w5-i:chapter}, or \ref{v4-arith:w6-a:ATP-direct-obstruction}.
Hurwitz 1882 and Weil 1952 appear earlier as proof sources; here they
enter only by cross-reference: the Hurwitz series
comes from Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}, and Weil's
explicit formula comes from Chapter~\ref{v4-arith:w5-a:chapter}.
