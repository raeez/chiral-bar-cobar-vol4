...
the identification.
The represented local modular operator in K3 is explicit. The native
embedding in K2, the archimedean pairing maps in K4, the finite maps
in K5 and their compatible assembly remain separate hypotheses.
...
\item[(K2)] \emph{Native \(L^2\)-embedding.} The comparison requires
an injective, Hecke-equivariant map from the specified native core
\(\mathcal V^{\mathrm{prim}}\) to the chosen automorphic \(L^2\)
space. Factorization of an already realized automorphic representation
does not construct this map from a native Hochschild carrier.
\textbf{Form: native realization hypothesis}.
...
half-density \(m^{1/2}\) is inserted on \(\mu_m\Omega\),
while \(m^{-1/2}\) multiplies \(\mu_m^*\Omega\). This is
proved for the represented prime Gibbs quotient in
\Cref{v4-arith:thermal:modular}. A native pairing lift further
requires \Cref{v4-arith:kp:hyp:native-pairing-maps}.
\textbf{Form: local operator construction with a separate native comparison}.
\item[(K4)] \emph{Archimedean normalization.} The Fock form has
explicit coefficient conjugation. Its identification with a native
Petersson integral requires the archimedean maps of
\Cref{v4-arith:kp:prop:Pet-is-Segal-infty}; a Gaussian integral alone
does not identify the full carriers. \textbf{Form: conditional transfer}.
\item[(K5)] \emph{Finite-place reconciliation.} The comparison with
Whittaker and Koszul forms requires the actual maps of
\Cref{v4-arith:bcglue-k5:conj:local-comparison-datum}. Adjoint factors
and Haar volumes are fixed in those maps and forms. The identity of the
normalized state has value one. \textbf{Form: conditional transfer
under native maps and scalar normalizations}.
...
\end{itemize}
The local modular construction does not remove the native embedding,
archimedean or finite pairing-map hypotheses. Their compatible adelic
assembly also remains necessary for the global identity.
\end{theorem}
...
(\Cref{v4-arith:sw:thm:KP-split}). K3 has the represented local
operator construction of Chapter~\ref{v4-arith:thermal:chapter}.
The native embedding K2, archimedean maps K4, finite maps K5 and their
compatible assembly remain hypotheses. Classical measure or spectral
decomposition results do not construct these native maps.
