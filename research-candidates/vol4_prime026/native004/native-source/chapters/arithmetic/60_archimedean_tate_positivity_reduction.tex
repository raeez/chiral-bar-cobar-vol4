...
domain. The finite-prime form has a signed translation representation,
and the full Weil form has the signed zero-evaluation representation
of equation~\eqref{v4-arith:sc:full-comparison}.  The
...
Chapter~\ref{v4-arith:prim-zeta-char} computes the mixed scalar
character; its native character interpretation requires
\Cref{v4-arith:prim-zeta:native-character-hypothesis}. Any native
duality conclusion using that interpretation retains this hypothesis.
...
The native positivity statements of
\Cref{v4-arith:w5-c:thm:holomorphic-cuspidal} and \Cref{v4-arith:w5-c:thm:maass-cuspidal}
require an injective automorphic realization and the pairing maps of
\Cref{v4-arith:kp:hyp:native-pairing-maps}. Under these maps the
Koszul--Petersson identity follows from the bilinear Hilbert--Schmidt
pairing and modular conjugation. On an actual cuspidal \(L^2\) space,
positivity is the positivity of its integral norm.

Ramanujan bounds constrain Satake parameters; they do not produce the
native maps. \Cref{v4-arith:w5-c:prop:KS-partial} gives an Euler-factor
bound and a separate norm criterion for comparing realization maps.
The discrete-spectrum conclusion of
\Cref{v4-arith:w5-c:thm:discrete-core-identity} also requires the
compatible pairing blocks of \Cref{v4-arith:w5-c:def:compatible-pairing-blocks}:
$c$ preserves the dual blocks, and $B$ vanishes between distinct
blocks. The stated adelic assembly is required on each block. For the continuous spectrum, the native
pairing measure and the regularized automorphic measure must be
identified before the Langlands--Shahidi normalization can transfer an
identity of forms. The Li trace needs its own spectral realization.

\subsection{Chapter 56: The Hilbert--P\'{o}lya Operator Skeleton}
\label{v4-arith:w5-synth:ch56}

\Cref{v4-arith:w5-b:thm:N-BC-self-adjoint} constructs the product GNS
operator \(N_{\mathrm{BC}}=-\tfrac12\log\Delta_1\) at inverse
temperature one. Its point eigenvalues are
\(\tfrac12\log(a/b)\), each with infinite multiplicity, and its
spectrum is \(\mathbb R\). The positive occupation weights belong to
\(N_{\mathrm{occ}}\) on \(\ell^2(\mathbb N^\times)\).
\Cref{v4-arith:w5-b:occupation-comparison} proves
\(WN_{\mathrm{occ}}=2N_{\mathrm{BC}}W\) onto the closed creation
subspace. The weighted zero-mode embedding is
\(\delta_m\mapsto m^{-1/2}[\mu_m]\). An oscillator extension requires
the additional lift of \Cref{v4-arith:w5-b:lem:fock-embeds-gns}.

The product GNS space is an incomplete Hilbert tensor product of local
GNS spaces; it is not identified with global Hilbert--Schmidt operators
at \(\beta=1\). Indeed the occupation Gibbs density has infinite trace
there. Its phases and its comparison with native arithmetic objects
are separate from this tensor construction.

The determinant and trace assertions of
\Cref{v4-arith:w5-b:prop:det-formal} and \Cref{v4-arith:w5-b:prop:weil-trace}
require a zero-divisor operator and a compatible regularization.
Occupation weights supply neither. Literal vacuum-preserving
intertwining with \(N_{\mathrm{occ}}\) is excluded by
\Cref{v4-arith:w5-b:vacuum-obstruction}. The proposed grading and the
self-adjoint zero operator must therefore be specified separately.

\subsection{Chapter 57: HP--Li Positivity and the Archimedean Residual}
\label{v4-arith:w5-synth:ch57}

For an actual cusp form, \Cref{v4-arith:w5-a:thm:RS-integral}
...
On the two-dimensional space $V_b$, Proposition~\ref{v4-arith:sc:two-signs}
...
\Cref{v4-arith:w7-d:thm:gauss-ATP} determines the exact Gaussian
...
\begingroup
\small
\begin{longtable}{p{0.21\textwidth}|p{0.17\textwidth}|p{0.22\textwidth}|p{0.17\textwidth}|p{0.12\textwidth}}
...
Arith.~Positselski (4) native character & Conditional &
Ch.~45 computes the mixed scalar & Native trace maps required &; \\
...
& Ch.~55 requires native maps, compatible pairing blocks and injective realization
& Conditional in every native sector & external \\
...
has occupation weights via \(N_{\mathrm{occ}}\) &
\textbf{Theorem (unconditional)} &; \\
...
Finite-prime restricted pairing & Signed & Explicit positive and
negative lines in $V_b$ & \textbf{Theorem} &
Proposition~\ref{v4-arith:sc:two-signs} \\
...
\(W(f*f^*)\geq 0\) on admissible Weil test functions.
\end{itemize}
...
\item \textbf{KP (Koszul--Petersson compatibility).}
On the native discrete core the identity requires the pairing maps,
compatible pairing blocks, injective automorphic realization,
and assembly hypotheses of
\Cref{v4-arith:w5-c:thm:discrete-core-identity}. These conditions apply
also to holomorphic and residual sectors; Maass parameter bounds are
additional analytic information.
\item \textbf{HP (Hilbert--P\'olya).}
The operator \(N_{\mathrm{occ}}\) supplies weights \(\log m\), with
\(WN_{\mathrm{occ}}=2N_{\mathrm{BC}}W\) at \(\beta=1\)
(\Cref{v4-arith:w5-b:occupation-comparison}). The determinant-divisor
and trace comparison in \Cref{v4-arith:w5-b:hyp:twist-existence}
remain additional data; literal vacuum-preserving intertwining fails.

\item \textbf{HPLi (HP--Li positivity).} The full compact-test form
has the signed representation \eqref{v4-arith:sc:full-comparison}.
The positive line in Proposition~\ref{v4-arith:sc:two-signs} realizes only the
...
\begin{tikzpicture}[node distance=1.2cm and 1.5cm,
 ...
 \node[closed] (KP) at (4,0) {KP / disc.~core \\ \textbf{Conditional} \\ {\footnotesize Ch.~55}};
 ...
 \node[closed] (HPLi) at (0,-4) {Signed Weil pairing \\ \textbf{Theorem} \\ {\footnotesize Ch.~57}};
...
\item HP (W) occupation weights
(\Cref{v4-arith:w5-b:occupation-comparison});
\item the signed Weil pairing on $\mathscr A$
(equation~\eqref{v4-arith:sc:full-comparison}).
\end{itemize}
...
self-adjoint operator on a specified arithmetic Hilbert space,
with a separate comparison to the product GNS carrier,
...
operator realization additionally requires the specified carrier and
trace maps; the \(\mathrm{VB}^{*}\) condition is the complex-analytic form on
...
(conditional on native pairing maps, compatible pairing blocks,
and injective automorphic realization in every sector) via Ch.~55.
...
\item \textbf{Signed arithmetic comparison.} The complete compact-test
identity and explicit positive and negative finite-prime lines are
equation~\eqref{v4-arith:sc:full-comparison} and
Proposition~\ref{v4-arith:sc:two-signs}.
...
Chapter 56 constructs the occupation operator \(N_{\mathrm{occ}}\)
with eigenvalues \(\log m\), and identifies it with
\(2N_{\mathrm{BC}}\) on the creation subspace at \(\beta=1\).
...
Consani--Marcolli 2006 supply the adele-class space framework; the realization of a zero-divisor operator with a compatible arithmetic
trace remains an additional construction. It cannot be literal
intertwining of the occupation grading on the vacuum, by
\Cref{v4-arith:w5-b:vacuum-obstruction}.
...
(\Cref{v4-arith:w5-d:combined:thm:closure}).
\item The Koszul--Petersson compatibility on the native
discrete-spectrum core requires the stated pairing and realization maps,
preservation of dual blocks by $c$, vanishing of mixed bilinear terms,
and the adelic assembly
(\Cref{v4-arith:w5-c:thm:discrete-core-identity}).
\item The occupation grading is constructed by \(N_{\mathrm{occ}}\),
with its precisely normalized GNS comparison
(\Cref{v4-arith:w5-b:occupation-comparison}); the zero-divisor
operator requires a separate construction.
\item The complete Weil form has the signed comparison
\eqref{v4-arith:sc:full-comparison}. The positive finite-prime line
is constructed in Proposition~\ref{v4-arith:sc:two-signs}; extending its norm
