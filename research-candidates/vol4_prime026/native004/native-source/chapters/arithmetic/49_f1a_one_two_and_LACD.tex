...
\emph{The functor \(\Phi^{\mathrm{DEG}}_{p,\chi}\) is analysed on three
orthogonal loci. On the unramified ordinary locus, Breuil--M{\'e}zard
geometry, Emerton--Gee smoothness, and cyclic base change identify the
compact generators. At \(p=2,3\), the semisimple blocks are separated by
the Pa{\v s}k{\=u}nas and Colmez--Dospinescu--Pa{\v s}k{\=u}nas block
calculi. On the cuspidal sph-finite automorphic category, Flath
restricted tensor products, Bernstein orthogonality, and
Mo{\oe}glin--Waldspurger spectral decomposition give adelic categorical
descent. The Eisenstein sector is governed by the normalised
intertwining operators.}

\section{Objects and Disjoint Sources}
\label{v4-arith:w5-e:domain}

\subsection{Inherited Objects}

All the categorical data used in this chapter is stated prior:
\begin{itemize}
...
list (Chapter~\ref{v4-arith:f1a3-BH};
...
(Chapter~\ref{v4-arith:fle-completion-table},
\Cref{v4-arith:fle-table:def:selmer-stack});
\item the four subconditions
\(\mathrm{L}_{\mathrm{ACD}}\).0--.3 of
Chapter~\ref{v4-arith:lacd-sub}, together with
\Cref{v4-arith:tate:thm:F2c-main} (F2.c tightened).
\end{itemize}

The constructions below use these objects and no additional moduli
problem.
...
for \(GL_{2}\).
\item[Carayol 1986] ``Sur les repr{\'e}sentations \(\ell\)-adiques
associ{\'e}es aux formes modulaires de Hilbert'', Ann.~Sci.~\'ENS~19
Th.~A. Local-global compatibility for the \(\ell\)-adic Galois
representation attached to a Hilbert modular newform at unramified
primes.
\item[Saito 1997] ``Modular forms and \(p\)-adic Hodge theory'',
Invent.~Math.~129. Local-global compatibility for the Galois
representation attached to a classical newform: the Frobenius
eigenvalue at an unramified prime \(\ell \neq p\) equals the
Hecke eigenvalue \(a_{\ell}\) of the newform.
...
The proof sources backing the DEG comparison
...
comparison sources used here. Breuil--M{\'e}zard + Kisin + Gee--Kisin
...
source in this chapter appears in DEG's primary derivation; hence the
...
\ClaimStatusProvedHereConditional. Under Setup~\ref{v4-arith:w5-e:setup:TW}
and the DEG essential-image theorem on the unramified ordinary
Emerton--Gee component, the
...
\]
is an equivalence. The Taylor--Wiles, Breuil--Mezard, Hida, and
local-global compatibility inputs identify the Hecke eigenvalue
geometry; the essential-image assertion is the additional local
categorical input.
\end{theorem}
...
distinguished-residual conditions. Local-global compatibility at
unramified primes (Carayol 1986 Ann.~Sci.~\'ENS~19 Th.~A;
Saito 1997 Invent.~Math.~129, local-global compatibility
for the Galois representation attached to a newform via
Carayol--Saito) identifies the unramified Hecke eigenvalue
\(\alpha_{p}+\beta_{p}\) with the trace of crystalline Frobenius
on the corresponding \(\ell\)-adic representation, matching the
Frobenius eigenvalues on
\(\mathcal{X}^{\mathrm{EG},p,\chi,\mathrm{unr\text{-}ord}}_{GL_{2}}\)
via CHT~\S 4. Harris--Taylor 2001
...
functor compatibility with Hida theory (Colmez 2010 Ch.~VII,
Ast{\'e}risque~330 for the fixed-weight ordinary match).
...
\subsection{Precise Ordinary Range}

\begin{corollary}[ordinary F1.a.1 range]
\label{v4-arith:w5-e:cor:f1a1-uncond-range}
\ClaimStatusProvedHereConditional. The essential surjectivity of
\(\Phi^{\mathrm{DEG}}_{p,\chi}\) holds on the unramified ordinary
summand under hypotheses (TW1)--(TW4) of
Setup~\ref{v4-arith:w5-e:setup:TW} and the DEG essential-image
theorem of \Cref{v4-arith:w5-e:thm:f1a1-ordinary}. Explicitly: for every prime
...
stated hypotheses. The three automorphic inputs
(\Cref{v4-arith:w5-e:lem:BM-unr-ord},
\Cref{v4-arith:w5-e:lem:unr-smooth},
\Cref{v4-arith:w5-e:lem:CHT-unr-Hecke}) are already theorems; the
remaining input is the local DEG essential-image theorem.
\end{proof}
...
\begin{remark}[non-ordinary obstruction]
...
the residual Frobenius-trace zero. Essential surjectivity on the
non-ordinary locus follows from either: (i) a Koszul-duality matching
of projective generators with structure sheaves of non-reduced
divisors, or (ii) a Bushnell--Kutzko-type Pa{\v s}k{\=u}nas exhaustion
at the non-ordinary simples. This is exactly
\Cref{v4-arith:deg-f1a:conj:block-exhaustion} specialised to the
non-ordinary locus; the ordinary locus is controlled by
\Cref{v4-arith:w5-e:thm:f1a1-ordinary}.
\end{remark}

\subsection{Source Disjointness and Convention Comparison for F1.a.1}

\begin{enumerate}
\item \textbf{Source disjointness.} Breuil--M{\'e}zard +
...
(fully-faithful-functor construction) and from
Fargues--Scholze arXiv:2102.13459 (spectral-action). The orthogonality
disjointness comparison passes on the unramified ordinary range.

\item \textbf{Hodge--Tate weight conventions.}
...
representations'', CMH~83.

\item \textbf{Categorical ambient.} LHS and RHS are stable
...
\item \textbf{Hypotheses.} (TW1)--(TW4) are
explicit. Big image excludes the Eisenstein ideal; distinguishedness
excludes the reducible non-split residual with equal diagonal
characters; the ordinary condition excludes the supersingular locus.

\item \textbf{Hecke functoriality.} The Hecke operators
\(T_{p}\) and \(S_{p}\) act on both sides; Clozel--Harris--Taylor
\S 4 establishes the eigenvalue match (Carayol 1986; Saito 1997,
as in \Cref{v4-arith:w5-e:lem:CHT-unr-Hecke}).

\item \textbf{Conditional inputs.} No conjecture is invoked
within the unramified ordinary range.
\end{enumerate}

The non-ordinary residual is recorded as
...
The block-diagonal local comparison functor is constructed on each
block separately, with the non-semisimple
\(\mathrm{Ext}^{1}\)-contributions recorded separately on each
non-semisimple block.

\subsection{Block Structure at \(p=2\)}

\begin{theorem}[Pa{\v s}k{\=u}nas JEMS 16 (2014) block decomposition at \(p=2\)]
...
representations at \(p=2\) (Breuil--Pa{\v s}k{\=u}nas 2012) and
...
principal block. The result builds on Pa{\v s}k{\=u}nas JEMS~16 (2014) for
\(p\geq 5\) using the stability of the
Breuil--M{\'e}zard / \((\varphi,\Gamma)\)-module structure at \(p=3\).
\end{proof}
...
Colmez 2010 (Ast{\'e}risque~330) \((\varphi,\Gamma)\)-module
equivalence directly; these blocks are semisimple on both sides,
and the equivalence is the Colmez Montreal functor restricted to
the block.
...
one simple. Pa{\v s}k{\=u}nas JEMS~16 (2014) \S 2.6 (supersingular) and
...
(dimension \(2\) at \(p=3\)). Pa{\v s}k{\=u}nas JEMS~16 (2014) \S 5 and
...
block \(\mathrm{Ext}^{1}\) is computed by Pa{\v s}k{\=u}nas JEMS~16 (2014) \S 4
...
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3}); the class-level match is
not recorded in the primary literature and is the conjectural
residual.
\end{remark}

\subsection{Source Disjointness and Convention Comparison for F1.a.2}

\begin{enumerate}
\item \textbf{Source disjointness.} Pa{\v s}k{\=u}nas JEMS~16 (2014)
and Colmez--Dospinescu--Pa{\v s}k{\=u}nas 2014
...
\item \textbf{Convention compatibility.} Pa{\v s}k{\=u}nas and
...
shift; \(\mathrm{Ext}^{1}\)-class conventions in Pa{\v s}k{\=u}nas
JEMS~16 (2014) \S 4 are Jantzen-style, compatible with
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p2}.

\item \textbf{Categorical ambient.} LHS: \(\mathrm{sph\text{-}fin}\)
derived category at a single prime; RHS:
\(\mathrm{IndCoh}_{\mathrm{Nilp}}\) on a formal algebraic stack over
\(\mathrm{Spf}\,\mathbb{Z}_{p}\). The block-diagonal decomposition
preserves both.

\item \textbf{Hypotheses.} Central character \(\chi\) is
fixed; block indexing uses Pa{\v s}k{\=u}nas block data.
\(p\in\{2,3\}\) is explicit. All residuals \(\bar\rho\) are enumerated
by the block theorem; no tacit restriction is imposed.

\item \textbf{Hecke functoriality.} The block-diagonal functor
is a direct sum of block functors; composition with Hecke operators,
which preserve blocks, is functorial per block.

\item \textbf{Conditional inputs.} Essential surjectivity on
non-semisimple blocks depends on
\Cref{v4-arith:f1a3-BH:conj:Paskunas-wild-ext} (class-level
\(\mathrm{Ext}^{1}\) match); this is the named residual
for non-semisimple blocks.
\end{enumerate}

Semisimple blocks close unconditionally; non-semisimple blocks close
up to the class-level \(\mathrm{Ext}^{1}\)-compatibility.
...
\(\bigotimes_{v}^{\prime,\mathrm{glob}}\mathcal{C}_{v}\) denote the
full subcategory of the restricted tensor product whose irreducible
objects are the local components of an actual cuspidal automorphic
representation. The globalization functor
\[
\bigotimes_{v\in|\overline{C}|}^{\prime,\mathrm{glob}}\mathrm{D}^{\mathrm{sph\text{-}fin}}([GL_{2}(\mathbb{Q}_{v})])
\;\xrightarrow{\;\sim\;}\;
\mathrm{D}^{\mathrm{sph\text{-}fin}}([GL_{2}])^{\mathrm{cusp}}
\]
is an equivalence on compact generators, conditional on the
categorical globalization theorem asserting full faithfulness and
essential surjectivity for the globalizable compact objects. It is
compatible with:
\begin{enumerate}
...
the globalizable part of the adelic restricted tensor product of
...
\textbf{Step 2: representation-level map on globalizable tuples.}
Flath 1979 (\Cref{v4-arith:w5-e:thm:Flath}) gives, for an actual
cuspidal automorphic representation, a factorization
\(\pi\simeq\bigotimes'_v\pi_v\). Hence on the globalizable subcategory
...
\]
sending \(\bigotimes'_{v}[\pi_{v}]\) to \([\pi]\). Flath does not
globalize an arbitrary tuple of local objects; globalization is part
of the definition of the source subcategory.

\textbf{Step 3: categorical globalization hypothesis.} The Flath map
of Step~2 extends to a functor
\[
F\colon
\bigotimes_{v}^{\prime,\mathrm{glob}}\mathcal{C}_{v}
\;\longrightarrow\;
\mathrm{D}^{\mathrm{sph\text{-}fin}}([GL_{2}])^{\mathrm{cusp}}
\]
only under the categorical globalization hypothesis: restriction to
local components has a compatible adjoint on the chosen compact
generators and the unit and counit are equivalences there. Flath
supplies the representation-level factorization; it does not by itself
construct this adjunction.
...
\S 1.7; Mo{\oe}glin--Waldspurger 1995 \S III.3). The categorical
globalization hypothesis says that the restriction of \(F\) to compact
generators is an equivalence. By Lurie HA~\S 5.3.5 (preservation of
compact generation under ind-completion of an equivalence on
generators), \(F\) is an equivalence of the stated globalizable stable
...
\begin{remark}[Eisenstein extension]
\label{v4-arith:w5-e:rem:LACD-Eisenstein-domain}
...
\end{itemize}
These hypotheses concern the non-cuspidal spectrum. The cuspidal
sph-fin locus is treated by \Cref{v4-arith:w5-e:thm:LACD-cuspidal};
the Eisenstein extension is \Cref{v4-arith:w5-e:conj:LACD-Eisenstein}.
\end{remark}

\subsection{Source Disjointness and Convention Comparison for \(\mathrm{L}_{\mathrm{ACD}}\)}

\begin{enumerate}
\item \textbf{Source disjointness.} Flath 1979, Bernstein 1988,
and Mo{\oe}glin--Waldspurger 1995 are spectral-decomposition and
Strong Multiplicity One results, disjoint from DEG, from
Fargues--Scholze, and from Lurie HA used in
Chapter~\ref{v4-arith:tate-tensor}.
Chapter~\ref{v4-arith:lacd-sub} (\(\mathrm{L}_{\mathrm{ACD}}\).0--.3)
uses Harish-Chandra, Vogan, Wallach, and Lurie HA; these are disjoint
from Flath, Bernstein, and Mo{\oe}glin--Waldspurger.

\item \textbf{Convention compatibility.} Flath 1979 uses the classical
adelic convention on \(GL_{2}\); Bernstein 1988 uses the smooth
representation convention (no fixed level); Mo{\oe}glin--Waldspurger
1995 uses the \(L^{2}\)-decomposition convention. The three conventions
coincide on cuspidal representations via Strong Multiplicity One
(Jacquet--Langlands SLN~114 \S 11).

\item \textbf{Categorical ambient.} Both sides are stable
...
product lies in \(\mathrm{Pr}^{\mathrm{L,st}}\) by F2.c.

\item \textbf{Hypotheses.} The cuspidality assumption is explicit.
Central-character factorisation: \(\chi\) factors
through a continuous character on \(\mathbb{A}^{\times}/\mathbb{Q}^{\times}\),
which is Flath's hypothesis for the restricted tensor product on
cuspidal representations.

\item \textbf{Hecke functoriality.} The Flath-lift functor
is defined by adjunction; compatibility with the Whittaker model,
the Bernstein centre, and the archimedean
\((\mathfrak{g},K_{\infty})\)-structure is verified place-by-place.
Cross-prime Hecke compatibility is inherited from
\Cref{v4-arith:lacd:thm:L2-closed}.

\item \textbf{Conditional inputs.} The Eisenstein extension
\Cref{v4-arith:w5-e:conj:LACD-Eisenstein} is a separate conjectural
statement; no unproved input is invoked on the cuspidal sph-fin locus.
\end{enumerate}

The cuspidal sph-fin equivalence holds; the Eisenstein extension is the
separate conjectural statement.

\section{Boundary of the F1 Comparison}
\label{v4-arith:w5-e:residuals}

\begin{proposition}[remaining F1 compatibility statements]
\label{v4-arith:w5-e:prop:F1-table-refined}
\ClaimStatusProvedHere. Let \(F_{\mathrm{ord}}\),
\(F_{\mathrm{ss}}\), and \(F_{\mathrm{cusp}}\) denote respectively the
unramified ordinary summand, the semisimple \(p=2,3\) block summand, and
the cuspidal sph-finite adelic summand. On
\[
F_{\mathrm{ord}}\oplus F_{\mathrm{ss}}\oplus F_{\mathrm{cusp}},
\]
the comparison functors of this chapter are equivalences. The extension
to the full F1 comparison is equivalent to the following five
statements:
\begin{enumerate}
\item essential surjectivity of DEG at
\(p\geq 5\) on the non-ordinary locus
(\Cref{v4-arith:w5-e:conj:f1a1-full}).
\item extension beyond Taylor--Wiles
big-image and distinguishedness.
\item class-level Pa{\v s}k{\=u}nas
...
\item Bernstein-centre diamond gluing after
\(\mathrm{L}_{\mathrm{ACD}}\).0--.3
(\Cref{v4-arith:bcglue-core}).
\end{enumerate}
\end{proposition}

\begin{proof}
The ordinary, semisimple, and cuspidal summands are handled by
\Cref{v4-arith:w5-e:thm:f1a1-ordinary},
\Cref{v4-arith:w5-e:cor:f1a2-ss-uncond}, and
\Cref{v4-arith:w5-e:thm:LACD-cuspidal}. Orthogonality of the block
decomposition leaves exactly the five displayed complements.
\end{proof}
