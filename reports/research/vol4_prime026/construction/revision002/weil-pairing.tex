...
For real $s>1$,
\begin{equation}
 \int_{\Gamma\backslash\mathbb H}|G(z)|^2y^kE(z,s)d\mu
...
$I:E\to L^2(\Gamma\backslash\mathbb H,y^k d\mu)$, then
\begin{equation}
 \langle v,w\rangle_I=\int (Iv)(z)\overline{(Iw)(z)}y^k d\mu
...
Changing $\mu$ or multiplying the proposed realization map by a
nonconstant function changes the pairing. A single scalar factor
does not select either datum. The sign-changing density above also
