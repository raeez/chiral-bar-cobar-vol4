...
The occupation operator of \Cref{v4-arith:w5-b:occupation-comparison}
provides a sequence of finite perturbations once an embedding into the
scattering space is chosen. Such an embedding is additional data; the
product GNS construction does not supply a map to the idele-class space.

\begin{definition}[scattering space and finite occupation perturbations]
\label{v4-arith:w7-j:def:scattering-space}
Write
\begin{equation}
\label{v4-arith:w7-j:eq:H-scatter}
 \mathcal K=L^2(\mathbb R,d\tau)\otimes L^2(C_{\mathbb Q}^1,d\mu_1),
 \qquad D_0=M_\tau\otimes1,
\end{equation}
with \(\mu_1(C_{\mathbb Q}^1)=1\) and maximal multiplication domain
\(\{f:\int\tau^2\|f(\tau)\|^2d\tau<\infty\}\).
Let \(P_0\) project onto the constant function in the second factor.
Choose an isometry \(I:\ell^2(\mathbb N^\times)\to L^2(\mathbb R)\).
For example, fix a smooth function \(h\), supported in \((0,1)\), with
\(\|h\|_2=1\), and put \(Iv_m=h_m\), where \(h_m(\tau)=h(\tau-m)\).
The disjoint supports make this an isometry. Let \(Q_M\) project onto
\(v_1,\ldots,v_M\), and define
\begin{equation}
\label{v4-arith:w7-j:eq:D-BC}
 V_M=(I Q_MN_{\mathrm{occ}}Q_MI^*)\otimes P_0,
 \qquad D_M=D_0+V_M \quad\hbox{on }\operatorname{Dom}D_0.
\end{equation}
A full arithmetic scattering operator \(D_{\mathrm{BC}}\) would be a
specified self-adjoint perturbation of \(D_0\), with a prescribed domain
and an arithmetic trace comparison. It is not defined by the formal
expression \(D_0+\iota_*N_{\mathrm{BC}}\otimes P_0\): an isometric
transport of either unbounded occupation or modular energy cannot be a
bounded perturbation.
\end{definition}

\begin{proposition}[finite perturbations and the undamped limit]
\label{v4-arith:w7-j:finite-perturbation}
The operator \(D_M\) is self-adjoint on \(\operatorname{Dom}D_0\), and
\[
 \|V_M\|=\log M,\qquad
 \|V_M\|_1=\sum_{m=1}^M\log m,\qquad
 \|V_M\|_2^2=\sum_{m=1}^M(\log m)^2.
\]
Here \(\|\cdot\|_1\) and \(\|\cdot\|_2\) are the trace and
Hilbert--Schmidt norms. The undamped sequence has no bounded,
trace-class or Hilbert--Schmidt limit. For real coefficients \(a_m\),
the diagonal series
\(V_a=\sum_m a_m|h_m\rangle\langle h_m|\otimes P_0\)
is trace class exactly when \(\sum_m|a_m|<\infty\), and is
Hilbert--Schmidt exactly when \(\sum_m|a_m|^2<\infty\).
\end{proposition}
\begin{proof}
The vectors \(h_m\otimes1\) are orthonormal. They diagonalize \(V_M\)
with eigenvalues \(\log m\); the operator is zero on their orthogonal
complement. This proves the three formulas and the series criteria.
For self-adjointness, \(V_M\) is bounded and self-adjoint. A vector
\(y\) is in the adjoint domain of \(D_0+V_M\) exactly when
\(x\mapsto\langle D_0x,y\rangle+\langle V_Mx,y\rangle\) is bounded
on \(\operatorname{Dom}D_0\) in the Hilbert norm. The second term is
already bounded, so this condition is precisely
\(y\in\operatorname{Dom}D_0^*=\operatorname{Dom}D_0\). The adjoint
therefore equals \(D_0+V_M\) on this domain.
\end{proof}

The original occupation coefficients are \(a_m=\log m\).
Replacing them by \((\log m)/m\) produces a Hilbert--Schmidt series,
but changes the operator and still does not give a trace-class series.
A damping or subtraction must therefore be part of a new specified
arithmetic construction, together with its trace comparison.
...
A relation between these two operators requires the arithmetic maps of
\Cref{v4-arith:w5-b:hyp:twist-existence} and a comparison of their
regularized traces. The scattering formulation additionally requires a
specified perturbation, complete wave operators and the relevant
spectral-shift identity. None follows from the occupation spectrum.
\end{remark}
...
\end{equation}
an operator that is unitary when both wave operators are complete,
meaning that each has range \(P_{\mathrm{ac}}(D_{\mathrm{BC}})\mathcal K\).
Existence alone does not assert completeness or unitarity.
\end{definition}

\begin{proposition}[wave operators for the smooth finite model]
\label{v4-arith:w7-j:prop:wave-existence}
For the smooth, disjoint-support choice \(Iv_m=h_m\) above, the limits
\(W_\pm(D_M,D_0)\) exist and are isometries on \(\mathcal K\).
Completeness of a specified pair and its spectral-shift formula are
additional hypotheses for the scattering comparisons below.
\end{proposition}
\begin{proof}
It suffices first to take \(f\) in the algebraic tensor product of
\(C_c^\infty(\mathbb R)\) with \(L^2(C_{\mathbb Q}^1)\). Only its
component in the constant character contributes to \(V_M e^{-itD_0}f\).
Each coefficient is an integral
\(\int\overline{h_m(\tau)}f_0(\tau)e^{-it\tau}d\tau\).
Twice integrating by parts bounds its absolute value by
\(C_f(1+|t|)^{-2}\). The sum has finitely many terms, so
\(\int_\mathbb R\|V_Me^{-itD_0}f\|dt<\infty\).
Differentiating \(e^{itD_M}e^{-itD_0}f\) on this common smooth domain
gives \(i e^{itD_M}V_Me^{-itD_0}f\). Its integrable norm proves the
Cauchy property as \(t\to\pm\infty\). The approximating operators are
unitary. Thus the limits preserve norms on a dense subspace and extend
by the uniform norm bound to isometries on all of \(\mathcal K\).
\end{proof}

\begin{remark}[the full scattering hypothesis]
\label{v4-arith:w7-j:rem:trace-class-domain}
A self-adjoint \(D_{\mathrm{BC}}\), complete wave operators, a
spectral-shift function and the asserted arithmetic trace matching
must be supplied for the full scattering problem. Trace class is a
condition on a bounded operator on a Hilbert space; convergence of a
subtracted distribution on Schwartz functions does not by itself
satisfy it. The finite construction proves existence of wave operators
for each \(M\); it does not prove convergence or completeness in an
arithmetic limit. Full modular energy is still further removed from
this finite-rank argument: each of its eigenprojections has infinite
rank. The choice of \(I\), the limiting or damped perturbation, and its
comparison with arithmetic phases are the first missing data.
\end{remark}
