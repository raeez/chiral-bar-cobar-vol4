...
The chiral bar--cobar adjunction on a complex curve is replaced, over
\(\overline{\mathrm{Spec}\,\mathbb Z}\), by an ordered
Parshin--Gersten complex. The de Rham differential is replaced by
boundary maps in the arithmetic flag complex.
...
and convergence to \(\Delta\) is a \(p\)-adic limit, not
complex-analytic.
\end{observation}
...
\begin{theorem}[one-dimensional reciprocity and the surface obstruction]
\label{v4-arith:thm:parshin-reciprocity}
\ClaimStatusProvedElsewhere (Tate, Parshin, Kato, Beilinson, Bloch).
On \(\mathrm{Spec}\,\mathcal{O}_{K}\) the reciprocity law is
one-dimensional: it is the product formula for tame symbols. The
triple Parshin symbol belongs to an arithmetic surface, or
equivalently to two-dimensional local fields. For a regular
arithmetic surface \(X\), the Arnold-type triple relation has the form
\begin{equation}
\sum_{(x,C)}\{\,a,b,c\,\}_{x,C}\;=\;0,
\end{equation}
a sum over flags \(x\in C\subset X\). The arithmetic \(\eta_{ij}\) (the Arnold \(2\)-form
\(d\log(z_i - z_j)\) on the complex configuration space, here
replaced by the tame symbol) is the map
\(\partial_{\mathfrak{p}}\colon K_{n+1}^{M}(K)\to K_{n}^{M}(k(\mathfrak{p}))\),
and the one-dimensional relation is the boundary-square-zero condition
in the Gersten complex:
\begin{equation}
...
For \(\mathcal{V}^{\mathrm{prim}}\) the Hochschild trace class of
the identity on the global arithmetic Iwahori Hecke category of
\(GL_2\), regarded as an \(E_1\)-algebra object in motivic sheaves
on \(\overline{C}\) with \(\Lambda\)-action
(\(\Lambda=\mathbb{Z}_{p}[[\Gamma]]\) the cyclotomic Iwasawa
algebra, \(\Gamma=\mathrm{Gal}(\mathbb{Q}(\mu_{p^{\infty}})/\mathbb{Q})\cong\mathbb{Z}_{p}\)),
\begin{equation}
...
\(\partial_{\mathfrak{p}}\).
It is a differential precisely when the Iwasawa, Gersten, and local bar
terms anticommute. This anticommutativity is not proved in this
definition.
\end{definition}

\begin{conjecture}[arithmetic bar nilpotency]
\label{v4-arith:bar:conj:dAr-square-zero}
\ClaimStatusConjectured. The pre-differential \(d^{\mathrm{Ar}}\) satisfies
\((d^{\mathrm{Ar}})^{2}=0\). Equivalently, the Iwasawa differential, the
Gersten differential, and the tame-symbol-weighted local bar
differential form a mixed complex.
\end{conjecture}
...
\section{Primary Bar Mellin Character}
\label{v4-arith:bar:heisenberg-zeta}

The arithmetic bar complex
\(B^{\mathrm{ch,Ar}}(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}})\)
carries a homological grading and a Hecke-monoid grading. The
ordinary Poincar\'e series in homological degree is not a zeta
function. At \(\kappa^{\mathrm{Ar}}=0\), the zero-mode primary
quotient has a Mellin character indexed by \(\mathbb{N}^{\times}\).

\begin{theorem}[primary bar Mellin character]
\label{v4-arith:thm:heisenberg-bar-zeta}
\ClaimStatusConjectured. For the primary zero-mode quotient of the arithmetic Heisenberg
\(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\) (the Hochschild
trace class specialised to \(\kappa^{\mathrm{Ar}}=0\), the locus
of arithmetic Heisenberg data without interacting corrections) at
the undeformed locus, the Hecke-monoid basis is indexed by
\(m\in\mathbb{N}^{\times}\). Its Mellin character is
\begin{equation}
\chi_{\mathrm{bar}}(s)
\;:=\;\sum_{m\geq 1}m^{-s}
\;=\;\prod_{p}(1-p^{-s})^{-1}
...
\end{equation}
This is a Mellin character of the primary Hecke grading, not a
Poincar\'e series of homological dimensions.
\end{theorem}
...
\section{The \(\mathsf{G}\)-Row Complementarity}
...
\begin{remark}
This is the essential theorem of the arithmetic branch. It is
...
derived-centre complementarity'' is the instance of ``Koszul
duality \(\Rightarrow\) functional equation of the Dirichlet
series generating bar cohomology'' for the arithmetic Heisenberg.
\end{remark}

\section{Arithmetic Positselski}
\label{v4-arith:bar:positselski-arith}

The classical Positselski theorem (Positselski, \emph{Two kinds of
derived categories}, 2011) is the equivalence between the co-derived
and the contraderived categories of a conilpotent coalgebra and its
cobar algebra, conditional on vanishing of the Koszul homology in
positive degree. The arithmetic avatar replaces the Koszul condition
by vanishing of the Iwasawa \(\mu\)-invariant.

\begin{theorem}[arithmetic Positselski]
\label{v4-arith:thm:arithmetic-positselski-preview}
...
The full Positselski equivalence with its four independent
verification subitems is \Cref{v4-arith:thm:arithmetic-positselski};
the statement above records the core equivalence in condensed form.
...
symbol \(\partial_{\mathfrak{p}}\). It is a differential under
\Cref{v4-arith:bar:conj:dAr-square-zero}.
\end{definition}

\begin{theorem}[verifications]
\label{v4-arith:thm:bar-verifications}
\ClaimStatusConjectured\ (items 2--4; item~1 is
\ClaimStatusProvedHere). The construction satisfies:
\begin{enumerate}
\item \textbf{Archimedean reduction.} (\ClaimStatusProvedHere,
\Cref{v4-arith:thm:archimedean-reduction}.) At \(\infty\), the
construction agrees with the classical bar complex.
\item \textbf{Nilpotency.} (\ClaimStatusConjectured,
\Cref{v4-arith:bar:conj:dAr-square-zero}.) The arithmetic
pre-differential squares to zero.
...
\(\kappa^{\mathrm{Ar}}+(\kappa^{!})^{\mathrm{Ar}}\in\{0,8,13,250/3,25/3\}\)
...
\item The triple Arnold identity is Parshin reciprocity on the
Gersten complex (\Cref{v4-arith:thm:parshin-reciprocity}).
...
\item Primary bar Mellin character \(=\zeta(s)\)
