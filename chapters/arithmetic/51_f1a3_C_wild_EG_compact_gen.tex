...
is compactly generated at the wild primes \(p\in\{2,3\}\). The answer is yes,
unconditionally for the classical Emerton--Gee stack, and the explicit generators
are identified. Emerton--Gee arXiv:1908.07185 constructs
\(\mathcal{X}^{\mathrm{EG},p}_{GL_{2}}\) as a Noetherian formal algebraic stack
at every prime; Bhatt--Scholze prism theory (arXiv:1905.08229) supplies
quasi-excellence of the complete local rings; Gaitsgory--Rozenblyum DAG
Vol.~II establishes compact generation of \(\mathrm{IndCoh}_{\mathrm{Nilp}}\)
on any locally Noetherian quasi-excellent stack by structure sheaves of closed
points. At \(p\in\{2,3\}\) the nilpotent cone acquires two strata beyond the
generic reducible and irreducible loci: an irregular-supersingular stratum at
\(p=2\) (carrying the \(\mathrm{sl}_{2}\) sign-defect sector of
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p2}) and a principal-block
wild-quiver stratum at \(p=3\) (the \(A_{2}\)-linear quiver of
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p3}). Each stratum is a locally
closed Noetherian substack; on each substack the compact generators are the
residual-gerbe structure sheaf tensored with Serre twists by the determinant
character, with the wild \(\mathrm{Ext}^{1}\)-data absorbed into the
gerbe-plus-twist family. The category
\(\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG},p,\chi}_{GL_{2}})\)
is thus a finite colimit of compactly generated pieces, with
the Bushnell--Henniart closed points
recovered as residual gerbes of the codimension-zero components in the
non-split sectors. The argument is independent of the wild-EG derived
enhancement conjecture \Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}:
compact generation is a property of the classical stack.
...
is finite by Bushnell--Henniart 2006 \S 13 (finite Jordan--H\"older type classification for \(GL_{2}(\mathbb{Q}_{p})\) at \(p\in\{2,3\}\)).
\end{proof}
...
\begin{theorem}[Compact generation of
\(\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG},p,\chi}_{GL_{2}})\)
at wild primes]
...
\begin{corollary}[Wild-prime compact generation implies the Bushnell--Henniart gerbe identification]
...
\subsection{Scope of the Classical Result}

\begin{proposition}[summary of unconditional results]
\label{v4-arith:w5-g:f1a3-C:prop:scope-summary}
\ClaimStatusProvedHere. The following hold without additional hypothesis:
\begin{enumerate}
...
\subsection{Scope Boundary: Derived Enhancement}

\begin{remark}[scope boundary: derived wild-EG enhancement]
...
chapter supplies it. The derived enhancement is a separate conjecture of
Chapter~\ref{v4-arith:deg-f1a}.
\end{remark}

\begin{remark}[completeness of the nilpotent-cone stratification]
\label{v4-arith:w5-g:f1a3-C:rem:no-boundary-obstruction}
The stratification of
\Cref{v4-arith:w5-g:f1a3-C:prop:nilp-stratification} is exhaustive:
the three strata at \(p=2\) or five strata at \(p=3\) cover the full
nilpotent cone. The tangent-complex analysis of
Emerton--Gee \S 5.5 shows no additional component at higher codimension;
all singular directions of the Galois deformation functor are accounted
for by the listed strata.
\end{remark}
...
(1) Emerton--Gee arXiv:1908.07185 (Noetherian formal algebraic structure),
Bhatt--Scholze arXiv:1905.08229 (quasi-excellence via prismatic theory),
and Dotto--Emerton--Gee arXiv:2603.26887 (reduction mod \(p\)) supply the
...
Vol.~II Ch.~4 Prop.~4.4). The hypotheses required
(\(\mathcal{X}\) locally Noetherian; quasi-excellent; tangent complex of
tor-amplitude \([-1,0]\) on each stratum) are all supplied by
Emerton--Gee arXiv:1908.07185 for every prime \(p\geq 2\); no
\(p\geq 5\) condition enters. There is no hidden equivalence: compact
generation follows directly from the stratification and the
per-stratum generators. No numerical engine is involved.
\end{remark}
...
classification. Supplies the Jordan--H\"older type classification that indexes
...
All sources are disjoint from the derivation sources of the Vol.~IV
...
\begin{remark}[consequence for the F1.a sub-residual ledger]
\label{v4-arith:w5-g:f1a3-C:rem:F1a-reduction}
With \Cref{v4-arith:w5-g:f1a3-C:cor:F1a3C-discharged} in hand, the
F1.a sub-residual ledger of Chapter~\ref{v4-arith:f1a3-BH} has two
remaining open items: the Bushnell--Henniart block-packaging conjecture
\Cref{v4-arith:f1a3-BH:conj:BH-packaging} and the Paskunas wild
\(\mathrm{Ext}^{1}\)-compatibility conjecture
\Cref{v4-arith:f1a3-BH:conj:Paskunas-wild-ext}. The compact-generation
item is closed by the present chapter.
\end{remark}
