...
\emph{Under wave-2 attacker A2-PosAr-01 (Costello + Functoriality)
...
The wave-2 attacker A2-PosAr-01 (Costello lens, Functoriality
secondary; see \Cref{v4-arith:sw:wave2}, the ``A2-PosAr-01''
entry) identifies five sub-obligations C1--C5 left open by the
wave-1 abelian-scope heal of the arithmetic Positselski
correspondence. In the wave-1 heal
...
wave-2 attacker observes that this scope restriction is
...
\Cref{v4-arith:pos-costello:attack-heal} runs the six-cycle
attack--heal ledger. Sections
...
wildly-ramified primes; super-Yangian chain-level chiral
coproduct). \Cref{v4-arith:pos-costello:summary} consolidates.

\section{Attack--Heal Ledger}
\label{v4-arith:pos-costello:attack-heal}

Six attack--heal cycles correspond to the five sub-obligations
plus a cross-check on P1 Step~3 functorial pullback.

\subsection{Cycle 1 (C1): Factorization-Coalgebra Coherence}

\paragraph{Attack.} The wave-1 bar complex
...
\end{tabular}
\caption{Costello-refined Positselski attack--heal ledger. Six
...
(\Cref{v4-arith:sw:def:pointed-En} strengthened by the wave-2
...
$E_{2}$-algebra morphisms $\eta_{v}$ (wave-2 A2-F2c-01
...
acts on $a_{-n}^{(p)}$. Crucially, the local bar differential
...
By the wave-1 abelian-scope heal
...
in the $\mu=0$ localisation. Wave-2 residual R4 (Etingof +
...
\item \emph{Super-Yangian chain-level chiral coproduct.} The
$\mu=0$ hypothesis and the abelian scope exclude the
super-Yangian sector where chain-level chiral coproduct
...
wave-2 attacker A2-PosAr-01 are closed.
...
$p=2,3$, super-Yangian chain-level chiral coproduct. Each is
catalogued explicitly and tied to a named wave-1 or wave-2
ledger entry.
...
This chapter refines the wave-2 A2-PosAr-01 ledger entry
...
and the five open Positselski sub-items of
\Cref{v4-arith:pos:rem:open-sub-items} (wave-3 audit
