...
The abelian-domain Positselski equivalence of
\Cref{v4-arith:sw:thm:PosAr-Heisenberg} leaves five subconditions
...
(C5). Each condition is reduced under the stated hypotheses. The arithmetic Heisenberg
biconditional
\(\kappa^{\mathrm{Ar}}_{\mathsf{G}}+(\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}=0\Leftrightarrow\xi(s)=\xi(1-s)\)
remains conditional for the abelian Heisenberg domain.

\section{Five Subconditions of the Abelian Positselski Domain}
\label{v4-arith:pos-costello:five-conditions}

\subsection{The Five Subconditions}
...
is its residual conditional hypothesis. This domain restriction is
necessary but not sufficient: five further subconditions must be
...
P1 Step~3 argument must be proved directly
...
\label{v4-arith:pos-costello:obstruction-analysis}

Six verification cycles address the five subconditions plus a
...
(\Cref{v4-arith:sw:thm:PosAr-Heisenberg}) is conditional on
base-change flatness. C1--C5 supply the additional domain checks:
...
With C1--C5 as hypotheses, the P1 Step~3 Positselski pairing
computation is conditional for the abelian arithmetic Heisenberg.
...
\small
\begin{tabular}{c|l|l}
Cycle & Subcondition & Conclusion \\
\hline
1 & C1 (factorization-coalgebra coherence) & Conditional check, CG~II + Beilinson adelic \\
2 & C2 (\(d^{2}=0\)) & Conditional check, Parshin reciprocity \(\{p,1-p\}=0\) \\
3 & C3 (renormalization) & Conditional check, pro-nilpotent BV + BC KMS\({}_{1}\) \\
4 & C4 (archimedean classical limit) & Conditional check, Witt + Segal--FBZ \\
5 & C5 (functoriality) & Conditional check, corrected naturality \\
6 & P1 Step~3 cross-check & Conditional \\
\end{tabular}
\caption{Costello-refined Positselski comparison table. Six
cycles; six conditional checks. Source citations attached per cycle.}
\label{v4-arith:pos-costello:tab:table}
\end{table}
...
\ClaimStatusProvedHereConditional \emph{(conditional on the \(\eta\)-algebra
...
\ClaimStatusProvedHereConditional \emph{(conditional on pro-nilpotent
...
Euler-product convergence condition. Specifically: the
...
\ClaimStatusProvedHereConditional \emph{(conditional on the Witt-truncation
...
\ClaimStatusProvedHereConditional \emph{(conditional on the C1--C5 domain
...
\Cref{v4-arith:thm:P1-resolution} are the classical inputs supplied by
...
By \Cref{v4-arith:sw:thm:PosAr-Heisenberg}, Step 3 is reduced for the
...
\Cref{v4-arith:pos-costello:thm:C5-closed}, the five C1--C5 checks
are available under their stated hypotheses. Base-change flatness
...
the cancellation is termwise before summation over primes.
...
\ClaimStatusProvedHereConditional \emph{(conditional as in
...
Positselski sub-items, for the abelian Heisenberg domain.
\end{corollary}
...
\Cref{v4-arith:pos-costello:cor:arith-Heis-Koszul} identify the
implication \(\kappa+\kappa^{!}=0\Rightarrow\xi(s)=\xi(1-s)\) for
the arithmetic Heisenberg under the stated hypotheses. They do not prove the Riemann
...
conditional arithmetic Positselski equivalence supplies a derivation
of the functional equation from arithmetic Koszul duality; the
full RH construction requires independently the VB\({}^{*}\) Virasoro
...
Positselski reduction.
\end{remark}

\section{Residual Conditions}
\label{v4-arith:pos-costello:residual}

The Costello-refined Positselski reduction of C1--C5 addresses the
abelian Heisenberg domain. Three residual items remain outside
this domain:
\begin{enumerate}
...
conjecture for non-commutative Iwasawa theory). Residual form:
conjectural (recorded as \Cref{v4-arith:pos:residual-domain}).
...
\begin{enumerate}
\item Five Costello-refined subconditions C1--C5 are recorded under
their named hypotheses.

C1 (local-global factorization-coalgebra coherence): recorded by
...
\item P1 Step 3 functorial pullback is conditional for the
...
reduction: non-abelian Iwahori level, wildly-ramified primes
\(p=2,3\), super Bethe-Hecke chain-level chiral coproduct. Each
is stated explicitly with its residual form in
\Cref{v4-arith:pos-costello:residual}.

\item Source disjoint list (per
\Cref{v4-arith:pos-costello:tab:table}): Costello--Gwilliam
...
\item The Costello-refined reduction does \emph{not} prove RH; it
refines the arithmetic Positselski conditional equivalence in
the abelian Heisenberg domain, which identifies the forward implication
\(\kappa+\kappa^{!}=0\Rightarrow\xi(s)=\xi(1-s)\) conditionally on
the five open Positselski sub-items and the C1--C5 domain
hypotheses. The full RH construction requires independently the
...
\emph{Conditional} form within the abelian domain: closed
conditionally on the C1--C5 domain hypotheses and the five open
