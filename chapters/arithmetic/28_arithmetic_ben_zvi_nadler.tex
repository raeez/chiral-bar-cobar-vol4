...
\emph{The arithmetic Ben-Zvi--Nadler candidate has a sharp obstruction.
The procyclic stack \(B\widehat{\mathbb{Z}}\) retains only the
inertia shadow of the finite-prime topology; it forgets Frobenius,
Weil-group, and \((\varphi,\Gamma)\) data. The corrected target requires
the full Weil/\((\varphi,\Gamma)\) parameter stack, with the
\(B\widehat{\mathbb{Z}}\) construction playing the role of a
forgetful procyclic shadow.}

\section{Statement}
\label{v4-arith:bzn:statement}

\begin{theorem}[F3.BZN\(^{\mathrm{Ar}}\), full target]
\label{v4-arith:bzn:thm:main}
\ClaimStatusConjectured\ \emph{(conditional beyond F1 and F2, i.e.\ beyond
...
global spectral and \(E_{2}\) inputs. The remaining inputs are the
identification of the full Weil/\((\varphi,\Gamma)\) parameter stack
with the inertia shadow and the profinite formal-loop continuity
required by the arithmetic Ben-Zvi--Nadler comparison.
\end{proof}

\section{Comparison Table}
\label{v4-arith:bzn:table}

The attempted proof of \Cref{v4-arith:bzn:thm:main} passes through
five independent conditions. The independent comparison identifies
which close and which yield residuals.

\begin{description}
\item[\textbf{MKR shadow and the inertia circle.}]
The classifying stack \(B\widehat{\mathbb{Z}}\) is the classifying
\(\infty\)-stack of the profinite abelian group \(\widehat{\mathbb{Z}}\);
its pro-\'etale fundamental group is
\(\pi_{1}^{\mathrm{\acute{e}t}}(B\widehat{\mathbb{Z}})=\widehat{\mathbb{Z}}\).
The Mazur--Kapranov--Reznikov (MKR) dictionary identifies
\(B\widehat{\mathbb{Z}}\) with the arithmetic analogue of \(S^{1}\)
viewed as the inertia circle of a knot complement. Closes
(\Cref{v4-arith:bzn:prop:MKR-inertia-circle}).

\item[\textbf{Loop-space identification for the inertia shadow.}]
For an \(n\)-geometric derived stack \(\mathcal{X}\), the arithmetic
free loop space \(L^{\mathrm{Ar}}\mathcal{X}:=\mathrm{Map}(B\widehat{\mathbb{Z}},\mathcal{X})\)
is a well-defined derived stack (To\"en--Vezzosi DAG framework). For
\(\mathcal{X}=BGL_{2}\), one computes
\(L^{\mathrm{Ar}}BGL_{2}=\mathrm{Hom}(\widehat{\mathbb{Z}},GL_{2})/GL_{2}\)
as a derived quotient stack. Closes
(\Cref{v4-arith:bzn:prop:L-ar-BGL2}).

\item[\textbf{Arithmetic transport of the Ben-Zvi--Nadler machine.}]
Ben-Zvi--Nadler prove
\(\mathrm{HH}_{\bullet}(\mathrm{IndCoh}(\mathcal{X}/G))=\mathcal{O}(L(\mathcal{X}/G))^{\mathrm{flat}}\)
over \(\mathbb{C}\) with \(S^{1}\) as the topological circle. The
proof factors through two ingredients: (i) the
Costello--Francis--Lurie identification of the Hochschild trace
of a symmetric monoidal \(\infty\)-category with factorization
homology over \(S^{1}\); (ii) the flatness assertion cutting out
ind-coherent sheaves inside functions on the loop space.
Ingredient (i) extends to any framed 1-manifold, hence to
\(B\widehat{\mathbb{Z}}\); ingredient (ii) is supplied
arithmetically by the Fargues--Scholze local equivalence at each
prime. Conditional
(\Cref{v4-arith:bzn:thm:HH-IndCoh-arith}).

\item[\textbf{Dunn additivity and the \(E_{1}\)-structure.}]
The global Iwahori Hecke category is \(E_{2}\)-monoidal by F2.
Dunn additivity (Lurie HA~5.1.2.2) supplies the canonical
\(E_{1}\)-algebra structure on \(\mathrm{HH}_{\bullet}\) of any
\(E_{2}\)-monoidal category, and the arithmetic Ben-Zvi--Nadler
map respects this structure via the factorization-homology
presentation. Closes under F2
(\Cref{v4-arith:bzn:prop:E1-on-HH}).

\item[\textbf{Full Weil/\((\varphi,\Gamma)\) target and the
\(\mathcal{V}^{\mathrm{prim}}\) pullback.}]
The structure sheaf on the full Weil/\((\varphi,\Gamma)\) flat target
should pull back along the Emerton--Gee comparison to
\(\mathcal{V}^{\mathrm{prim}}\) via the spectral-side identification
of F1. The forgetful map from the Emerton--Gee stack to the inertia
shadow \(\mathrm{LocSys}^{\mathrm{Ar}}_{GL_{2}}(B\widehat{\mathbb{Z}})\)
records only the procyclic restriction; the full Frobenius and
\((\varphi,\Gamma)\) data are retained only in the complete target.
Open
(\Cref{v4-arith:bzn:thm:pullback-to-V-prim}).
\end{description}

The independent criterion for each condition is recorded in the
...
\subsection{Source Comparison: MKR Dictionary and Inertia Circle}

\paragraph{Obstruction 1 (source separation).} \(B\widehat{\mathbb{Z}}\) is
defined via Bhatt--Scholze pro-\'etale formalism
...
\paragraph{Obstruction 2 (conventions).} Pro-\'etale fundamental group
convention: Bhatt--Scholze; matches Morishita's profinite inertia
convention.

\paragraph{Obstruction 3 (ambient categories).} \(B\widehat{\mathbb{Z}}\)
is a pro-\'etale \(\infty\)-stack. Derived stacks in the
To\"en--Vezzosi DAG setting admit pro-\'etale enhancements
(To\"en DAG \S2.3); so \(B\widehat{\mathbb{Z}}\) sits in
the same ambient category as the Emerton--Gee stack.

\paragraph{Obstruction 4 (missing hypothesis).} None: the definition
requires only that \(\widehat{\mathbb{Z}}\) be a profinite group,
which is classical.

\paragraph{Obstructions 5--7.} Vacuous (no functoriality claimed here,
no equivalence claimed here, no numerical computation).

The MKR dictionary condition closes as a definition of the procyclic
shadow; its arithmetic interpretation remains heuristic.
...
is the procyclic shadow of the full Fargues--Scholze or Emerton--Gee
flat parameter space; the full space retains Frobenius, Weil, and
\((\varphi,\Gamma)\) data beyond what the procyclic shadow records.
The provisional locus is cut out by:
\begin{enumerate}
...
\end{equation}
as a map of pro-\'etale derived stacks. Whether this inertia-only
restriction is a closed immersion is an open question: on points, the
Colmez--Fontaine theorem shows the full \((\varphi,\Gamma)\) data carries
strictly more information than the procyclic restriction.
\end{proposition}
...
\((\varphi,\Gamma)\) or Weil parameter retains Frobenius data beyond
the inertia restriction. The map \(\iota^{\mathrm{EG},p}\) is
surjective onto the locally-admissible \(\mathbb{Z}_{p}\)-representations
...
(Colmez--Fontaine 2000). The inertia restriction forgets Frobenius
and unramified data; the closed-immersion property accordingly
belongs to the full Weil/\((\varphi,\Gamma)\) target.
\end{proof}

\subsection{Source Comparison: Loop-Space Computation for \(BGL_{2}\)}

\paragraph{Obstruction 1 (source separation).} To\"en DAG
DAG; Bhatt--Scholze arXiv:1309.1198 pro-\'etale; Emerton--Gee
arXiv:1908.07185; Fargues--Scholze arXiv:2102.13459. All disjoint
from Ben-Zvi--Nadler.

\paragraph{Obstruction 2 (conventions).} \((\varphi,\Gamma)\)-module
convention: Fontaine. Local reciprocity convention: Artin arithmetic
normalisation sends a uniformizer to arithmetic Frobenius; geometric
Frobenius is its inverse and acts by \(p^{-1}\) on the Tate line. These
are the Emerton--Gee conventions; they match Fargues--Scholze after the
standard twist.

\paragraph{Obstruction 3 (ambient categories).} \(L^{\mathrm{Ar}}BGL_{2}\)
...
\paragraph{Obstruction 4 (missing hypothesis).} The
...
\Cref{v4-arith:bzn:def:flat-locus}. \emph{hypothesis
explicit.}

\paragraph{Obstruction 5 (false functoriality).} The Emerton--Gee
...
\paragraph{Obstruction 6 (unproved equivalence).} The closed-immersion
property requires the full Weil/\((\varphi,\Gamma)\) target:
\(B\widehat{\mathbb{Z}}\) records only the procyclic shadow, omitting
Frobenius and unramified data retained by the Colmez--Fontaine datum.
\textbf{Residual: full target comparison.}

\paragraph{Obstruction 7.} No numerical computation.

The loop-space condition closes for the procyclic mapping-stack
computation. The Emerton--Gee comparison to the full arithmetic target
remains open.
...
its role in (F1) Obstruction~2 (\Cref{v4-arith:cl:F1-closure}).
\end{remark}

\subsection{Source Comparison: Arithmetic Transport of Ben-Zvi--Nadler}

\paragraph{Obstruction 1 (source separation).} Francis arXiv:1206.5522
(factorization homology), Lurie HA~5.5.3 (Hochschild trace),
Ben-Zvi--Francis--Nadler arXiv:0805.0157 (integral transforms)
form one source list. Emerton--Gee, Fargues--Scholze,
Bhatt--Scholze, Gaitsgory--Rozenblyum form another. Ben-Zvi--Nadler
2009 is not used as a proof source; only its statement is
referenced. The proof uses disjoint machinery.

\paragraph{Obstruction 2 (conventions).} Factorization-homology
...
\paragraph{Obstruction 3 (ambient categories).} \(E_{2}\)-monoidal
...
\paragraph{Obstruction 4 (missing hypothesis).} Noetherianness and
...
properties are additional conditional hypotheses. \textbf{Residual
named.}

\paragraph{Obstruction 5 (false functoriality).} The arithmetic
...
\paragraph{Obstruction 6 (unproved equivalence).} Step 2 invokes
...
The arithmetic transport condition is conditional. The
pro-\'etale lift of Ben-Zvi--Francis--Nadler and the compatibility
with formal Noetherian Emerton--Gee stacks are residual inputs.
...
\begin{remark}[domain of F2-conditionality]
\label{v4-arith:bzn:rem:F2-domain}
...
\begin{proof}
The following is a conditional strategy; each step names its
...
\medskip\noindent\textbf{Step 4: Full Weil/\((\varphi,\Gamma)\) target comparison.}
The required comparison involves a morphism from the procyclic
loop space to the full Weil/\((\varphi,\Gamma)\) target.
\(\mathrm{Map}(B\widehat{\mathbb{Z}},\mathcal{X}^{\mathrm{Sel}}_{\bar\rho,\det,\mathcal{L}})\)
is the procyclic shadow of the arithmetic loop problem; the target
\(\mathrm{LocSys}_{GL_{2}}^{\mathrm{Ar}}(B\widehat{\mathbb{Z}})\)
records only the inertia restriction, while the full Weil group data
lives in \(\mathrm{LocSys}^{W,\varphi\Gamma}_{GL_{2}}\). The
required morphism is:
\begin{equation}
...
The unconditional content consists of the definition
of the procyclic shadow and Dunn additivity under F2. The arithmetic
Ben-Zvi--Nadler machinery, specifically
\Cref{v4-arith:bzn:thm:arith-fact-homology},
\Cref{v4-arith:bzn:thm:HH-IndCoh-arith},
\Cref{v4-arith:bzn:prop:EG-to-LocSys},
\Cref{v4-arith:bzn:prop:EG-image-flat}, carries
\ClaimStatusConjectured\ form until the full Weil/\((\varphi,\Gamma)\)
target and formal-loop continuity are established.
\end{remark}
...
has as its remaining input the corrected BZN comparison against the
...
The corrected BZN comparison and the full-target pullback are the
remaining inputs; their supply closes F3.
\end{proof}
...
the Emerton--Gee stack. The full-target comparison requires that the map from Emerton--Gee
to the Weil/\((\varphi,\Gamma)\) flat target has the required pullback
on structure sheaves; the inertia-only map to
\(\mathrm{LocSys}^{\mathrm{Ar}}_{GL_{2}}(B\widehat{\mathbb{Z}})\)
records only the procyclic restriction, and the full Weil group data
must enter through the complete target.
\end{proof}

\subsection{Source Comparison: Dunn Additivity and Full-Target Pullback}

\paragraph{Dunn additivity and \(E_{1}\)-structure.}

\paragraph{Obstruction 1 (source separation).} Lurie HA~5.1.2
(Dunn additivity) is a higher-algebra result, disjoint from
F2's perfectoid geometry.

\paragraph{Obstruction 2 (conventions).} \(E_{n}\)-operad convention:
Lurie HA (little cubes). \(E_{2}\)-monoidal category convention
matches Fresse's operadic conventions via the Dunn--Lurie theorem.

\paragraph{Obstruction 3 (ambient categories).} \(E_{2}\)-monoidal
stable presentable \(\infty\)-categories in
pro-\'etale derived geometry. Lurie HA applies unchanged in
this ambient category.

\paragraph{Obstruction 4 (missing hypothesis).} Stability and
...
\paragraph{Obstruction 5 (false functoriality).} Dunn additivity is a
natural transformation of \((\infty,2)\)-functors, functorial in
\(\mathcal{C}\). No malpractice.

\paragraph{Obstructions 6, 7.} Vacuous.

The Dunn additivity condition closes under F2.

\paragraph{Full Weil/\((\varphi,\Gamma)\) target and \(\mathcal{V}^{\mathrm{prim}}\) pullback.}

\paragraph{Obstruction 1 (source separation).} Pullback along closed
...
\paragraph{Obstruction 2 (conventions).} Structure-sheaf pullback
...
\paragraph{Obstruction 3 (ambient categories).} Ind-coherent sheaves
...
\paragraph{Obstruction 4 (missing hypothesis).} The identification of
\(\mathcal{V}^{\mathrm{prim}}\) with the Emerton--Gee structure
sheaf requires F1 (spectral-side reading). \emph{hypothesis
explicit as F1.}

\paragraph{Obstruction 5 (false functoriality).} The pullback
\(\iota^{\mathrm{EG},*}\) is the standard functoriality, which
preserves structure sheaves (trivially: \(\iota^{*}\mathcal{O}_{Y}=\mathcal{O}_{X}\)
for any morphism \(\iota\colon X\to Y\)).

\paragraph{Obstruction 6 (unproved equivalence).} The identification of
\(\mathcal{V}^{\mathrm{prim}}\) with the Emerton--Gee structure
...
Fargues--Scholze; F1 globalises. \emph{Conditional on F1.}

\paragraph{Obstruction 7.} No numerical computation.

The full-target pullback condition closes under F1.

\section{Independent Comparison Path}
\label{v4-arith:bzn:independent-verification}

The comparison path for
\Cref{v4-arith:bzn:thm:main-body}.

\subsection{Proof Source}
...
\item To\"en DAG framework.
\end{itemize}

\subsection{Comparison Source}

The independent comparison uses:
\begin{itemize}
\item Mazur 2000 Selecta, Morishita arXiv:0904.3399 (MKR
dictionary);
\item Kapranov 1996 (arithmetic topology);
\item Antieau--Nikolaus (TR, THH, arithmetic
traces);
...
\begin{proposition}[disjointness of the two source lists]
...
lists are independent in the Vol~IV orthogonal sense:
\begin{enumerate}
...
or Hochschild-trace machinery. Its invariant is independent of
Ben-Zvi--Francis--Nadler's \(\mathrm{IndCoh}\)/loop-space identity.
\item Antieau--Nikolaus compute THH, TC, TR for
...
\subsection{Comparison Sketch}

\begin{remark}[the comparison path]
\label{v4-arith:bzn:rem:verification-sketch}
A non-formal comparison theorem would proceed as follows.
\begin{enumerate}
...
\end{enumerate}
The domain of this comparison is deliberately smaller than the
non-formal theorem main proof: the three steps above constitute a
\emph{consistency check}, showing that the arithmetic
Ben-Zvi--Nadler equivalence of \Cref{v4-arith:bzn:thm:main-body}
is consistent with independent computations from disjoint source
lists.
\end{remark}
...
The independent comparison leaves F3 with genuine residual conditions.
...
\begin{remark}[residual F3 ingredients]
\label{v4-arith:bzn:rem:what-remains}
The open ingredients are:
\begin{itemize}
...
two are bar-cobar-structural conditions internal to the arithmetic
Heisenberg.
\end{remark}

\subsection{Domain: Archimedean Place}
...
\subsection{Domain: Non-\(GL_{2}\) Groups}

\begin{remark}[extension to \(GL_{n}\)]
\label{v4-arith:bzn:rem:GLn}
The machinery extends to \(G=GL_{n}\) for any \(n\geq 1\),
since:
\begin{itemize}
...
\section{Ben-Zvi--Nadler Boundary}
...
forgetful procyclic shadow; whether the restriction is a closed
immersion is an open question
(\Cref{v4-arith:bzn:prop:EG-to-LocSys},
...
\(\mathcal{X}\); this remains conditional for formal Noetherian
...
shadow.
\item F3 remains conditional after F1 and F2
...
pullback.
\item The independent comparison path uses disjoint sources
...
(Chapter~\ref{v4-arith:synthesis}) remains conditional. F1 and
F2 reduce F3 to a sharper residual: construct the corrected arithmetic
BZN equivalence over the full Weil/\((\varphi,\Gamma)\) target and
prove the profinite formal-loop and flat-locus comparisons.
