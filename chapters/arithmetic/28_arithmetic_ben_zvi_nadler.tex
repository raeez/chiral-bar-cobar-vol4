...
and the obstruction exposed by the attack--heal swarm. The
...
attack--heal cycles. The swarm found that several cycles produce
...
\subsection{The Emerton--Gee Morphism}

\begin{proposition}[Emerton--Gee to LocSys morphism]
...
\iota^{\mathrm{EG}}\;\colon\;\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}\;\longrightarrow\;\mathrm{LocSys}^{\mathrm{Ar}}_{GL_{2}}\bigl(B\widehat{\mathbb{Z}}\bigr)
\end{equation}
...
Globally, the assembly over all primes is the inductive limit
$\iota^{\mathrm{EG}}=\varinjlim_{S}\prod_{p\in S}\iota^{\mathrm{EG},p}$
indexed by finite sets of primes; this is compatible with the
(F1.c) presentation of $\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}$
as an ind-formal algebraic stack.
\end{proof}
...
\begin{proposition}[image of the Emerton--Gee morphism]
\label{v4-arith:bzn:prop:EG-image-flat}
\ClaimStatusConjectured. The full Emerton--Gee comparison morphism
is expected to factor through the flat full-parameter locus. The
forgetful inertial shadow $\iota^{\mathrm{EG}}$ of
\Cref{v4-arith:bzn:prop:EG-to-LocSys} factors through the flat
locus
\begin{equation}
\iota^{\mathrm{EG}}\colon\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}\longrightarrow\mathrm{LocSys}^{\mathrm{Ar}}_{GL_{2}}(B\widehat{\mathbb{Z}})^{\mathrm{flat}}
\end{equation}
...
The almost-everywhere-unramified condition (2) is the restriction
of $\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}$ to the finite-type
Noetherian locus (Emerton--Gee Thm.~B.0.1), which automatically
has almost-all-unramified structure at primes outside a finite
ramification set.
...
\subsection{Attack--Heal Trace for Cycle 2}
...
and $\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}$ both live in
pro-\'etale derived stacks; the morphism $\iota^{\mathrm{EG}}$
...
\subsection{Attack--Heal Trace for Cycle 3}
...
global Emerton--Gee stack satisfies these (Emerton--Gee
arXiv:1908.07185 Thm~B.0.1). \textbf{Pass, hypothesis explicit.}
...
$\mathrm{IndCoh}_{\mathrm{Nilp}}$ of the Emerton--Gee stack.}
If F1 is proved after the local comparison and adelic descent
residuals, it gives
\begin{equation}
\mathrm{Hk}^{\mathrm{Iw}}_{GL_{2},\mathrm{glob}}\;\simeq\;\mathrm{D}([GL_{2}])^{\mathrm{sph\text{-}fin}}\;\simeq\;\mathrm{IndCoh}_{\mathrm{Nilp}}\bigl(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}\bigr).
\end{equation}
...
$\mathcal{X}=\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}$ (Noetherian
formal algebraic by Emerton--Gee), it yields
\begin{equation}
\mathrm{HH}_{\bullet}\bigl(\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})\bigr)\;\simeq\;\mathcal{O}\bigl(L^{\mathrm{Ar}}\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}\bigr)^{\mathrm{flat}}.
\end{equation}
...
L^{\mathrm{Ar}}\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}
\longrightarrow
\mathrm{LocSys}^{W,\varphi\Gamma}_{GL_{2}},
\end{equation}
whose pullback on flat functions must identify
$\mathcal{O}(L^{\mathrm{Ar}}\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})^{\mathrm{flat}}$
with
...
\begin{align}
\mathrm{HH}_{\bullet}(\mathrm{Hk}^{\mathrm{Iw}}_{GL_{2},\mathrm{glob}})\;&\overset{\text{F1}}{\simeq}\;\mathrm{HH}_{\bullet}(\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}))\\
&\overset{\text{arith BZN}}{\simeq}\;\mathcal{O}(L^{\mathrm{Ar}}\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})^{\mathrm{flat}}\\
...
\subsection{Attack--Heal Trace for Cycles 4 and 5}
...
The decorator discipline of Vol~IV (see the CLAUDE.md Part~B
protocol) requires a disjoint verification path for each
inscribed theorem. We exhibit one for
...
$L^{\mathrm{Ar}}\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}$ corresponds
...
The attack--heal run leaves F3 with genuine residual obligations.
...
\item The Emerton--Gee stack $\mathcal{X}^{\mathrm{EG,glob}}_{GL_{n}}$
is defined for all $n$ in Emerton--Gee arXiv:1908.07185.
