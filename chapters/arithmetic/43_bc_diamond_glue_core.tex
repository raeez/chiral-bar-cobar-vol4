...
the global Bernstein-centre pullback under the diagonal restriction of
...
Chenevier-level statement amenable to primary-source attack via
...
Assembling the three preceding reductions: local
...
\begin{remark}[scope of the local Bernstein identification]
...
\(\mathrm{IndCoh}_{\mathrm{Nilp}}\) is computed by the global-function
ring over which it is defined % RECTIFICATION-FLAG: citation needed;
% "DAG V" is Lurie arXiv:0905.0459 "Structured Spaces"; the specific
% centre-computation result may be Gaitsgory's 2023 notes on
% IndCoh_Nilp or Lurie HA 5.3 -- verify before finalizing.
. On the spectral side, the centre is
...
(cf.\ \Cref{v4-arith:acd:thm:L2-reduction}% RECTIFICATION-FLAG:
% verify that L2-reduction governs this adelic product;
% global trace-class decomposition vs adelic Bernstein centre
% may require a more specific reference from Bernstein 1984 or
% Flicker--Kazhdan 1988 for the adelic restricted tensor product.
).
...
local BZCHN realisation
(\Cref{v4-arith:bcglue-core:thm:local-bernstein}), the original
two-prime diagonal restriction equality in
\(\mathrm{Coh}(\mathcal{X}^{\mathrm{EG},\{p,q\}}_{G})\) follows by
descent along the GL\(_{2}\times GL_{2}\)-invariant quotient.
...
Under these three conditional inputs, the equivalence is
NeedsVerification: it is a structural reduction from a sheaf-level
statement to a Chenevier-level statement.
\end{proof}
...
categorical-Langlands-compatible way.% RECTIFICATION-FLAG: citation
% needed; the centre-determines-derived-Picard principle on each
% Bernstein block should cite Gaitsgory's IndCoh notes or Lurie HA 5.3
% -- not "DAG V" (Lurie arXiv:0905.0459 "Structured Spaces");
% verify and replace.
...
and a centre-determination principle (established in Gaitsgory's
theory of IndCoh but conjectural in the arithmetic setting with
\(\overline{\mathrm{Spec}\,\mathbb{Z}}\) playing the role of the
curve) on the backward direction.
\end{proof}
...
statement amenable to primary-source attack via Chenevier 2014 and
...
\begin{remark}[status boundary for (BCglue.core)]
...
commutative-algebra statement attackable by primary-source methods,
and naming the irreducible categorical residual (AG.det). The
former is genuine scope tightening; the latter is an honest scope
boundary.
\end{remark}
...
\begin{proof}
Assemble \Cref{v4-arith:bcglue-k5:cor:LACD-tight-catalog} with
...
scope-tightened commutative-algebra core and (AG.det) is the
irreducible categorical residual.
\end{proof}

\begin{remark}[scope reduction of (BCglue.core): quantitative summary]
\label{v4-arith:bcglue-core:rem:scope-delta}
Prior to \Cref{v4-arith:bcglue-core:thm:core-final-reduction}, the
residual (BC-diamond-glue.core) was \(10\)--\(15\) pp Conjectural
conditional on arithmetic AG. The
reduction of \Cref{v4-arith:bcglue-core:thm:core-final-reduction}
brings it to \(5\)--\(8\) pp Conjectural for (Ch1) plus (AG.det), the
arithmetic AG centre statement. The quantitative tightening:
\begin{itemize}
\item Pairwise sheaf content: \(10\)--\(15\) pp \(\to\) \(5\)--\(8\) pp
(a \(40\)--\(50\%\) reduction), by moving from the coherent-sheaf
level to the Chenevier pseudocharacter ring level via
\Cref{v4-arith:bcglue-core:thm:global-bernstein-chenevier}.
...
\end{itemize}
The decomposition isolates an attackable commutative-algebra component
(Ch1) and a named irreducible conjecture (AG.det); this is the minimal
residual achievable without inscribing new global results.
\end{remark}

\begin{remark}[HZ-IV realization status of (Ch1) and (AG.det)]
\label{v4-arith:bcglue-core:rem:realization-discipline}
Under the HZ-IV realization protocol
(\Cref{v4-arith:platonic:rem:sources}):
(Ch1) is the candidate for a \texttt{RealizedHere} decorator
once a primary-source transition theorem is written with disjoint
source catalogs Chenevier 2014 and Bhatt--Scholze arXiv:1905.08229 /
Bhatt--Lurie arXiv:2201.06120 (prismatic cohomology).
(AG.det) carries a \texttt{ScopeRestricted} or \texttt{Conditional}
tag, since realization requires a primary-source closure of arithmetic
AG. The scope reduction is inscribed at \ClaimStatusNeedsVerification;
the residual (AG.det) is inscribed at \ClaimStatusConjectured.
\end{remark}
...
(\Cref{v4-arith:bcglue-core:thm:colimit-to-global-reduces-to-AG}).
\item The scope tightening of
\Cref{v4-arith:bcglue-core:thm:core-final-reduction} shrinks the
pairwise-sheaf content of (BCglue.core) from \(10\)--\(15\) pp to
\(5\)--\(8\) pp (a \(40\)--\(50\%\) reduction), isolating the irreducible
residual as (AG.det)
