...
\emph{Route~4 attack-and-repair on the smallest $\mathrm{L}_{\mathrm{ACD}}$
residual. Route~1 Ch.~\ref{v4-arith:bcglue-k5} reduced the
...
Four attack-and-repair cycles are executed; each produces a sharper
...
\begin{proof}
Assemble
\Cref{v4-arith:bcglue-k5:cor:LACD-tight-catalog} (route~2
tight catalog) with
\Cref{v4-arith:bcglue-core:thm:core-final-reduction} (route~4 core
...
\begin{remark}[route~4 scope delta]
\label{v4-arith:bcglue-core:rem:scope-delta-route~4}
Route~3 left (BC-diamond-glue.core) at $10$--$15$ pp Conjectural
conditional on arithmetic AG. Route~4 (the present chapter) reduces
...
\end{itemize}
The route~4 reduction isolates the residual into an attackable
...
(\Cref{v4-arith:platonic:rem:sources} analogue for Vol~IV):
...
\item The route~4 scope tightening shrinks the pairwise-sheaf content
...
$\mathrm{L}_{\mathrm{ACD}}$ after route~4 is (Ch1) (amenable at the
