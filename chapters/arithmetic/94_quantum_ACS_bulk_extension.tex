...
finite level, iii on the open \(\mathrm{Spec}\,\mathbb{Z}\setminus\{\infty\}\))
...
problem in the Clausen--Scholze condensed-mathematics framework.}
...
on the archimedean boundary, where \(\mathcal{V}^{\mathrm{prim}}\) is the Hochschild
trace class of the identity on the global arithmetic Iwahori--Hecke category of
\(GL_{2}\) (Ch.~\ref{v4-arith:3d-acs-E1bar});
...
is a cocycle on the congruence tower.
\end{proposition}
...
disjoint from Bost--Connes engines
(Ch.~\ref{v4-arith:connes-soibelman}). No conflict.
\textbf{Pass.}
...
state-sum over triangulations. If the category
\(\mathcal{MTC}^{\mathrm{Ar}}\) of admissible modules for
\(\mathcal{V}^{\mathrm{prim}}\) at level \(k^{\mathrm{Ar}}\) is modular,
this route produces a 3D arithmetic Chern--Simons directly.
...
along \(\Sigma_{\infty}\), this trace is the arithmetic Zhu regularised
character of \(\mathcal V^{\mathrm{prim}}\) in the sense of
Ch.~\ref{v4-arith:virasoro-bootstrap}. The regularised-character identification
with \(\xi\) is \Cref{v4-arith:3d-acs:cor:partition-bar}.
\end{proof}

\subsection{Attack-heal on Route II}
\label{v4-arith:w10-a:ah:route-II}

\paragraph{Attack 1 (source collision).} Reshetikhin--Turaev 1991 and Turaev 1994;
Huang--Lepowsky 1994--1999 (MTC from VOA, in the complex-analytic setting
where the relevant object is a vertex operator algebra; arithmetic extension
is the residual); arithmetic Zhu of Ch.~\ref{v4-arith:virasoro-bootstrap}.
Three disjoint sources. \textbf{Pass.}

\paragraph{Attack 2 (sign/convention).} The RT quantum dimension is positive
on a modular category; the Wilson-loop trace is the quantum trace.
...
\paragraph{Attack 5 (false functoriality).} The
...
\paragraph{Attack 7 (engine-sync).} No engine conflict. \textbf{Pass.}
...
\paragraph{Attack 1 (source collision).} Costello--Gwilliam 2017/2021; Milne 1980
\'etale cohomology; Kim 2015 ACS. Three disjoint sources. \textbf{Pass.}

\paragraph{Attack 2 (sign/convention).} \'Etale descent preserves tensor structure;
...
\paragraph{Attack 5 (false functoriality).} Functoriality in the level \(G_{N}\) is
preserved by the finite-groupoid pullback. \textbf{Pass.}

\paragraph{Attack 6 (unproved equivalence).} The extension
...
\paragraph{Attack 1 (source collision).} Lurie 2008 (cobordism hypothesis);
Douglas--Schommer-Pries--Snyder 2013 (dualizability in dimension 3); Turaev
1994 (RT). Three disjoint sources. \textbf{Pass.}

\paragraph{Attack 2 (sign/convention).} Framing and orientation conventions for
3-dimensional cobordism are fixed by Douglas--Schommer-Pries--Snyder
Thm.~1.1. \textbf{Pass.}

\paragraph{Attack 3 (ambient category).} Symmetric monoidal
\((\infty,3)\)-category \(\mathrm{ArithTC}\). Consistent. \textbf{Pass.}
...
\paragraph{Attack 5 (false functoriality).} Functoriality in the MTC is by construction.
\textbf{Pass.}

\paragraph{Attack 6 (unproved equivalence).} The arithmetic-cobordism / smooth-cobordism
...
\paragraph{Attack 1 (source collision).} Scholze 2020 (diamonds); Clausen--Scholze 2020
(condensed mathematics); Fargues--Scholze 2021 (geometric Langlands).
Three disjoint sources. \textbf{Pass.}

\paragraph{Attack 2 (sign/convention).} Six-functor sign convention is fixed by
Scholze lecture notes; proper-support and shriek-pull conventions match
...
\paragraph{Attack 5 (false functoriality).} Functoriality of \(f^{!}\) under refinement of
diamonds is by the naturality of the six-functor formalism. \textbf{Pass.}

\paragraph{Attack 6 (unproved equivalence).} The equivalence of the six-functor ACS with
the Reshetikhin--Turaev state-sum is at the level of Fargues--Scholze
speculation. \textbf{Residual.}

\paragraph{Attack 7 (engine-sync).} No engine conflict. \textbf{Pass.}

\begin{theorem}[Route V conditional closure]
\label{v4-arith:w10-a:thm:route-V-closure}
\ClaimStatusProvedHereConditional \emph{(on the arithmetic diamond of
...
character identification; but it does not supply A-TP positivity
of the scaling operator. Under both descent and A-TP, RH follows.
The archimedean Hilbert descent is the bulk-construction
obstruction; A-TP is the separate positivity obstruction. After the descent,
no further bulk obstruction remains: A-TP is the only RH-equivalent
open condition.
\end{remark}
...
\begin{theorem}[condensed-Hilbert descent equivalent to the quantum ACS conjecture]
...
computed by the regularised trace character (Ch.~\ref{v4-arith:virasoro-bootstrap}), hence by
...
The seven-point attack-and-repair audit of
\Cref{v4-arith:w10-a:thm:common-residual} is as follows.
...
\(\chi(\overline{\mathrm{Spec}\,\mathbb Z})=0\), so the renormalisation
factor \(|G_{N}|^{-\chi}\) is trivial; the Arakelov K\"ahler
...
Three residuals remain at the close of this chapter: each is a
precise formulation of an obstruction already identified in the five-route analysis.
...
Five construction paths to
\(\mathrm{ACS}^{\mathrm{qu}}_{k^{\mathrm{Ar}}}\) converge on a single
obstruction (\Cref{v4-arith:w10-a:thm:common-residual}), and that
obstruction is equivalent to the conjecture itself
(\Cref{v4-arith:w10-a:thm:descent-equivalent}): the archimedean
condensed-Hilbert descent of
\Cref{v4-arith:w10-a:conj:condensed-descent}. Routes I and III close
unconditionally on the finite-level and open-curve parts; Routes II, IV, V
reduce to modularity of \(\mathcal{MTC}^{\mathrm{Ar}}\) or the
arithmetic diamond, each a strictly stronger sufficient condition.
Under the descent, both Wilson observables become unconditional; under the
descent together with A-TP, the Riemann Hypothesis holds
(\Cref{v4-arith:w10-a:thm:descent-plus-ATP-equals-RH}).

The bulk-construction residual of the arithmetic chiral-algebra
programme is a single well-posed condensed-mathematics
statement. The archimedean fibre is the load-bearing
