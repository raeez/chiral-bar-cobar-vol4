...
This chapter closes all four by first-principles tools: Pontryagin
...
holds unconditionally on the abelian Heisenberg sector, conditional
only on the non-commutative Iwahori extension residual of
\S\ref{v4-arith:pos:att-6-noncommutative}.
...
repairs: \(p\cdot\mathrm{id}_{M}\) has \((p\cdot\mathrm{id})\otimes\mathbb{F}_{p}=0\)
...
resolution of \(\overline{V}\)). Uniqueness up to isomorphism is the
conservativity of \Cref{v4-arith:w5-d:lift:thm:faithful}: any two lifts
inducing the same map mod \(p\) are isomorphic.
% RECTIFICATION-FLAG: the base change -(x)F_p is NOT an equivalence
% (the source D^b(Lambda_p-mod) is far larger than the target). The
% lifting is essential surjectivity (derived completion) plus uniqueness
% of the derived-complete lift (conservativity); the phrase "full
% compact-object equivalence" overstated this. Verify that
% Cref{v4-arith:pos:prop:lifting} needs only existence+uniqueness of the
% derived-complete lift, not a categorical equivalence.
\end{proof}
...
Parshin--Gersten cobar; Consani--Marcolli \S2.5 Thm.~2.4%
% RECTIFICATION-FLAG: Gate 1, load-bearing citation. Consani--Marcolli
% 2006 \S2.5 Thm.~2.4 identifies E^BC with the C*-algebraic completion
% of the arithmetic cobar sheaf. Verify the precise statement of
% Thm.~2.4 in Noncommutative Geometry, Arithmetic, and Related Topics
% (Johns Hopkins 2006) against the usage here before publication.
). Under this
...
previously inscribed as \ClaimStatusConjectured, is realized
unconditionally by \Cref{v4-arith:w5-d:bc-koszul:thm:main}.
\end{proof}
...
arithmetic Heisenberg bar. The equivalence holds
unconditionally on the abelian Heisenberg sector.
\end{proof}
...
\ClaimStatusProvedHereConditional. The biconditional
\begin{equation}
...
\end{equation}
of \Cref{v4-arith:thm:G-row-Riemann-functional-equation} holds
unconditionally on the abelian Heisenberg sector, conditional only
on the non-commutative Iwahori extension of
\S\ref{v4-arith:pos:att-6-noncommutative}, which is explicit
residual scope outside the abelian Heisenberg application.
\end{corollary}

\begin{proof}
\Cref{v4-arith:pos:thm:weight-comp} combined with
\Cref{v4-arith:w5-d:combined:thm:closure} gives both directions of
the biconditional unconditionally on the abelian sector. The
non-commutative Iwasawa residual of
\S\ref{v4-arith:pos:att-6-noncommutative} lies outside the abelian
Heisenberg scope and is not removed by the arguments of this chapter.
\end{proof}
...
\S\ref{v4-arith:pos:att-6-noncommutative}, which is explicit
out-of-scope. For the abelian Heisenberg
...
The five sub-item closures proceed by mutually disjoint primary
sources, so the full arithmetic Positselski closure
(\Cref{v4-arith:w5-d:combined:thm:closure}) rests on five
