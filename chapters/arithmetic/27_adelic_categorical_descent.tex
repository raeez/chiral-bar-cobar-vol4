...
is the load-bearing adelic gluing statement for conjecture F1 (global
arithmetic Arinkin--Gaitsgory for \(GL_{2}/\mathbb{Q}\)), with
estimated closure scope of \(80\)--\(120\) pages
(App.~\ref{v4-arith:app:three-conjectures-dep}). This chapter decomposes
\(\mathrm{L}_{\mathrm{ACD}}\) into four sub-obligations
\((L_{1})\)--\((L_{4})\), closes \((L_{1})\) and \((L_{4})\)
unconditionally from primary literature (Raskin JAMS 2021;
Jacquet--Langlands SLN 114), reduces \((L_{2})\) to the local
categorical comparison plus a Whittaker-model compatibility, and
identifies \((L_{3})\) with a single Bernstein-centre
diamond-gluing compatibility on local coherent Springer sheaves over the
Selmer-derived global spectral stack (Ben-Zvi--Chen--Helm--Nadler
arXiv:2010.02321). The main result
(\Cref{v4-arith:acd:thm:final-reduction}) is that \(\mathrm{L}_{\mathrm{ACD}}\)
is equivalent to this single compatibility.
...
Six independent lines of attack on \(\mathrm{L}_{\mathrm{ACD}}\) are
executed below. Each line produces a structural decomposition, a partial
closure, or a named residual, following the schema of
\Cref{v4-arith:cl:protocol}.

\subsection{Modular Decomposition of \(\mathrm{L}_{\mathrm{ACD}}\)}
\label{v4-arith:acd:attack-1}

\(\mathrm{L}_{\mathrm{ACD}}\) packages four ingredients into a single
statement:
...
decomposition admits four disjoint sub-obligations,
\((L_{1})\)--\((L_{4})\), each a precise statement at the
\(\infty\)-categorical level, disjoint from the others, closable by a
named argument.
...
\subsection{Function-Field Precedent and Transport Obstructions}
\label{v4-arith:acd:attack-2}

In the function-field case (base curve \(X\) over \(\mathbb{F}_{q}\))
the analogue of \(\mathrm{L}_{\mathrm{ACD}}\) is a theorem, due to the
Gaitsgory--Rozenblyum--Raskin--Lin collective work (2019--2024),
culminating in the proof of the geometric Langlands conjecture by
Arinkin--Gaitsgory--Kazhdan--Raskin--Rozenblyum--Varshavsky (AGKRRV,
2020--2024). The function-field proof does not transport directly to
the number-field case. The obstructions are:
\begin{enumerate}
...
\((L_{1})\) and \((L_{4})\) close from the primary literature (Lurie HA,
Jacquet--Langlands); \((L_{2})\) reduces to function-field transport plus
a single Whittaker-model compatibility; the remaining content is
...
\Cref{v4-arith:acd:thm:final-reduction}. Its scope:
\begin{itemize}
...
(BC-diamond-glue), these established results constitute the primary
literature input for \(\mathrm{L}_{\mathrm{ACD}}\).
\end{proposition}
...
as the complete obstruction to F1.
\end{proof}
...
\(\mathcal{O}(\mathcal{X})\) on the constant-loop locus
(\(\mathrm{Map}(S^{1},\mathcal{X})\to\mathcal{X}\) along the constant
map \(S^{1}\to\mathrm{pt}\)). Tracking through:
...
\subsection{Remaining Residual}

After the reduction of \Cref{v4-arith:acd:thm:final-reduction}, the
remaining obstruction to full closure of \(\mathrm{L}_{\mathrm{ACD}}\)
and hence F1 is:
...
spectrally identifies, at the level of categorical trace, with the
structure sheaf on the Selmer-derived global spectral stack
...
\(\mathrm{L}_{\mathrm{ACD}}\) reduces from a single large conjecture to
three disjoint closed sub-obligations plus one narrower residual
(BC-diamond-glue). The structural content, that F1 is equivalent to
BC-diamond-glue, is new and inscribed here; it decomposes the F1
obligation to its minimal sharp form, supported by eight disjoint
...
RH-closure spine (Spine B). (BC-diamond-glue) closes the categorical-trace presentation of
\(\mathcal{V}^{\mathrm{prim}}\); the RH-closure spine (Spine B)
...
after the reduction of \Cref{v4-arith:acd:thm:final-reduction}, this
entry retires to (BC-diamond-glue), with revised scope \(30\)--\(50\) pp.
\end{itemize}
