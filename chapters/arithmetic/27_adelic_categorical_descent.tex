...
\emph{This chapter executes the attack-and-repair protocol on the single
...
Springer sheaves over the Selmer-derived global spectral stack. The final
...
\label{v4-arith:acd:attack-and-repair}

We execute six attack-and-repair cycles on $\mathrm{L}_{\mathrm{ACD}}$.
...
this automatic from the local Fargues--Scholze spectral action and
the expected F1.a comparison, or is there a non-trivial global
compatibility?
...
\begin{equation}
\mathcal{V}^{\mathrm{prim}}\;\leftrightarrow\;\mathcal{O}\bigl(\mathcal{X}^{\mathrm{Sel}}_{\bar\rho,\det,\mathcal{L}}\bigr)
\end{equation}
...
the structure sheaf of the Selmer-derived global spectral stack.
\end{corollary}
...
$\mathcal{V}^{\mathrm{prim}}\leftrightarrow\mathcal{O}(\mathcal{X}^{\mathrm{Sel}}_{\bar\rho,\det,\mathcal{L}})$.
\end{proof}
