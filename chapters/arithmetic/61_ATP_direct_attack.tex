...
equivalent to the Weil positivity form of the Riemann Hypothesis. This chapter attacks A-TP directly via elementary first-principles
...
irreducible obstruction: A-TP on the complementary class reduces,
via the Weil explicit formula identity, to the bounded-interval
inequality \(A[f]/(2\pi)\leq B[f]/(2\pi)+R[f]\), where \(A[f]\) is
...
The unconditional content is extracted; the irreducible obstruction is
named precisely; three further reductions within elementary analysis
are ruled out.}
...
chapter proves it in full. The obstruction's irreducible location is
established in \S\ref{v4-arith:w6-a:bounded-interval}.
...
\(g(t_{\ast})=0\).
% RECTIFICATION-FLAG: the numerical value t_*~0.794 was used illustratively
% throughout the ATP chapters (this chapter and Chs 72-80: core boundaries
% [-t_*,t_*], the prefactor in the HFD majorant, the L^1 estimate ~0.521).
% That value is WRONG; the mpmath-verified sign change is
% t_*=0.850662304 (g(0.81)=-0.086<0, not +0.035). Since t_* enters the
% attacks only symbolically via g(t_*)=0 (this remark), no load-bearing
% theorem is affected, but each illustrative 0.794 instance should be
% updated to 0.8507 and the dependent illustrative constants (the HFD
% prefactor, the L^1 bound) recomputed with the correct core width.
\end{remark}
...
inequality. Three apparent routes to close this inequality are
...
to the following bounded-interval positivity statement \((\star)\):
\begin{equation}
...
\Cref{v4-arith:w5-a:rem:H3-removable}; the finite-prime term satisfies
\(W_{\mathrm{fin}}(f*\widetilde{f})\geq 0\) by
\Cref{v4-arith:w5-a:thm:prime-side}. Statement \((\star)\) asserts
that the weighted \(L^{2}\)-mass of \(\widehat{f}\) on
\([-t_{\ast},t_{\ast}]\) against \(-g\) is bounded above by the
high-frequency tail mass and \(R[f]\).
\end{theorem}
...
\end{equation}
A-TP states \(W(f*\widetilde{f})=\sum_{\rho}|\widehat{f}(\gamma_{\rho})|^{2}\geq 0\).
...
The non-trivial zero sum \(\sum_{\rho}|\widehat{f}(\gamma_{\rho})|^{2}\)
does not appear in \((\star)\); it is recovered from
\eqref{v4-arith:w6-a:eq:weil-rearranged} as a consequence:
\(\sum_{\rho}|\widehat{f}(\gamma_{\rho})|^{2}=R[f]-W_{\infty}(f*\widetilde{f})\geq 0\)
exactly when \((\star)\) holds. The original phrasing with
\(\sum_{\rho}\) on the right-hand side of the reduction is a
non-equivalent restatement; \((\star)\) above is the sharp form.
\end{remark}
...
\subsection{Three Elementary Routes, Ruled Out}
\label{v4-arith:w6-a:bounded-interval:ruled-out}

Three apparent routes to close \eqref{v4-arith:w6-a:eq:bounded-reduction-star}
...
only on \(\widehat{f}|_{[-t_{\ast},t_{\ast}]}\) is RH-equivalent, hence
unavailable by elementary analysis.
\end{proposition}

\begin{proof}
The right-hand side of \((\star)\) includes \(B[f]/(2\pi)\), which
depends on \(\widehat{f}\) for \(|t|>t_{\ast}\). Any reduction to
\(\widehat{f}|_{[-t_{\ast},t_{\ast}]}\) alone must set \(B[f]=0\),
i.e.~force \(\widehat{f}\equiv 0\) on \((t_{\ast},\infty)\). Under this
restriction \((\star)\) becomes \(A[f]/(2\pi)\leq R[f]\), equivalently
\(-W_{\infty}(f*\widetilde{f})\leq R[f]\), which by
\eqref{v4-arith:w6-a:eq:weil-rearranged} is
\(\sum_{\rho}|\widehat{f}(\gamma_{\rho})|^{2}\geq 0\). The first
non-trivial zero of \(\zeta\) has ordinate
\(\gamma_{1}\approx 14.135>t_{\ast}\), so a restricted \(f\) with
\(\widehat{f}\equiv 0\) on \((t_{\ast},\infty)\) contributes zero to the
zero sum unconditionally; the reduced statement says \(0\geq 0\) and is
trivially true but carries no content. Removing the \(B[f]\) term from
\((\star)\) for general \(f\in\mathrm{PW}^{\mathrm{hol}}_{F}\) is
...
The direct elementary attack yields the following partition.
...
\begin{enumerate}
\item \emph{Spectral density of non-trivial zeros.} The zero-sum
\(\sum_{\rho}|\widehat{f}(\gamma_{\rho})|^{2}=R[f]-W_{\infty}(f*\widetilde{f})\)
(from \eqref{v4-arith:w6-a:eq:weil-rearranged}) requires precise
control of the zero density \(N(T)=\#\{\rho:|\gamma_{\rho}|\leq T\}\)
to evaluate the residue \(R[f]\) along a path disjoint from the
archimedean integral. The Riemann--von Mangoldt formula
\(N(T)=(T/2\pi)\log(T/2\pi)-T/2\pi+O(\log T)\) is classical; its
sharpening to an integrable weight against
\(|\widehat{f}(\gamma_{\rho})|^{2}\) is pursued in
...
The bounded-interval positivity statement \((\star)\) admits a de
...
Connes--Deninger routes. The direct elementary attack establishes
that the bounded-interval residual is the only irreducible content
...
open. The direct elementary attack (Chapter~\ref{v4-arith:w6-a:ATP-direct-attack})
refines this:
\begin{itemize}
...
bounded-interval inequality \(A[f]/(2\pi)\leq B[f]/(2\pi)+R[f]\) on
\([-t_{\ast},t_{\ast}]\), exactly equivalent to A-TP
...
content is located on a bounded interval and stated as a precise
inequality. The elementary ceiling is established; no further
elementary advance is available without entering one of the three
non-elementary programmes of \Cref{v4-arith:w6-a:rem:live-routes}.
\end{remark}
