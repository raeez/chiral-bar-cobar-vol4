...
arithmetic input is required, and F2.c closes unconditionally. The
mixed-characteristic Iwahori \(E_{2}\) local input F2.b remains open;
F2 itself is therefore theorem-waiting.
...
The derivation sources of \Cref{v4-arith:tate:thm:F2c-main} are
Lurie~HA~4.8 (tensor products of presentable \(\infty\)-categories) and
Fresse (integral \(E_{n}\) operad). The independent verification sources
...
derivation and verification sources are disjoint: Lurie~HA~4.8 does
...
\begin{attack}[filtered colimit of \(\mathcal{O}\)-algebras]
...
\begin{heal}[colimit preservation]
...
\begin{attack}[factorization axiom]
...
\begin{attack}[Dunn additivity compatibility]
...
\begin{heal}[operad-variable functoriality]
...
\begin{attack}[uniqueness modulo cofinal filtration]
...
\begin{heal}[cofinality invariance]
...
\begin{attack}[ramified archimedean places]
...
\begin{heal}[cofinite spherical locus]
...
This is the engine of colimit-preservation
...
Both routes around~\eqref{v4-arith:tate:eq:unital-incl-commute} differ
...
Factorization homology (Ayala--Francis) provides a verification path
...
A further independent verification comes from Ginot,
...
one answer; the source-disjointness requirement is satisfied.
\end{remark}
...
\begin{theorem}[closure of F2.c]
\label{v4-arith:tate:thm:F2-closure}
\ClaimStatusProvedHere. F2.c closes: assuming the local
mixed-characteristic Iwahori Hecke categories
\(\mathrm{Hk}^{\mathrm{Iw}}_{GL_{2},v}\) are already equipped with
...
\begin{remark}[epistemic status of F2]
\label{v4-arith:tate:rem:F2-status-final}
\Cref{v4-arith:tate:thm:F2-closure} promotes F2.c from conjecture
to theorem. F2 itself remains theorem-waiting: the local
mixed-characteristic Iwahori \(E_{2}\) enhancement F2.b is not yet
available, and downstream uses of F2 remain conditional on it.
\end{remark}
...
route; the derivation and verification sources do not share a common
sub-argument.
\item The restricted-tensor part F2.c closes unconditionally, but
F2 itself remains theorem-waiting pending the mixed-characteristic
Iwahori \(E_{2}\) local input. Any simplification of F3 that assumes
F2 remains conditional on that local input.
\end{enumerate}
...
itself remains theorem-waiting pending the local Iwahori
\(E_{2}\)-monoidal input.
