...
\emph{Chapter~\ref{v4-arith:w5-b:hp-operator} constructs the Bost--Connes
log-modular operator \(N_{\mathrm{BC}}=-\tfrac12\log\Delta_{1}\) with
unconditional self-adjointness and pure point spectrum
\(\{\log m\mid m\in\mathbb{N}^{\times}\}\). The target is a
Hilbert--P\'olya operator \(H_{\mathrm{HP}}\) whose spectral divisor,
through the regularised determinant of
\(\tfrac12+iH_{\mathrm{HP}}\), is the non-trivial zero divisor of
\(\xi_{\mathbb{R}}\). The intertwining operator \(A\) taking
\(N_{\mathrm{BC}}\) to \(H_{\mathrm{HP}}\) is the classical
Hilbert--P\'olya twist. The spectral-flow problem is a precise
operator-theoretic statement equivalent to the Connes archimedean
absorption-trace condition; the unconditional pieces are exactly the
Bost--Connes logarithmic Hamiltonian, the pole-cleared zeta
determinant, and their common scaling normalisation in the
triple-identification formalism of
Chapters~\ref{v4-arith:w8-a:deninger-chiral-homology},
\ref{v4-arith:w7-j:ATP-adelic-scattering}, and
\ref{v4-arith:3d-acs-E1bar}. The determinant-trace condition is not a
consequence of these unconditional pieces.}

\section{Setup: \(N_{\mathrm{BC}}\) and \(\Theta_{\xi}\) as Target Operators}
\label{v4-arith:w9-r4:setup}

The two endpoints of the spectral flow are the operators \(N_{\mathrm{BC}}\) and
...
intertwining reads
\(U|A|N_{\mathrm{BC}}|A|^{-1}U^{*}=H_{\mathrm{HP}}\) on its natural
...
\(U\,\exp(it N_{\mathrm{BC}})\,U^{*}=\exp(itH_{\mathrm{HP}})\). We use
...
explicit formula and its Hadamard inversion, which holds at the level
of tempered distributions on \(\mathbb{R}\) and is not an operator
identity.
...
\end{equation}
on \(\mathbb{R}_{\geq 0}\). The \emph{ordinate shadow} of the
zero-counting distribution is
\begin{equation}
\label{v4-arith:w9-r4:eq:mu-zero}
\mu_{\mathrm{ord}}\;:=\;\sum_{\rho\in\mathcal{Z}}\delta_{\gamma_{\rho}}
\end{equation}
on \(\mathbb{R}\). The actual Weil zero functional is the functional
on entire test functions
\[
\mathfrak Z_{\xi}(h):=\sum_{\rho\in\mathcal Z}h(z_{\rho}),
\qquad
z_{\rho}:=-i(\rho-\tfrac12).
\]
The equality \(\mathfrak Z_{\xi}(h)=\int_{\mathbb R}h\,d\mu_{\mathrm{ord}}\)
for all such \(h\) is equivalent to RH.
\end{definition}
...
For every even Paley--Wiener test function \(h\) for which the Weil
explicit formula is valid,
\begin{equation}
\label{v4-arith:w9-r4:eq:mellin-dual}
\mathfrak Z_{\xi}(h)\;=\;\int_{\mathbb{R}}h\,dW_{\infty}\;-\;2\sum_{m\geq 2}\frac{\Lambda(m)}{\sqrt{m}}\widehat{h}(\log m),
\end{equation}
with \(W_{\infty}\) the archimedean Weil density of
\eqref{v4-arith:w7-d:eq:W-arch}. If RH holds, then
\(\mathfrak Z_{\xi}(h)=\int_{\mathbb R}h\,d\mu_{\mathrm{ord}}\). Without
RH, this replacement is the projection \(z_{\rho}\mapsto\gamma_{\rho}\)
and is not the explicit formula.
\end{proposition}
...
\Cref{v4-arith:w9-r4:prop:mellin-dual} is a statement about a
zero-functional on entire test functions and a prime-side tempered
distribution on \(\mathbb{R}\). It is not an operator identity. In
particular, the duality does not by itself produce a linear map from
one Hilbert space to another; it produces a pairing between zero
samples and prime distributions that agrees up to the archimedean
term.
...
densely defined (Meyer 2005 \S3 construction) or (b) a direct
...
(Costello--Gwilliam construction). Neither is delivered unconditionally
...
\ClaimStatusProvedHere. For the critical-line Mellin completion
compatible with the spectral-flow condition, the Hadamard kernel
\(K_{\mathrm{Had}}\) is not Hilbert--Schmidt. Its
Hilbert-Schmidt norm
\(\|K_{\mathrm{Had}}\|_{\mathrm{HS}}^{2}=\sum_{\rho}\int_{\mathrm{Re}(s)=1/2}|\xi_{\mathbb{R}}(s)|^{-2}|s-\rho|^{-2}ds\)
diverges at every zero \(\rho\) lying on the critical line.
\end{lemma}

\begin{proof}
If \(\rho\) lies on the critical line, then
\(|\xi_{\mathbb{R}}(s)|^{-2}\sim C_{\rho}|s-\rho|^{-2m_{\rho}}\) along
the contour near \(s=\rho\). The integrand is then
\(|s-\rho|^{-2m_{\rho}-2}\), non-integrable on a one-dimensional
contour for \(m_{\rho}\geq 1\). Hardy's theorem gives infinitely many
zeros on the critical line. Hence the critical-line Hadamard residue
kernel is not Hilbert--Schmidt. Off-line zeros do not give this local
contour singularity; they are not needed for the conclusion.
\end{proof}

\begin{remark}[unboundedness is intrinsic]
\label{v4-arith:w9-r4:rem:unboundedness-intrinsic}
\Cref{v4-arith:w9-r4:lem:had-not-HS} rules out a Hilbert--Schmidt
Hadamard-residue kernel in the critical-line completion. A bounded
non-Hilbert--Schmidt operator is not thereby excluded. The unbounded
spectral-flow problem is the stronger demand imposed by the pole
residue prescription together with the zero-divisor determinant and
trace identities; it requires a closed densely defined operator and a
specified domain.
\end{remark}
...
\(\sum_{m\leq M}c_{m}e_{m}\) for \(M\) finite. The associated Dirichlet
polynomial has residues at every non-trivial zero after division by
\(\xi_{\mathbb{R}}\); this is a formal algebraic core, not a Hilbert
domain, unless the residue sequence is square-summable.
...
\begin{proof}
Item (1): a finite sum \(\sum_{m\leq M}c_{m}e_{m}\) maps formally under
\(A\) to
...
is a local expression involving derivatives of \(\xi_{\mathbb{R}}^{-1}\)
at \(\rho\). The sum over zeros is infinite; summability depends on
...
of \(D_{\mathrm{Connes}}\)'s absorption trace with the zero
distribution and critical-line support is equivalent to RH. In the
spectral-flow vocabulary, Connes supplies
...
``construct \(A\)'' to ``prove the right absorption trace for
\(D_{\mathrm{Connes}}\)''. The two questions have the same strength.
\end{remark}
...
Ch.~\ref{v4-arith:HPAr-vbconverse})}. The Connes absorption-trace
condition
(\Cref{v4-arith:w9-r4:thm:connes-absorption}) is equivalent to the
Hilbert--P\'olya hypothesis
H-P\(^{\mathrm{Ar}}\) (\Cref{v4-arith:rh:hyp:HPAr}).
\end{proposition}

\begin{remark}[the adele-class construction transfers the problem]
\label{v4-arith:w9-r4:rem:adele-transfers}
The Connes construction transfers the spectral-flow problem from the
...
candidate for the zero-divisor operator (scaling generator on cuspidal
subspace), but the intrinsic candidates are equivalent to the
abstract candidates of
...
the determinant-trace conjecture.
...
\begin{theorem}[Meyer determinant-trace intertwiner]
...
with \(H_{\mathrm{ord}}^{\mathrm{formal}}\); it intertwines
...
\ClaimStatusProvedElsewhere \emph{(Meyer 2005 \S5 conditional)}. The
Meyer determinant-trace conjecture (that
\(U_{\mathrm{Meyer}}\) extends continuously to a unitary intertwiner
carrying the Bost--Connes trace to the zero distribution) is equivalent
to the Connes absorption-trace conjecture
...
\(L^{2}(\mathbb{R}_{1/2+it})\). It does \emph{not} close the
	...
	\(\ell^{2}\)-sum over zeros; that extension is the Meyer determinant-trace
	conjecture, equivalent to Connes absorption trace, equivalent to
	H-P\(^{\mathrm{Ar}}\). Meyer therefore supplies the left half of the
	spectral flow (Mellin transform) unconditionally and leaves the
	right half (Hadamard inversion as determinant-trace realisation) as the
	conjectural kernel.
\end{remark}
...
The irreducible residual is the absorption-trace property of the
archimedean scaling on the cuspidal subspace, isolated and named below.
...
\(\mathcal{E}\subset\mathcal{H}^{\mathrm{id}}\), has absorption trace
equal to the zero distribution of \(\xi_{\mathbb R}\); after
zero-placement that trace is supported at
\(\{\gamma_{\rho}\mid\rho\in\mathcal{Z}\}\) with multiplicity.
Equivalently, the closure of \(U_{\mathrm{Meyer}}\) lifts
...
unconditional and obstructs the bare symbolic intertwining:
...
H-P\(^{\mathrm{Ar}}\) \(\Leftrightarrow\) Connes absorption trace \(\Leftrightarrow\)
Meyer determinant-trace matching \(\Leftrightarrow\) A-TP\(^{\mathrm{arch}}\) is
proven within the Vol~IV equivalence chain
...
it is the absorption-trace property, not derivable from the source-side
data or the Mellin unitary by any known argument.
\end{theorem}
...
\[
A\;=\;(\text{determinant-trace inclusion})\circ U_{\mathrm{Meyer}}\circ(\text{embedding }\mathcal{H}_{\mathrm{BC}}\to\mathcal{S}_{\mathrm{Meyer}}(\mathbb{A}))
\]
in which the embedding and \(U_{\mathrm{Meyer}}\) are unconditional,
while the determinant-trace inclusion into an \(\ell^{2}\)-direct sum
is the irreducible residual. The embedding
...
\eqref{v4-arith:w9-r4:eq:spec-id} requires the absorption-trace
...
each finite prime, the local trace matching is elementary: the
...
requires the scaling trace to be the zero distribution on the
cuspidal sub-space. With zero-placement this trace is supported at
\(\{\gamma_{\rho}\}\). The latter condition is equivalent to A-TP
...
finite-prime trace matching is delivered by Bost--Connes
unconditionally.
\end{remark}

\subsection{Spectral-Flow Logical Table}
...
Embedding \(\mathcal{H}_{\mathrm{BC}}\hookrightarrow\mathcal{S}_{\mathrm{Meyer}}(\mathbb{A})\) & Unconditional after Meyer renormalisation & \Cref{v4-arith:w7-j:rem:trace-class-domain} \\
...
Absorption trace of \(D_{\mathrm{Meyer}}|_{\mathcal{E}}\) equals the zero distribution & Conjectural (A-TP\(^{\mathrm{arch}}\)) & \Cref{v4-arith:w9-r4:thm:irreducible-residual} \\
Connes equivalence with H-P\(^{\mathrm{Ar}}\) & Proved equivalent & \Cref{v4-arith:w9-r4:prop:connes-eq-HPAr} \\
Meyer determinant-trace equivalence with A-TP\(^{\mathrm{arch}}\) & Proved equivalent & \Cref{v4-arith:w9-r4:thm:meyer-eq-HPAr} \\
...
the trace distribution of \(D_{\mathrm{Meyer}}|_{\mathcal{E}}\) is
determined by the Tate-Mellin transform of the cuspidal forms modulo
the residue structure at non-trivial zeros. The passage from pole data
to the zero trace distribution is precisely the determinant-trace
conjecture; any finer
reduction would require an \emph{a priori} realisation of the
cuspidal forms as an \(\ell^{2}\)-sum over non-trivial zeros, which
presupposes the critical-line placement. No unconditional construction
...
transfers the problem to an equivalent absorption-trace or positivity
statement, each equivalent to RH.

A direct obstruction: the bare symbolic intertwining
...
\end{enumerate}
H-P\(^{\mathrm{Ar}}\) remains the determinant-trace hypothesis. The
irreducible obstruction is the spectral-flow problem in the arithmetic
formalism.
\end{remark}
...
appears three more times in the Vol~IV arithmetic branch under
different names, in
Chapters~\ref{v4-arith:w8-a:deninger-chiral-homology},
\ref{v4-arith:w7-j:ATP-adelic-scattering}, and
\ref{v4-arith:3d-acs-E1bar}; the present section records these
identifications.
...
operator \(A\) is identified with the polar-unitary part of the
\(L_{0}\)-conjugacy only under the additional closed-range, domain
invariance, and BC--GNS Heisenberg-lift hypotheses. Under those
hypotheses the induced conjugacy carries \(N_{\mathrm{BC}}\) to the
zero-side operator \(H_{\mathrm{HP}}\) on the completed primary sector.
\end{proof}
...
\begin{proof}
The Meyer determinant-trace conjecture
...
\(\mathcal{H}_{\mathrm{BC}}\hookrightarrow\mathcal{S}_{\mathrm{Meyer}}(\mathbb{A})\)
of the spectral-flow setup, the absorption-trace condition
...
the level-derivative operator on the primary sector of
\(\mathcal{V}^{\mathrm{prim}}\) (the Hochschild trace class of the
identity on the global arithmetic Iwahori--Hecke category of \(GL_2\),
which is not a vertex algebra; the 3D ACS conjecture endows it with a
Hilbert-space completion on the bulk primary sector).
\end{proposition}
...
operator must have determinant divisor \(\mathrm{div}\,\xi_{\mathbb R}\)
and zero trace distribution under the
Arakelov-K\"ahler Wilson-surface realisation
(\Cref{v4-arith:w8-a:rem:wilson-infinity}). This is equivalent to
Deninger axiom (2), to Meyer determinant-trace matching, to Connes absorption,
...
\ClaimStatusProvedHereConditional \emph{(conditional on the referenced
...
the arithmetic formalism:
\begin{enumerate}
\item \emph{H-P\(^{\mathrm{Ar}}\)} (\Cref{v4-arith:rh:hyp:HPAr}):
existence of a self-adjoint operator \(H_{\mathrm{HP}}\) with
canonical spectral resolution on a Petersson completion and
\[
\det\nolimits_{\infty}\bigl(s\mathrm{id}-(\tfrac12\mathrm{id}+iH_{\mathrm{HP}})\bigr)
\doteq \xi_{\mathbb R}(s)
\]
after removal of the polar Tate factors.
\item \emph{Connes absorption}
(\Cref{v4-arith:w9-r4:thm:connes-absorption}): absorption trace of
the scaling generator on the cuspidal subspace of the adele class
space.
\item \emph{Meyer determinant-trace matching}
...
\begin{proof}
In the arithmetic formalism, the five equivalences are:
...
\begin{remark}[spectral-flow boundary]
\label{v4-arith:w9-r4:rem:honest-summary}
The spectral-flow problem \(N_{\mathrm{BC}}\rightsquigarrow H_{\mathrm{HP}}\)
is the classical Hilbert--P\'olya twist. The five formulations above
are equivalent after their stated hypotheses; each supplies different
geometric or analytic structure, but none supplies the residual
operator. The residual is irreducible in this formalism: it is the
absorption-trace property of the archimedean scaling on the cuspidal
subspace, equivalent to RH.

The unconditional content is the Bost--Connes logarithmic Hamiltonian,
the Mellin unitary on Schwartz sub-spaces, and the formal
Mellin--Hadamard obstruction. The missing object is the irreducible
operator-theoretic absorption-trace property. It does not close
H-P\(^{\mathrm{Ar}}\). Thus the operator construction remains the
Hilbert--P\'olya residual itself.
\end{remark}

\section{Polyakov--Dirac Normal Form for the Residual}
\label{v4-arith:w9-r4:polyakov-dirac-normal-form}

The Polyakov rank-residue calculation fixes the determinant scalar
\(r_{\zeta}=2\), closing off any attempt to hide the problem in a
different Euler-product normalisation. The Dirac reformulation converts the
remaining content into a first-order essential self-adjointness
question on the residual symmetric core, eliminating the possibility
of concealing the Hilbert--P\'olya content inside an unspecified
unbounded operator.
...
into a different determinant normalisation. Its remaining
...
inside the stated primary Heisenberg domain. The only remaining
unsettled part is therefore the closed-operator passage from the
Meyer/Connes absorption symbol to a self-adjoint zero-divisor operator,
...
residual conditions isolated below.
...
superscript ``\(\mathrm{sym}\)'' is part of the condition: if the
...
there exists a self-adjoint operator \(H_{\mathrm{HP}}\) whose normal
translate has determinant divisor \(\mathrm{div}\,\xi_{\mathbb{R}}\).
\item A-TP\(^{\mathrm{arch}}\):
the Meyer/Connes absorption trace on the cuspidal subspace is the
zero distribution, supported after zero-placement at the
non-trivial-zero ordinates with multiplicity.
...
\end{enumerate}
Thus the residual is the essential self-adjointness and determinant
identity of one first-order
...
determinant \(\xi_{\mathbb{R}}\), hence its determinant divisor is the
non-trivial-zero divisor with multiplicity. This is
H-P\(^{\mathrm{Ar}}\) in the convention of
\Cref{v4-arith:rh:hyp:HPAr}. The Polyakov rank-residue rigidity is used
only to fix the determinant scalar at \(r_{\zeta}=2\); it
...
been constructed, and the closure has determinant
...
quotient, and Meyer-coordinate graph statement below is a statement
...
With the Polyakov rank-residue rigidity fixing \(r_{\zeta}=2\), the
only remaining scalar renormalisation freedom is gone. The obstruction
is purely operator-theoretic: the boundary quotient
\(\mathcal{Q}_{\mathrm{res}}\) of the residual symmetric core must vanish.
...
\ClaimStatusProvedHere. In the primary Heisenberg domain, the only
remaining operator-theoretic obstruction to spectral-flow closure is
the non-vanishing of the residual boundary quotient:
\begin{equation}
...
\(\mathcal{D}(T_{0}^{*})/\mathcal{D}(\overline{T_{0}})\), together
...
Heisenberg domain. What remains is therefore the operator-boundary
...
\begin{theorem}[Dirac two-test form of the residual]
\label{v4-arith:w9-r4:thm:dirac-two-test}
\ClaimStatusProvedHere. After Polyakov scalar rigidity, the residual
...
\begin{remark}[deficiency kernels as the operative test]
\label{v4-arith:w9-r4:rem:smartest-next-calculation}
The deficiency kernels
\(\ker(T_{0}^{*}\mp i)\) on the Meyer/Hadamard residual core are the
operative test. If they vanish, the operator obstruction is gone and
the remaining datum is the determinant identity
\eqref{v4-arith:w9-r4:eq:closure-det}. If either kernel is non-zero,
the failure is localised as a specific boundary state in
\(\mathcal{Q}_{\mathrm{res}}\). The Polyakov--Dirac decomposition is:
the determinant residue fixes the scalar to \(r_{\zeta}=2\);
first-order operator theory isolates the only surviving obstruction.
\end{remark}

\subsection{The Deficiency Calculation}
\label{v4-arith:w9-r4:pdnf:deficiency-calculation}

In Meyer coordinates the deficiency kernels are the orthogonal
...
the inner product. The computation is unconditional given a residual
Meyer chart and the closed Tate-polar operator quotient of
...
\;:=\;
U_{\mathrm{Meyer}}(\mathcal{C}_{\mathrm{res}}^{\circ})
\;\subset\;
\mathcal{D}(M_{t}),
\end{equation}
and
...
\ClaimStatusProvedHereConditional. Assume a Meyer-coordinate residual chart in
...
\(\tilde f=U_{\mathrm{Meyer}}f\). Since \(T_{0}\) is represented by
...
\ClaimStatusProvedHere. In the ordinary Meyer graph topology,
...
the closed Tate-polar quotient and residual Meyer chart have been
...
\end{enumerate}
These are presentations of the same A--TP\(^{\mathrm{arch}}\) /
Burnol--de Branges residual, not new independent residuals. The
stronger-topology approach would require, at minimum, a continuous
...
Three graph-norm closures are available in explicit ambient spaces.
The classical Meyer line closes but has absolutely continuous spectrum
\(\mathbb{R}\), not determinant divisor
\(\mathrm{div}\,\xi_{\mathbb R}\). The formal zero space has the right
divisor only by using the zero divisor as input. The non-formal
quotient is exactly A-TP\(^{\mathrm{arch}}\).
...
ordinary Meyer-line graph completion is the unconditional Mellin
unitary part, not the Hadamard-residue determinant-trace realisation.
This is exactly the limitation of Meyer's construction recorded in
\Cref{v4-arith:w9-r4:rem:meyer-schwartz-only}.
\end{proof}
...
\begin{theorem}[residual Dirac criterion]
\label{v4-arith:w9-r4:thm:execution-result}
\ClaimStatusProvedHere. On the residual Meyer/Hadamard core, a
non-formal Dirac closure with the xi-determinant is equivalent to the
RH-strength residual. More precisely, the following statements are
equivalent:
\begin{enumerate}
\item The post-quotient residual operator \(T_{0}=H_{0}^{\circ}\)
...
\(\xi_{\mathbb{R}}\).
\end{enumerate}
Thus the unquotiented classical line and the formal zero space give
opposite degenerate limits.  On the residual core, the missing
assertion is the Hilbert--P\'olya/RH-strength step itself.
\end{theorem}
...
the latter imports the desired zero divisor by definition.
\end{proof}

\begin{remark}[graph-norm and determinant together require RH]
...
non-formal, independent residual Hilbert space. That conjunction
is precisely the absorption-trace property of Connes--Meyer, the
A-TP positivity statement, and the Hilbert--P\'olya realisation.
An unconditional proof of this conjunction is an unconditional proof
of RH.
\end{remark}
...
The formal residue-obstruction analysis is drawn
(\Cref{v4-arith:w9-r4:lem:formal-intertwine}) from the
Riemann--Hadamard--von Mangoldt classical list, disjoint from the
Bost--Connes / Takesaki modular-theoretic list of
Ch.~\ref{v4-arith:w5-b:hp-operator}; its Meyer determinant-trace
analysis from Meyer 2005 arXiv:math/0311468, disjoint from the
Birman--Krein scattering list of
Ch.~\ref{v4-arith:w7-j:ATP-adelic-scattering}; its Connes absorption-trace
analysis from Connes 1999 \S IV, disjoint from the Bost--Connes 1995
\(C^{*}\)-algebraic list of Ch.~\ref{v4-arith:w5-b:hp-operator}. No
