...
deficiency kernels
\(\ker(T_{0}^{*}\mp i)\) on the Meyer/Hadamard residual core. If they
...
\ClaimStatusDefinitional. Let
\[
\Gamma_{\xi}:=\{\,\gamma\in\mathbb R\mid
\tfrac12+i\gamma\text{ is a non-trivial zero of }\zeta\,\}
\]
with multiplicities forgotten. A continuous functional
\[
...
\qquad
\text{for every }
\phi\in C_c^\infty(\mathbb R\setminus\Gamma_{\xi}).
\]
This is the graph-norm formulation of the idea that the boundary
condition sees only the Hadamard residue data at the zero ordinates.
\end{definition}
...
\qquad
\operatorname{supp}\beta\subset\Gamma_{\xi}
...
condition supported only at the zero ordinates.
\end{theorem}
...
interval. Hence \(\Gamma_{\xi}\) is locally finite as a subset of
\(\mathbb R\). By
\Cref{v4-arith:w9-r4:lem:point-conditions-graph-invisible},
\[
C_c^\infty(\mathbb R\setminus\Gamma_{\xi})
\]
...
locally finite set \(\Gamma_{\xi}\), which has Lebesgue measure zero.
\end{proof}

\begin{corollary}[the smart calculation returns a no-go]
...
boundary functional. Therefore any surviving residual boundary
obstruction must take one of the following forms:
\begin{enumerate}
\item the residual Hadamard condition is graph-invisible, so it cannot
obstruct density of \(\mathcal{C}_{\mathrm{res}}^{M}\);
\item the true residual boundary lives in a stronger rigged topology
than \(\mathcal{D}(M_t)_{\mathrm{graph}}\);
\item the boundary functional is nonlocal, represented by an
\(L^2\)-vector as in
\Cref{v4-arith:w9-r4:lem:rank-one-graph-boundary}, and is therefore
not merely Hadamard residue at the zero ordinates.
\end{enumerate}
\end{corollary}
