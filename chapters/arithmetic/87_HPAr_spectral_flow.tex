...
\eqref{v4-arith:w7-d:eq:W-arch}. The pairing is Mellin-dual: under
...
Connes' adele class space construction is the canonical partial
resolution of the spectral-flow problem: it delivers an explicit
Hilbert space and scaling generator whose absorption spectrum would
equal \(\{\gamma_\rho\}\) if and only if the Riemann Hypothesis holds.
...
The irreducible residual is the absorption-spectrum property of the
...
\subsection{Spectral-Flow Status Table}
...
The Polyakov scalar rigidity fixes the Virasoro anomaly to
\(c_{\mathrm{reg}}=2\), closing off any attempt to shift the problem
by rescaling the central charge. The Dirac reformulation converts the
...
Polyakov regularisation is decisive for the anomaly coefficient,
...
With the Polyakov scalar rigidity fixing \(c_{\mathrm{reg}}=2\), the
...
the anomaly coefficient fixes the scalar to \(c_{\mathrm{reg}}=2\);
...
forming a trichotomy. The classical Meyer line closes but has
absolutely continuous spectrum \(\mathbb{R}\), not the discrete zero
ordinate set. The formal zero space has the right determinant but
inserts the desired spectrum by definition. The non-formal residual
quotient is exactly A-TP\(^{\mathrm{arch}}\). No non-formal
...
The sources below are organised into four groups, each internally
disjoint and explicitly disjoint from the corresponding source
groups of the upstream Vol~IV arithmetic chapters.
