...
\emph{This chapter proves the wild-prime block \(\mathrm{Ext}^{1}\)-class
...
\(p\geq 5\). The present chapter replaces the \(p=2\) input by
...
\subsection{Disjoint Derivation Sources}

The derivation in this chapter uses:
\begin{description}
...
inputs. The derivation of this chapter replaces the connecting map at the
...
both sides. No source in this chapter supplies a Montreal-functor connecting
map; no source in the proof of \Cref{v4-arith:w6-f:thm:class-match} supplies
a pseudo-character tangent comparison.
Source-disjointness holds at the per-arrow level.
...
\begin{corollary}[wild-prime class-match by two source-disjoint derivations]
\label{v4-arith:w10-b:cor:retires-disjoint}
\ClaimStatusProvedHere. The wild-prime class-match is proved by two
source-disjoint derivations: \Cref{v4-arith:w6-f:thm:class-match} via the
Colmez connecting map and \Cref{v4-arith:w10-b:thm:class-match} via the
pseudo-character tangent. The two derivations are source-disjoint at the
...
source-disjointness is verified in
...
\section{Derivation Verification}
...
\paragraph{Source collision.} Derivation sources of this chapter:
...
(spectral cotangent). Derivation sources of \Cref{v4-arith:w6-f:thm:class-match}
(Pa{\v s}k{\=u}nas 2013
...
\Cref{v4-arith:w6-f:thm:class-match}. \textbf{Pass.}
...
\paragraph{Compute engine.} No compute engine is invoked. \textbf{Pass.}

All seven conditions pass: the class-match is \(\mathrm{ProvedHereConditional}\)
by a derivation source-disjoint from \Cref{v4-arith:w6-f:thm:class-match}
...
up to scalar.
\item Source-disjointness at the first arrow is verified; both derivations
...
\item The seven verification conditions all pass; no new residual
