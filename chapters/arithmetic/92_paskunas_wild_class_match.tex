...
\emph{Wave-10 agent B reopens the wild-prime class-match residual at
$p\in\{2,3\}$ with a derivation path disjoint from wave-6 agent F
(\Cref{v4-arith:w6-f:thm:class-match}). Wave-6 routes the class-level
...
Pa{\v s}k{\=u}nas--Tung 2020 (arXiv:2003.03129) and the $p=3$ input by
Colmez--Dospinescu--Pa{\v s}k{\=u}nas 2014 (arXiv:1310.7499) \S 5
combined with the Hu--Tan 2018 (arXiv:1611.01304) multiplicity
framework; it then matches the resulting block $\mathrm{Ext}^{1}$-class
to the Emerton--Gee wild-inertia cotangent fibre
(arXiv:2012.12403 \S 9) via the pseudo-character deformation route
rather than the Colmez-connecting-map route used by wave-6. Block-by-block
...
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement} as wave-6 but with
...
\subsection{The Residual after Wave-6}

Wave-6 agent F retired F1.a.2 class-match conditional on
...
(arXiv:1311.3048) Theorem 1.4 is explicitly conditional on $p\geq 5$
at the Montreal-functor step; the extension to $p=2$ is the subsequent
Pa{\v s}k{\=u}nas--Tung 2020 (arXiv:2003.03129) refinement, and the
$p=3$ case is Colmez--Dospinescu--Pa{\v s}k{\=u}nas 2014
(arXiv:1310.7499) \S 5. Wave-6 cites \S 4.11 of Pa{\v s}k{\=u}nas 2013
...
The wave-10 B closure uses:
\begin{description}
\item[Pa{\v s}k{\=u}nas--Tung 2020, arXiv:2003.03129.] Theorems 1.3,
...
block.
\item[Colmez--Dospinescu--Pa{\v s}k{\=u}nas 2014, arXiv:1310.7499 \S 5.]
...
\item[Hu--Tan 2018, arXiv:1611.01304.] Theorem 1.1: very-special
Breuil--Mezard multiplicities, used to pin the tangent multiplicity
at the reducible closed point.
\item[Emerton--Gee, arXiv:2012.12403 \S 9.] Wild-inertia stack
...
Wave-6 derivation sources (used to produce the wave-6 proof of
...
inputs. Wave-10 B derivation sources replace the connecting map at the
...
the spectral cotangent fibre. The only shared input with wave-6 is the
...
no source in wave-6 supplies a pseudo-character tangent comparison.
...
\begin{proof}
Emerton--Gee arXiv:2012.12403 \S 9 identifies the formal completion of
...
\begin{theorem}[wild block Ext$^{1}$-class match, wave-10 B route]
...
the wave-6 isomorphism $\mathsf{c}^{\mathrm{class}}_{p}$ of
...
Agreement with wave-6 up to scalar: both isomorphisms factor through
...
\begin{corollary}[wave-10 B retires class-match with disjoint route]
\label{v4-arith:w10-b:cor:retires-disjoint}
\ClaimStatusProvedHere. The wild-prime class-match is retired by two
routes: wave-6 via the Colmez connecting map
(\Cref{v4-arith:w6-f:thm:class-match}) and wave-10 B via the
...
\begin{proof}
Direct: \Cref{v4-arith:w6-f:thm:class-match} supplies the wave-6 route;
\Cref{v4-arith:w10-b:thm:class-match} supplies the wave-10 B route;
...
\label{v4-arith:w10-b:attack-heal}
...
2009 (tangent space identification); Emerton--Gee arXiv:2012.12403 \S 9
(spectral cotangent). Wave-6 derivation sources (Pa{\v s}k{\=u}nas 2013
...
wave-6 as preparation for the connecting map. \textbf{Pass,
...
derivation path source-disjoint from wave-6 at the first arrow, with
...
\begin{enumerate}
\item Wave-6 class-match (\Cref{v4-arith:w6-f:thm:class-match}) routes
...
\item Wave-10 B (\Cref{v4-arith:w10-b:thm:class-match}) replaces the
...
identification. The class match agrees with wave-6 up to scalar.
