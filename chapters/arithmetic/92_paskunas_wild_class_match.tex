...
\emph{The wild-prime block \(\mathrm{Ext}^{1}\)-class
match at \(p\in\{2,3\}\) by a derivation path disjoint from that of
\Cref{v4-arith:w6-f:thm:class-match}. The proof of
\Cref{v4-arith:w6-f:thm:class-match} threads the class-level
...
\(p\geq 5\). The \(p=2\) input is replaced by
Pa{\v s}k{\=u}nas--Tung 2020 and the \(p=3\) input by
Colmez--Dospinescu--Pa{\v s}k{\=u}nas 2014 (CDP 2014) \S 5
combined with the Hu--Tan 2018 multiplicity
framework; it then matches the resulting block \(\mathrm{Ext}^{1}\)-class
to the Emerton--Gee wild-inertia cotangent fibre
(arXiv:1908.07185 \S 9) via the pseudo-character deformation tangent,
rather than the Colmez connecting map used in the proof of
\Cref{v4-arith:w6-f:thm:class-match}. Block-by-block
class identification is proved for each wild non-semisimple
Pa{\v s}k{\=u}nas block at \(p\in\{2,3\}\), stated
\(\mathrm{ProvedHereConditional}\) on the same wild-EG enhancement
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement} as
\Cref{v4-arith:w6-f:thm:class-match}, but with
disjoint proof sources. The class-match target
...
\section{Domain and Disjoint Derivation}
\label{v4-arith:w10-b:domain}

\subsection{The Residual after \texorpdfstring{\Cref{v4-arith:w6-f:thm:class-match}}{the Chapter~66 class-match}}

The proof of \Cref{v4-arith:w6-f:thm:class-match} established the
wild-prime class-match conditional on
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement} via the composition
\begin{equation}
\label{v4-arith:w10-b:eq:ch66-colmez-arrow}
...
\end{equation}
The first arrow in \eqref{v4-arith:w10-b:eq:ch66-colmez-arrow} is the Colmez
Montreal functor's connecting map. Pa{\v s}k{\=u}nas 2013
Theorem 1.4 is explicitly conditional on \(p\geq 5\)
at the Montreal-functor step; the extension to \(p=2\) is the subsequent
Pa{\v s}k{\=u}nas--Tung 2020 refinement, and the
\(p=3\) case is Colmez--Dospinescu--Pa{\v s}k{\=u}nas 2014
(CDP 2014) \S 5. The proof of \Cref{v4-arith:w6-f:thm:class-match}
cites \S 4.11 of Pa{\v s}k{\=u}nas 2013
for both wild cases, which is technically outside the \(p\geq 5\) domain.
The same class-match closes by a different first arrow,
via the pseudo-character tangent space, and states the conclusion
with disjoint proof sources.

\subsection{Disjoint Proof Sources}
...
Breuil--M{\'e}zard multiplicities, used to pin the tangent multiplicity
...
residual.
\item[Kisin 2008 (J.~AMS~21), Bella{\"\i}che--Chenevier 2009
...
The derivation of \Cref{v4-arith:w6-f:thm:class-match}
(cf.\ \eqref{v4-arith:w10-b:eq:ch66-colmez-arrow}) draws on Pa{\v s}k{\=u}nas 2013
...
inputs. The derivation replaces the connecting map at the
first arrow by the pseudo-character tangent: the Pa{\v s}k{\=u}nas--Tung
\(p=2\) block centraliser structure and the CDP \(p=3\) structure, both as
...
the spectral cotangent fibre. The only shared input with
\Cref{v4-arith:w6-f:thm:class-match} is
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement} (the wild-EG enhancement
conjecture), which supplies the degree-\(1\) derived structure on
both sides. No source used here supplies a Montreal-functor connecting
map; no source in the proof of \Cref{v4-arith:w6-f:thm:class-match} supplies
a pseudo-character tangent comparison.
Independence holds at the per-arrow level.
...
identified with the pseudo-character deformation tangent
\begin{equation}
...
pseudo-character tangent path avoids this: the residual trace
...
Taking \(k\)-linear duals of the tangent identification gives
...
\begin{theorem}[wild block \(\mathrm{Ext}^{1}\)-class match, pseudo-character derivation]
...
the isomorphism \(\mathsf{c}^{\mathrm{class}}_{p}\) of
...
Agreement with \Cref{v4-arith:w6-f:thm:class-match} up to scalar: both
isomorphisms factor through the same pseudo-character tangent space;
the Colmez connecting map and the Auslander--Reiten graded decomposition
produce the same cohomology class up to a normalisation choice of generator,
which is a scalar because each block's cotangent direction is a \(k\)-line.
The wild-EG enhancement is the only conditional input; the remaining
derivation is unconditional block algebra.
\end{proof}

\begin{corollary}[wild-prime class-match by two independent derivations]
\label{v4-arith:w10-b:cor:removes-disjoint}
\ClaimStatusProvedHere. The wild-prime class-match is proved by two
independent derivations: \Cref{v4-arith:w6-f:thm:class-match} via the
Colmez connecting map and \Cref{v4-arith:w10-b:thm:class-match} via the
pseudo-character tangent. The two derivations are independent at the
first arrow and agree up to scalar. The realization pair for the
class-match is
...
\begin{proof}
\Cref{v4-arith:w6-f:thm:class-match} supplies the Colmez-connecting-map
derivation; \Cref{v4-arith:w10-b:thm:class-match} supplies the
pseudo-character-tangent derivation;
independence of sources is verified in
\S\ref{v4-arith:w10-b:domain}\,; agreement up to scalar is the last
paragraph of the proof of \Cref{v4-arith:w10-b:thm:class-match}.
\end{proof}

\section{Derivation Comparison}
\label{v4-arith:w10-b:obstruction-analysis}

Seven conditions are checked against the class-match
\Cref{v4-arith:w10-b:thm:class-match}.

\paragraph{Source separation.} Proof sources:
...
(spectral cotangent). Proof sources of \Cref{v4-arith:w6-f:thm:class-match}
(Pa{\v s}k{\=u}nas 2013
...
the proof of \Cref{v4-arith:w6-f:thm:class-match} as preparation for
the connecting map. \emph{independent at the first arrow.}

\paragraph{Sign and convention.} Chenevier pseudo-character
...
\paragraph{Ambient category.} Block is in
...
\paragraph{Hypotheses.} The only hypothesis is
...
pro-representable at every prime.

\paragraph{Functoriality.} Both factor isomorphisms
...
\paragraph{Unproved equivalences.} No new unproved
...
unconditional (Pa{\v s}k{\=u}nas--Tung 2020, CDP 2014).

\paragraph{Numerical comparison.} No numerical comparison is invoked.

The seven conditions hold: the class-match is \(\mathrm{ProvedHereConditional}\)
by a proof independent from \Cref{v4-arith:w6-f:thm:class-match}
at the first arrow, with the same conditional dependency on the wild-EG
enhancement \Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}.

\section{Wild Class Boundary}
\label{v4-arith:w10-b:summary}

\begin{enumerate}
\item \Cref{v4-arith:w6-f:thm:class-match} identifies
the wild-prime block \(\mathrm{Ext}^{1}\) with Galois cohomology via the
...
domain of the Montreal functor's original construction.
\item \Cref{v4-arith:w10-b:thm:class-match} replaces the
...
identification. The class match agrees with \Cref{v4-arith:w6-f:thm:class-match}
up to scalar.
\item Independence at the first arrow is verified; both derivations
share only the wild-EG enhancement
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}.
\item The realization pair for the class-match is
\((\mathsf{c}^{\mathrm{class}}_{p},\mathsf{c}^{\mathrm{class},\mathrm{B}}_{p})\)
with independent derivations.
\item The seven source, convention, and domain conditions hold; no new residual
is introduced; no numerical constant.
\end{enumerate}
