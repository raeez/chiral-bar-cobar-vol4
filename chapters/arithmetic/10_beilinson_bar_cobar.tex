...
programme. Its arithmetic avatar is the adjunction
\(\Omega^{\mathrm{ch,Ar}}\dashv B^{\mathrm{ch,Ar}}\) on the
Arakelov-compactified arithmetic curve
\(\overline{\mathrm{Spec}\,\mathbb{Z}}\), with the Parshin--Gersten
complex replacing the de Rham complex.}
...
archimedean. The arithmetic \(\eta_{ij}\) (the Arnold \(2\)-form
...
and the Arnold relation is the \(3\)-cocycle condition on the
Gersten complex:
\begin{equation}
...
\(\partial_{\mathfrak{p}}\).
% RECTIFICATION-FLAG: (d^Ar)^2=0 is the load-bearing nilpotency
% condition for B^{ch,Ar} to be a cochain complex. The three
% summands square to zero individually; cross-terms
% d_Iw * d_Gersten + d_Gersten * d_Iw and mixed terms with
% d_bar^local require verification that the Iwasawa--Gersten
% anticommutativity and the tame-symbol compatibility hold.
% Independent verification path: Parshin--Kato exactness of the
% Gersten complex (Kato, J. reine angew. Math. 366, 1986) combined
% with Coleman--Perrin-Riou Iwasawa theory (Perrin-Riou, Invent.
% Math. 115, 1994). Cross-term cancellation not yet exhibited in
% this chapter. Gate 1 HARD: leave flagged.
\end{definition}
...
carries a grading by homological degree; the question is what
Dirichlet series its Poincar\'e series computes. At
\(\kappa^{\mathrm{Ar}}=0\), the Euler product structure of the
Parshin--Gersten base forces an identification with \(\zeta(s)\).
...
\ClaimStatusConjectured\ (items 2--3; item~1 is
