...
\emph{This chapter addresses the two smallest sub-residuals of F1.a.3
introduced by Chapter~\ref{v4-arith:f1a3-BH}: the BH type-block
comparison residual (F1.a.3.A) and the Paskunas wild-block
\(\mathrm{Ext}^{1}\)-vanishing residual (F1.a.3.B). Both are handled
by first-principles block-category arguments. The BH type-block comparison
...
after Paskunas 2013 and Paskunas 2015, into three regimes: generic blocks where the block
...
new \(\mathrm{ProvedHere}\) statement is attempted for F1.a.3.A or
F1.a.3.B as a whole; the unconditional content is the block-additivity
...
blocks only trivially, because the Bernstein centre is commutative
and decomposes along the block product. Restriction to a fixed
\(\chi\) selects the sub-product over \(\mathfrak{B}(G)_{\chi}\).
\end{proof}
...
regime at \(p\in\{2,3\}\). In this three-way stratification,
``block level'' means the corresponding block regime preserved by the
argument; proofs use only structure preserved across the
regimes.
\end{remark}
...
(Lurie HA~4.8.1, stable presentable additivity). Hence
...
\begin{theorem}[block-wise BH type-block comparison]
...
\subsection{Conditional Residual on Each Block}

\begin{proposition}[obstruction taxonomy for F1.a.3.A]
...
Publ.~IHES~118:1--191, Theorems~1.4, 1.5, and 1.6.
\end{proof}

\begin{remark}[location of the two cases in Paskunas 2013]
\label{v4-arith:w5-f:F1a3-AB:rem:Paskunas-centraliser-location}
The regular-local dimension for the generic case is computed in
Paskunas 2013 \S 6, with explicit pseudo-character-deformation
generators. The non-generic (ordinary block) case is in \S 10.
Updated extent of the ordinary block is in Paskunas 2015
(Duke Math.\ J.\ 164 (2015), 297--359; arXiv:1306.2691), ``On
...
\(\mathrm{Ext}^{1}\)-vanishing required for BH type-block essential
surjectivity under
...
multiplicity formula for \(\rho\) (Paskunas 2015
...
multiplicity formula is not used here). The very-special stratum is
therefore recorded as residual (R.B.1). The dimension bound
...
The ``ordinary block'' covered by Paskunas 2013
...
\begin{remark}[gap in the wild-prime block centraliser argument]
\label{v4-arith:w5-f:F1a3-AB:rem:centraliser-wild-conditional}
The conjecture in
...
but has no published lift in the cited range. The expected dimension,
\(2\) at \(p=2\)
...
\subsection{Unconditional F1.a.3.A+B Content}

\begin{theorem}[unconditional content of F1.a.3.A+F1.a.3.B]
\label{v4-arith:w5-f:F1a3-AB:thm:unconditional-AB}
\ClaimStatusProvedHere. The following hold without conjectural input:
\begin{enumerate}
...
\subsection{Conditional F1.a.3.A+B Residual}

\begin{proposition}[remainder of the F1.a.3.A+B residual]
\label{v4-arith:w5-f:F1a3-AB:prop:remainder-AB}
\ClaimStatusProvedHere. The F1.a.3.A+B residual consists of exactly
three conditional items:
\begin{enumerate}
\item[\textbf{(R.A)}] Per-block BH type-block comparison at each
...
\begin{remark}[obstruction factorization]
\label{v4-arith:w5-f:F1a3-AB:rem:reduction-summary}
F1.a.3.A is the product of block-additivity and the per-block
comparison core (R.A). F1.a.3.B is the product of generic-locus
Ext\({}^{1}\)-vanishing at \(p\geq5\), the very-special ordinary
block condition (R.B.1), and wild-prime centraliser regularity
(R.B.2). The support of (R.A), (R.B.1), and (R.B.2) is strictly smaller
than the support of the two un-factored conditions.
\end{remark}

\subsection{Obstruction Decomposition}

\begin{proposition}[F1.a.3 obstruction decomposition]
\label{v4-arith:w5-f:F1a3-AB:prop:F1a3-table-updated}
\ClaimStatusProvedHere. The F1.a.3 obstruction decomposition consists
of:
\begin{enumerate}
\item \textbf{(R.A)} Per-block BH type-block comparison
(\Cref{v4-arith:w5-f:F1a3-AB:prop:remainder-AB}(R.A)), substantial
non-formal theorem at each block; block-local.
...
substantial non-formal theorem.
...
from Chapter~\ref{v4-arith:f1a3-BH}), substantial non-formal theorem.
\end{enumerate}
The four supports are pairwise named: (R.A) is block-local, (R.B.1) is
very-special ordinary, (R.B.2) is wild-centraliser, and (R.C) is
...
\Cref{v4-arith:f1a3-BH:conj:wild-compact-generation}. The last term is
independent of the block-additivity and Ext\({}^{1}\)-vanishing
arguments.
\end{proof}

\section{Source Disjointness and Convention Comparison}
\label{v4-arith:w5-f:F1a3-AB:source-comparison}

Each principal result of this chapter is verified against five
independent criteria: source disjointness, sign and convention
consistency, ambient-category compatibility, hypothesis minimality,
and functoriality. The following subsections record these checks for
the four main arguments.

\subsection{Block-Additivity: Source and Convention Comparison}
\label{v4-arith:w5-f:F1a3-AB:comparison-block-additivity}

\paragraph{Source disjointness.} The block-additivity argument draws on
Bernstein 1984 and Bernstein--Deligne 1984 (Bernstein centre and product
decomposition of \(\mathrm{Mod}^{\mathrm{sm}}_{G}\)) and
Bushnell--Henniart 2006 (BH type list at \(GL_{2}\)). These are
jointly disjoint from Paskunas 2013 and Paskunas 2015 (the
mod-\(p\) block-centraliser list), from Ben-Zvi--Nadler
arXiv:0904.1247 (the Hochschild-trace source), and from
Dotto--Emerton--Gee arXiv:2603.26887 (the DEG construction source).

\paragraph{Sign and convention.} Bernstein--Deligne use the left-regular
representation of the Bernstein centre; Paskunas uses the right-regular;
both yield equivalent block-projection functors under the
opposite-algebra convention. Bushnell--Henniart uses
Langlands-normalised central characters; the \(\chi\) convention here
matches. The Hochschild homology orientation follows the ``co-Hochschild''
convention from Lurie HA~5.5.3, consistent with Ben-Zvi--Nadler and
Costello--Gwilliam.

\paragraph{Ambient category.} The block decomposition of
\(\mathrm{Mod}^{\mathrm{sm},\chi}_{GL_{2}(\mathbb{Q}_{p})}\) lives in the
...
\paragraph{Hypothesis minimality.} The block-additivity lemma
\Cref{v4-arith:w5-f:F1a3-AB:lem:HH-block-additive} requires only
cocontinuity of Hochschild homology on presentable stable
\(\infty\)-categories. No \(p\)-adic or Langlands hypothesis enters.

\paragraph{Functoriality.} Block-decomposition is functorial under
central-idempotent projection; Hochschild homology is functorial on
stable presentable \(\infty\)-categories; their composite is
block-additive. No unverified equivalence is introduced.

\subsection{Per-Block Residual Isolation: Source and Convention Comparison}
\label{v4-arith:w5-f:F1a3-AB:comparison-per-block}

\paragraph{Source disjointness.} The per-block residual argument adds
Paskunas 2013 (block-by-block projective-cover analysis) and Colmez
2010 Ast\'erisque~330 (the Montreal functor bridging automorphic and
Galois sides) to the preceding source set. These are disjoint from the
Bernstein--Deligne and Ben-Zvi--Nadler sources.

\paragraph{Convention alignment.} Paskunas' projective cover
\(P_{\rho}\) uses the Montreal functor normalisation; this matches the
\(P_{\tau}\) of \Cref{v4-arith:f1a3-BH:conj:BH-packaging} under the
BH-to-residual correspondence
\(\tau\mapsto\rho_{\tau}\) of
\Cref{v4-arith:f1a3-BH:prop:type-to-component-p2}.

\paragraph{Hypothesis.} The per-block hypothesis is named explicitly as
\Cref{v4-arith:w5-f:F1a3-AB:prop:A-residual-taxonomy}(2); no further
hypothesis is required for the structural factorisation. The residual
(R.A) is the named open content.

\subsection{Generic-Block \(\mathrm{Ext}^{1}\)-Vanishing: Source and Convention Comparison}
\label{v4-arith:w5-f:F1a3-AB:comparison-ext1-generic}

\paragraph{Source disjointness.} The generic-block
\(\mathrm{Ext}^{1}\)-vanishing argument uses Paskunas 2013 \S~6
(block centraliser regular local of dimension \(2\) or \(3\)),
Auslander--Buchsbaum 1957 (Koszul resolution on a regular local ring),
and Paskunas 2015 (Breuil--Mezard multiplicities). These are jointly
disjoint from Bernstein--Deligne and from Ben-Zvi--Nadler.

\paragraph{Convention.} The regular local ring Koszul complex follows
Matsumura 1989 Ch.~6; this matches the Gaitsgory--Rozenblyum Vol.~II
treatment of \(\mathrm{IndCoh}\) on a regular local scheme.
Localisation at the generic point is functorial on \(A_{\rho}\)-Mod;
the Morita equivalence preserves \(\mathrm{Ext}\).

\paragraph{Hypothesis minimality.} The hypothesis ``\(p\geq 5\), generic
\(\rho\), fixed \(\chi\)'' is explicit in
\Cref{v4-arith:w5-f:F1a3-AB:prop:Ext1-generic-p5}. No implicit
Langlands-side hypothesis enters; all inputs are from the block-algebra
structure. The generic-fibre \(\mathrm{Ext}^{1}\)-vanishing is a
pointwise statement; no functoriality claim beyond Morita is made.
The result is unconditional.

\subsection{Wild-Prime Block Centraliser: Source and Convention Comparison}
\label{v4-arith:w5-f:F1a3-AB:comparison-wild-prime}

\paragraph{Source disjointness.} The wild-prime argument uses
Bushnell--Henniart 2006 \S~12--15 (wild types at \(p=2,3\)) and
Auslander--Reiten--Smal\o{} 1995 (linear-quiver block algebra theory).
Paskunas 2013 does not cover \(p\in\{2,3\}\) directly; this is the
source of the gap named as (R.B.2).

\paragraph{Convention.} The \(\mathrm{Ext}^{1}\) convention uses the
\(A_{2}\)-linear quiver \(L_{0}\leftarrow L_{1}\leftarrow L_{2}\) with
radical-square-zero relations, consistent with
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p3}. The block centraliser
algebra at wild primes is complete Noetherian local by the general
representation theory of reductive groups over local fields.

\paragraph{Hypothesis.} The key hypothesis is regularity of the block
centraliser at wild primes, named
\Cref{v4-arith:w5-f:F1a3-AB:prop:centraliser-wild}. The expected
dimensions are \(2\) at \(p=2\) and \(\leq 3\) at \(p=3\), consistent
with the \(A_{2}\)-quiver dimensions of
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p2} and
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p3}. Conditional on this
hypothesis, the Koszul-resolution argument transfers from the
\(p\geq 5\) case. The residual (R.B.2) is the named open content.

\paragraph{Ordinary-block controlled deformation.} The ordinary-block
argument uses Paskunas 2013 \S~10 and Paskunas 2015 (Breuil--Mezard
multiplicity update). No further preprint is used to close the
very-special sub-stratum. The very-special Breuil--Mezard multiplicity
is the named residual (R.B.1).

\section{Paskunas Block Boundary}
\label{v4-arith:w5-f:F1a3-AB:summary}

\begin{enumerate}
\item F1.a.3.A (BH type-block comparison) factors into two parts: block-additivity
...
(R.B.2).
\item The combined F1.a.3.A+B decomposition has four items:
(R.A), (R.B.1), (R.B.2), and the unchanged (R.C) of
Chapter~\ref{v4-arith:f1a3-BH}. Total substantial
non-formal theorem
(\Cref{v4-arith:w5-f:F1a3-AB:prop:F1a3-table-updated}), within the
non-formal theorem condition of Chapter~\ref{v4-arith:f1a3-BH}. The
partition is strictly finer: two original conjectures A and B become
three named sub-items, each strictly narrower.
\item The unconditional content is: (i) block-additivity
...
(iv) the obstruction decomposition
(\Cref{v4-arith:w5-f:F1a3-AB:prop:F1a3-table-updated}).
\item The full F1.a.3.A and F1.a.3.B statements factor through the
three named residuals. The source disjointness and convention comparison of
\Cref{v4-arith:w5-f:F1a3-AB:source-comparison} confirms three named
residuals: (R.A), (R.B.1), and (R.B.2).
\end{enumerate}

Thus two block conditions factor into three strictly supported
residuals and four unconditional reductions.
...
arithmetic branch's inherited list):
...
\item V.~Paskunas, ``On the Breuil--M\'ezard conjecture'', Duke
Math.\ J.\ 164 (2015), no.\ 2, 297--359 (arXiv:1306.2691).
...
derivation of unconditional results. Disjointness comparison:
\begin{itemize}
...
disjoint inputs.
\end{itemize}
Disjointness holds throughout; no tautological source reuse.
