...
by first-principles block-category arguments. The BH-packaging residual
...
% RECTIFICATION-FLAG: Gate 1 -- HA 4.8.1.17 is Prop on presentable-category products; verify this is the correct statement for product-coproduct coincidence in Pr^L_st (may need HA 4.8.1.15--4.8.1.17 range or HTT 5.5.3.13).
...
% RECTIFICATION-FLAG: Gate 1 -- HA 5.5.3 is the section on THH/circle actions; the specific statement needed is that HH_bullet is cocontinuous and that factorisation homology on S^1 commutes with colimits; a more precise subsection pin (e.g., HA 5.5.3.1 or 5.5.3.6) should be supplied.
...
Updated coverage of the ordinary block is in Paskunas 2015
...
% RECTIFICATION-FLAG: Gate 1 -- Paskunas--Schönberg arXiv:2401.* is a wildcard; supply the full arXiv ID (the preprint on very-special Breuil--Mézard multiplicities) before submission.
% RECTIFICATION-FLAG: Gate 1 -- Emerton--Paskunas cited without venue/arXiv; the paper on effective local Langlands for GL(2)/Q_p should carry arXiv:1407.0428 (or the correct ID); verify before submission.
noted in Paskunas--Sch\"onberg (arXiv:2401, preprint) if needed; see also
Emerton--Paskunas arXiv:1407.0428). The dimension bound
...
\section{Source Disjointness and Convention Audit}
\label{v4-arith:w5-f:F1a3-AB:source-audit}
...
\subsection{Block-Additivity: Source and Convention Audit}
\label{v4-arith:w5-f:F1a3-AB:audit-block-additivity}
...
Bushnell--Henniart 2006 (BH type catalog at \(GL_{2}\)). These are
jointly disjoint from Paskunas 2013 and Paskunas 2015 (the
mod-\(p\) block-centraliser catalog), from Ben-Zvi--Nadler
...
\subsection{Per-Block Residual Isolation: Source and Convention Audit}
\label{v4-arith:w5-f:F1a3-AB:audit-per-block}
...
\subsection{Generic-Block \(\mathrm{Ext}^{1}\)-Vanishing: Source and Convention Audit}
\label{v4-arith:w5-f:F1a3-AB:audit-ext1-generic}
...
\subsection{Wild-Prime Block Centraliser: Source and Convention Audit}
\label{v4-arith:w5-f:F1a3-AB:audit-wild-prime}
...
multiplicity update); Emerton--Paskunas
% RECTIFICATION-FLAG: Gate 1 -- Emerton--Paskunas "On effective local Langlands for GL(2)/Q_p" cited without venue or arXiv; verify arXiv ID (expected arXiv:1407.0428) and year before submission.
provides independent convention alignment for the very-special
sub-stratum (Paskunas 2013 Remark~10.5). These sources are disjoint
from the generic-block source set. The very-special sub-stratum
Breuil--Mezard multiplicity is the named residual (R.B.1).
...
unconditional. The source disjointness and convention audit of
\Cref{v4-arith:w5-f:F1a3-AB:source-audit} confirms three named
...
for \(GL(2)/\mathbb{Q}_{p}\)''
% RECTIFICATION-FLAG: Gate 1 -- venue missing; this may be arXiv:1407.0428 or a different preprint; verify arXiv ID and publication status before submission.
(preprint, 2015).
