...
The route~5 derivation sources for F1.a.2 and F1.a.3.A+B are:
...
The present closures draw on: Paskunas--Tung 2020, Hu--Tan 2018,
Bella\"iche--Chenevier 2009, Kisin 2008, and Emerton--Gee arXiv:1908.07185 \S 9.

Pa\v skunas--Tung 2020 extends Pa\v skunas 2013 to the Cohen--Macaulay axis:
the 2013 paper bounds Krull dimension but does not establish Cohen--Macaulay regularity;
the 2020 paper adds Theorems~1.3, 1.5, 1.7 on that axis.
Hu--Tan 2018 fills the very-special sub-stratum that Pa\v skunas 2015 explicitly left open.
...
automorphic input for the class-match closure (\Cref{v4-arith:w6-f:prop:Paskunas-equals-Galois}),
and its \S 5.3 Theorems~5.15--5.22 on centraliser dimensions serve as the spectral input
for the R.B.2 closure (\Cref{v4-arith:w6-f:thm:CDP-p3-centraliser}).
These two uses draw on distinct propositions within \S 5.3; disjointness holds at the
proposition level, not at the section-reference level. The disjointness audit is therefore
proposition-level: the connecting-map result (automorphic) and the centraliser-dimension
results (spectral) are mathematically independent within the paper.
...
closing the sub-stratum left open in Pa\v skunas 2015 (``On the Breuil--M\'ezard
conjecture,'' Duke Math.~J.~164 (2015), arXiv:1208.3779). The result
uses Emerton--Pa\v skunas 2015 (``On effective local Langlands for
\(GL(2)/\mathbb{Q}_{p}\),'' arXiv:1503.04796) Proposition~5.7 to reduce
...
\section{Source-Disjointness and Convention Verification}
\label{v4-arith:w6-f:disjointness-verification}

\subsection{Class-Match: Source-Disjointness and Conventions}

The primary sources for the class-match closure are
...
Kisin 2008 (deformation cotangent complex),
and Emerton--Gee arXiv:1908.07185 \S 9 (wild component structure on the spectral side).
All five are disjoint from the route~5 derivation set (\Cref{v4-arith:w5-e:sources}).

The disjointness of Colmez--Dospinescu--Pa\v skunas 2014 \S 5.3 in the class-match closure requires
...
The two uses are therefore source-disjoint at the level of the specific propositions invoked,
...
restricted to the block, and the cotangent fibre in
...
The only non-trivial hypothesis is the wild-EG enhancement
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}; no further input is required.
Both factor isomorphisms are natural in \(\bar\rho_{\mathfrak{B}}\), so
composition preserves naturality. No compute engine is invoked.

\subsection{R.B.1: Source-Disjointness and Conventions}

Hu--Tan 2018 and Emerton--Pa\v skunas 2015 are disjoint from the
route~5 Pa\v skunas 2015 primary set: Hu--Tan fills the very-special
sub-stratum that Pa\v skunas 2015 explicitly left open (Pa\v skunas 2015
Remark following Theorem~1.2), and Emerton--Pa\v skunas 2015 is a
companion reduction paper distinct from the main Pa\v skunas 2015 article.

Hu--Tan 2018 uses the Breuil--M\'ezard geometric convention for multiplicities;
Pa\v skunas 2015 uses the numerical convention; the equivalence between them
is Kisin 2009 Theorem~2.3.5. Both multiplicities are computed on the deformation
ring of the scalar residual \(\bar\rho\sim\chi\oplus\chi\omega\). The very-special
hypothesis is explicit in Hu--Tan Theorem~1.1, which covers this case precisely.
Multiplicity is an integer function of the residual; no functoriality claim arises.
No compute engine is invoked.

\subsection{R.B.2: Source-Disjointness and Conventions}

Pa\v skunas--Tung 2020 and Colmez--Dospinescu--Pa\v skunas 2014 \S 5.3 Theorems~5.15--5.22
(centraliser structure) are disjoint from the route~5 inputs as follows.
Pa\v skunas--Tung 2020 is a post-2013 paper that adds the Cohen--Macaulay axis
to Pa\v skunas 2013; it extends the Krull-dimension result to a full regularity statement
and is therefore a strict refinement, not a restatement.
Colmez--Dospinescu--Pa\v skunas 2014 \S 5.3 Theorems~5.15--5.22 are independent
of Pa\v skunas 2013 at the level of the specific propositions invoked.
Both sources are also disjoint from the Auslander--Reiten--Smal\o{} quiver inputs
(Auslander--Reiten--Smal\o{} \emph{Representation Theory of Artin Algebras}, Ch.~III
\S 1.14) used in the route~5 block-quiver argument.
The Krull-dimension convention and Cohen--Macaulay property are absolute
and match both Pa\v skunas' and the present notation.
Block centralisers \(A_{\mathfrak{B}}\) are complete Noetherian local \(W(k)\)-algebras
in both references, so the ambient category is identical. No compute engine is invoked.
...
\item Source-disjointness and convention verification for all three closures
is recorded in \Cref{v4-arith:w6-f:disjointness-verification}; no source
appears on both the derivation and verification sides of any single closure
at the proposition level.
\end{enumerate}

The chapter retires the full route~5 Pa\v skunas-lane residual set
(R.B.1, R.B.2, class-match) and leaves only two items in the F1.a.3
ledger. The conditional dependency on
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement} for class-match is explicit
and is the only surviving hypothesis.
...
Disjointness audit (proposition-level):
\begin{itemize}
...
and Bella\"iche--Chenevier 2009 residual gerbe, Kisin 2008 deformation cotangent,
Emerton--Gee arXiv:1908.07185 \S 9 spectral side (cotangent identification input).
...
\item R.B.1 (\Cref{v4-arith:w6-f:cor:R-B-1-closed}) uses Hu--Tan 2018 Theorem~1.1
and Emerton--Pa\v skunas 2015 Proposition~5.7 to fill the sub-stratum that
Pa\v skunas 2015 left open; these are disjoint from the Pa\v skunas 2015 main statement
(route~5 primary set) because they address a sub-stratum explicitly excluded from it.
\item R.B.2 (\Cref{v4-arith:w6-f:thm:R-B-2-unified}) uses
Pa\v skunas--Tung 2020 Theorems~1.3, 1.5, 1.7 at \(p=2\) (Cohen--Macaulay axis,
absent from Pa\v skunas 2013) and CDP 2014 \S 5.3 Theorems~5.15--5.22 at \(p=3\)
(centraliser dimension, independent of Pa\v skunas 2013 at the proposition level).
CDP 2014 appears here through Theorems~5.15--5.22 only, not through Proposition~5.9
(class-match input); the two uses are proposition-disjoint.
\end{itemize}
At the proposition level, no result appears on both the derivation and verification
sides of a single closure. The proposition-level disjointness audit passes.
