...
Bombieri--Lagarias shortcut. Neither route addresses the
...
Beurling--Malliavin density) is not used in the route~2 derivation
of the four-way equivalence; and the route~2 derivation (operator-positivity
reformulations, Jacobi-matrix spectral theory, B\'aez-Duarte decay)
is not used in the route~5 closure. The two catalogs share two foundational references: de Branges 1968
(Hermite--Biehler theory, common to both routes) and Akhiezer 1965
(moment theory, used in route~2 for the Jacobi matrix and in route~5
for Hamburger existence and Carleman's criterion). These shared
references serve logically distinct roles: route~2 uses Akhiezer for
operator spectral methods; route~5 uses Akhiezer for classical moment
existence.
Simon 1998 (spectral theory of Jacobi matrices) appears in the
route~2 catalog and is not cited in route~5; Shohat--Tamarkin 1943
is likewise route~2-only. The derivation and verification paths are
therefore source-disjoint on the operative arguments, with the
...
(\(\mathrm{Im}\,\gamma=0\)) and symmetric under the Klein four group
...
\begin{proof}
By \Cref{v4-arith:w5-j:def:HB-decomposition},
\(E_{\xi}=A_{\xi}-iB_{\xi}=\Xi\) as entire functions, so the zero
multi-sets of \(\Xi\) and \(E_{\xi}\) coincide. Under (HB\(^{+}\)),
\Cref{v4-arith:w5-j:prop:HBplus-iff-kernel-positive}(2) gives that
every zero of \(E_{\xi}\) lies in the closed lower half plane
\(\{\mathrm{Im}\,z\leq 0\}\). Hence every
\(\gamma\in\mathcal{Z}_{\xi}\) satisfies \(\mathrm{Im}\,\gamma\leq 0\).
\end{proof}
...
is equivalent to RH. (HB\(^{+}\)) is therefore RH-equivalent. Route~5 closes the
implication from (HB\(^{+}\)) + (BD\(^{+}\)) + (Det) to
spectral zero-placement, making the chain of equivalences
explicit and identifying the exact verification gap.
\end{remark}
...
\end{equation}
has dense range; every \(f\in\mathcal{H}(E_{\xi})\) is determined by
its values on \(\mathcal{Z}_{\xi}\) and is reconstructed by the
Paley--Wiener inversion formula associated with \(E_{\xi}\).
If, in addition, \(D_{\mathrm{BM}}(\mathcal{Z}_{\xi})>1\) strictly,
the evaluation map is also bounded below (a Riesz basis property).
\end{proposition}

\begin{proof}
The exponential type of \(E_{\xi}\) is \(\pi\) (normalised so that
\(\Xi\) has exponential type \(\pi\); de Branges 1994 and
...
system gives dense range of the sampling map. When
\(D_{\mathrm{BM}}>1\), the Beurling--Malliavin theorem gives
stability of the exponential system, hence boundedness below of the
sampling map.
\end{proof}
...
complete family for \(\mathcal{H}(E_{\xi})\): every element of
\(\mathcal{H}(E_{\xi})\) is uniquely determined by its values on
\(\mathcal{Z}_{\xi}\) and is recovered by the Paley--Wiener
inversion formula.
\end{corollary}
...
\Cref{v4-arith:w5-j:prop:BDplus-reconstruction} give dense range of
the evaluation map and hence uniqueness of reconstruction.
\end{proof}
...
Step 6. (BD\(^{+}\)) and
\Cref{v4-arith:w5-j:cor:zeros-sample} confirm that
\(\mathcal{Z}_{\xi}\subset\mathbb{R}\) is a sampling sequence for
\(\mathcal{H}(E_{\xi})\), verifying that the zero multi-set is
locally finite in \(\mathbb{R}\) and that the Hamburger measure
\(\nu_{\xi}\) of Step 5 is supported exactly on \(\mathcal{Z}_{\xi}\):
the Paley--Wiener reconstruction determines \(f\in\mathcal{H}(E_{\xi})\)
from its values on \(\mathcal{Z}_{\xi}\), and \(\nu_{\xi}\) assigns
unit mass to each point of \(\mathcal{Z}_{\xi}\) counted with
multiplicity, so \(\mu_{\xi}=\nu_{\xi}\) as positive Borel measures
on \(\mathbb{R}\).
...
uniqueness and identification data, ensuring the conclusion holds without degenerate representations.
\end{remark}
...
\Cref{v4-arith:w5-j:cor:no-UHP-zeros}).
\item (SH) + (Det) + (BD\(^{+}\)) + functional equation: the
Hamburger representation route establishes \(\mathcal{Z}_{\xi}\subset\mathbb{R}\)
from the positivity of the representing measure and the symmetry
\(\mu_{\xi}=\mu_{\xi}|_{\mathbb{R}}\), without invoking (HB\(^{+}\))
directly.
% RECTIFICATION-FLAG: Gate 1 — verify that (SH)+(Det)+(BD+) alone
% imply Z_xi subset R without HB+; the current Steps 3-6 use Step 2
% which requires HB+. Either supply the missing argument or remove
% this item.
...
\label{v4-arith:w5-j:rem:route2-relation}
\Cref{v4-arith:hpvbconv:thm:VBstar-converse-spectral-regularity}
...
These sources furnish the classical foundations for the route~5
unconditional closure. None supplies the verification of HB\(^{+}\),
BD\(^{+}\), or (Det) on \(\xi_{\mathbb R}\), which remains the single
classical obstruction equivalent to RH.
