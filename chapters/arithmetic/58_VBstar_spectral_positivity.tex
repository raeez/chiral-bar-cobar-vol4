...
wave-2/wave-3 structural analyses.}

\section{Wave-5 Context and Orientation}
\label{v4-arith:w5-j:context}

The wave-2 structural analysis
...
Wave-3 (Chapter~\ref{v4-arith:hpar-det}) reduced the
...
\Cref{v4-arith:platonic:def:RH-package}. Wave-5 agent J closes
...
The wave-2 source catalog for the VB$^{*}$ converse uses
de Branges 1968, de Branges 1994, B\'aez-Duarte 2003, Burnol 2000,
Akhiezer 1965, Simon 1998, Shohat--Tamarkin 1943. Wave-5's
...
derivation sources of waves 2 and 3 (determinacy reductions,
operator-positivity reformulations) are kept disjoint from the
verification sources of wave-5 (classical complex analysis):
...
the shift).
\item[Lagarias, arXiv:math/9805135 (1999).] Survey of de Branges
reformulations of RH; explicit statement of the main-direction
theorem for entire functions of order 1.
\item[Conrey--Li, arXiv:math/9911100 (2000).] Construction of
...
Disjointness rationale: the wave-5 complex analysis (Hadamard
factorisation of $\xi_{\mathbb R}$, de Branges structure theorem,
Hamburger existence, Beurling--Malliavin density) is not used in
the wave-2 derivation of the four-way equivalence; and the wave-2
derivation (unbounded moment problem theory, Akhiezer Jacobi
matrix formalism) is not used in the wave-5 closure. The two
...
to be real. Wave-5's task is to identify sufficient unconditional
...
question whether the zeros are \emph{actually} real.
\end{remark}
...
wave-5 closure uses (Det) as a \emph{hypothesis}: the Carleman sum
...
Lagarias arXiv:math/9805135 \S3). The following are equivalent:
\begin{enumerate}
...
arXiv:math/9805135 \S4): proving that $E_{\xi}$ is Hermite--Biehler
is equivalent to RH. (HB$^{+}$) is therefore RH in disguise. The
purpose of wave-5 is not to prove HB$^{+}$ but to close the
...
Proc.~AMS 121 (1994). (4): Conrey--Li arXiv:math/9911100 Thm.~2.
\end{proof}
...
R}$ in the wave-5 work. Each of these verifications is strictly
...
\begin{remark}[wave-5 contribution in one sentence]
\label{v4-arith:w5-j:rem:contribution}
The wave-5 J contribution is the unconditional closure of the
...
\begin{remark}[relation to wave-2 structural equivalence]
...
Wave-5 inscribes the complementary arrow: (SH) + (Det) + (HB$^{+}$) + (BD$^{+}$)
...
(structural, wave-2) and jointly sufficient for RH (main-direction,
wave-5). RH is the unique missing verification.
\end{remark}

\section{Bibliography Anchors}
\label{v4-arith:w5-j:bibliography}

The primary sources for the wave-5 J closure, beyond the wave-2
...
\item[Conrey--Li, arXiv:math/9911100 (2000).] Theorem~2: the
...
These sources underpin the wave-5 unconditional closure; none
...
\item The wave-5 J closure matches the Vol~IV RH completion
