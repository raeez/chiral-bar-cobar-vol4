...
\emph{Route~7, voice H. The archimedean Tate positivity conjecture A-TP
(\Cref{v4-arith:w5-a:conj:archimedean-tate-positivity}) is
equivalent to negative-semidefiniteness of an infinite Gram matrix
...
strictly equivalent to RH. This chapter proves: (a) the explicit
Hermite--Mellin formula for \(Q_{mn}\) as a digamma-moment integral
\eqref{v4-arith:w7-h:eq:gram-formula}; (b) a Hadamard decomposition
\(Q_{mn}=-\sum_{\rho}\widehat{\phi_{m}}(\gamma_{\rho})\overline{\widehat{\phi_{n}}}(\gamma_{\rho})+(Q_{\mathrm{polar,fixed}})_{mn}\)
matching the Riemann-zero and polar Weil sectors; (c) three natural
regularizations (Tikhonov shift, Gaussian windowing, finite-rank
perturbation), each ruled out as unable to reduce A-TP to a
finite-dimensional statement; (d) the ceiling theorem: uniform
negative-semidefiniteness
\(\sup_{N}\sigma_{\max}(Q_{N})\leq 0\) is strictly equivalent to A-TP,
hence to RH. The Gram-matrix reformulation is explicit, computable,
and tautological at the \(N\to\infty\) limit; this is the
Bombieri--Lagarias ceiling.}
...
to negative-semidefiniteness \(Q\preceq 0\) of the self-adjoint Weil
operator \(Q\) on \(\mathrm{PW}(\mathbb{R})\); computing its Gram
matrix in the Weber--Hermite basis converts the operator inequality
into a countable system of explicitly computable entry-level constraints.
...
\ClaimStatusProvedElsewhere. \(\widehat{\phi_{n}}(t)=i^{n}\phi_{n}(t)\)
under the Fourier normalization \(\widehat{f}(t)=\int f(x)e^{itx}\,dx\)
and the normalization of \eqref{v4-arith:w7-h:eq:hermite}. The Hermite
...
Q_{mn}\;:=\;\frac{1}{2}\Bigl(\mathcal{Q}[\phi_{m}+\phi_{n}]-\mathcal{Q}[\phi_{m}]-\mathcal{Q}[\phi_{n}]\Bigr)
...
\(\widehat{\phi_{m}}\overline{\widehat{\phi_{n}}}=i^{m}\phi_{m}\cdot\overline{i^{n}\phi_{n}}=i^{m}\overline{i^{n}}\phi_{m}\phi_{n}=i^{m-n}\phi_{m}\phi_{n}\).
With the convention \(Q_{mn}=\langle Q\phi_{m},\phi_{n}\rangle_{L^{2}}\)
(the \((m,n)\) matrix entry), the integral becomes
\((2\pi)^{-1}\int i^{m-n}\phi_{m}\phi_{n}\,g\,dt\).
Writing \(i^{m-n}=i^{-(n-m)}\), and restricting to \(n-m\equiv 0\pmod{4}\)
...
(Q_{\mathrm{arch}})_{00}
\;=\;\frac{1}{2\pi}\int_{\mathbb{R}}|\phi_{0}(t)|^{2}\,g(t)\,dt
...
\begin{proof}
Direct substitution: \(\phi_{0}(t)=(2\pi)^{1/4}e^{-\pi t^{2}}\),
so \(|\phi_{0}(t)|^{2}=(2\pi)^{1/2}e^{-2\pi t^{2}}\) and
\[
(Q_{\mathrm{arch}})_{00}=\frac{1}{\sqrt{2\pi}}\int_{\mathbb{R}}e^{-2\pi t^{2}}g(t)\,dt.
\]
% RECTIFICATION-FLAG: Gate 1. The stated value -1.32 and the
% intermediate -2.8 are inconsistent with each other and with the
% formula. A direct numerical integral of (1/sqrt(2pi)) * e^{-2pi t^2}
% * g(t) over R, using g(0) = Re psi(1/4) + log pi approx -3.083, yields
% approximately -0.68, not -1.32. The value -2.8 corresponds to
% (1/(2pi)) * sqrt(2pi) * (-3.083 * 0.92 + 0.04) * (2pi) which is not
% the formula. Requires recomputation with a verified numerical engine
% before the stated constant can be trusted. The sign (negative) is
% unambiguous from g < 0 on the low-frequency core.
...
Q_{mn}\;=\;-\!\sum_{\rho}\widehat{\phi_{m}}(\gamma_{\rho})\overline{\widehat{\phi_{n}}}(\gamma_{\rho})+(Q_{\mathrm{polar,fixed}})_{mn},
\end{equation}
where the sum runs over non-trivial zeros \(\rho=\tfrac{1}{2}+i\gamma_{\rho}\)
with multiplicity, and
\((Q_{\mathrm{polar,fixed}})_{mn}=
\widehat{\phi_{m}*\widetilde{\phi_{n}}}(1/(2i))+\widehat{\phi_{m}*\widetilde{\phi_{n}}}(-1/(2i))\)
is the fixed polar term. Under RH, the ordinate \(\gamma_{\rho}\in\mathbb{R}\),
and \(\widehat{\phi_{n}}(\gamma_{\rho})=i^{n}\phi_{n}(\gamma_{\rho})\)
...
negative-semidefinite on all finite vectors \(c=(c_{m})\) if and only if,
for every such \(c\),
\begin{equation}
\label{v4-arith:w7-h:eq:Q-NSD}
\sum_{m,n}c_{m}c_{n}\cdot(Q_{\mathrm{polar,fixed}})_{mn}\;\leq\;
\sum_{\rho}\bigl|\sum_{m}c_{m}\widehat{\phi_{m}}(\gamma_{\rho})\bigr|^{2},
\end{equation}
which is strictly equivalent to RH via Weil positivity on the
Hermite-spanned Schwartz-dense sub-class.
\end{corollary}

\begin{proof}
Negative-semidefiniteness of the Gram matrix \((Q_{mn})\) on all finite
vectors is equivalent to Weil positivity \(\mathcal{Q}[f]\leq 0\) on the
Hermite-spanned sub-class, which is Schwartz-dense in
\(\mathrm{PW}(\mathbb{R})\); this equals A-TP, hence RH, by
...
\sum_\rho|\sum_m c_m \widehat{\phi_m}(\gamma_\rho)|^2 \leq 0\) iff
...
(\(p\geq 300\) decimal digits). By \Cref{v4-arith:w7-h:thm:finite-N-li},
the truncated Gram matrix \(Q_{N}\prec 0\) (strictly negative-definite)
for the same range.
\end{theorem}
...
direct numerical evaluation of \eqref{v4-arith:w7-h:eq:gram-formula}
yields \(Q_{2}\prec 0\) (strictly negative-definite) with all
eigenvalues negative.
% RECTIFICATION-FLAG: Gate 1. The specific numerical matrix
% Q_2 = ((-1.32, 0.41, -0.14), (0.41, -2.07, 0.65), (-0.14, 0.65, -2.48))
% and eigenvalues {-2.80, -2.08, -0.99} require verification against
% a compute engine implementing eq:gram-formula with eq:gram-fin
% and eq:gram-polar. The (0,0) entry -1.32 is the total Q_{00}
% (archimedean + finite-prime - polar), not just the archimedean
% diagonal; requires recomputation to confirm. The sign (Q_2 prec 0)
% is consistent with Coffey 2005 and is not in doubt.
\end{proposition}
...
Stirling tail correction. All eigenvalues are negative; the exact
values require verification by a separate numerical engine.
\end{proof}
...
routes to reducing A-TP to a finite-dimensional statement: a Tikhonov
shift, Gaussian windowing of matrix indices, and a finite-rank
perturbation. Each is equivalent to A-TP on a dense subspace and
therefore does not reduce the problem.
...
For \(N\leq 10^{5}\), Coffey 2005 establishes \(\sigma_{\max}(Q_{N})<0\),
so this condition holds for all sufficiently small \(\varepsilon>0\)
depending on \(N\).
\end{proposition}
...
Route~7-H via Gram matrix positivity in the Weber--Hermite basis establishes:
\begin{description}
...
(c) Small-\(N\) computation (\(N=2\): \(Q_{2}\prec 0\), all eigenvalues
negative) consistent with RH (\Cref{v4-arith:w7-h:prop:small-N};
explicit values subject to RECTIFICATION-FLAG therein).
...
and Coffey 2005 (extension to \(n\leq 10^{5}\)); the explicit Hermite
basis change and the ceiling theorem are the new contributions.
\end{description}
\end{remark}

\begin{remark}[comparison to route~6-A]
\label{v4-arith:w7-h:rem:compare-w6}
Route~6-A (\Cref{v4-arith:w6-a:ATP-direct-attack}) reduced A-TP via
...
further elementary reductions. Route~7-H attacks A-TP via Gram matrix positivity on the
...
\(Q_{mn}=i^{n-m}(2\pi)^{-1}\int\phi_{m}\phi_{n}\,g\,dt+\text{(finite prime)}-\text{(polar)}\)
...
established by Coffey 2005 (\Cref{v4-arith:w7-h:thm:coffey}); the
