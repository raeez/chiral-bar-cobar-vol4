\chapter{Archimedean Tate Kernel via Riemann--Siegel Theta and Stirling-Certified Bounds}
\label{v4-arith:w7-a:chapter}

\emph{The bounded-interval reduction~A (Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}) proved
\(W_{\infty}\ge0\) on the high-frequency dominance sub-class
...
Riemann--Siegel form gives a disjoint expression. The archimedean digamma
...
phase. Four consequences follow. First, an unconditional
...
threshold \(T_{\mathrm{St4}}=0.872959\ldots\) above which the
rational-plus-logarithmic closed form \(g_{\mathrm{St4}}\) controls
\(g\) to an absolute error at most \(|B_{4}|/(4|z|^{4})\), decaying as
\(|t|^{-4}\). Second, an
...
defined via the sharper Stirling weight, on which \(W_{\infty}\ge0\)
holds
unconditionally. Third, a Stirling-certified value of the sign-change
locus, \(t_{\ast}=0.850662\ldots\), sharpening the elementary Hurwitz
bracket \(t_{\ast}\in(0.84,0.86)\) of
Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}. Fourth, a
precise diagnosis of the obstruction in Z-function language: A-TP on
the complementary class reduces to the same inequality as
\Cref{v4-arith:w6-a:thm:bounded-reduction}; its archimedean
sign-changing term is a phase-derivative expression for
\(Z(t)=e^{i\theta_{\mathrm{RS}}(t)}\zeta(1/2+it)\) on the bounded
interval, while the polar and finite-prime terms remain in \(R[f]\).
The Stirling refinement outcome is an enlarged unconditional sub-class, a
rigorous Stirling-pin of the digamma kernel, and the archimedean part
of the irreducible obstruction re-expressed in classical
Riemann--Siegel language.}
...
sign-change \(t_{\ast}\). \Cref{v4-arith:w6-a:thm:HFD-ATP} proves \(W_{\infty}\ge0\)
on the high-frequency dominance sub-class via the crude bound
\(|g(t)|\leq|g(0)|\) on the core. A sharper bound via classical Stirling asymptotics with explicit
truncation-error control, and the reformulation of A-TP via the
Riemann--Siegel theta function, give the sharper expression below.

\subsection{Domain}
\label{v4-arith:w7-a:domain}
...
for which \(W_{\infty}\ge0\) follows by the Stirling-refined weight;
\item a numerical correction of the Hurwitz-partial-sum value of
\(t_{\ast}\);
\item the Z-function reformulation of the \(R[f]\)-coupled
bounded-interval inequality.
\end{itemize}

This does not prove A-TP on the full class
...
The obstruction uses three classical lists disjoint from the Hurwitz
...
1974 do not appear as proof sources in
...
\ref{v4-arith:w6-a:ATP-direct-obstruction}. The Hurwitz series, Gauss
reflection, and Plancherel--P\'olya theorem cited in
\Cref{v4-arith:w6-a:sources} are disjoint from the
Riemann--Siegel list used here: the former are classical
...
\begin{remark}[source and sign conventions for the exact identity]
\label{v4-arith:w7-a:rem:identity-conventions}
\begin{description}
\item[Source separation.] The identity combines
\Cref{v4-arith:w6-a:def:g} (Hurwitz list) with
\Cref{v4-arith:w7-a:def:theta-RS} (Riemann--Siegel list). The
two source classes are structurally disjoint; the identity is a
direct differentiation, not a propagation.
...
\(\arg\Gamma\) convention (Edwards 1974, \S6.5), fixed by
...
non-vanishing on the vertical line \(1/4+it/2\).
\item[False functoriality.] We assert no functorial property of
\(\theta_{\mathrm{RS}}\) beyond real-analyticity.
...
\end{equation}
Thus the integral on the right-hand side of
\eqref{v4-arith:w7-a:eq:W-as-Z} is the archimedean term. A-TP is the
separate domination inequality \(R[f]\geq W_{\infty}(f\ast\widetilde f)\).
\end{corollary}
...
The kernel \(g(t)\) admits an explicit rigorous expansion with bounded
truncation error, valid for every \(t\in\mathbb{R}\); this is the
Stirling-4 formula, from which the threshold \(T_{\mathrm{St4}}\) follows
...
\end{equation*}
using \(1-4t^{2}=2-u\). Expanding \(-(4/3)(2-u)/u^{2}=-8/(3u^{2})+4/(3u)\) and
differentiating term by term:
\begin{equation*}
\frac{d g^{-}_{\mathrm{St4}}}{du}
=\frac{1}{2u}+\frac{2}{u^{2}}+\frac{16}{3\,u^{3}}-\frac{4}{3u^{2}}+\frac{64}{15\,u^{3}}
=\frac{1}{2u}+\frac{2}{3u^{2}}+\frac{144}{15\,u^{3}}.
\end{equation*}
Each term is positive for all \(u\geq 1\), so
\(dg^{-}_{\mathrm{St4}}/du>0\) on \(u\in[1,\infty)\). Strict monotonicity
...
\begin{remark}[source and sign conventions for the threshold]
\label{v4-arith:w7-a:rem:threshold-conventions}

\begin{description}
\item[Source separation.] Stirling-4 error bound (Edwards 1974)
...
g(t)\;\geq\;\tfrac{1}{2}\log(1+4t^{2})+\log\pi-2\log 2-\frac{4}{1+4t^{2}}
\;\geq\;\log(|t|\pi/2)-\frac{4}{1+4t^{2}}.
\end{equation}
In particular \(g(t)/\log|t|\to 1\) as \(|t|\to\infty\), and
\(g(t)/\log|t|\geq 1/2\) for every \(|t|\geq 2\).
\end{corollary}

\begin{proof}
From \Cref{v4-arith:w7-a:prop:g-stirling-4}:
\begin{align*}
g(t)&\geq g_{\mathrm{St4}}(t)-\frac{32}{15(1+4t^{2})^{2}}\\
&=\tfrac{1}{2}\log(1+4t^{2})+\log\pi-2\log 2-\frac{2}{1+4t^{2}}-\frac{4}{3}\cdot\frac{1-4t^{2}}{(1+4t^{2})^{2}}-\frac{32}{15(1+4t^{2})^{2}}.
\end{align*}
For \(|t|\geq 2\), \(1-4t^{2}\leq 0\), so \(-(4/3)(1-4t^{2})/(1+4t^{2})^{2}\geq 0\) and
\(-2/(1+4t^{2})-32/(15(1+4t^{2})^{2})\geq -4/(1+4t^{2})\), since the residual
\((80t^{2}-52)/(15(1+4t^{2})^{2})\leq 2/(1+4t^{2})\) is direct to verify for
\(|t|\geq 2\) (both sides decay as \(|t|^{-2}\); at \(|t|=2\) the inequality
reads \(268/4335\leq 2/17\), i.e., \(268\cdot 17\leq 2\cdot 4335\)).
Combining the non-negative term with the bound on the remainder
establishes the first inequality of \eqref{v4-arith:w7-a:eq:ratio-bound}.
The second inequality \(\tfrac{1}{2}\log(1+4t^{2})+\log\pi-2\log 2\geq\log(|t|\pi/2)\)
is \(\tfrac{1}{2}\log(1+4t^{2})\geq\log(2|t|)\), which holds since
\(1+4t^{2}\geq 4t^{2}\). The asymptotic \(g(t)\sim\log(|t|\pi/2)\)
gives the ratio claim.
\end{proof}

\section{The Enlarged Sub-Class \(\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\)}
\label{v4-arith:w7-a:SHF-class}

the bounded-interval reduction's HFD sub-class uses the crude bound \(|g(t)|\leq|g(0)|\) on
\([-t_{\ast},t_{\ast}]\). The Stirling-4 analysis tightens the core
control and enlarges the sub-class on which \(W_{\infty}\ge0\).
...
\begin{remark}[Stirling-certified value of \(t_{\ast}\)]
\label{v4-arith:w7-a:rem:t-star-corrected}
\Cref{v4-arith:w6-a:prop:t-star-numerical} brackets
\(t_{\ast}\in(0.84,0.86)\) by an elementary Hurwitz partial sum. A
higher-precision Hurwitz evaluation using \(N=200\) partial-sum terms
plus the uniform tail bound \(|\text{tail}|\leq 3/(4(N+1)(N+1/4))\)
sharpens this to \(t_{\ast}=0.850662304\ldots\), in agreement with the
Stirling-4 threshold analysis above. At \(N=20\) the tail is of order
\(10^{-3}\); since \(g\) is small near \(t_{\ast}\) (one has
\(g(0.85)\approx-1.3\times 10^{-3}\)), localising \(t_{\ast}\) to full
precision requires the larger \(N\), at which the tail drops to
\(10^{-5}\) and the bisection pins
\(t_{\ast}=0.850662304\ldots\).

This numerical correction does not affect the bounded-interval reduction structural
theorems
...
All the Stirling refinement constants using \(t_{\ast}\) are computed with the
corrected value.
\end{remark}
...
\begin{theorem}[unconditional \(W_{\infty}\)-positivity on the SHF sub-class]
\label{v4-arith:w7-a:thm:SHF-ATP}
\ClaimStatusProvedHere. For every
\(f\in\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\),
\(W_{\infty}(f\ast\widetilde{f})\geq 0\). Full A-TP follows on the
sublocus where \(R[f]\geq W_{\infty}(f\ast\widetilde f)\).
\end{theorem}
...
\(W_{\infty}(f\ast\widetilde{f})=(B[f]-A[f])/(2\pi)\geq 0\). The full
A-TP assertion is then \Cref{v4-arith:w6-a:thm:bounded-reduction} with
\(R[f]\geq W_{\infty}(f\ast\widetilde f)\).
\end{proof}
...
Stirling refinement sub-class contains all HFD plus approximately twice as
...
\(\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\supsetneq\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\)
is a strict enlargement, giving a genuine Stirling refinement gain over
the bounded-interval reduction~A. The enlargement is approximately by the factor
...
contains the irreducible A-TP residual, which admits a reformulation
in terms of \(Z(t)\).
...
\ClaimStatusProvedHere. The archimedean inequality
\(W_{\infty}(f\ast\widetilde f)\geq0\) on
\(\mathrm{PW}^{\mathrm{hol}}_{F}\setminus\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\)
...
(\Cref{v4-arith:w6-a:lem:g-monotone}) into a sign statement about
\(\theta''_{\mathrm{RS}}\). For \(f\) real and even,
\(|\widehat{f}(t)|^{2}\) is an even function of \(t\), and integration
by parts on \([-t_{\ast},t_{\ast}]\) with antiderivative
\(G(t):=-2(\theta_{\mathrm{RS}}(t)+t\log\pi)\) of \(-g(t)\) gives
\begin{equation}
\label{v4-arith:w7-a:eq:integration-by-parts}
A[f]\;=\;\int_{-t_{\ast}}^{t_{\ast}}|\widehat{f}|^{2}(-g)\,dt
=\Bigl[-2(\theta_{\mathrm{RS}}+t\log\pi)|\widehat{f}|^{2}\Bigr]_{-t_{\ast}}^{t_{\ast}}
+2\int_{-t_{\ast}}^{t_{\ast}}(\theta_{\mathrm{RS}}(t)+t\log\pi)\cdot(|\widehat{f}|^{2})'\,dt.
\end{equation}
The boundary term vanishes: by oddness of \(\theta_{\mathrm{RS}}\) and
evenness of \(|\widehat{f}|^{2}\), the boundary contributions at
\(t=t_{\ast}\) and \(t=-t_{\ast}\) cancel. The integral on the right
pairs the spectral derivative \((|\widehat{f}|^{2})'\) (odd for \(f\)
real and even) against \(\theta_{\mathrm{RS}}(t)+t\log\pi\) (odd, since
\(\theta_{\mathrm{RS}}\) is odd). The sign of \((|\widehat{f}|^{2})'\)
on \((0,t_{\ast})\) is unconstrained by the SHF hypothesis alone, so
the integral is sign-indefinite on the SHF complement. The
irreducibility of the bounded-interval obstruction persists.
\end{remark}
...
\ClaimStatusProvedHere. After the Stirling refinement:

\begin{description}
\item[\textbf{Unconditional archimedean positivity.}] The inequality
\(W_{\infty}\geq0\) holds on
\(\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\supsetneq\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\)
...
\begin{remark}[source and numerical conventions for the residual theorem]
\label{v4-arith:w7-a:rem:irreducible-conventions}

\begin{description}
\item[Source separation.] The Stirling list (Edwards 1974) is
strictly disjoint from the Hurwitz list (Hurwitz 1882) used in
Chapter~\ref{v4-arith:w6-a:ATP-direct-obstruction}. The two analyses
agree on the numerical values of \(g\) (verified at 50-digit
precision); they agree because they are two expansions of the
same entire function.
...
its proof is a direct substitution.
...
verified to 50-digit mpmath precision.
\end{description}
\end{remark}

\section{Non-Elementary Residual Constructions}
\label{v4-arith:w7-a:remains-open}

\begin{remark}[four non-elementary obstruction constructions]
\label{v4-arith:w7-a:rem:open-constructions}
The Stirling refinement leaves the SHF-complement as the irreducible
residual. Four non-elementary constructions remain, in
parallel with the three of
\Cref{v4-arith:w6-a:rem:live-constructions} plus one added by the
...
\(|z|\approx 0.43\) gains unconditional ground. Form: elementary
cap reached.
...
\([-t_{\ast},t_{\ast}]\). Form: not applicable to the residual.
...
for the shifted phase. Form: non-elementary, equivalent in
strength to HB\(^{+}\) of
...
Chapter~\ref{v4-arith:w5-i:chapter}). Form:
non-elementary, equivalent in strength to the Gaussian explicit-formula
closure.
\end{enumerate}

None of (1)--(4) closes A-TP on the full cone; each is a mathematical
route equivalent in strength to de Branges, Connes, Deninger,
or Hardy. The Stirling refinement sharpens the elementary ceiling by a factor \(\approx 2\)
in the sub-class, names the obstruction precisely in Riemann--Siegel
language, and exhausts the elementary constructions.
\end{remark}

\begin{remark}[comparison to the bounded-interval reduction residual]
\label{v4-arith:w7-a:rem:compare-w6-a}
the bounded-interval reduction
(\Cref{v4-arith:w6-a:ATP-direct-obstruction}) closed \(W_{\infty}\ge0\) on
the HFD sub-class and left full A-TP as the inequality
\(B[f]\leq A[f]+2\pi R[f]\). The Stirling refinement refines this:
...
(\Cref{v4-arith:w7-a:prop:T-St4}).
\item \emph{Stirling-certified \(t_{\ast}\)}: \(t_{\ast}=0.850662304\ldots\),
sharpening the elementary Hurwitz bracket
...
The RH-equivalent residual persists. What the Stirling refinement adds is sharper
quantitative control, a corrected numerical constant, and a
reformulation tying A-TP to the Riemann--Siegel phase list.
\end{remark}

\section{Stirling Boundary}
...
(\Cref{v4-arith:w7-a:rem:t-star-corrected}).
\item The archimedean term \(W_{\infty}\) is non-negative on the enlarged sub-class
\(\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\supsetneq\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\),
...
\([-t_{\ast},t_{\ast}]\), paired against \(|\widehat{f}|^{2}\), after
the same \(R[f]\) functional is retained
(\Cref{v4-arith:w7-a:thm:Z-phase-reformulation}).
\item The full inequality, not the archimedean interval term alone, is
strictly RH-equivalent by
\Cref{v4-arith:w5-a:thm:ATP-equals-weil-positivity}; four
non-elementary constructions (\Cref{v4-arith:w7-a:rem:open-constructions})
remain open and are each equivalent in strength to classical
theories beyond elementary analysis.
\end{enumerate}

\begin{remark}[closing Beilinson principle for the Stirling refinement]
\label{v4-arith:w7-a:rem:closing-beilinson}
the Stirling refinement sharpens but does not overturn the bounded-interval reduction ceiling. The
smaller unconditional theorem on
\(\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\) is strictly larger
than the bounded-interval reduction theorem on
...
\Cref{v4-arith:w7-a:rem:open-constructions}.
\end{remark}
