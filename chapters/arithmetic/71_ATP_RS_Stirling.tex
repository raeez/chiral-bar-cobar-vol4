...
\emph{Wave-6~A (Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}) closed
...
\Cref{v4-arith:w6-a:thm:bounded-reduction}. The wave-7~A outcome is
...
\label{v4-arith:w7-a:rem:attack-heal-identity}
...
\label{v4-arith:w7-a:rem:attack-heal-threshold}
...
Wave-6 A's HFD sub-class uses the crude bound $|g(t)|\leq|g(0)|$ on
...
gives $t_{\ast}=0.850662304\ldots$. The discrepancy in the wave-6 A
...
This numerical correction does not affect the wave-6 A structural
theorems
...
All wave-7~A constants using $t_{\ast}$ are computed with the
corrected value.
\end{remark}
...
wave-7 A sub-class contains all HFD plus approximately twice as
...
$\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}\supsetneq\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}$
is a strict enlargement, giving a genuine wave-7~A gain over
wave-6~A. The enlargement is approximately by the factor
...
\ClaimStatusProvedHere. After the wave-7 A Stirling analysis:
...
\label{v4-arith:w7-a:rem:attack-heal-irreducible}
...
The wave-7 A Stirling-certified closure leaves the SHF-complement as
...
Chapter~\ref{v4-arith:w5-i:R-analytic-weil-gaussians}). Status:
...
Deninger, or Hardy. The wave-7 A closure is that the Stirling
...
\begin{remark}[comparison to wave-6 A residual]
...
against the Hurwitz-series-pinned weight $-g$. Wave-7 A refines
this:
...
The RH-equivalent residual persists. What wave-7 A adds is sharper
...
\begin{remark}[closing Beilinson principle for wave-7 A]
\label{v4-arith:w7-a:rem:closing-beilinson}
Wave-7 A sharpens but does not overturn the wave-6 A ceiling. The
smaller unconditional theorem on
$\mathrm{PW}^{\mathrm{hol},\mathrm{SHF}}_{F}$ is strictly larger
than the wave-6 theorem on
