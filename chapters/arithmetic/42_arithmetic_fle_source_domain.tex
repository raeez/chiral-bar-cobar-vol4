\chapter{Arithmetic FLE: Source Domain and Logical Corrections}
\label{v4-arith:fle-comparison}
...
records the domain boundaries of the geometric FLE sources, explains
why F1/F2/F3 require a distinct mixed-characteristic framework, and
corrects the form of claims that previously conflated the two
settings. No independent comparison pair is constructed here;
arithmetic-FLE realization remains conditional on the boundary
conditions of Section~\ref{v4-arith:fle-comparison:open-conditions}.

\section{Source Theorems}
\label{v4-arith:fle-comparison:sources}

\begin{theorem}[geometric critical-level FLE]
\label{v4-arith:fle-comparison:thm:critical-FLE-source}
\ClaimStatusProvedElsewhere. The 2024 geometric-Langlands proof
constructs the critical-level fundamental local equivalence in the
...
\label{v4-arith:fle-comparison:thm:quantum-FLE-source}
...
\begin{remark}[domain of the geometric FLE sources]
\label{v4-arith:fle-comparison:rem:source-boundary}
...
\section{Domain Distinction: Geometric FLE vs F1/F2/F3}
\label{v4-arith:fle-comparison:non-identification}

\begin{proposition}[F1/F2/F3 as a distinct mixed-characteristic programme]
\label{v4-arith:fle-comparison:prop:not-FLE}
...
\begin{proof}
The geometric FLE (Theorem~\ref{v4-arith:fle-comparison:thm:critical-FLE-source})
...
\label{v4-arith:fle-comparison:rem:finite-prime-replacement}
...
\section{Domain Corrections Across Source Categories}
\label{v4-arith:fle-comparison:table}

\begin{remark}[local/global type distinction]
\label{v4-arith:fle-comparison:rem:local-global-type}
...
\label{v4-arith:fle-comparison:rem:chiral-at-finite-prime}
...
\begin{remark}[quantum-group and root-of-unity source domain]
\label{v4-arith:fle-comparison:rem:qgroup-domain}
...
\begin{remark}[function-field transport and number-field domain]
\label{v4-arith:fle-comparison:rem:function-field-transport}
...
\label{v4-arith:fle-comparison:rem:mixed-char-substrate}
...
\begin{remark}[independent form for arithmetic FLE]
\label{v4-arith:fle-comparison:rem:hz-iv-form}
Arithmetic FLE has no pair of independent constructions in
the present arithmetic formalism. An arithmetic FLE theorem requires first
the boundary conditions of
Section~\ref{v4-arith:fle-comparison:open-conditions}.
\end{remark}

\begin{remark}[AGKRRV 2024 source and target]
\label{v4-arith:fle-comparison:rem:agkrrv-target}
Arinkin--Beraldo--Campbell--Chen--Faergeman--Gaitsgory--Lin--Raskin--%
...
\begin{remark}[F3/BZN form against full Weil target]
\label{v4-arith:fle-comparison:rem:f3-bzn}
...
\label{v4-arith:fle-comparison:rem:q-vs-ps}
...
parameter; identifying \(q = p^{-s}\) requires an explicit independent
bridge; this bridge is the sixth condition in
Section~\ref{v4-arith:fle-comparison:open-conditions}.
\end{remark}

\begin{remark}[Fargues--Scholze, Emerton--Gee, and Dotto--Emerton--Gee]
\label{v4-arith:fle-comparison:rem:fs-eg-deg}
...
\begin{remark}[F1/F2/F3 logical-form correction]
\label{v4-arith:fle-comparison:rem:f123-form}
F1 carries form \emph{Conditional}: the local categorical
...
and BC-diamond-glue steps. F2 carries form
\emph{Conditional}: the mixed-characteristic \(E_{2}\) Hecke
enhancement and restricted tensor product are formally accessible but
unproved. F3 carries form \emph{Conjecture}: the full
Weil/\((\varphi,\Gamma)\) target comparison is beyond current
techniques. Chapters previously marking F1, F2, F3 with premature
theorem-closure have been sharpened accordingly.
\end{remark}

\begin{remark}[local Satake/Yangian normalization]
\label{v4-arith:fle-comparison:rem:satake-yangian}
...
structure, if it exists, must be grounded in an independent
...
\label{v4-arith:fle-comparison:rem:bc-diamond-glue}
...
\label{v4-arith:fle-comparison:rem:wild-prime}
...
\section{Corrected Logical Form}
\label{v4-arith:fle-comparison:form-matrix}

\begin{theorem}[geometric FLE: domain and logical form]
\label{v4-arith:fle-comparison:thm:geometric-fle-form}
...
\begin{remark}[archimedean chiral structure and finite-prime domain]
\label{v4-arith:fle-comparison:rem:chiral-domain}
...
\begin{remark}[corrected form of F1, F2, F3, and BC-diamond-glue]
\label{v4-arith:fle-comparison:rem:corrected-form}
...
carries form \emph{Conditional}: the comparison requires
adelic-descent and BC-diamond-glue steps.
F2 (mixed-characteristic \(E_{2}\) Hecke enhancement and restricted
tensor product for the adelic assembly) carries form
\emph{Conditional}: the \(\infty\)-categorical Tate restricted
...
wild inertia and Frobenius data) carries form \emph{Conjecture}.
...
\label{v4-arith:fle-comparison:open-conditions}
...
\item Construct an independent bridge between the Satake spectral
...
\item Establish two independent constructions of arithmetic FLE.
\end{enumerate}

\begin{remark}[source role of the geometric FLE results]
\label{v4-arith:fle-comparison:rem:integration-rule}
The geometric FLE (Theorems~\ref{v4-arith:fle-comparison:thm:critical-FLE-source}
and~\ref{v4-arith:fle-comparison:thm:quantum-FLE-source}), quantum
...
requires an explicit independent bridge; the bridges are the
