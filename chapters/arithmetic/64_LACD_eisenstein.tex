...
induced representation is reducible.}

\section{Scope, Disjointness, and Parallelism with W6-C}
\label{v4-arith:w6-d:scope}
...
\subsection{Relation to W6-C}
\label{v4-arith:w6-d:scope:w6c}

W6-C establishes the Koszul--Petersson density correction on the
Eisenstein continuum, producing the corrected pairing
\(\langle\,\cdot\,,\,\cdot\,\rangle^{\mathrm{Koszul},\mathrm{LS}}\)
of \Cref{v4-arith:w5-c:prop:LS-absorption} via the Langlands--Shahidi
normalisation. This chapter installs \(N(s,\chi)\) as the intertwining
...
\(N_{v}(s,\chi)\) of \Cref{v4-arith:w5-c:def:LS-factor}; W6-C supplies
the density-match identity on the continuum, this chapter supplies
the categorical lift. The density computation is not reproduced here;
\Cref{v4-arith:w6-d:prop:W6C-crossref} records the cross-compatibility.

\subsection{Primary Sources}
\label{v4-arith:w6-d:scope:sources}
...
\(|N(s,\chi)|/\sqrt{2}\) on the critical line \(\mathrm{Re}(s)=1/2\):
the factor \(\sqrt{2}\) reflects the restriction of the adelic
Plancherel measure to \(i\mathbb{R}_{\geq 0}\) (the Weyl symmetry
\(s\mapsto -s\) folds the axis by two, so the Plancherel density
\(\frac{1}{2}|\xi(1+2it)|^{-2}dt\) on \(i\mathbb{R}\) yields
\(|\xi(1+2it)|^{-2}dt\) on \(i\mathbb{R}_{\geq 0}\), and
\(|N^{\mathrm{LS}}|^{2} = |\xi(1+2it)|^{-2}\) is the squared
Plancherel density on the positive half-axis; see Shahidi 2010
Thm~3.5.1). The Koszul density absorption of
\Cref{v4-arith:w5-c:prop:LS-absorption} is the specialisation of
Shahidi's Plancherel density at \(GL_{2}\). W6-C establishes the
density-match from the Koszul-pairing side; this chapter establishes
...
The W6-C density \(|N^{\mathrm{LS}}(s)|^{2} = |\xi(1+2it)|^{-2}\)
...
\section{Source Audit and Hypothesis Verification}
\label{v4-arith:w6-d:source-audit}

\subsection{Source Disjointness}

\paragraph{Derivation and verification sources.}
The derivation of route~5E cuspidal closure used Flath 1979,
Bernstein 1988, Mo{\oe}glin--Waldspurger 1995 \S III (cuspidal
orthogonality). The present Eisenstein closure uses Langlands 1976,
...
and route~5E cuspidal (\Cref{v4-arith:w5-e:thm:LACD-cuspidal}).
...
over from route~5E through the construction of \(u^{\mathrm{cont}}_v\).
...
\paragraph{Unverified input at the exceptional locus.}
...
This chapter contains no computational engine; the expected-value
verification is through the analytic Plancherel formula.
...
\label{v4-arith:w6-d:residual-ledger}
...
page obligation. W6-C establishes the density-match from the
...
The remaining open obligation is
...
must be reconciled. The non-exceptional locus carries no further
obstruction.
