...
three inputs are disjoint sources in the route~1/route~2 ledger sense.
...
The theorem is assembled through four attack-and-repair cycles.
...
\end{enumerate}
All four are route~0 sources (pre-programme) with no internal
...
\begin{enumerate}
\item[(1)] Completed continuous duals (route~3 OC1 sub-item 1).
\item[(2)] Derived completion (route~3 OC1 sub-item 2).
\item[(3)] Faithful lifting along Nakayama (route~3 OC1 sub-item 3).
\item[(5)] Koszul-dual Bost--Connes presentation (route~3 OC1
sub-item 5).
\end{enumerate}
...
$\mathrm{char}(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis},\mathrm{prim}})(s)=\xi(s)$,
unconditionally.
\item The four attack-and-repair cycles
...
\item \Cref{v4-arith:prim-zeta:cor:p1-residual-reduced}: route~3
