...
\emph{Chapter~\ref{v4-arith:closure} recorded F1.a as theorem-waiting,
...
and the Langlands parameter package; they do not supply the displayed
equivalence
...
source catalog disjoint from Fargues--Scholze; each partial closure
is scoped. No unconditional closure of F1.a is asserted.}

\section{The DEG Result in Context: Scope and Obstructions}
...
\emph{local categorical comparison at every prime} with status
\emph{theorem-waiting}, citing Fargues--Scholze arXiv:2102.13459
as the primary input. That citation is accurate in substance and
loose in scope: Fargues--Scholze supply three components of F1.a
...
\subsection{Partial Closures and Residuals}

Each obstruction reduces to a named conjecture or closes unconditionally:
\begin{itemize}
...
the Bushnell--Henniart packaging), and (ii) a non-semisimple
enhancement of the Emerton--Gee stack encoding the wild inertia
component (\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}).
\item[(c)] \emph{Central character variation} closes unconditionally
once (a) closes, by a pushforward along the central-character
fibration (\Cref{v4-arith:deg-f1a:thm:central-character-variation}).
\end{itemize}

F1.a closes modulo two conjectural inputs: block exhaustion (for (a)
at \(p\geq 5\)) and the wild-EG enhancement (for (b) at \(p=2,3\)).
The strongest unconditional statement is a fully-faithful family DEG
embedding at every \(p\geq 5\), conditional on block exhaustion for
essential surjectivity and unconditional on central-character variation.

\section{Obstruction Ledger}
\label{v4-arith:deg-f1a:ledger}

Six independent verification cycles address the three obstructions
(a), (b), (c). Each applies the seven-attack protocol of \Cref{v4-arith:cl:protocol}
(source collision, sign/convention, ambient category, missing
hypothesis, false functoriality, unproved equivalence,
compute-engine sync). The outline is recorded here; the detail
...
this character torus. The standard decomposition
\(\mathrm{D}([GL_{2}(\mathbb{Q}_{p})])\simeq\bigoplus_{\chi}\mathrm{D}([GL_{2}(\mathbb{Q}_{p})])_{\chi}\)
over central characters allows pushforward of DEG block-by-block
(\S\ref{v4-arith:deg-f1a:central-char}).
Residual: none; assembly is unconditional once~(I) closes.
...
Residual: conditional closure; two conjectural inputs.
...
Seven-attack verifications are recorded inline within each
section. All items close either unconditionally or with named
residuals.
...
% RECTIFICATION-FLAG: arXiv:2603.26887 (Dotto--Emerton--Gee) — theorem number and
% section references in this chapter (Thm 1.1, §4, §5, §2) require verification
% against the published/posted preprint. Cannot verify without access; left as cited.
...
% RECTIFICATION-FLAG [GATE 1]: Step 1 is proved here from EG arXiv:1908.07185 Thm 3.2.1.
% Step 2 (block compatibility) cites DEG §4; the ClaimStatusProvedHere covers only
% the spectral decomposition (Step 1) plus the consequence that DEG respects it;
% the full statement depends on DEG §4, so status is partially ProvedElsewhere.
...
\subsection{Verification Trace for Obstruction (a), \texorpdfstring{\(p\geq 5\)}{p>=5}}
...
\subsection{Verification Trace for Obstruction (b), \texorpdfstring{\(p\in\{2,3\}\)}{p in {2,3}}}
...
% RECTIFICATION-FLAG [GATE 1]: The smoothness claim and density argument in this step
% are informal; "fibres vary continuously" and "determined by generic fibre" for
% sheaves of presentable infty-categories require a precise reference
% (Gaitsgory--Rozenblyum Vol. I Ch. 10 may suffice; needs explicit theorem cite).
...
(Gaitsgory--Rozenblyum Vol.~I Ch.~9, base-change for \(\mathrm{IndCoh}\)
along the determinant fibration). The Serre-weight stratification is
constant over each connected component of \(\mathcal{T}^{\mathrm{cent}}_{p}\)
(Emerton--Gee arXiv:1908.07185 \S 5.4), so the fibre structure is
locally constant over \(\mathcal{T}^{\mathrm{cent}}_{p}\).
...
\subsection{Verification Trace for Obstruction (c)}
...
fibre-to-total passage uses local constancy of the Serre-weight
stratification (Emerton--Gee arXiv:1908.07185 \S 5.4) and the
sheaf-of-categories totalisation theorem (Gaitsgory--Rozenblyum
Vol.~I Ch.~10 Thm~10.3.2.1). \textbf{Pass, hypotheses explicit.}

\paragraph{Attack 5 (false functoriality).} The twisting morphism
\(\chi'\to\chi\) is compatible with both sides of DEG by the naturality
established in Step~3 of the proof of
...
No new residual is introduced; the assembly is unconditional once block exhaustion closes.
...
\item[\textbf{(H3)}] \Cref{v4-arith:deg-f1a:conj:BH-block-exhaustion}
at \(p\in\{2,3\}\) and every \(\chi\).
\end{itemize}
...
\subsection{Verification Trace for Global Assembly}
...
The global consequence is unconditional given the three local residuals plus \(\mathrm{L}_{\mathrm{ACD}}\).
...
of wild-enhanced EG stack) plus
