...
\(\mathbb{F}_{1}\)-descent data. The question is whether
\(\mathcal{V}^{\mathrm{prim}}\) carries a Borger \(\Lambda\)-ring
structure.}
...
The Wilkerson congruence requires a candidate Adams operation \(\psi_{n}\) on
\(\mathcal{V}^{\mathrm{prim}}\). The underlying vector space is the completed
Fock space of a free Heisenberg algebra: for each prime \(p\), the operators
...
\(p\)). A \(\mathbb{Z}\)-lattice in \(\mathcal{V}^{\mathrm{prim}}\)
spanned by eigenstates of the conformal \(L_{0}\) is not preserved
...
with the further intertwining property
...
interaction is a separate datum.
\end{theorem}
...
the natural Fock lattice does not supply (Theorem~\ref{v4-arith:thm:healed-lambda-status}
...
forms are distinct (\Cref{v4-arith:obs:lattice-mismatch-Z}). At the
level of vertex algebras, \(T_{n}\) scales conformal weight
(\Cref{v4-arith:prop:T-n-not-VA-endo}), so the correct enriched
structure is the category \(\mathrm{HVA}\) of Hecke-equivariant vertex
algebras (\Cref{v4-arith:thm:HVA-category}). The \(\mathbb{F}_{1}\)-descent
