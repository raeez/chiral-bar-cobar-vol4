...
Macdonald theory, DAHA, and elliptic Hall algebras depend on uniform
parameters. The primon construction carries a prime-indexed family of
parameters, and this non-uniformity is the obstruction.
...
\;=\;1.
\end{equation}
Equivalently, the unnormalised vector \(p_{k}^{(p)}\) has norm \(k\).
...
free-boson Heisenberg norm for the unnormalised vector. The
normalised vector \(p_k^{(p)}/\sqrt{k}\) has norm \(1\). A
\(\log p\)-weighted arithmetic inner product, if imposed, is an
external grading convention:
\begin{equation}
\langle\!\langle p_k^{(p)},p_k^{(p)}\rangle\!\rangle_{\mathrm{prim}}
\;=\;k\cdot(\log p)^{k}.
\end{equation}
No Kesten--McKay measure is asserted at this limit. That measure is a
graph-theoretic input, not a consequence of the Macdonald
specialisation.
\end{theorem}
...
algebra of Schiffmann--Vasserot. Its degeneration is used only as a
rank-one Hall degeneration; no prime is a DAHA rank.
...
arXiv:1202.2756, Theorem~1.2). Under the degeneration normalisation of
Schiffmann--Vasserot, the spherical elliptic Hall algebra has
\(W_{1+\infty}\) as its limiting rank-one algebra. This chapter uses
only that limiting Hall algebra, not a prime-dependent Yangian
identification.
\end{theorem}
...
\begin{equation}
\omega_{p}(x,y)\;=\;\frac{(1-q_{p}x/y)(1-t_{p}^{-1}x/y)}{(1-x/y)(1-q_{p}t_{p}^{-1}x/y)}.
\end{equation}
...
\end{enumerate}
The trace class \(\mathcal{V}^{\mathrm{prim}}\) is obtained from the
Schiffmann--Vasserot rank-one degeneration in each prime factor;
arithmetic data sits in the tensor-indexing over primes, not in the
deformation parameters.
\end{theorem}
