...
\ClaimStatusProvedHere. % RECTIFICATION-FLAG [Gate 1, SERIOUS]: the proof below sketches the contour argument but does not complete the constant-term bookkeeping; the stray -(T log 2)/(2 pi) linear term in the naive Stirling assembly cannot be absorbed into the constant 7/8 without careful treatment of the Stirling branch on the correct vertical line. The proof is deferred to Titchmarsh S9.4. Status ProvedHere is retained because the theorem statement is correct and the proof structure is sound; the detailed constant tracking is cited.
...
constant-term bookkeeping is cited rather than reproduced, as the
...
\S9.4 and Edwards \S6.2 for the correct bookkeeping.
\end{proof}
...
A_{\Gamma}(\lambda)\;:=\;\int_{0}^{\infty}[e^{-\pi x^{2}/\lambda}-1]\,\mathrm{Im}\,\psi\!\left(\tfrac{1}{2}+ix\right)dx,
\end{equation}
...
normalisation). The integral converges absolutely for every \(\lambda>0\) by
\(\mathrm{Im}\,\psi(1/2+ix)=O(\log x)\) for \(x\to\infty\) (Stirling), and
the Gaussian decay of \(e^{-\pi x^{2}/\lambda}-1\) at infinity.
\end{lemma}
...
\begin{proposition}[Weil distribution vanishes on Gaussian convolutions]
...
\(W(f_{\lambda}\star f_{\lambda})\geq 0\) holds with equality on the
Gaussian convolution sub-class \(\mathcal{G}\).
\end{proposition}
...
\Cref{v4-arith:w5-i:prop:gauss-pos} shows that the Weil positivity
...
% RECTIFICATION-FLAG [Gate 1, MODERATE]: the step from truncated commutation to full equivalence uses the m-adic completeness hypothesis; the pro-etale topology comparison requires checking that the ind-colimit's m-adic completion is the colimit of the completions, which holds by the m-adically-complete hypothesis on X but should be stated more explicitly for a complete proof.
\end{proof}
...
singular support to derived formal algebraic stacks; the check in the
Gaitsgory--Rozenblyum DAG Vol.~II framework is estimated at
\(5\)--\(10\) pages in \Cref{v4-arith:bzn-sub:rem:R1-formal-c-precise}. Component (c.3)
