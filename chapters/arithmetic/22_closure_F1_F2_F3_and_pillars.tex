...
The three conjectures F1, F2, F3 and the three pillars P1, P2, P3 record what the arithmetic branch requires in order to complete the Riemann Hypothesis reformulation of Chapter~\ref{v4-arith:rh-reformulation}. F1 asserts global categorical arithmetic Langlands for \(GL_{2}/\mathbb{Q}\); F2 asserts that the global Hecke category is \(E_{2}\)-monoidal; F3 identifies its Hochschild trace with \(\mathcal{V}^{\mathrm{prim}}\). P1 is the load-bearing equivalence \(\kappa^{\mathrm{Ar}}_{\mathsf{G}}+(\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}=0\Leftrightarrow\xi(s)=\xi(1-s)\); P2 is arithmetic Zhu modularity; P3 is unitarity. Each conjecture and each pillar is subjected to the seven-attack protocol below and either reduced to named components or proved conditionally. The residual obstructions at the end of this chapter are precisely stated, independently attackable claims.

\section{Seven-Attack Reduction Protocol}
...
each of which is either a theorem in the literature, a theorem-waiting
with identified literature gap, or a conjecture with a precise
statement. The seven attacks test whether a given equivalence claim
is structurally sound and, if not, isolate the defect. For each
obligation the following seven attacks are run, in order:
\begin{enumerate}
...
here with derived-Noetherian properties theorem-waiting), and
...
theorem-waiting and conjectural inputs in (F1.a) and (F1.b).
\end{remark}
...
via Scholze's tilting equivalence arXiv:1111.4914. \textbf{Pass.}
...
infrastructure; the missing ingredient is a Tate-style additivity
...
conditional characterisations pending F1--F3. Under this definition,
...
are the arithmetic Koszul levels of the Heisenberg and its dual. Status: Theorem-waiting.
...
\end{enumerate}
Step~1 is Julia's primon gas (1990 proceedings).
...
and Tate (adelic Poisson); they close unconditionally. Step 3 is
conditional on arithmetic Positselski
...
the functional equation \(\xi(s)=\xi(1-s)\) holds. Arithmetic
...
with the adelic Mellin transform computing \(\xi(s)\). Under the
...
of \(\xi(s)=\xi(1-s)\). Step 3 (bar-cobar weight computation) is not
...
adelic Poisson summation, \(\xi\) functional equation) plus the
Riemann--von Mangoldt explicit formula. The arithmetic chiral
interpretation: \(\xi(s)\) as the character of
\(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\), the functional equation
as Zhu \(S\)-modular invariance, \(c^{\mathrm{Ar}}=2\) as the residue. This
...
Tata Institute Bombay 1956). Disjoint. \textbf{Pass.}
...
These five residual obligations cover the full scope of F1, F2, F3,
with two narrower items (\(\mathrm{L}_{\mathrm{ACD}}\) and Koszul--Pet compat
each independently stated and paper-length) and three items of
comparable scope to the original conjectures. P2 closes unconditionally;
