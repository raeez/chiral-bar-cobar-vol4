...
The global Emerton--Gee stack satisfies
\begin{equation}
\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}\;\simeq\;\varinjlim_{S\text{ finite}}\prod_{p\in S}\mathcal{X}^{\mathrm{EG},p}_{GL_{2}}
\end{equation}
as an ind-Noetherian formal algebraic stack, with the inductive
system indexed by finite sets of primes $S$.
\textbf{Status: Theorem} (Emerton--Gee arXiv:1908.07185 constructs
local stacks; the global inductive limit is immediate from the
definition).
...
(F1.d); (F1.c) is a theorem; (F1.a), (F1.b), and (F1.d) are
the residual obstructions.
\end{theorem}
...
$\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}=\varinjlim_{S}\prod_{p\in S}\mathcal{X}^{\mathrm{EG},p}_{GL_{2}}$,
so $\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG,glob}})=\varinjlim_{S}\bigotimes_{p\in S}\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG},p})$
(continuity of $\mathrm{IndCoh}$ under formal-stack inductive limits;
Gaitsgory--Rozenblyum's AMS DAG monographs). By (F1.a), each factor
matches $\mathrm{D}^{\mathrm{sph\text{-}fin}}([GL_{2}(\mathbb{Q}_{p})])$.
By (F1.d), the resulting family assembles compatibly. By (F1.b),
the restricted tensor product of local automorphic categories is
the global automorphic category. Composing gives (\ref{v4-arith:cl:eq:F1-statement}).
\end{proof}
...
to a conjunction of four items, one of which is a theorem and three of
which are strictly narrower theorem-waiting or conjectural inputs.
...
statement; (F1.c) is already a theorem and (F1.a) is a named local
comparison input. Applying \Cref{v4-arith:thm:F1-reduction} reduces F1
...
\end{itemize}
Under this reduction, F2 closes given (F2.a) + (F2.b) + (F2.c);
...
circle $S^{1}=B\mathbb{Z}$ by an arithmetic inertia circle. The
natural candidate is $B\widehat{\mathbb{Z}}=B\prod_{p}\mathbb{Z}_{p}$
(profinite $S^{1}$), which matches the MKR dictionary
(Mazur--Kapranov--Reznikov) identifying $\overline{\mathrm{Spec}\,\mathbb{Z}}$
with an arithmetic 3-manifold. \textbf{Residual: arithmetic
Ben-Zvi--Nadler extension.}
...
The identification with $\mathcal{V}^{\mathrm{prim}}$ follows from
the six-window equivalence
(\Cref{v4-arith:thm:six-window-placement}): $\mathcal{V}^{\mathrm{prim}}$
is defined as the Hochschild trace class, and its six presentations
are extensional characterisations.
