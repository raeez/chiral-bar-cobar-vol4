...
The three conjectures F1, F2, F3 and the three pillars P1, P2, P3 record what the arithmetic branch requires in order to complete the Riemann Hypothesis reformulation of Chapter~\ref{v4-arith:rh-reformulation}. F1 asserts global categorical arithmetic Langlands for \(GL_{2}/\mathbb{Q}\); F2 asserts that the global Hecke category is \(E_{2}\)-monoidal; F3 identifies its Hochschild trace with \(\mathcal{V}^{\mathrm{prim}}\). P1 is the essential equivalence between \(\kappa^{\mathrm{Ar}}_{\mathsf{G}}+(\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}=0\) and the Tate functional equation for \(\Lambda_{\zeta}\), equivalently for the pole-cleared \(\xi_{\mathbb R}\); P2 is the Tate functional equation and explicit formula; P3 is unitarity. Each conjecture and each pillar is subjected to the seven-obstruction criterion below and either reduced to named components or proved conditionally. The residual obstructions at the end of this chapter are precisely stated, independently testable claims.

\section{Seven-Obstruction Reduction Criterion}
\label{v4-arith:cl:criterion}

A conjecture \(C\) is a structured reduction of \(C_{0}\) if
\(C_{0}\Leftrightarrow C\) and \(C\) is a conjunction of named inputs,
each of which is either a theorem in the literature, a conditional
with identified literature gap, or a conjecture with a precise
statement. The seven obstructions are the structural tests for an
equivalence claim. For each condition they are:
\begin{enumerate}
\item \emph{Source separation.} Are the proof source and the
comparison source disjoint in the orthogonal sense?
...
\item \emph{Numerical constant / numerical comparison.} Do
numerical-comparison checks rely on the same source as the derivation?
\end{enumerate}
Each obstruction produces either a valid implication, a correction (reformulation
...
\subsection{Obstruction Analysis Trace}

\paragraph{Obstruction 1 (source separation).} Fargues--Scholze
arXiv:2102.13459 (local geometrisation via Fargues--Fontaine
curve) and Arinkin--Gaitsgory arXiv:1201.6343 (nilpotent
singular support) are disjoint source lists. The
Emerton--Gee stack arXiv:1908.07185 is disjoint from both.

\paragraph{Obstruction 2 (conventions).} Singular-support conventions
...
arXiv:2201.06120, which bridges both. \emph{The comparison is mediated by prismatic Nygaard.}

\paragraph{Obstruction 3 (ambient categories).} LHS is a stable
...
\paragraph{Obstruction 4 (missing hypothesis).} The sph-finite cut on the
...
\paragraph{Obstruction 5 (false functoriality).} Functoriality under
...
\paragraph{Obstruction 6 (unproved equivalence).} The local categorical
comparison
...
change: Caraiani--Scholze
arXiv:1511.02418 for signature \((1,n-1)\) unitary Shimura varieties;
...
\textbf{Form: conditional theorem}. Fargues--Scholze arXiv:2102.13459
supplies the local geometrisation, spectral action, and parameter
spaces. Emerton--Gee arXiv:1908.07185 supplies the
...
\textbf{Form: conjectural, reduces to Tate--Weil restricted
tensor product for automorphic representations} (Tate 1950,
Weil 1967, Jacquet--Langlands 1970 for the representation-level
statement; the categorical lift is conjectural, cf.~Gan--Savin 2022
for partial change).
...
\textbf{Form: definitional here; derived and Noetherian
properties are conditional/conditional on the local deformation
...
\begin{remark}[open content after reduction]
\label{v4-arith:cl:rem:F1-form}
The four items of \Cref{v4-arith:thm:F1-reduction} isolate the open
content of F1: local categorical comparison (F1.a), adelic categorical
descent (F1.b), the Selmer-derived spectral target (F1.c, definitional
here with derived-Noetherian properties conditional), and
local-global diagram commutativity (F1.d). The Selmer spectral target
is correctly formulated; the categorical equivalence depends on
conditional and conjectural inputs in (F1.a) and (F1.b).
\end{remark}
...
\begin{proof}
\(\mathrm{L}_{\mathrm{ACD}}\) combines (F1.b) and (F1.d) into a single
...
\begin{remark}[substantial estimate]
\label{v4-arith:cl:rem:F1-substantial}
...
adapted to the number-field setting. The non-formal theorem condition of
App.~C still applies.
\end{remark}
...
\paragraph{Obstruction 1 (source separation).} Local \(E_{2}\) from
...
\paragraph{Obstruction 2 (conventions).} The \(E_{2}\)-monoidal structure
...
via Scholze's tilting equivalence arXiv:1111.4914.

\paragraph{Obstruction 3 (ambient categories).} \(E_{2}\)-monoidal stable
...
\paragraph{Obstruction 4 (missing hypothesis).} The restricted tensor
...
\(GL_{2}/\mathbb{Q}\) this is every prime.

\paragraph{Obstruction 5 (false functoriality).} The \(E_{2}\)-structure at
...
\paragraph{Obstruction 6 (unproved equivalence).} The precise statement
...
structure.
\textbf{Form: theorem-grade local input} (Fargues--Scholze
...
\textbf{Form: conditional theorem in mixed characteristic}. Bezrukavnikov
...
\textbf{Form: conjectural} (open \(\infty\)-categorical lemma;
expected to follow from Lurie HA~4.8.1.6 (tensor products of
presentable \(\infty\)-categories) plus a Tate-style additivity lemma
for \(E_{n}\)-algebras, but neither that additivity lemma nor the
full compatibility check is in the literature).
\end{itemize}
Under this reduction, F2 would close given (F2.a) + (F2.b) + (F2.c);
(F2.a) is theorem-grade local input, (F2.b) is conditional in
...
\begin{remark}[domain of the open lemma]
\label{v4-arith:cl:rem:F2-open-lemma}
(F2.c) is a general \(\infty\)-categorical statement whose proof would
apply to any restricted tensor product of \(E_{n}\)-categories with
distinguished units, not only to the \(GL_{2}/\mathbb{Q}\) case.
Fresse's \emph{Homotopy of Operads} and Lurie HA provide the operadic
formalism; the missing ingredient is a Tate-style additivity
lemma for \(E_{n}\)-algebras in presentable \(\infty\)-categories.
\end{remark}
...
\paragraph{Obstruction 1--4.} Independent (Ben-Zvi--Nadler
...
is sph-finiteness (matches F1).

\paragraph{Obstruction 5 (false functoriality).} The Hochschild trace is
...
identified via Ben-Zvi--Nadler's categorical-character--Hecke--trace
triangle (Ben-Zvi--Nadler arXiv:0904.1247 \S1). This compatibility is
the residual content. \textbf{Residual: arithmetic Ben-Zvi--Nadler
compatibility.}

\paragraph{Obstruction 6.} Ben-Zvi--Nadler over \(\mathbb{C}\) is a theorem.
...
\paragraph{Obstruction 7.} No numerical comparison.

\subsection{Conditional Closure}

\begin{theorem}[F3 conditional closure]
\label{v4-arith:thm:F3-reduction}
\ClaimStatusProvedHereConditional. Assume (F1.a)--(F1.d) and
...
is defined as the Hochschild trace class, and the other windows are
conditional characterisations conditional on F1--F3. Under this definition,
F3 reduces to (F3.BZN\(^{\mathrm{Ar}}\)), the arithmetic extension of
Ben-Zvi--Nadler Thm~1.1 from the complex setting to the full
Weil/\((\varphi,\Gamma)\) target.
\end{proof}
...
\section{P1: \texorpdfstring{\(\mathsf{G}\)}{G}-Row Complementarity and the Functional Equation}
...
\kappa^{\mathrm{Ar}}_{\mathsf{G}}\,+\,\bigl(\kappa^{!}\bigr)^{\mathrm{Ar}}_{\mathsf{G}}\;=\;0\;\Longleftrightarrow\;\Lambda_{\zeta}(s)\;=\;\Lambda_{\zeta}(1-s)
\end{equation}
for the arithmetic Heisenberg \(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\),
where \(\kappa^{\mathrm{Ar}}_{\mathsf{G}}\) and \((\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}\)
are the arithmetic Koszul levels of the Heisenberg and its dual. Form: conditional theorem.

\subsection{Obstruction Analysis Trace}

\paragraph{Obstruction 1--5.} All conditions hold. Sources are Vol I Koszul
duality (chiral-bar-cobar) and Tate 1950 (adelic Poisson
summation), disjoint.

\paragraph{Obstruction 6 (unproved equivalence).} The forward direction
``Koszul self-duality \(\Rightarrow\) functional equation'' is the
non-trivial content. It requires:
\begin{enumerate}
\item Explicit identification of the primary bar Mellin character
\(\chi_{\mathrm{bar}}(s)=\sum_{m\geq1}m^{-s}\) with \(\zeta(s)\).
...
\end{enumerate}
Step~1 is the conditional primary character input, modelled on
Julia's primon gas (1990 proceedings).
...
\ClaimStatusProvedHereConditional \emph{(conditional on arithmetic
Positselski, \Cref{v4-arith:thm:arithmetic-positselski}, and the
primary bar-character identification
\Cref{v4-arith:thm:heisenberg-bar-zeta})}. For the
arithmetic Heisenberg \(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\),
\begin{equation}
\kappa^{\mathrm{Ar}}_{\mathsf{G}}\,+\,\bigl(\kappa^{!}\bigr)^{\mathrm{Ar}}_{\mathsf{G}}\;=\;0\;\Longleftrightarrow\;\Lambda_{\zeta}(s)\;=\;\Lambda_{\zeta}(1-s).
\end{equation}
Equivalently, after multiplying by \(\tfrac12s(s-1)\), the same
statement holds for \(\xi_{\mathbb R}\).
\end{theorem}

\begin{proof}
The proof has four steps; the bar-character and Positselski steps are
the stated conditional inputs.
...
to the primary zero-mode quotient of
\(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\) and its arithmetic bar
complex (\Cref{v4-arith:def:arith-bar-concrete}), the Mellin character
of the Hecke-monoid grading is
\begin{equation}
\chi_{\mathrm{bar}}(s)\;=\;\sum_{m\geq1}m^{-s}\;=\;\zeta(s).
\end{equation}
This is the conditional character input
...
is \(\zeta(\beta)\) for \(\beta>1\); outside this half-plane one uses
the meromorphic continuation of the scalar \(\zeta\), not a convergent
KMS trace. Under Koszul duality the primary bar character of
\(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\) pairs
with the Cobar of \(\mathbb{A}^{\mathrm{BC}}\); this pairing is the
adelic Mellin transform computing
\(\Lambda_{\zeta}(s)=\pi^{-s/2}\Gamma(s/2)\zeta(s)\).
The archimedean \(\Gamma\)-factor contributes the determinant-rank
residue \(r_{\zeta}=2\cdot\mathrm{Res}_{s=1}\zeta(s)\)
...
weight vanishes termwise under
\(\alpha^{(p)}_{-n}\leftrightarrow\mu_{p^{n}}\), yielding
...
\begin{equation}
\Lambda_{\zeta}(s)\;=\;\int_{\mathbb{A}^{\times}}\Theta(a)|a|^{s}\,d^{\times}a\;=\;\int_{\mathbb{A}^{\times}}\mathcal{F}_{\mathbb{A}}\Theta(a)|a|^{s}\,d^{\times}a\;=\;\Lambda_{\zeta}(1-s),
\end{equation}
where the second equality is Fourier invariance of \(\Theta\) and
the third is change of variables \(a\mapsto a^{-1}\). Multiplication
by the symmetric Tate-pole factor \(\tfrac12s(s-1)\) gives
...
equation \(\Lambda_{\zeta}(s)=\Lambda_{\zeta}(1-s)\), equivalently
\(\xi_{\mathbb R}(s)=\xi_{\mathbb R}(1-s)\). Conversely, the functional equation
forces the bar-cobar pairing to be \(\mathbb{Z}/2\)-equivariant under
\(s\mapsto 1-s\), which by Positselski implies
\(\kappa^{\mathrm{Ar}}_{\mathsf{G}}+(\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}=0\).
\end{proof}

\begin{remark}[conditional structure of \Cref{v4-arith:thm:P1-resolution}]
\label{v4-arith:cl:rem:P1-conditional}
Steps 1, 2, 4 of \Cref{v4-arith:thm:P1-resolution} draw on
Julia--Spector (primon gas), Bost--Connes (Hecke \(C^{*}\)-algebra),
and Tate (adelic Poisson); these are the classical inputs. Step 3 is
...
Ann.~Math.~109, 1979 for the abelian case; conjectural in general,
but satisfied for the abelian Heisenberg).
\end{remark}

\begin{corollary}[forward implication of P1]
\label{v4-arith:cl:cor:P1-unconditional-forward}
\ClaimStatusProvedHereConditional \emph{(conditional on
\Cref{v4-arith:cor:koszul-dual-BC})}. Assuming \(\kappa^{\mathrm{Ar}}_{\mathsf{G}}+(\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}=0\),
the functional equation for \(\Lambda_{\zeta}\), and hence for
\(\xi_{\mathbb R}\), holds. Arithmetic
Positselski (\Cref{v4-arith:thm:arithmetic-positselski}) is not
required.
\end{corollary}

\begin{proof}
Assume \(\kappa^{\mathrm{Ar}}_{\mathsf{G}}+(\kappa^{!})^{\mathrm{Ar}}_{\mathsf{G}}=0\).
Step 1 (Julia--Spector primon gas) identifies the bar generating
function with \(\zeta(s)\). Step 2 (Bost--Connes Koszul duality,
\Cref{v4-arith:cor:koszul-dual-BC}) identifies the bar-cobar pairing
with the adelic Mellin transform computing \(\Lambda_{\zeta}(s)\). Under the
hypothesis, the bar-cobar pairing is \(\mathbb{Z}/2\)-equivariant.
Step 4 (Tate 1950) gives the Fourier symmetry \(\mathcal{F}_{\mathbb{A}}\Theta=\Theta\),
and change of variables \(a\mapsto a^{-1}\) completes the derivation
of \(\Lambda_{\zeta}(s)=\Lambda_{\zeta}(1-s)\). The pole-cleared
\(\xi_{\mathbb R}\) then has the same involution. Step 3
(bar-cobar weight computation) is not
invoked; the hypothesis supplies equivariance directly.
\end{proof}

\section{P2: Tate Functional Equation and Explicit Formula}
\label{v4-arith:cl:P2-resolution}

\subsection{Statement}

P2 asserts that the meromorphic Tate character
\(\Lambda_{\zeta}(s)=\pi^{-s/2}\Gamma(s/2)\zeta(s)\) satisfies the
Mellin-theta presentation, the functional equation, the explicit
formula, and the residue identity. No global VOA or Zhu flat
connection is used for \(\mathcal{V}^{\mathrm{prim}}\).

\subsection{Obstruction Analysis Trace}

\paragraph{Obstructions 1--7.} Sources: Tate 1950 (Mellin-theta) and
Riemann--von Mangoldt (explicit formula). Disjoint. Conventions: Tate's normalised
\(\Lambda_{\zeta}(s)=\Gamma(s/2)\pi^{-s/2}\zeta(s)\). Ambient category:
meromorphic functions with Tate boundary poles at \(s=0,1\). \emph{All conditions hold.}

\subsection{Theorem}

\begin{theorem}[Tate functional equation and explicit formula]
\label{v4-arith:thm:P2-resolution}
\ClaimStatusProvedHere. The meromorphic completed zeta
\(\Lambda_{\zeta}(s)=\pi^{-s/2}\Gamma(s/2)\zeta(s)\) is the Tate
character of the arithmetic Heisenberg trace and satisfies the
following four identities:
\begin{enumerate}
\item \emph{Mellin-theta presentation.}
\begin{equation}
\Lambda_{\zeta}(s)\;=\;\frac{1}{2}\int_{0}^{\infty}\bigl(\theta(t)-1\bigr)\,t^{s/2-1}\,dt,\qquad\theta(t)\;=\;\sum_{n\in\mathbb{Z}}e^{-\pi n^{2}t}.
\end{equation}
\item \emph{Logarithmic-derivative residue.} The logarithmic
derivative has contour integral
\begin{equation}
\frac{1}{2\pi i}\oint\Lambda_{\zeta}'(s)/\Lambda_{\zeta}(s)\,ds,
\end{equation}
equal to the divisor count inside the contour, with the Tate boundary
poles included with sign.
...
\(\Lambda_{\zeta}\)-line by \(\Lambda_{\zeta}(s)=\Lambda_{\zeta}(1-s)\).
\item \emph{Arithmetic rank residue.} The primary determinant has
rank residue
...
(2) The contour integral is the argument-principle form of the
explicit formula of analytic number theory: for any test function \(h\) of
suitable growth,
\begin{equation}
...
\end{equation}
Riemann--von Mangoldt explicit formula (Riemann 1859; von Mangoldt
Crelle 114, 1895). No Virasoro zero-mode on
\(\mathcal{V}^{\mathrm{prim}}\) is used.

(3) The Jacobi theta functional equation \(\theta(t)=t^{-1/2}\theta(1/t)\)
plus change of variables \(t\mapsto 1/t\) in (1) gives
\(\Lambda_{\zeta}(s)=\Lambda_{\zeta}(1-s)\);
this is Riemann's 1859 proof of the functional equation. Multiplying
by \(\tfrac12s(s-1)\) gives the same involution
for \(\xi_{\mathbb R}\).

(4) Residue at \(s=1\) of \(\Lambda_{\zeta}(s)\): from \(\Lambda_{\zeta}(s)=\Gamma(s/2)\pi^{-s/2}\zeta(s)\)
and \(\mathrm{Res}_{s=1}\zeta(s)=1\), \(\Gamma(1/2)\pi^{-1/2}=\pi^{1/2}\pi^{-1/2}=1\),
giving \(\mathrm{Res}_{s=1}\Lambda_{\zeta}(s)=1\). Hence \(r_{\zeta}=2\).
\end{proof}

\begin{remark}[P2 as Tate's thesis]
\label{v4-arith:cl:rem:P2-is-Tate}
The mathematical content of P2 is Tate 1950 (Mellin-theta symmetry,
adelic Poisson summation, the \(\Lambda_{\zeta}\) functional equation) plus the
Riemann--von Mangoldt explicit formula. The arithmetic chiral
interpretation: \(\Lambda_{\zeta}(s)\) as the Tate character of
\(\mathcal{V}^{\mathrm{prim}}_{\mathrm{Heis}}\), the functional equation
as Tate self-duality, and
\(\xi_{\mathbb R}(s)=\tfrac12s(s-1)\Lambda_{\zeta}(s)\) as the
pole-cleared determinant for zeros. The number \(r_{\zeta}=2\) remains
the residue. This
is the content of the arithmetic branch; the underlying analytic inputs
are classical. P2 closes unconditionally.
\end{remark}
...
\paragraph{Obstructions 1--4.} Sources: Weil 1967 (Basic Number Theory),
Bost--Connes 1995, Selberg 1956 (Rankin--Selberg positivity,
Tata Institute Bombay 1956). Disjoint.

\paragraph{Obstruction 5 (false functoriality).} Positive-definiteness at
...
\paragraph{Obstruction 6 (unproved equivalence).} Compatibility between
...
\ClaimStatusProvedHereConditional \emph{(conditional on identification of
...
compatible Adams endomorphism \(\psi_{p}\) (the degree-\(p\) Adams
operation acting on the local Bost--Connes algebra at \(p\)). The inner product is the
...
\begin{equation}
\langle f,g\rangle^{\mathrm{Pet}}\;=\;\int_{[GL_{2}]}f(h)\overline{g(h)}\,dh,
\end{equation}
with Tamagawa measure \(dh\) on the double coset. Positive-definite
...
\section{Logical Form Table}
\label{v4-arith:cl:form-update}

\begin{table}[h]
\centering
\small
\begin{tabular}{l|l|l}
Condition & Initial form & Final form \\
\hline
...
P1 (\(\mathsf{G}\)-row) & Conditional & ProvedHere forward implication; full equivalence conditional on arith.~Positselski \\
P2 (Tate/expl.~formula) & Unstated & ProvedHere (Tate 1950 framing) \\
P3 (unitarity) & Unstated & ProvedHere conditional on Koszul--Petersson compatibility \\
\end{tabular}
\caption{Logical forms after \Cref{v4-arith:closure}.}
\label{v4-arith:cl:table:form}
\end{table}
...
\begin{enumerate}
\item F1 passes through \(\mathrm{L}_{\mathrm{ACD}}\), but its present
table also contains the local categorical comparison, the global
restricted-tensor identification, and BC-diamond-glue
...
(F3.BZN\(^{\mathrm{Ar}}\)).
\item P1 has a forward implication under the stated Bost--Connes input
(\Cref{v4-arith:cl:cor:P1-unconditional-forward}) and a conditional
full equivalence (\Cref{v4-arith:thm:P1-resolution}).
\item P2 closes unconditionally: the Tate functional equation is
framed via Mellin-theta and the explicit formula
...
restricted over places. It is a non-formal theorem, number-field adaptation
of Gaitsgory--Rozenblyum function-field results.
\item[(F2.c)] \(\infty\)-categorical Tate restricted tensor product
of \(E_{n}\)-categories with distinguished units. It is a non-formal theorem,
\(\infty\)-operadic.
\item[(F3.BZN\(^{\mathrm{Ar}}\))] Arithmetic Ben-Zvi--Nadler
identification: \(E_{1}\)-algebra equivalence
\(\mathrm{HH}_{\bullet}(\mathrm{Hk}^{\mathrm{Iw}}_{GL_{2},\mathrm{glob}})\simeq\mathcal{O}(\mathrm{LocSys}^{W,\varphi\Gamma}_{GL_{2}})^{\mathrm{flat}}\)
(Ben-Zvi--Nadler arXiv:0904.1247 extended to the
Weil/\((\varphi,\Gamma)\) arithmetic target).
It is a non-formal theorem.
\item[(arith.~Positselski)] Arithmetic Positselski equivalence for
\(\Lambda\)-coalgebras with \(\mu=0\)
(\Cref{v4-arith:thm:arithmetic-positselski}). It is a non-formal theorem,
...
(\Cref{v4-arith:cl:cj:koszul-Petersson-compat}). It is a non-formal theorem,
verification.
\end{description}

These five residual conditions cover the full domain of F1, F2, F3,
with two narrower items (\(\mathrm{L}_{\mathrm{ACD}}\) and Koszul--Pet compat
each independently stated and substantial) and three items of
comparable domain to the original conjectures. P2 closes unconditionally;
P1 and P3 close conditionally on arith.~Positselski and Koszul--Pet
compat respectively. The Riemann Hypothesis reformulation of
Chapter~\ref{v4-arith:rh-reformulation} proceeds from this basis.
