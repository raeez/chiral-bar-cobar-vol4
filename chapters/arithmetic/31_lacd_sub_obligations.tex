...
\emph{Route~2 of the adversarial audit
(\Cref{v4-arith:sw:wave2}) refined the route~1 decomposition of
...
(archimedean stratum admissibility). The route~1
residual $\mathrm{L}_{\mathrm{ACD}}.4$ (strong approximation for
$GL_{2}/\mathbb{Q}$) was already theorem by Kneser 1966 /
Platonov--Rapinchuk 1994. This chapter closes all four route~2
sub-obligations, each through a source-disjoint route. The route~1
...
page compatibility lemma that sister-route $\varepsilon$ attacks
independently.}

\section{Route~2 Context and Statement of Sub-Obligations}
\label{v4-arith:lacd:context}

\subsection{Genealogy of the Sub-Obligation List}

The route~1 modular decomposition (\Cref{v4-arith:acd:thm:modular-decomposition})
split $\mathrm{L}_{\mathrm{ACD}}$ into $(L_{1})$--$(L_{4})$. Route~1
of the adversarial audit (\Cref{v4-arith:sw:thm:LACD-split})
...
\end{itemize}
Route~2 (\Cref{v4-arith:sw:wave2}), specifically the auditor
...
The combined route~2 list of sub-obligations is:
\begin{itemize}
...
route, of the first four sub-obligations. The route~1 route through
...
The derivation sources in route~1 were Raskin 2021 JAMS for $(L_{1})$,
...
We execute five attack-and-repair cycles, one per sub-obligation plus a
...
\begin{remark}[reconciliation with Route~1 $(L_{4})$]
\label{v4-arith:lacd:rem:L4-reconciliation}
The route~1 $(L_{4})$ of \Cref{v4-arith:acd:thm:modular-decomposition}
was the archimedean-extension requirement, stated as ``the archimedean
factor extends the finite-place product by direct tensoring''. Route~2
...
rather than Vec. The route~1 statement is preserved; the route~2
...
sheaves on the Selmer-derived global spectral stack.
\end{corollary}

\begin{proof}
By the route~1 split of $\mathrm{L}_{\mathrm{ACD}}$
(\Cref{v4-arith:sw:thm:LACD-split}) plus route~2 refinement
...
Platonov--Rapinchuk 1994, route~1).
\end{itemize}
...
\Cref{v4-arith:tate-tensor}, closed by route~1 attack-and-repair.
...
\begin{remark}[(BC-diamond-glue) scope after Route~2]
...
Bernstein-centre construction). Route~2 does not change this
...
Before route~1: $\mathrm{L}_{\mathrm{ACD}}$ estimated at $80$--$120$
pages. After route~1 four-way split plus (BC-diamond-glue) isolation:
$30$--$50$ pages. After route~2 closure of $\mathrm{L}_{\mathrm{ACD}}.0$
and the route~2 definitional tightening of F2.c: $30$--$50$ pages,
...
cleanly closed by the source-disjoint route constructed here). The net
...
\small
\begin{tabular}{l|l|l}
Sub-obligation & Status after route~2 + this chapter & Source \\
\hline
...
(BC-diamond-glue) & \textbf{Conjectural} & Ben-Zvi--Chen--Helm--Nadler 2010.02321; route $\varepsilon$ \\
\end{tabular}
\caption{Status of $\mathrm{L}_{\mathrm{ACD}}$ components after
route~2 + this chapter. Six theorems; single conjectural residual.}
...
Springer sheaves. Sister-route $\varepsilon$ attacks
...
Raskin 2021 JAMS & Restricted tensor product for finite primes & Route~1 residual \\
Tate 1950 & Adelic Mellin/theta and functional equation & Route~1 residual \\
Kneser 1966 & Strong approximation for $GL_{2}$ & $\mathrm{L}_{\mathrm{ACD}}.4$ (theorem) \\
(BC-diamond-glue) & Bernstein-centre gluing & Route $\varepsilon$, not yet theorem \\
\end{tabular}
\caption{Hypotheses used in this chapter and their sources. The
only non-theorem is (BC-diamond-glue), handled by sister-route
$\varepsilon$.}
...
\begin{enumerate}
\item Route~2 of the adversarial audit identified five sub-obligations
...
\item This chapter closes the four route~2 sub-obligations
...
(BC-diamond-glue), attacked independently by sister-route
...
the route~1 Fargues--Scholze-based catalog: the route~2 catalog
...
With this chapter, the four route~2 sub-obligations of
