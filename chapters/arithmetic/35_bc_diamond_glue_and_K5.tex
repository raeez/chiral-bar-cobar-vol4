...
Two obligations from \Cref{v4-arith:acd:thm:final-reduction} and
...
\Cref{v4-arith:kp:thm:main-full} was inscribed conditionally.
This chapter decomposes (i) into three logically independent
sub-obligations and isolates the irreducible cross-prime compatibility
(``BC-diamond-glue.core'') as the minimal residual. It closes (ii) on the cuspidal sph-fin locus by specifying the
Koszul-dual normalisation convention and verifying its adelic
consistency.

\section{Incoming Obligations and Disjoint-Source Discipline}
\label{v4-arith:bcglue-k5:context}

Two compatibility obligations are inherited from upstream chapters.
...
consistently with the global Bernstein-centre action. The scope estimate
from \Cref{v4-arith:acd:rem:bc-diamond-glue-scope} was \(30\)--\(50\) pp;
this chapter tightens the reduction to isolate the irreducible
...
conditional on the absorption convention. This chapter pins the
convention explicitly, verifies its adelic consistency with
Tamagawa normalisation, and closes K5 on the cuspidal sph-fin locus.
...
theory). The pairs of catalogs satisfy the source-disjointness requirement
of the Vol~IV verification programme: no primary author
appears at overlapping work, no derivation and verification path
share a primary source.

\section{Verification Ledger for the Six Sub-Obligations}
\label{v4-arith:bcglue-k5:ledger}

The six sub-obligations are verified in order; each is checked
against an independent primary source from \Cref{v4-arith:bcglue-k5:context}.
...
first two sub-obligations fall within reach of
...
on the global stack, of scope \(10\)--\(15\) pp.
...
Each of the six verifications is a cross-check against an independent
primary source from \Cref{v4-arith:bcglue-k5:context}; none introduces
new construction beyond the primary-source inputs.
...
(BC-diamond-glue.core): a single cross-prime compatibility of
scope \(10\)--\(15\) pp, conditional on arithmetic AG. The primary-source
obligations remaining are the local Emerton--Gee comparison, the
...
and on (BCglue.local) and (BCglue.glue) being proved)}. After the
reductions of this chapter, \(\mathrm{L}_{\mathrm{ACD}}\)
(\Cref{v4-arith:acd:thm:LACD-single-lemma})
has the following residual catalog:
\begin{enumerate}
\item (BCglue.local): \textbf{Theorem-waiting}; Ben-Zvi--Chen--Helm--Nadler supply the
...
\begin{remark}[scope decomposition of (BC-diamond-glue)]
\label{v4-arith:bcglue-k5:rem:scope-delta}
\Cref{v4-arith:acd:rem:bc-diamond-glue-scope} estimated
(BC-diamond-glue) at \(30\)--\(50\) pp past Ben-Zvi--Chen--Helm--Nadler.
The decomposition of \Cref{v4-arith:bcglue-k5:thm:decomposition}
partitions that scope into \(5\)--\(10\) pp for (BCglue.local), \(5\)--\(10\) pp
for (BCglue.glue), and \(10\)--\(15\) pp for (BC-diamond-glue.core),
...
equals the local Whittaker/Kirillov pairing under the Koszul-dual
normalisation convention of \Cref{v4-arith:bcglue-k5:def:koszul-dual-norm},
where \(\sigma^{\mathrm{BC}}_{p}\) is the Tomita--Takesaki modular
involution of the KMS\(_{1}\) state on \(\mathcal{H}^{(p)}\)
(Bost--Connes 1995 \S5):
\begin{equation}
...
cuspidal sph-fin locus (\Cref{v4-arith:w3-audit-B:repair-35}).
\end{remark}
...
% RECTIFICATION-FLAG [Gate 1: Jacquet 1979 Corvallis Prop.~3.1 -- pin-cite likely wrong for
% the local Rankin-Selberg L-factor factorisation L_p(s,pi x pi^v) = L_p(s,Ad)·ζ_p(s).
% The Corvallis Proceedings (Jacquet 1979, "Automorphic L-functions") contains multiple
% propositions; Prop.~3.1 there concerns the functional equation, not the factorisation.
% The factorisation L_p(s,pi x pi^v) = L_p(s,Ad)·ζ_p(s) is in Jacquet--Shalika 1981
% (Ann. Math. 114, "On Euler products and the classification of automorphic representations
% I") or Bump 1997 §3.8.5. Verify before removing flag.]
...
% RECTIFICATION-FLAG [Gate 1: Step 4 -- L_p(1,Ad) cancellation is mis-identified.]
% The equation as written asserts that the Whittaker pairing factors as
% L_p(1,Ad)·(1-p^{-1}) times the Koszul-bare pairing, based on Step 3's
% evaluation of Z(1,W_{|0>},W_{|0>}) for the zonal spherical vector.
% Two steps are missing: (a) the conversion from Z(1,W,W) at the spherical
% Whittaker vector to the full Whittaker pairing ⟨v,σ^BC(w)⟩^Whit for
% general v,w in H^{(p)}_{sph-fin,cusp}; (b) the conversion from the
% Whittaker pairing to the Koszul-bare pairing.  After L_p(1,Ad) cancels,
% the residual factor is (1-p^{-1}) = ζ_p(1)^{-1}; this matches K5 only
% if (1-p^{-1})·⟨v,w⟩^{Koszul-bare} = ⟨v,w⟩^BC, which is the
% Plancherel-volume identification, but this identification requires
% Whittaker→Koszul-bare as an intermediate step that is not supplied.
% Do not guess: supply the missing conversion from a primary source before
% removing this flag.
...
cuspidal core under the Koszul-dual normalisation. The prior
...
% RECTIFICATION-FLAG [Gate 1: Bump 1997 §3.8 citation for global Rankin-Selberg
% factorisation of Petersson pairing likely mis-cited. The identity
% ⟨v,w⟩^Pet = L(1,Ad)·⟨v,w⟩^{Pet,reduced} is in newform normalisation, not a
% global pairing rescaling by L(1,Ad) as a scalar. Bump 1997 §3.8 concerns the
% analytic properties of Rankin-Selberg L-functions, not a Petersson pairing
% factorisation. Verify against Hoffstein-Lockhart 1994 or Iwaniec-Kowalski
...
\begin{remark}[K5 status upgrade]
\label{v4-arith:bcglue-k5:rem:K5-delta}
\Cref{v4-arith:sw:thm:KP-split}(K5) inscribed K5 as
...
\Cref{v4-arith:bcglue-k5:thm:K5-adelic-consistent}
close K5 on the cuspidal sph-fin locus. The
...
K5 was conditional on the GJ absorption convention. This chapter closes
...
\Cref{v4-arith:kp:cor:P3-unconditional} inscribed P3 conditional on
K-P compat, which was itself conditional on K5
(\Cref{v4-arith:sw:thm:KP-split}).
\Cref{v4-arith:bcglue-k5:thm:K5-absorption-canonical} closes K5
on the cuspidal sph-fin locus. The logical chain
\[
...
The K5 closure of this chapter does not remove additional residuals
from the RH path but upgrades the status of K-P compat from
``conditional on convention'' to the explicit scope restriction of
...
After the closures and reductions of this chapter, the open residuals
split as follows.

\begin{description}
\item[(BCglue.local).] Theorem-waiting;
\Cref{v4-arith:bcglue-k5:thm:local-BZCHN-plus-EG}.
Residual scope: \(5\)--\(10\) pp.
...
\item[Arithmetic AG (global).] Open conjecture. Scope: substantial
...
Arith.~Positselski: \(\sim 30\)--\(50\) pp residual scope.
VB\(^{*}\) is the 100-year-old open problem.
\end{description}

\begin{remark}[chapter residual summary]
\label{v4-arith:bcglue-k5:rem:residual-summary}
This chapter tightens:
\begin{enumerate}
\item (BCglue.local): \(5\)--\(10\) pp, status \emph{theorem-waiting}; the
...
primary-source writing obligation; see
\Cref{v4-arith:bcglue-k5:thm:local-BZCHN-plus-EG}.
\item (BCglue.glue): \(5\)--\(10\) pp, status \emph{conjectural} conditional
...
\item (BCglue.local) is theorem-waiting: Ben-Zvi--Chen--Helm--Nadler +
...
past F2.c and the sub-obligations of
...
full-sector extension and KMS\(_{1}\)/GNS identification remain open.
\item Residual scope: this chapter tightens \(\sim 20\)--\(40\) pp and
reduces (BC-diamond-glue) by a factor of \(2\)--\(3\); the primary open
...
conditional on arithmetic AG, and K5 is closed on the cuspidal sph-fin
locus under the explicit Koszul-dual normalisation convention. The
K-P compatibility and P3 are scope-restricted theorems on the
cuspidal sph-fin locus. The minimal RH path is unaffected.

The primary sources are drawn from seven disjoint catalogs:
...
the source-disjointness requirement of the verification programme
is satisfied.
