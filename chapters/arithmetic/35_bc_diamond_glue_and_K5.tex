...
\emph{Wave-2 attack-heal. Two bundled obligations left open by wave-1:
...
\begin{description}
\item[\textbf{BC-diamond-glue, wave-1 status.}]
\Cref{v4-arith:acd:thm:final-reduction} reduced
...
(arXiv:2010.02321) glue to a global coherent sheaf on the global
Emerton--Gee stack, and that the glued sheaf acts on
$\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})$
consistently with the global Bernstein-centre action. Wave-1
estimated scope $30$--$50$ pp; wave-2's mandate is to tighten the
reduction to the smallest-possible residual and to isolate the
irreducible primary-source obligation.
\item[\textbf{K5, wave-1 status.}]
...
Godement--Jacquet local functional equation.'' Status at wave-1:
Theorem conditional on the absorption convention. Wave-2's
...
required, wave-1's 30--50 pp estimate would collapse. Verify
...
$\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}}$ agrees with the external
...
over finite sets $S$ of primes to obtain
$\mathcal{F}_{\mathrm{glob}}^{\mathrm{prism}}\in\mathrm{Coh}(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})$.
...
\]
would exist in $\mathrm{Coh}(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})$
...
single cross-prime compatibility on the global Emerton--Gee stack.
\end{theorem}
...
actions on $\mathrm{IndCoh}_{\mathrm{Nilp}}(\mathcal{X}^{\mathrm{EG,glob}}_{GL_{2}})$:
...
arithmetic AG.
\end{center}
Wave-2's contribution is to isolate (BC-diamond-glue.core) as the
...
current Theorem-waiting / Conjectural statuses)}. After wave-2's
...
\begin{remark}[scope delta from wave-1]
\label{v4-arith:bcglue-k5:rem:scope-delta}
Wave-1 \Cref{v4-arith:acd:rem:bc-diamond-glue-scope} estimated
(BC-diamond-glue) at $30$--$50$ pp past BZCHN. Wave-2 decomposes
...
cuspidal sph-fin locus (wave-3 audit
...
cuspidal core under the Koszul-dual normalisation. The wave-1
...
Wave-1 \Cref{v4-arith:sw:thm:KP-split}(K5) inscribed K5 as
``Theorem conditional on Godement--Jacquet absorption convention.''
Wave-2's contribution, \Cref{v4-arith:bcglue-k5:def:koszul-dual-norm},
...
K5 was conditional on the GJ absorption convention. Wave-2 closes
...
unconditional after wave-1 K-P compat. Wave-1's K-P compat was
itself conditional on K5 (swarm-ledger \S\ref{v4-arith:sw:thm:KP-split}
and \Cref{v4-arith:sw:table:residuals}); wave-2 upgrades K5 to
unconditional via
...
two (arith.~Pos., VB$^{*}$) after wave-1. Wave-2's K5 closure
...
unaffected by wave-2 and remain open.
\end{remark}

\section{Residual Scope}
\label{v4-arith:bcglue-k5:residual}

After the two wave-2 closures of this chapter, the open residuals
attached to wave-1's conditional obligations split as follows.
...
wave-2. Scope: substantial (primary-literature-length); this is
...
wave-3 or wave-4: arith.~Positselski $\sim 30$--$50$ pp; VB$^{*}$
is the $100$-year-old open problem.
\end{description}

\begin{remark}[wave-2 residual summary]
...
literature as of the wave-2 inscription date.
...
past F2.c and the wave-2 sub-obligations of
...
full-sector extension and KMS$_{1}$/GNS identification remain open.
\item Residual scope: wave-2 tightens $\sim 20$--$40$ pp; reduces
...
The chapter converts the two wave-1 obligations, (BC-diamond-glue)
...
locus. The minimal RH path is unaffected by wave-2.

The primary sources carrying wave-2's closures are drawn from
