...
The arithmetic Koszul-duality programme assembles an ordered-incidence
...
\ClaimStatusProvedElsewhere. Write nontrivial zeros of the completed
Riemann zeta function \(\xi(s)\) as \(\rho = \frac{1}{2} + i\gamma_\rho\).
...
\end{equation}
where \(W_\infty(h)\) is the archimedean gamma-factor contribution
...
prime-power index set; none supplies the zero-side spectral operator.
\end{remark}
...
is whether the zero sum on its left side admits a spectral
interpretation, i.e., whether some natural operator has its spectrum
at the zero ordinates $\gamma_\rho$. Connes' adele-class-space
construction provides a partial answer.
...
\eqref{v4-arith:tfe:eq:weil}. The scaling operator is not a
self-adjoint Hilbert-space operator: its spectrum is not automatically
confined to the critical line, and the distributional trace is not
the spectral measure of a self-adjoint operator with determinant
\(\xi(s)\).
\end{theorem}
...
\Cref{v4-arith:3d-acs:cor:partition-bar})}. Let \(\Gamma\) be a
positively oriented vertical contour in the critical strip enclosing
all nontrivial zeros of \(\xi\). The ACS argument-principle functional
\begin{equation}
...
h(s)\frac{Z'(s)}{Z(s)}\,ds
\end{equation}
equals the Weil explicit-formula functional \eqref{v4-arith:tfe:eq:weil}
whenever \(Z(s)=\xi(s)\) is assumed, and hence agrees with the
Connes--Meyer scaling trace on \(\mathcal H\).
\end{theorem}

\begin{proof}
Under the stated assumptions \(Z=\xi\). Applying the argument
principle to \(\xi'/\xi\) inside \(\Gamma\) gives the zero sum
\(\sum_\rho h(\gamma_\rho)\). Expanding \(\xi'/\xi\) in the
...
The arithmetic Gutzwiller package of the previous section requires a
...
this structure, or whether the Gutzwiller route is genuinely new input.
...
it exists, would place the spectrum on the critical line by
...
\begin{remark}[open obligations as explicit hypotheses]
\label{v4-arith:tfe:rem:residual}
\Cref{v4-arith:tfe:thm:stat-upside} rests on seven open obligations:
...
not gaps in the present chapter. The ordered bar supplies obligation~(3)
via the Euler-product character
\eqref{v4-arith:tfe:eq:ord-bar-product}. Obligations~(1), (4), (6),
and~(7) constitute the spectral-flow programme of
\Cref{v4-arith:3d-acs:conj:spectral-flow}.
\end{remark}
