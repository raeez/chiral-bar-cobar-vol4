...
The Riemann Hypothesis is equivalent to the conjecture F-RH:
...
Zhu modularity, under which the non-trivial zeros of the complete
character \(\xi_{\mathbb{R}}(s)\) lie on \(\mathrm{Re}(s)=1/2\).
Chapter~\ref{v4-arith:closure} resolves the three structural pillars
P1, P2, P3; the residual obligation is the arithmetic Virasoro
bootstrap (VB), or equivalently, a positive Hilbert--P\'olya Li trace.
...
\ClaimStatusConditional{(VB)}. Suppose:
\begin{itemize}
...
\begin{remark}[VB as residual obligation]
\label{v4-arith:rh:rem:VB-single-open}
With P1, P2, P3 resolved to Theorem or Theorem-conditional,
F-RH reduces to VB, the arithmetic Virasoro bootstrap: the
statement that a unitary self-dual arithmetic chiral algebra with
arithmetically modular character places all non-trivial character
zeros on the self-dual locus. VB is the arithmetic analogue of the
\(c<1\) Friedan--Qiu--Shenker bootstrap for unitary Virasoro
representations, adapted to \(c=2\) and the arithmetic modular setting.
\end{remark}
...
arithmetic chiral-algebra framework and exploit the Petersson inner
...
ordinate operator, three incompatible roles (see
\Cref{v4-arith:rh:rem:spectral-convention}). The correct statement
is the one-directional HP--Li positivity
\(\Rightarrow\) F-RH, where H-P\(^{\mathrm{Ar}}\) supplies
\(\Theta_{\xi}\). The converse, whether F-RH supplies a positive
Weil trace representation of the full zero distribution, is open:
it is the Hilbert--P\'olya/Connes--Deninger obstruction.
\end{remark}

\begin{remark}[content of F-RH as a reformulation]
\label{v4-arith:rh:rem:what-is-gained}
F-RH does not prove RH; it reformulates it in four ways.
\begin{enumerate}
\item The spectral target is made precise: H-P\(^{\mathrm{Ar}}\) plus
HP--Li positivity are together equivalent to RH once the positive
trace is supplied.
...
\item The reformulation connects RH programs (Connes, Deninger,
Meyer) to unitarity bootstrap methods in the style of
Friedan--Qiu--Shenker, Cardy, and Zhu, with the arithmetic
modular setting replacing the compact-CFT setting.
\item A computational target becomes accessible: admissible
approximants to the Li/Weil test functions can be evaluated against
the Bost--Connes and Hecke trace packages. Positivity is a
...
Weil-positive trace rather than through the bare trace-formula route.
\end{enumerate}
\end{remark}
...
the F-RH reformulation above, RH reduces to the following
six precisely-stated obligations, any one of which closes the
circuit at a different level:
...
form the minimal sufficient set for RH through the
...
the chiral-algebra framework is:
\begin{center}
...
chain-level trace setting where unitarity-bootstrap and factorisation
techniques may be applied without conflating them with the
zero-spectrum theorem.
\end{remark}
...
F-RH asks whether Koszul self-duality, which delivers the functional
equation, combined with the positivity of P3 and the modularity of
P2, suffices to deliver the spectral zero-placement that RH asserts.
The answer within the chiral-algebra framework is: yes, conditional
on H-P\(^{\mathrm{Ar}}\) and HP--Li positivity. Those two statements
are the Hilbert--P\'olya form of RH native to this framework.

\section{The F-RH Reduction: Complete Statement}
...
\item F-RH is implied by HP--Li positivity
...
positive-trace equivalent of RH within the chiral-algebra framework.
The operator-dictionary repair of
