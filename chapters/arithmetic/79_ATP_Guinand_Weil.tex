...
\emph{Chapters~\ref{v4-arith:w5-a:chapter} and~\ref{v4-arith:w6-a:ATP-direct-attack}
attack A--TP from the Weil 1952 side: the archimedean kernel is the
...
which Weil-axis and Guinand--Burnol attacks deliver unconditional
...
The derivation sources for this chapter are disjoint from those of
...
(Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}), while the Guinand
...
% RECTIFICATION-FLAG: Gate 1 -- the claim that the polar terms h(+/- i/2) >= 0
% for f real even requires either a non-negativity hypothesis on f or a separate
% argument. For general real even f in PW, h(i/2) = int f(u) e^{-u/2} du can
% be negative. The proof below bounds the archimedean pairing by the zero-sum
% alone via (CP3); the positivity of polar+prime terms is invoked as a
% fortiori step but should be stated as a separate lemma or hypothesis in a
% full treatment.
\begin{proof}
By \Cref{v4-arith:w7-i:prop:guinand-as-hilbert}, A--TP
is \eqref{v4-arith:w7-i:eq:guinand-hilbert}. Condition (CP3) gives
\(\int_{0}^{\infty}\varphi\widetilde g\,dy/y
\leq\sum_{\rho}|\widehat f(\gamma_{\rho})|^{2}\). The right-hand side
of \eqref{v4-arith:w7-i:eq:guinand-hilbert} exceeds
\(\sum_{\rho}|\widehat f(\gamma_{\rho})|^{2}\) by the polar correction
\(h(\pm i/2)\) and the prime-side term
\(-2\sum_{n}\Lambda(n)n^{-1/2}g(\log n)\). By
\Cref{v4-arith:w5-a:thm:prime-side}, the prime-side correction is
non-negative when (CP2) holds. Hence the full right-hand side of
\eqref{v4-arith:w7-i:eq:guinand-hilbert} is at least
\(\int_{0}^{\infty}\varphi\widetilde g\,dy/y\), giving
\eqref{v4-arith:w7-i:eq:guinand-hilbert}, which is A--TP.
\end{proof}
...
\(T\) small, where \(\widetilde g(1)>0\) by the leading asymptotics of
\Cref{v4-arith:w7-i:rem:sign-tilde-g} (since \(1>1/\pi\)). The parameter \(a\) is determined by a single equation \(\widetilde\varphi_a(0)=0\),
which has a solution by the intermediate value theorem applied to
the continuous function \(a\mapsto\widetilde\varphi_a(0)\).
\end{remark}
...
Chapter~\ref{v4-arith:w6-a:ATP-direct-attack} in the following
...
zero-sum plus the positive tail on \((1/\pi,\infty)\). The theorem
...
Chapter~\ref{v4-arith:w6-a:ATP-direct-attack} obstruction
\begin{equation}
\int_{-t_{\ast}}^{t_{\ast}}|\widehat f(t)|^{2}\cdot(-g(t))\,dt
\end{equation}
in the following sense: Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}
...
\item \textbf{Sub-class closure:} A--TP closes unconditionally on
\(\mathrm{PW}^{\mathrm{CP}}\)
(\Cref{v4-arith:w7-i:thm:copoisson-closure}), which is non-empty
(\Cref{v4-arith:w7-i:rem:CP-nonempty}) and orthogonal to the HFD
sub-class of Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}
...
\begin{remark}[combined coverage and residual obstruction]
\label{v4-arith:w7-i:rem:honest-status}
A--TP closes unconditionally on two disjoint sub-classes:
\(\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\) (spectral axis,
Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}) and
...
than either individually, but the full class \(\mathrm{PW}(\mathbb{R})\)
is not covered. The residual is a two-dimensional obstruction:
...
Chapter~\ref{v4-arith:w6-a:ATP-direct-attack} identified the A--TP
