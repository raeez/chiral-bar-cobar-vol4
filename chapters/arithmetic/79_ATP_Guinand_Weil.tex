...
\emph{Wave-5 (Chapter~\ref{v4-arith:w5-a:chapter}) and wave-6
...
$t_{\ast}\approx 0.794$. Wave-7 opens a second axis. Guinand's 1948
...
sub-class, and this sub-class is \emph{orthogonal} to the wave-6 HFD
...
\section{Orientation and Relation to Wave-5/Wave-6}
...
Wave-5 (\Cref{v4-arith:w5-a:chapter}) closed the prime side by a
Rankin--Selberg isometry; wave-6 (\Cref{v4-arith:w6-a:ATP-direct-attack})
analysed the digamma kernel $g$ and closed the sub-case of
high-frequency spectral dominance.

Wave-7 shifts the axis. Instead of the Weil 1952 distributional form
...
Hilbert-transform positivity sub-class, orthogonal to wave-6 HFD).
...
The wave-7 attack uses four classical catalogs, disjoint from the
derivation sources used in wave-5 (Rankin 1939, Selberg 1940,
Hecke 1936, Deligne 1974), wave-6 (Hurwitz 1882, Gauss 1813,
...
sign-change at $t_{\ast}\approx 0.794$ (wave-6), while the Guinand
...
\emph{orthogonal} to the wave-6 HFD class
...
\int_{0}^{1/\pi}\varphi(y)\cdot\bigl(-\widetilde g(y)\bigr)\,\frac{dy}{y}
\end{equation}
against the zero-side sum. This is \emph{orthogonal} to the wave-6
obstruction
\begin{equation}
\int_{-t_{\ast}}^{t_{\ast}}|\widehat f(t)|^{2}\cdot(-g(t))\,dt
\end{equation}
in the following precise sense: wave-6 controls the \emph{spectral}
behaviour on the bounded spectral interval $[-t_{\ast},t_{\ast}]$ with
$t_{\ast}\approx 0.794$; wave-7 controls the \emph{time-domain}
...
\section{Wave-7 I Deliverables}
\label{v4-arith:w7-i:deliverables}

\begin{theorem}[wave-7 A--TP status]
\label{v4-arith:w7-i:thm:wave7-status}
\ClaimStatusProvedHere. The wave-7 I attack on A--TP via the
...
\emph{orthogonal} to the wave-6 HFD sub-class
...
orthogonal to the wave-6 bounded-spectral-interval residual on
...
\begin{remark}[honest status after wave-7 I]
\label{v4-arith:w7-i:rem:honest-status}
A--TP unconditionally closes on two disjoint sub-classes:
$\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}$ (wave-6, spectral
axis) and $\mathrm{PW}^{\mathrm{CP}}$ (wave-7, time-domain axis).
...
\begin{remark}[comparison to wave-6 residual]
\label{v4-arith:w7-i:rem:compare-w6}
Wave-6 A identified the A--TP residual as a bounded-spectral-interval
problem on $[-t_{\ast},t_{\ast}]\subset\mathbb{R}_{t}$ with
$t_{\ast}\approx 0.794$. Wave-7 I identifies a second,
...
combined wave-6 plus wave-7 closure covers $\mathrm{PW}^{\mathrm{hol},
...
wave-6 HFD sub-class (\Cref{v4-arith:w7-i:rem:orthogonal-HFD}).
...
wave-6 bounded-spectral-interval residual on
$[-t_{\ast},t_{\ast}]$.
...
\item The two sub-class closures (wave-6 HFD and wave-7 CP) cover
...
The wave-7 I deliverable obeys the Beilinson principle: a smaller
...
sign-control on $(0,1/\pi)$) and orthogonal to the wave-6 spectral
residual. The combined wave-6 plus wave-7 coverage expands the
