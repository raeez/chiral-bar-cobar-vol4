...
\emph{Construction \(\gamma\) in Chapter~\ref{v4-arith:deg-f1a} reduced F1.a at
\(p\in\{2,3\}\) to the wild-enhanced Emerton--Gee stack, the
Bushnell--Henniart-compatible wild block functor, and the
Bushnell--Henniart version of block exhaustion at the wild primes.
F1.a.3 is a small local residual: non-formal theorem at a
first substantial estimate, bounded in difficulty by the finite
cardinality of the Bushnell--Kutzko type list and by the precise
\(\mathrm{Ext}^{1}\)-structure recorded in
Chapter~\ref{v4-arith:bzn-rcompletion}. The analysis attacks F1.a.3
directly. It does not close it unconditionally. It proves it conditional
on three disjoint, individually tractable inputs, each shorter than
the residual itself: the BH type-block comparison
...
(\Cref{v4-arith:f1a3-BH:conj:wild-compact-generation}). Under these
three hypotheses the wild block functor is an equivalence; no
essential-surjectivity gap remains beyond them.}
...
central character
\(\chi\colon\mathbb{Z}_{p}^{\times}\to k^{\times}\), and a conductor
bound \(N\). We write \(\mathbf{T}^{\mathrm{BH}}_{p,\chi}\) for the
Bushnell--Kutzko types with central character \(\chi\) and conductor
exponent at most \(N\). We work with the
...
\[
\mathrm{Mod}_{GL_{2}(\mathbb{Q}_{p})}^{\mathrm{sm},\chi,\leq N}
\;=\;\prod_{\tau\in\mathbf{T}^{\mathrm{BH}}_{p,\chi}}\mathrm{Block}^{\mathrm{BH}}_{\tau},
\qquad p\in\{2,3\},
\]
the conductor-\(\leq N\) summand of the Bushnell--Kutzko block
decomposition. On the spectral side, the wild-enhanced EG stack
...
\begin{itemize}
\item[\textbf{(A)}] The BH type-block comparison conjecture
...
\end{itemize}
Then for every \(p\in\{2,3\}\), every central character \(\chi\), and
every conductor bound \(N\), every
block
...
application of the direct-sum decomposition; the conjectural content
is concentrated in (A), (B), (C).
\end{proof}

\begin{remark}[why F1.a.3 is the smallest residual]
\label{v4-arith:f1a3-BH:rem:smallest-residual}
Chapter~\ref{v4-arith:deg-f1a} records the following local residuals
for F1.a:
F1.a.1 (block exhaustion at \(p\geq 5\), substantial non-formal theorem),
F1.a.2 (wild-EG enhancement at \(p\in\{2,3\}\), substantial \(20\)--\(40\)
pp), the wild block functor datum, and F1.a.3 (a substantial
non-formal theorem). The
smaller size reflects three simplifying features: (i) the BH type
list is finite, not varying over a deformation space; (ii) the
...
reduction makes each BH type a local computation whose domain is
bounded.
\end{remark}

\section{Proof Reduction and Source Comparison}
\label{v4-arith:f1a3-BH:table}

The five analyses below apply the seven-source comparison criterion of
\Cref{v4-arith:cl:criterion} (source separation,
sign/convention, ambient category, missing hypothesis, false
functoriality, unproved equivalence, numerical comparison).

\begin{description}
\item[\textbf{BH tame-wild enumeration at \(p=2\).}]
\emph{Obstruction.} Enumerate Bushnell--Kutzko types in
\(\mathbf{T}^{\mathrm{BH}}_{2,\chi}\). Show that wild types at \(p=2\)
are classified in conductor \(\leq N\) by the unramified principal
series, the tame supercuspidal, the wild supercuspidal types of
bounded conductor,
and the spherical (Iwahori-fixed) type absorbing the
Ext\({}^{1}\)-extension of
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p2}.
\emph{Calculation.} The BH classification
(Bushnell--Henniart 2006 \S 11--13) gives the exact list; the wild
conductor-exponent bound is \(a(\chi)+2\), cf.\ BH \S 12.1.
\emph{Obstruction: (A), the BH type-block comparison conjecture.}

\item[\textbf{BH tame-wild enumeration at \(p=3\).}]
\emph{Obstruction.} Enumerate Bushnell--Kutzko types in
...
\emph{Calculation.} The three BH types in the principal block match the
...
\item[\textbf{Essential surjectivity via type-by-type exhaustion.}] \emph{Obstruction.} Fix a hypothetical
...
\emph{Calculation.} By (C), \(\mathcal{F}_{\tau}\) is a colimit of shifts of
...
\item[\textbf{Comparison with the wild-EG decomposition of Chapter~\ref{v4-arith:deg-f1a}.}] \emph{Obstruction.} Does the BH-component
decomposition coincide with the Bushnell--Henniart
fibre-correspondence decomposition of
\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}(3)? \emph{Calculation.}
...
\item[\textbf{Unconditional closure test.}]
\emph{Obstruction.} Can (A), (B), (C) collectively close on existing
primary literature? \emph{Calculation.} (A) is known for \(p\geq 5\) by Paskunas
...
structure sheaves not yet stated. (B) is known numerically (both
...
case. None of the three is closable from currently-stated primary
literature; all three are individually tractable.
\emph{Final obstruction: (A), (B), (C), three named conjectures.}
\end{description}

The source comparison for each reduction step is recorded in
\S\ref{v4-arith:f1a3-BH:residuals-traces}. The first four analyses
close conditionally on (A), (B), (C); the fifth identifies the three
as the minimal conjectural set.
...
\(\chi\colon\mathbb{Z}_{2}^{\times}\to k^{\times}\) and a conductor
bound \(N\). The bounded set
\(\mathbf{T}^{\mathrm{BH}}_{2,\chi}\) of Bushnell--Kutzko types with
central character \(\chi\) and conductor exponent at most \(N\)
consists of three sorts of types:
\begin{enumerate}
...
\item \textbf{Wild supercuspidal types.} One type per admissible wild
character of a ramified quadratic extension \(E/\mathbb{Q}_{2}\), with
conductor exponent in the range imposed by \(\chi\) and by \(n\leq N\).
\end{enumerate}
The bounded set is finite:
\(|\mathbf{T}^{\mathrm{BH}}_{2,\chi}|<\infty\), bounded in terms of
\(N\) and \(a(\chi)\).
\end{proposition}
...
supercuspidal classification and conductor exponents at \(p=2\);
...
\ClaimStatusProvedHereConditional. Assume the BH type-block comparison
...
\ClaimStatusProvedElsewhere. Fix
\(\chi\colon\mathbb{Z}_{3}^{\times}\to k^{\times}\) and a conductor
bound \(N\). The bounded set \(\mathbf{T}^{\mathrm{BH}}_{3,\chi}\)
consists of:
\begin{enumerate}
...
\item \textbf{Wild supercuspidals.} One per admissible wild character
of a ramified quadratic extension \(E/\mathbb{Q}_{3}\), with conductor
exponent \(a(\chi)+1\leq n\leq N\).
...
small primes). The conductor bound cuts the infinite admissible
character families to a finite Bushnell--Kutzko set.
\end{proof}
...
\begin{proposition}[dimensional comparison \(p=2\)]
\label{v4-arith:f1a3-BH:prop:dim-comparison-p2}
...
\begin{proposition}[dimensional comparison \(p=3\)]
\label{v4-arith:f1a3-BH:prop:dim-comparison-p3}
...
dimensional comparisons \Cref{v4-arith:f1a3-BH:prop:dim-comparison-p2} and
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3}. The non-semisimple locus
...
\subsection{(A) BH Type-Block Comparison}

\begin{conjecture}[BH type-block comparison]
...
\begin{remark}[evidence for the BH type-block comparison]
...
because the block structure is more complex; the comparison at
\(p\in\{2,3\}\) is the extension of Paskunas' result to the BH type
list, with the same projective-to-structure-sheaf identification.
...
indicator-character-module; the BH comparison conjecture asserts that this
...
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p2} and
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3} each close the dimensional
...
in degree \(1\); extending the generation statement to the derived-smooth
case is the conjectural content of (C).
\end{remark}
...
\item F1.a.3: (A), (B), (C) of
\Cref{v4-arith:f1a3-BH:thm:main} (BH type-block comparison, Paskunas wild Ext\({}^{1}\),
...
\]
The central-character variation
\Cref{v4-arith:deg-f1a:thm:central-character-variation} also requires
the family datum
\Cref{v4-arith:deg-f1a:conj:central-character-family}.
\end{corollary}

\begin{proof}
Taking the direct sum of
...
(central-character variation, conditional on the family datum).
\end{proof}

\subsection{The Updated F1 Obstruction Decomposition}

\begin{proposition}[F1 obstruction decomposition after BH block exhaustion]
\label{v4-arith:f1a3-BH:prop:F1-table-updated}
\ClaimStatusProvedHere. The total F1 obstruction decomposition (combining
F1.a.1--F1.a.3 with \(\mathrm{L}_{\mathrm{ACD}}\)) factors into the
following named conjectural inputs:
\begin{enumerate}
\item F1.a.1: block exhaustion at \(p\geq 5\)
(\Cref{v4-arith:deg-f1a:conj:block-exhaustion}), substantial
non-formal theorem.
\item F1.a.2: wild-EG enhancement at \(p\in\{2,3\}\)
(\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}), substantial
non-formal theorem.
\item Wild block functor datum at \(p\in\{2,3\}\)
(\Cref{v4-arith:deg-f1a:conj:wild-block-functor}).
\item F1.a.3.A: BH type-block comparison
(\Cref{v4-arith:f1a3-BH:conj:BH-packaging}), substantial non-formal theorem.
\item F1.a.3.B: Paskunas wild Ext\({}^{1}\)-compatibility
(\Cref{v4-arith:f1a3-BH:conj:Paskunas-wild-ext}), substantial
non-formal theorem.
\item F1.a.3.C: wild-EG compact-generator identification
(\Cref{v4-arith:f1a3-BH:conj:wild-compact-generation}), substantial
non-formal theorem.
\item Central-character family datum
(\Cref{v4-arith:deg-f1a:conj:central-character-family}).
\item \(\mathrm{L}_{\mathrm{ACD}}\): adelic categorical descent
(Ch.~\ref{v4-arith:closure}), substantial non-formal theorem.
\end{enumerate}
The three sub-residuals of F1.a.3 together form a non-formal theorem, matching
the aggregate F1.a.3 domain identified in Chapter~\ref{v4-arith:deg-f1a}.
\end{proposition}

\begin{proof}
The enumeration is direct. F1.a.1, F1.a.2, the wild block functor
datum, and the central-character family datum are inherited from
Chapter~\ref{v4-arith:deg-f1a}. F1.a.3 factors as (A)+(B)+(C) by
\Cref{v4-arith:f1a3-BH:thm:main}. \(\mathrm{L}_{\mathrm{ACD}}\) is the
adelic residual of Chapter~\ref{v4-arith:closure}.
\end{proof}

\section{Residual Conditions and Source Comparisons}
\label{v4-arith:f1a3-BH:residuals-traces}

\subsection{Named Residuals Introduced}

F1.a.3 factors into three sub-conjectures:
...
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p2} and
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3}).
...
\subsection{Unconditional BH Comparisons}

Three statements are unconditional:

\begin{enumerate}
\item Dimensional comparison at \(p=2\)
(\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p2}): \(1=1\).
\item Dimensional comparison at \(p=3\)
(\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3}): \(2=2\).
\item Structural consistency with Chapter~\ref{v4-arith:deg-f1a}
(\Cref{v4-arith:f1a3-BH:prop:structural-consistency}): the
BH-component decomposition matches the wild-EG
decomposition of Chapter~\ref{v4-arith:deg-f1a} structurally.
\end{enumerate}

These unconditional statements establish the decomposition and the
numerical match of F1.a.3. The conjectural content is the fine
identification:
class-level match, compact generation, and projective-to-structure-sheaf
comparison.

\subsection{Source and Convention Comparison: BH Tame-Wild Classification at \(p=2\)}

\paragraph{Source separation.} Bushnell--Henniart 2006
\S 11--13, Carter 1989 \S 7.3, Lusztig 1980, Jantzen 2003 Ch.~II
\S 8, and Paskunas 2013 \S 6 are disjoint from Fargues--Scholze
arXiv:2102.13459 and from Dotto--Emerton--Gee arXiv:2603.26887.
DEG is confined to \(p\geq 5\); the \(p=2,3\) extension is the
conjectural content of (A).

\paragraph{Conventions.} BH uses Langlands-normalised
L-parameters; the EG stack follows Emerton--Gee arXiv:1908.07185, which
matches. Character conductor exponents agree.

\paragraph{Ambient category.} The category
...
\paragraph{Hypotheses.} The conjectural inputs
(A), (B), (C) are explicit. The unconditional inputs are BH 2006
classification (proved), dimensional match (proved here), structural
consistency (proved here). \emph{The hypotheses are explicit.}

\paragraph{Functoriality.} Block restriction is
...
\paragraph{Unproved equivalences.} Three conjectures (A),
(B), (C); explicit. \textbf{Named residuals.}

\paragraph{Numerical comparison.} No numerical computation.

The BH tame-wild classification at \(p=2\) closes with three named sub-residuals.

\subsection{Source and Convention Comparison: BH Tame-Wild Classification at \(p=3\)}

\paragraph{Source separation.} Same primary sources as the \(p=2\) comparison
plus Auslander--Reiten--Smal{\o} 1995 Ch.~VII \S 2 for the
\(A_{2}\)-linear-quiver block structure. All are disjoint from
Dotto--Emerton--Gee arXiv:2603.26887 and from Fargues--Scholze
arXiv:2102.13459.

\paragraph{Conventions.} Conventions match BH and EG as in the \(p=2\) comparison;
\(A_{2}\)-quiver conventions follow Auslander--Reiten--Smal{\o}
(radical-square-zero relations).

\paragraph{Ambient category.} Same ambient stable presentable \(\infty\)-category
as the \(p=2\) case.

\paragraph{Hypotheses.} Same hypotheses (A), (B), (C), instantiated at
\(p=3\); the Ext\({}^{1}\)-dimension is \(2\) not \(1\), matching
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p3}.

\paragraph{Functoriality.} Block restriction functorial as in the \(p=2\) case;
the \(A_{2}\)-quiver is functorial under block restriction.

\paragraph{Unproved equivalences.} Same three conjectures (A), (B), (C),
instantiated at \(p=3\). \textbf{Named residuals.}

\paragraph{Numerical comparison.} No numerical computation.

The BH tame-wild classification at \(p=3\) closes with the same three named sub-residuals.

\subsection{Source and Convention Comparison: Essential Surjectivity via Block Exhaustion}

\paragraph{Source separation.} Sources:
...
\paragraph{Conventions.} Compact-generation conventions: shifts and finite
colimits (not infinite), matching the DAG monograph.

\paragraph{Ambient category.} Stable presentable \(\infty\)-categories; the
compact-generator transfer is an equivalence in \(\mathrm{Pr}^{\mathrm{L}}_{\mathrm{st}}\).

\paragraph{Hypotheses.} (A), (B), (C) all named; the essential-surjectivity
argument uses all three. \emph{The hypotheses are explicit.}

\paragraph{Functoriality.} The block-by-block exhaustion is
block-diagonal (blocks are orthogonal on both sides); no false
functoriality arises.

\paragraph{Unproved equivalences.} No new conjecture introduced.

\paragraph{Numerical comparison.} No numerical computation.

Essential surjectivity holds conditional on (A), (B), (C).

\subsection{Source and Convention Comparison: Wild-EG Decomposition Cross-Check}

\paragraph{Source separation.} Sources: Chapter~\ref{v4-arith:deg-f1a}
...
Vol~IV's arithmetic branch.

\paragraph{Conventions.} The degree-\(1\) derived-fibre
dimension in Ch.~\ref{v4-arith:deg-f1a} equals the
Ext\({}^{1}\)-dimension in Ch.~\ref{v4-arith:bzn-rcompletion} by
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p2} and
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3}.

\paragraph{Ambient category.} Both objects sit in derived formal algebraic
stacks over \(\mathrm{Spf}\,\mathbb{Z}_{p}\).

\paragraph{Hypotheses.} Structural consistency assumes (B) for the
class-level match; dimensional match is unconditional.

\paragraph{Functoriality.} Functoriality of the BH fibre correspondence
(BH 2006 \S 18) is used; this is explicit.

\paragraph{Unproved equivalences.} No new unproved equivalence; (B) is already
named.

\paragraph{Numerical comparison.} No numerical computation.

The cross-check confirms structural consistency without introducing new residuals.

\subsection{Source and Convention Comparison: Unconditional Closure}

\paragraph{Source separation.} Literature: Paskunas 2013, BH 2006,
EG 2019, Gaitsgory--Rozenblyum monographs, Pridham--Toen--Vezzosi DAG
VIII. All disjoint.

\paragraph{Conventions.} All sources use Langlands-normalised L-parameters
and agree on conductor exponents.

\paragraph{Ambient category.} Ambient stable presentable \(\infty\)-categories
are fixed throughout.

\paragraph{Hypotheses.} Each of (A), (B), (C) requires input not currently
recorded in primary literature at \(p\in\{2,3\}\):
\begin{itemize}
\item (A) requires extending Paskunas' projective-to-structure-sheaf
match from \(p\geq 5\) to \(p\in\{2,3\}\); the concrete technical
domain is non-formal theorem.
\item (B) requires identifying the specific Ext\({}^{1}\)-classes
with cotangent-fibre directions via deformation theory;
required theorem non-formal theorem.
\item (C) requires extending Gaitsgory--Rozenblyum's compact
generation theorem from classical formal stacks to the derived-enhanced
case; required theorem non-formal theorem using DAG VIII.
\end{itemize}
None of (A), (B), (C) admits unconditional closure from currently
stated primary literature. \textbf{Three named residuals.}

\paragraph{Functoriality.} Each of (A), (B), (C) is functorial in the
input data; no false-functoriality gap.

\paragraph{Unproved equivalences.} (A), (B), (C) are the minimal
conjectural set. \textbf{Named residuals.}

\paragraph{Numerical comparison.} No numerical computation.

The three sub-conjectures (A), (B), (C) constitute the
minimal conditional input set for F1.a.3, each individually tractable
at non-formal theorem. The total is within the
non-formal theorem condition of Chapter~\ref{v4-arith:deg-f1a}.

\section{Block Exhaustion Boundary}
\label{v4-arith:f1a3-BH:summary}

\begin{enumerate}
\item F1.a.3 is a small local residual on F1.a: \(15\)--\(25\)
pp domain, bounded by the finite BH type list and the explicit
Ext\({}^{1}\)-structure of Chapter~\ref{v4-arith:bzn-rcompletion}
(\Cref{v4-arith:f1a3-BH:rem:smallest-residual}).
\item The condition is: essential surjectivity of the twisted
DEG-type functor
\(\Phi^{\mathrm{wild}}_{p,\chi}\) of
...
\Cref{v4-arith:bzn-rcompletion:prop:R4-Ext1-p3}.
\item Dimensional comparison (unconditional): both \(p=2\) (dimension \(1\))
and \(p=3\) (dimension \(2\)) match between the Ext\({}^{1}\)-side and the
derived-fibre side
(\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p2},
\Cref{v4-arith:f1a3-BH:prop:dim-comparison-p3}).
\item Structural consistency (unconditional): the BH-component
decomposition coincides with the wild-EG
...
\item Three named sub-conjectures: (A) BH type-block comparison
(\Cref{v4-arith:f1a3-BH:conj:BH-packaging}), non-formal theorem;
(B) Paskunas wild Ext\({}^{1}\)
(\Cref{v4-arith:f1a3-BH:conj:Paskunas-wild-ext}), non-formal theorem;
(C) wild-EG compact generation
(\Cref{v4-arith:f1a3-BH:conj:wild-compact-generation}), non-formal theorem.
Total: non-formal theorem, consistent with Chapter~\ref{v4-arith:deg-f1a}'s
estimate.
\item F1.a.3 does not close unconditionally; it is
factored into three individually tractable sub-residuals, each
strictly narrower than the full F1.a.3 statement. No
\ClaimStatusProvedHere{} statement for the main theorem; only
conditional closures.
\item The consequence for the F1 table
(\Cref{v4-arith:f1a3-BH:prop:F1-table-updated}) is that F1.a factors
into named conjectural inputs: F1.a.1, F1.a.2, the wild block
functor datum, F1.a.3.A--C, and the central-character family datum;
F1 adds \(\mathrm{L}_{\mathrm{ACD}}\). This is the finest
factorisation of the F1 residual recorded here.
\end{enumerate}

There is no unconditional closure of F1.a.3. The strongest
unconditional results are the dimensional comparison and the structural
consistency; all remaining closures are conditional on (A), (B), (C).
