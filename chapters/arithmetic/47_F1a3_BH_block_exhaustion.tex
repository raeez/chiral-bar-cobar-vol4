...
\emph{Route \(\gamma\) in Chapter~\ref{v4-arith:deg-f1a} reduced F1.a at
...
\section{Proof Reduction and Source Audit}
\label{v4-arith:f1a3-BH:ledger}

The five analyses below apply the seven-source-verification protocol of
\Cref{v4-arith:cl:protocol} (source collision,
...
\item[\textbf{Essential surjectivity via type-by-type exhaustion.}] \emph{Attack.} Fix a hypothetical
...
\item[\textbf{Cross-check with the wild-EG decomposition of Chapter~\ref{v4-arith:deg-f1a}.}] \emph{Attack.} Does the BH-component
...
\item[\textbf{Unconditional-closure assessment.}]
...
The source verification for each reduction step is recorded in
...
because the block structure is more complex; the packaging
statement at \(p\in\{2,3\}\) is the extension of Paskunas' result to the BH type
catalog, with the same projective-to-structure-sheaf identification.
...
\section{Residual Scope and Source Audits}
...
These unconditional results establish the decomposition and the numerical
match of F1.a.3. The conjectural content is the fine identification:
class-level match, compact generation, and projective-to-structure-sheaf
packaging.

\subsection{Source and Convention Audit: BH Tame-Wild Classification at \(p=2\)}

\paragraph{Source collision.} Bushnell--Henniart 2006
\S 11--13, Carter 1989 \S 7.3, Lusztig 1980, Jantzen 2003 Ch.~II
\S 8, Paskunas 2013 \S 6. All disjoint from Fargues--Scholze
arXiv:2102.13459 and from Dotto--Emerton--Gee arXiv:2603.26887% RECTIFICATION-FLAG: verify DEG arXiv:2603.26887 is the correct identifier for the tame-DEG paper cited here
(DEG is at \(p\geq 5\); the \(p=2,3\) extension is the conjectural
content of (A)). \textbf{Pass.}
...
\paragraph{Compute engine.} No engine. \textbf{Pass.}

The BH tame-wild classification at \(p=2\) closes with three named sub-residuals.

\subsection{Source and Convention Audit: BH Tame-Wild Classification at \(p=3\)}

\paragraph{Source collision.} Same primary sources as the \(p=2\) audit
plus Auslander--Reiten--Smal{\o} 1995 Ch.~VII \S 2 for the
\(A_{2}\)-linear-quiver block structure. All disjoint from Dotto--Emerton--Gee arXiv:2603.26887% RECTIFICATION-FLAG: verify DEG arXiv:2603.26887 identifier
and from Fargues--Scholze arXiv:2102.13459. \textbf{Pass.}

\paragraph{Conventions.} Conventions match BH and EG as in the \(p=2\) audit;
...
the \(A_{2}\)-quiver is functorial under block restriction. \textbf{Pass.}
...
\subsection{Source and Convention Audit: Essential Surjectivity via Block Exhaustion}

\paragraph{Source collision.} Sources:
...
functoriality arises. \textbf{Pass.}

\paragraph{Unproved equivalences.} No new conjecture introduced. \textbf{Pass.}

\paragraph{Compute engine.} No engine. \textbf{Pass.}

Essential surjectivity holds conditional on (A), (B), (C).

\subsection{Source and Convention Audit: Wild-EG Decomposition Cross-Check}

\paragraph{Source collision.} Sources: Chapter~\ref{v4-arith:deg-f1a}
...
\subsection{Source and Convention Audit: Unconditional-Closure Assessment}

\paragraph{Source collision.} Literature: Paskunas 2013, BH 2006,
EG 2019, Gaitsgory--Rozenblyum monographs, Pridham--Toen--Vezzosi DAG
VIII. All disjoint. \textbf{Pass.}

\paragraph{Conventions.} All sources use Langlands-normalised L-parameters
and agree on conductor exponents. \textbf{Pass.}

\paragraph{Ambient category.} Ambient stable presentable \(\infty\)-categories
are fixed throughout. \textbf{Pass.}
...
case; estimated scope \(5\)--\(10\) pp using DAG VIII.
\end{itemize}
None of (A), (B), (C) admits unconditional closure from currently
inscribed primary literature. \textbf{Three named residuals.}
...
at \(5\)--\(10\) pp. The total is within the
...
adds \(\mathrm{L}_{\mathrm{ACD}}\); six total, constituting the finest
factorisation of the F1 residual recorded in this programme.
\end{enumerate}

The chapter produces no unconditional closure of F1.a.3. The strongest
unconditional results are the dimensional audit and the structural
