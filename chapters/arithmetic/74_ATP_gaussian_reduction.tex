...
\emph{The wave-6 direct attack on A-TP
...
$[-t_{\ast},t_{\ast}]$ with $t_{\ast}\approx 0.794$. Wave-5 agent I
...
The wave-7 D attack runs these two inputs in concert. The Gaussian
...
Moreover, the \emph{normalised function}
\begin{equation}
...
$W(f_{\lambda}*f_{\lambda})=0$ via the Weil formula): the wave-7 D
...
\subsection{Comparison with Wave-6 HFD Sub-Class}
...
\begin{theorem}[wave-7 D refinement of the A-TP obstruction]
...
\begin{remark}[wave-7 D deliverable]
\label{v4-arith:w7-d:rem:deliverable}
Wave-7 D produces three deliverables.
...
$\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\cup\mathcal{C}_{\mathrm{Gauss}}^{+}$
strictly enlarges either the wave-6 HFD class or the wave-7 D
...
The wave-7 D attack obeys the Beilinson principle: A-TP closes on
