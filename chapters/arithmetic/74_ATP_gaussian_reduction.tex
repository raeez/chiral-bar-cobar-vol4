...
\emph{Chapter~\ref{v4-arith:w6-a:ATP-direct-attack} reduced the archimedean
...
\(I(\lambda_{c})=0\). A-TP on the Gaussian sub-class
...
produces a restricted-class theorem: the density
...
where the extension-by-density argument fails.}
...
with \(t_{\ast}\approx 0.8507\); the irreducible obstruction is
...
The Gaussian sub-class of PW collapses the full \(L^{2}\)-weighted positivity
...
The chapter uses four source catalogs. None enters the Weil 1952 derivation
...
\(f_{\lambda}*\widetilde{f}_{\lambda}=\sqrt{1/(2\lambda)}\cdot f_{\lambda/2}\)
...
\section{Extension by Gaussian Density: Boundary of the Positive Cone}
...
via continuity. The extension closes on a strict positive sub-cone;
on the complement the quadratic structure of the A-TP inequality is
incompatible with linear density.
...
\begin{proposition}[boundary of Gaussian density extension of A-TP]
...
the A-TP inequality is quadratic in \(f\),
...
\begin{theorem}[Gaussian reduction refinement of the A-TP obstruction]
...
\begin{remark}[three outputs of the Gaussian reduction]
...
\section{The Gaussian A-TP Theorem: Results}
...
Chapter~\ref{v4-arith:w6-a:ATP-direct-attack} or the Gaussian
positive cone \(\mathcal{C}_{\mathrm{Gauss}}^{+}\) alone; the complement in \(\mathrm{PW}^{\mathrm{hol}}_{F}\)
...
The Gaussian reduction of this chapter obeys the Beilinson principle: A-TP closes on
...
is imported from Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}, and Weil's
explicit formula is imported from Chapter~\ref{v4-arith:w5-a:chapter}).
