...
Chapter~\ref{v4-arith:w5-e:chapter}. The closure of (AG.det.transition)
rests on four independent source contributions:
...
flat locus as the centre. The BC-diamond-glue final closure
...
(AG~2015, GKRW~2024, BL~2022, BFM~2005).
...
\begin{theorem}[BFM 2005 equivariant cohomology of the flat locus]
...
by the four-source assembly AG 2015 + GKRW 2024 + BL 2022 + BFM 2005.
\end{theorem}
...
prismatic--Nygaard filtration (no cellular \(E_{k}\)-input); BFM 2005
...
\begin{remark}[scope of route~6 G: residual conditional inputs]
...
prismatic-cohomology literatures, and their closure lies in those
literatures. Arithmetic AG at the
...
\begin{remark}[HZ-IV decorator candidates for this chapter]
\label{v4-arith:w6-g:rem:decorators}
Three HZ-IV realization decorators are registered against
\texttt{compute/lib/realization\_registry.py} for the claims of
this chapter. Their derivation and verification sources are mutually
disjoint per the Vol IV disjointness check in the registry.
The candidates are:
\begin{itemize}
...
AG 2015 + GKRW 2024 + BL 2022 + BFM 2005 on the codim-\(\leq 2\)
