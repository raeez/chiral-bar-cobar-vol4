\chapter{Wave-6 G: (AG.det.transition) Codim-2 $E_{2}$-Centre Glue and BC-Diamond-Glue Final Closure}
\label{v4-arith:w6-g:chapter}

\emph{Wave-6 closure of the two residuals left by wave-5 after
Chapters~\ref{v4-arith:bcglue-core} (wave-4) and
\ref{v4-arith:w5-h:bcglue-core-and-F2c} (wave-5): the
...
\subsection{Wave-5 Input}

Wave-5 (Chapter~\ref{v4-arith:w5-h:bcglue-core-and-F2c}) left the
...
general $\mathcal{O}$-level and the wave-5 elementary $E_{2}$-level
(\Cref{v4-arith:w5-h:thm:f2c-e2-transport}). The remaining
residuals after wave-5 are:
\begin{enumerate}
...
Wave-6 assumes $\mathrm{L}_{\mathrm{ACD}}$.$0$--.$3$ are closed via
...
compatibility (\ref{v4-arith:w5-h:eq:agdet-e2-compat}) of wave-5, is
...
Wave-5 estimated (AG.det.transition) at $5$--$10$ pp past the
...
$\sim 1$~pp. Total: $\sim 8$~pp, within the wave-5 estimate.
\end{remark}
...
and (BCglue.glue) (both stood on their wave-2 conditional inputs),
...
manifolds; no Galois input) and wave-5 (Ch1)
(\Cref{v4-arith:w5-h:thm:ch1-closure}) + wave-6 (AG.det.transition)
...
inherited wave-5/wave-6 closures.
\end{proof}
...
from wave-2; (BCglue.core) closed via
\Cref{v4-arith:w6-g:cor:bcglue-core-closed}.
\end{proof}

\section{Wave-6 Residual Catalog and Net Delta}
\label{v4-arith:w6-g:residuals}

\begin{theorem}[$\mathrm{L}_{\mathrm{ACD}}$ residual catalog after wave-6 G]
\label{v4-arith:w6-g:thm:lacd-wave6}
\ClaimStatusConjectured (conditional on the named standing residuals).
After wave-6 G, $\mathrm{L}_{\mathrm{ACD}}$ has residual catalog:
\begin{enumerate}
\item (BCglue.local): Theorem-waiting (unchanged from
wave-2 \Cref{v4-arith:bcglue-k5:thm:local-BZCHN-plus-EG}).
...
\begin{proof}
Assemble \Cref{v4-arith:w5-h:cor:LACD-wave5} (wave-5 catalog) with
...
\begin{remark}[net wave-6 G scope delta]
\label{v4-arith:w6-g:rem:scope-delta}
Wave-5 left BC-diamond-glue at $\sim 50$--$80$ pp conditional scope
(decomposed as $20$--$30$~pp for (BCglue.local), $25$--$40$~pp for
(BCglue.glue), $5$--$10$~pp for (AG.det.transition)). Wave-6 G
...
Section~\ref{v4-arith:w6-g:bcglue-final} ($\sim 5$~pp). Net wave-6 G
delta: $\sim 13$~pp removed by (AG.det.transition) closure +
BC-diamond-glue final assembly. Remaining residuals past wave-6 G
are the two wave-2 conditional inputs (BCglue.local) Theorem-waiting
(scope $20$--$30$~pp) and (BCglue.glue) Conjectural (scope
$25$--$40$~pp), both untouched by wave-6 G. The scope of
BC-diamond-glue past wave-6 G is therefore $45$--$70$~pp,
compared with the pre-wave-6 scope of $50$--$80$~pp: a $5$--$10$~pp
reduction obtained by closing the $E_{2}$-centre cleavage gluing
unconditionally past the remaining conditional inputs.
\end{remark}

\begin{remark}[what wave-6 G cannot touch]
\label{v4-arith:w6-g:rem:what-wave6-cannot-touch}
Wave-6 G does not touch (BCglue.local) or (BCglue.glue): these are
...
centre is inherited from wave-4 as a conditional input; the
...
Wave-6 G also does not close the Eisenstein extension of
...
and the inherited wave-5 (Ch1) + wave-6 G (AG.det.transition)
...
\begin{enumerate}
\item Wave-6 G targets two residuals: (AG.det.transition) left by
wave-5 ($5$--$10$~pp) and the BC-diamond-glue final closure
...
\item Net wave-6 G scope delta: $\sim 13$~pp removed. Residual
BC-diamond-glue scope past wave-6 G is $45$--$70$~pp, distributed
as (BCglue.local) Theorem-waiting ($20$--$30$~pp) and
(BCglue.glue) Conjectural ($25$--$40$~pp), both inherited unchanged
from wave-2.
\item Wave-6 G installs two candidate \texttt{RealizedHere}
decorators
(\Cref{v4-arith:w6-g:thm:agdet-transition-closure},
