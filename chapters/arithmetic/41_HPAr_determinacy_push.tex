...
\subsection{What Wave-2 Settled}
\label{v4-arith:hpar-det:wave2-recap}

The wave-2 structural analysis of
Chapter~\ref{v4-arith:HPAr-vbconverse} established the following
four-point ledger against which wave-3 operates:
...
Wave-3 does not repair the wave-2 ledger; the ledger is correct. Wave-3
...
Wave-2's \Cref{v4-arith:hpvbconv:cor:HPAr-single-residual} names the
single residual as ``the choice of Bost--Connes zero-mode weights
$\{\lambda(m)\}_{m\in\mathbb{N}^{\times}}$ such that the combined
trace matches the zero generating function''. Wave-3 tests four
...
C^{n}n!^{2}$. Wave-3 tests the tempting Bombieri--Lagarias route and
...
after the conditional Ch~37 package is supplied. Wave-3 produces the
sharpest reduction the framework allows without that closure.

\subsection{Disjoint Source Catalog for Wave-3}
\label{v4-arith:hpar-det:sources}

Wave-2 used: Connes arXiv:math/9811068, Deninger 1994, Berry--Keating
1999, Reed--Simon, Akhiezer--Glazman, de Branges 1968/1994,
B\'aez-Duarte 2003, Burnol 2000. Wave-3 draws on a complementary
...
These sources are disjoint from the wave-2 catalog in the Vol~IV
sense: none of the wave-2 primary derivations are cited wave-3, and
none of the wave-3 verifications are cited wave-2.

\section{Attack-Heal Ledger}
\label{v4-arith:hpar-det:ledger}

Wave-3 executes six attack-heal cycles, four on weight selection
...
\paragraph{target:} check that any candidate weight $\lambda(m)$
produces $\mathrm{tr}(e^{-tL_{0}^{\mathrm{Ar}}})=\sum_{\rho}e^{-t\rho}$
under the Friedrichs construction.
...
by the Dedekind-eta inverse, does \emph{not} equal $\sum_{\rho}e^{-t\rho}$;
...
\paragraph{deciding evidence:} by Mellin inversion, the trace identity
$\mathrm{tr}(e^{-tL_{0}^{\mathrm{Ar}}})=\sum_{\rho}e^{-t\rho}$ is
equivalent to the identity of meromorphic functions
$\det_{\infty}(s\cdot\mathrm{id}-L_{0}^{\mathrm{Ar}})=\xi(s)$ via
$\mathrm{tr}(e^{-tL_{0}^{\mathrm{Ar}}})=-\partial_{t}\sum_{\rho}e^{-t\rho}/(\ldots)$
...
\paragraph{broken step:} this is the wave-3 closure target; it
succeeds.
\paragraph{evidence type:} synthesis.
\paragraph{evidence ref:} accumulated wave-2 and wave-3 sources.
\paragraph{files read:} all of above.
\paragraph{confidence:} complete for the naming; zero for independent
closure.
\paragraph{blast radius:} consolidates all of wave-3's reductions
...
remain conditional inputs from wave-2 and Ch~37.
...
The wave-2 candidate weight system inscribed was the Berry--Keating
...
quadratic finite-place possibilities used in the attack-heal audit.
\end{remark}
...
\ClaimStatusProvedHere. Let \(\lambda=\lambda_{\mathrm{BK}}\) be the
...
\widehat{h}_{t}(\log n)
\end{equation}
for the heat/Laplace test family associated to
\(e^{-tL_{0}^{\mathrm{Ar}}}\), after the required admissible
regularisation is supplied. This is not an unconditional application
of the classical Weil formula to compactly supported Fourier-test
functions; it is precisely the regularisation bridge needed for
H-P$^{\mathrm{Ar}}$.
\end{theorem}
...
descendant contribution (trivial zeros of $\xi$) via the
...
\end{equation*}
with the trivial-zero and archimedean terms removed by the
Bost--Connes/Dedekind-eta prescription. The Hadamard product for
\(\xi\) rewrites the zero side as a regularised zero trace. The
classical Weil explicit formula supplies this identity for its
standard admissible test spaces; the Vol~IV condition is exactly the
additional assertion that the heat/Laplace family for
\(L_{0}^{\mathrm{Ar}}\) lies in the admissible regularised extension
and that the BC trace equals that extension. Therefore the theorem is
a reformulation of the trace-compatibility hierarchy, not a proof of
the hierarchy.
\end{proof}

\begin{corollary}[the hierarchy is the Connes absorption-spectrum bridge]
\label{v4-arith:hpar-det:cor:hierarchy-is-weil}
\ClaimStatusProvedHere. Under the BK weight selection, the
H-P$^{\mathrm{Ar}}$ trace-compatibility condition is the
Connes--Meyer--Weil absorption-spectrum bridge for the
Bost--Connes heat-trace family. It is not discharged by citing the
classical Weil explicit formula alone.
\end{corollary}

\begin{proof}
The previous theorem identifies the needed equality with the
regularised explicit-formula extension for the BC heat-trace family.
...
under the zeta/KMS normalisation. But whether the Berry--Keating
weight actually satisfies the trace compatibility is the
regularised Connes absorption-spectrum problem. The reduction is
tight: wave-3 determines the canonical weight under explicit
...
(purely imaginary iff RH). The \emph{symmetrised shifted-zero moment
sequence} is
\begin{equation}
...
\begin{remark}[relation to the symmetrised zero measure of wave-2]
...
This is the central repair of the wave-3 attack. A lower bound on
...
\ClaimStatusProvedHere. Under the wave-2 structural reductions and
the wave-3 analytic reductions, the Hamburger-determinacy component
...
\begin{theorem}[wave-3 consequence for the Ch~37 single positivity
corollary]
...
wave-3 analysis, to the conjunction:
\begin{enumerate}
...
\begin{corollary}[structure of the wave-3 reduction]
\label{v4-arith:hpar-det:cor:wave3-structure}
\ClaimStatusProvedHere. Wave-3 reduces the Ch~37 single positivity
...
After the wave-3 reductions, the precise remaining gaps in the
...
\begin{remark}[what wave-3 contributes]
\label{v4-arith:hpar-det:rem:wave3-contribution}
Wave-3 does not close either (A) H-P$^{\mathrm{Ar}}$ spectral
matching or (B) Hamburger determinacy; both remain RH-equivalent or
RH-adjacent open problems. What wave-3 produces:
\begin{enumerate}
...
\item \textbf{Three precisely-named wave-3 residuals}
(\Cref{v4-arith:hpar-det:cor:wave3-structure}): weight
normalisation (W), H-P trace bridge (H), and Carleman positivity
\((\mathrm{R}_{\mathrm{analytic}})\).
\item \textbf{What wave-3 does not do}. Wave-3 does not close
H-P$^{\mathrm{Ar}}$ (it reduces to the Connes absorption-spectrum
conjecture, which is equivalent to H-P$^{\mathrm{Ar}}$). Wave-3
does not close Hamburger determinacy. Wave-3 does not produce new
RH-equivalent positivity languages beyond the wave-2 catalog of Det,
HB+, BD+.
\item \textbf{What wave-3 does do}. Wave-3 produces the sharpest
