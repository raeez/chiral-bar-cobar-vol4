...
\(35\)--\(55\) pp to two sub-cores: the genuinely supersingular non-pBT
core at \(p\geq 5\) (\Cref{v4-arith:w6-e:conj:f1a1-ss-residual},
\(6\)--\(12\) pp) and the non-solvable non-nearly-ordinary \(A_{5}\)
icosahedral core
(\Cref{v4-arith:w6-e:conj:TW-removal-ns-residual}, \(3\)--\(6\) pp).
Each sub-core admits a primary-source assembly that closes its
dominant sub-locus unconditionally and isolates a strictly narrower
residual tail. The supersingular core closes on the Fontaine--Laffaille
sub-locus (Hodge--Tate weights in \([0,p-2]\)) via Kisin~2009
...
\(k\in\{p-1,p\}\), of scope \(2\)--\(4\) pp. The icosahedral core closes
on the odd-determinant sector via Taylor~2003 Pub.~IHES~98 Part~II
potential automorphy, Barnet-Lamb--Gee--Geraghty~2012 J.~AMS~25
automorphic-lifting descent, and Emerton~2011 Invent.~Math.~184
patching without Taylor--Wiles; the residual is the local--global
descent defect, of scope \(1\)--\(2\) pp. Both closures are
source-disjoint from the pBT and Serre-modular closures of
...
The two pending sub-cores from Chapter~\ref{v4-arith:w6-e:chapter}
...
source-disjointness of the two closures from all prior routes is
verified: the six assembled sources are pairwise disjoint in
methodology, as recorded in the audit above.
...
(\(6\)--\(12\) pp) covered the full genuinely supersingular non-pBT locus.
...
range. The scope reduction from \(6\)--\(12\) pp to \(2\)--\(4\) pp
reflects that the residual is a boundary computation at two weights,
not a stratum of dimension \(3\).
\end{remark}

\subsection{Verification of Theorem~\ref{v4-arith:w10-f:thm:f1a1-ss-FL}}
...
\paragraph{Hypothesis scope.} The Fontaine--Laffaille hypothesis
...
\paragraph{Unconditional inputs.} No conjecture is invoked on the
Fontaine--Laffaille stratum; all inputs are theorems. The small-weight
tail residual is \Cref{v4-arith:w10-f:conj:ss-small-weight}, of scope
\(2\)--\(4\) pp, a strict reduction from the
\Cref{v4-arith:w6-e:conj:f1a1-ss-residual} scope of \(6\)--\(12\) pp.

\paragraph{Compute verification.} No compute engine backs F1.a.1.

The six verification checks close on the Fontaine--Laffaille
supersingular stratum; the small-weight tail residual is explicitly
named and strictly narrower than the residual of
...
The sub-sector \(S^{A_{5},\mathrm{odd}}\) is closed via three primary
...
odd-determinant \(A_{5}\) sub-sector is the dominant part of this
sector, covered by Taylor~2003 plus BLGG~2012 plus Emerton~2011. The
residual defect captures only the sparse case where the solvable chain
to \(\mathbb{Q}\) is not of the Arthur--Clozel cyclic form; scope
reduction: \(3\)--\(6\) pp to \(1\)--\(2\) pp.
\end{remark}

\subsection{Verification of Theorem~\ref{v4-arith:w10-f:thm:A5-odd-closure}}
...
\paragraph{Hypothesis scope.} Odd determinant is explicit: it is the
...
the icosahedral sector is verified via the chain of \(R=T\) and
...
\paragraph{Unconditional inputs.} No conjecture is invoked on
\(S^{A_{5},\mathrm{odd}}\); all inputs are theorems. The local--global
defect residual is \Cref{v4-arith:w10-f:conj:ns-defect}, of scope
\(1\)--\(2\) pp, a strict reduction from the
\Cref{v4-arith:w6-e:conj:TW-removal-ns-residual} scope of
\(3\)--\(6\) pp.

\paragraph{Compute verification.} No compute engine backs F1.a.1.

The six verification checks close on the odd-determinant icosahedral
sub-sector; the local--global defect residual is explicitly named and
strictly narrower than the residual of
\Cref{v4-arith:w6-e:conj:TW-removal-ns-residual}.

\section{Combined F1.a.1 Closure on the Five Unconditional Strata}
\label{v4-arith:w10-f:combined}

\begin{theorem}[F1.a.1 equivalence on the five unconditional strata]
...
\begin{proposition}[F1.a.1 residual scope after Chapter~\ref{v4-arith:w10-f:chapter}]
\label{v4-arith:w10-f:prop:F1a1-ledger-w10f}
\ClaimStatusProvedHere. After the closures of this chapter, the
F1.a.1 residual has two sub-cores of combined scope \(3\)--\(6\) pp:
\begin{enumerate}
...
\end{enumerate}
Total F1.a.1 scope is \(3\)--\(6\) pp, down from the
\Cref{v4-arith:w6-e:prop:F1-ledger-refined} scope of \(9\)--\(18\) pp.
The reduction in this chapter is approximately \(67\) percent
(both endpoints yield \((9-3)/9=(18-6)/18=2/3\)) on top of the
\Cref{v4-arith:w6-e:chapter} reduction of \(65\)--\(70\) percent; the
compound reduction from the \Cref{v4-arith:w5-e:chapter} starting
scope of \(35\)--\(55\) pp is \(89\)--\(91\) percent.
\end{proposition}

\begin{proof}
Tabulation from \Cref{v4-arith:w10-f:cor:ss-core-reduction} and
\Cref{v4-arith:w10-f:cor:ns-core-reduction}. The
\Cref{v4-arith:w6-e:prop:F1-ledger-refined} residual is \(9\)--\(18\) pp
(\(6\)--\(12\) ss-core plus \(3\)--\(6\) ns-core); the present residual is
\(3\)--\(6\) pp (\(2\)--\(4\) ss-small-weight plus \(1\)--\(2\) ns-defect).
The reduction \((9-3)/9=(18-6)/18=2/3\) yields approximately
\(67\) percent;
combined with the \Cref{v4-arith:w6-e:chapter} reduction of
\(65\)--\(70\) percent from the \Cref{v4-arith:w5-e:chapter} starting
scope of \(35\)--\(55\) pp, the compound reduction is
\(1-3/35=0.914\) to \(1-6/55=0.891\), i.e.\ \(89\)--\(91\) percent.
\end{proof}

\begin{proposition}[Full F1 residual scope after Chapter~\ref{v4-arith:w10-f:chapter}]
...
residuals from \Cref{v4-arith:w6-e:prop:F1-ledger-combined}, the full
F1 residual scope has five sub-residuals:
\begin{enumerate}
...
\end{enumerate}
Total scope \(68\)--\(116\) pp, down from the
\Cref{v4-arith:w6-e:prop:F1-ledger-combined} scope of \(74\)--\(128\) pp.
The F1.a.1 reduction carries through the total without affecting the
other sub-residuals.
\end{proposition}

\begin{proof}
Tabulation from \Cref{v4-arith:w10-f:prop:F1a1-ledger-w10f}
and \Cref{v4-arith:w6-e:prop:F1-ledger-combined}
(the other four sub-residuals are unchanged).
\end{proof}

The two F1.a.1 residuals isolated in
Chapter~\ref{v4-arith:w6-e:chapter} close on their dominant sub-loci:
...
The two residual tails, the small-weight Fontaine--Laffaille
boundary (\Cref{v4-arith:w10-f:conj:ss-small-weight}) and the
local--global cyclic descent defect
(\Cref{v4-arith:w10-f:conj:ns-defect}), have combined scope
\(3\)--\(6\) pp and each admits a specific primary-source extension:
Breuil--M\'ezard~2010 plus Caraiani--Levin~2020 Kisin-module
refinement for the former; Arthur--Clozel cyclic-descent refinement
for the latter. The six assembled sources are source-disjoint from
those used in Chapter~\ref{v4-arith:w6-e:chapter}.
