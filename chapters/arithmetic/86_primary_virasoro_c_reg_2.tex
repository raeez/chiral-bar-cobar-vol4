...
\emph{Chapter~\ref{v4-arith:polyakov}
states that the naive sum \(c_{\mathrm{full}}=\sum_{p}c^{(p)}\)
diverges; Chapter~\ref{v4-arith:rh-reformulation} records the
reconciliation conjecture (\Cref{v4-arith:rh:conj:primary-Virasoro})
as open. The present chapter proves the
...
derives it from the Sugawara tensor rule and supplies the
Heisenberg-coordinate-rescaling check, needed for the proof of
per-state finiteness in \S\ref{v4-arith:w9-r3:per-state-finite}.
\end{remark}
...
% RECTIFICATION-FLAG [Gate 1: scope] The proposition as stated claims individual
% L_m^{Ar} v is a finite sum for all m in Z. For m <= -2 and p not in supp(v),
% L_m^{(p)}|0>_p is nonzero (a level-|m| Fock state), so the sum over all p
% is infinite. The proof below establishes finiteness of the commutator
% [L_m^{Ar}, L_{-m}^{Ar}] acting on v, not of individual modes L_m^{Ar} v.
% The proposition statement should be restricted to m >= -1 for individual
% mode action, with a separate claim for commutator finiteness. Left for
% next repair cycle; below we add a parenthetical to flag the restriction.
\begin{proposition}[per-state finiteness of the Virasoro commutator]
...
and every \(m\geq -1\) the mode action
\begin{equation}
...
\end{equation}
is a finite sum. For all \(m\in\mathbb{Z}\), the commutator
\([L_{m}^{\mathrm{Ar}},L_{-m}^{\mathrm{Ar}}]\) acts on \(v\) as a finite
Fock expression plus a universal scalar central term.
\end{proposition}
...
\S\ref{v4-arith:w9-r3:dirichlet}.
\end{remark}
...
\(\mathbb{P}\) as its summation domain; the arithmetic Heisenberg central
term requires the Riemann zeta \(\zeta(s)\), which is meromorphic on
\(\mathbb{C}\), not the prime zeta \(\zeta_{\mathbb{P}}\), which has a
natural boundary at \(\mathrm{Re}(w)=0\). The per-state-finite primary
trace already has a finite Fock action
(\Cref{v4-arith:w9-r3:prop:per-state-finite}); what requires regularisation
is the universal scalar multiplier, and that scalar is extracted from the
completed-zeta Euler product via the residue at \(s=1\), as established in
\Cref{v4-arith:pol:prime-zeta-obstruction}.
\end{remark}
...
Chern--Simons thermal determinant used for the central-charge
regularisation in \S\ref{v4-arith:w9-r3:partition-reg}.
...
\begin{remark}[logarithmic derivative of \(Z\) and the central-charge pole]
...
In Polyakov's convention (\S\ref{v4-arith:pol:c-as-residue}), the central
charge is the residue of this pole.
\end{remark}
...
c_{\mathrm{reg}}\;:=\;\mathrm{Res}_{s=1}\Bigl(-\frac{d}{ds}\log Z(s)\Bigr).
\end{equation}
Equivalently, if \(\log Z(s)=-r\log(s-1)+\mathrm{holomorphic}\) near \(s=1\),
then \(c_{\mathrm{reg}}:=r\), where \(r\) is the order of the logarithmic
singularity.
\end{definition}
...
\log Z(s)\;=\;-2\log(s-1)\;+\;2\sum_{n\geq 2}\log\zeta(n)\;+\;2\gamma_{E}(s-1)\;+\;O((s-1)^{2}),
\end{equation}
...
is holomorphic at \(s=1\) and evaluates there to \(\sum_{n\geq 2}\log\zeta(n)\).
...
\[
\log Z(s)
=-2\log(s-1)+2\sum_{n\geq 2}\log\zeta(n)+2\gamma_{E}(s-1)+O((s-1)^{2}),
\]
...
\(-\frac{d}{ds}\log Z(s)=\frac{2}{s-1}-2\gamma_{E}+O(s-1)\)
near \(s=1\). The residue at \(s=1\) is \(2\).
By Definition~\ref{v4-arith:w9-r3:def:c-reg},
\(c_{\mathrm{reg}}=\mathrm{Res}_{s=1}(-\frac{d}{ds}\log Z)=2\).
\end{proof}
...
\(c^{(p)}=2\), so the per-place charges are uniformly \(2\). The content
of \Cref{v4-arith:w9-r3:thm:polyakov-identity} is that the global
regularised sum over all finite places equals the same number \(2\),
not the naive sum \(\sum_{p}2=\infty\): the zeta regularisation selects
the finite part.
\end{remark}
...
\begin{remark}[disjoint derivation and verification sources]
...
sum over \(|S_{v}(m)|<\infty\) primes; for all \(m\in\mathbb{Z}\),
the commutator \([L_{m}^{\mathrm{Ar}},L_{-m}^{\mathrm{Ar}}]\) acts on
\(v\) by a finite Fock expression plus a universal scalar central term
...
c_{\mathrm{reg}}\;=\;\mathrm{Res}_{s=1}\Bigl(-\frac{d}{ds}\log Z(s)\Bigr)\;=\;2.
\end{equation}
...
\subsection{Falsification and repair of six potential objections}
\label{v4-arith:w9-r3:attack-and-repair}

\begin{remark}[prime-zeta sum does not regularise the central charge]
...
\begin{remark}[\((\log p)\)-rescaling leaves the central charge unchanged]
\label{v4-arith:w9-r3:attack-2}
\textbf{Objection.} ``The \((\log p)\)-rescaling changes the central
charge \(c^{(p)}\mapsto(\log p)^{2}\cdot c^{(p)}\).''
\textbf{Answer.} The rescaling is of the \emph{Heisenberg coordinate}
...
\begin{remark}[the universal scalar central is not per-state-finite]
...
\textbf{Answer.} The commutator splits as (Fock action) \(+\)
...
\textbf{Objection.} ``The regularisation \(c_{\mathrm{reg}}=2\) is
...
\begin{remark}[the central charge equals \(2\) in the Iwasawa-completion as well]
...
rank). Thus \(c^{(p)}=2\) in both the complex Heisenberg setting of
Ch.~\ref{v4-arith:polyakov} and the Iwasawa-coefficient setting of
...
the scalar half \(c_{\mathrm{reg}}=2\) is established by this chapter.
\end{remark}
...
requires operator-norm estimates on
