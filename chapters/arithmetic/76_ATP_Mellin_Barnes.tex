...
\emph{Route~7 F. The archimedean Tate positivity conjecture A-TP is
attacked via the Mellin--Barnes contour representation of the digamma
...
left open after route~6 Chapter~\ref{v4-arith:w6-a:ATP-direct-attack}.
The Mellin--Barnes route reveals a structural obstruction: the residue
...
Hermite--Biehler. A fourth non-elementary route, orthogonal to all
...
Out of scope: unconditional closure of A-TP (RH-equivalent). As in
routes~5 and 6, the irreducible obstruction is named and the spectral
subclass closed by the Binet route is identified.
...
Chapter~\ref{v4-arith:w6-a:ATP-direct-attack} and the
...
Parseval). Combining and using \(|\widehat{f}(0)|^{2}=|\int f|^{2}\) yields
...
R_{n}(t)\;=\;\frac{(-1)^{n}}{2(n!)^{2}}\cdot\frac{d^{2}}{dw^{2}}\!\left[\mathrm{Re}\!\left(\frac{1}{\Gamma(s+w)}\right)\right]\bigg|_{w=-n}
+\text{(lower-order terms from the }w\text{-expansion of }\pi/\sin\pi w\text{)},
\end{equation}
% RECTIFICATION-FLAG: Gate 1 -- the exact closed-form residue formula for a triple
% pole requires careful Laurent expansion; the leading term is as displayed.
% Full formula with all terms from the triple-pole expansion deferred.
...
\end{equation}
the union of the route~6 high-frequency dominance class and the
route~7 F Hankel dominance class. This union strictly contains both
...
the zero-density route, the Gaussian route, or the de Branges route of
Chapter~\ref{v4-arith:w5-j:VBstar-spectral-positivity}.
\end{remark}
...
route is a computational instance of the de Branges programme
...
strictly contains the route~6 closure
...
\begin{remark}[Beilinson principle applied to route~7 F]
\label{v4-arith:w7-f:rem:beilinson-closing}
Route~7 F closes A-TP on the union
\(\mathrm{PW}^{\mathrm{hol},\mathrm{HFD}}_{F}\cup\mathrm{PW}^{\mathrm{hol},\mathrm{HD}}_{F}\)
via a contour-integration route disjoint from the Hurwitz-series route
of route~6. The residual complement \eqref{v4-arith:w7-f:eq:wave7f-residual}
