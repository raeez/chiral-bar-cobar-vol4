\chapter{Route~10 E: Bernstein-Centre (BCglue.local) + (BCglue.glue) Residual Closure}
\label{v4-arith:w10-e:chapter}

\emph{Route~10 route E closure on the $45$--$70$~pp residual left by
Route~6 G
...
seven primary-source catalogs disjoint from the ones used by route~5 H
and route~6 G, reduces the load-bearing content to a pointwise
...
block-tangent match inherited from route~10 B
...
\subsection{The Route~6 G Residual Restated}

Route~6 G closed BC-diamond-glue conditional on the three inputs:
...
The seven primary catalogs routed through route~10 E are:
\begin{description}
...
partially closed by route~10 B
...
residual scope after route~10 E: $\sim 8$--$12$~pp, concentrated on
...
block structure extends to $p\in\{2,3\}$). Route~6 F and route~10 B
have attacked this residual through the Pa{\v s}k{\=u}nas
block-$\mathrm{Ext}^{1}$ route; route~10 E routes around it by using
...
$\mathrm{Ext}^{1}$-class-match of route~10 B
...
spectral-side action by route~10 B's pseudo-character block-tangent
...
Route~10 B (\Cref{v4-arith:w10-b:thm:class-match}) supplies the
...
enhancement); this conjecture is inherited from route~6 F and
route~10 B and is not touched by route~10 E.
...
2014 (block structure at $p=3$) + route~10 B class-match (pseudo-char
...
route~10 B: Kisin pseudo-character deformation).
\end{proof}
...
$R^{\mathrm{Ch}}$ by
\Cref{v4-arith:bcglue-core:thm:global-bernstein-chenevier} (route~4)
combined with (Ch1) closure of
\Cref{v4-arith:w5-h:thm:ch1-closure} (route~5 H). The factorization
...
from route~2 (\Cref{v4-arith:bcglue-k5:lem:prismatic-flatness}) and
...
closures of route~5 H and route~6 G. The three catalogs are disjoint:
...
EG enhancement inherited from route~6 F and route~10 B), the
conjunction (BCglue.local) $\wedge$ (BCglue.glue) is closed via the
route~10 E assembly of
...
conditional on the EG wild-inertia enhancement and route~10 B
...
\begin{corollary}[net route~10 E scope delta]
\label{v4-arith:w10-e:cor:scope-delta}
\ClaimStatusProvedHere. Route~10 E removes $\sim 40$~pp of the
$45$--$70$~pp residual left by route~6 G, reducing the surviving
...
route~2.
\item EG wild-inertia enhancement + route~10 B class-match at $\ell=v$:
$\sim 5$--$7$~pp, inherited from route~6 F and route~10 B
(\Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement},
\Cref{v4-arith:w10-b:thm:class-match}). Not touched by route~10 E.
\end{itemize}
\end{corollary}

\begin{proof}
Route~6 G left $45$--$70$~pp distributed as (BCglue.local)
$20$--$30$~pp and (BCglue.glue) $25$--$40$~pp
(\Cref{v4-arith:w6-g:rem:scope-delta}). Route~10 E closes:
\begin{itemize}
...
at $v\in\{2,3\}$ past the EG wild-inertia enhancement and route~10 B
...
the route~10 E assembly combined with route~6 G.
\end{corollary}

\begin{proof}
Route~6 G (\Cref{v4-arith:w6-g:cor:bcglue-final}) closes BC-diamond-glue
conditional on three inputs: (BCglue.local), (BCglue.glue), and
arithmetic AG at the centre. Route~10 E
...
\begin{remark}[what route~10 E cannot touch]
\label{v4-arith:w10-e:rem:what-wave10-e-cannot-touch}
Route~10 E does not touch:
\begin{itemize}
...
factorization-homology + Bernstein--Galois route of route~10 E.
\item \Cref{v4-arith:deg-f1a:conj:EG-wild-enhancement}: this is
the wild-inertia enhancement of the Emerton--Gee stack to
accommodate wild-inertia types at $p\in\{2,3\}$; route~6 F and
route~10 B have attacked this via the Pa{\v s}k{\=u}nas-block route,
but the enhancement itself is conjectural.
\item Arithmetic AG at the centre: unchanged from route~6 G.
\end{itemize}
Each of these three residuals is a primary-source obligation
external to Vol IV; route~10 E's contribution is to remove the
...
\begin{remark}[route~10 E HZ-IV realization decorator candidates]
...
enhancement and route~10 B class-match hold.
...
Bella\"iche--Chenevier 2009 Chapter 8 + inherited route~5 H (Ch1) +
route~6 G (AG.det.transition). The three catalogs are disjoint.
...
The route~10 E closure ran seven adversarial attack-and-repair cycles:
...
Can $\Pi$-flat be closed unconditionally by route~10 E? \emph{Heal.}
...
of route~10 E, which treats the cross-prime glue as a discrete-base
...
route~10 B class-match (\Cref{v4-arith:w10-b:thm:class-match})
...
Primary sources invoked by route~10 E:
...
route~10 B and inherited here.
\item[Kisin, M.] (2008). \emph{Potentially semi-stable deformation
rings}. Ann.\ Math.\ 170 (2008). Pseudo-character tangent space.
Inherited via route~10 B.
\end{description}

All ten catalogs are HZ-IV-disjoint under the route~10 E partition
...
\begin{enumerate}
\item Route~10 E targets the $45$--$70$~pp residual left by route~6 G
...
$\ell=v$ wild to the route~10 B class-match plus the EG wild-inertia
...
\item Net route~10 E scope delta: $\sim 38$--$48$~pp removed;
surviving residual $\sim 8$--$12$~pp, distributed as $\Pi$-flat
($\sim 3$--$5$~pp) and EG wild-inertia enhancement plus route~10 B
...
\item Route~10 E installs four candidate \texttt{RealizedHere}
