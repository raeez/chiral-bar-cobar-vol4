...
\emph{Route~6 G
(\Cref{v4-arith:w6-g:rem:scope-delta}) closed the BC-diamond-glue
compatibility conditional on three inputs: (BCglue.local)
(pointwise Bernstein-centre matching at finite primes, Theorem-waiting,
\(20\)--\(30\)~pp), (BCglue.glue) (cross-prime factorization assembly,
Conjectural conditional on \(\Pi\)-flat, \(25\)--\(40\)~pp), and arithmetic
...
revisited since. Seven primary-source catalogs disjoint from those
of route~5 H and route~6 G reduce (BCglue.local) to a pointwise
Bernstein-centre-matching datum at the finite primes and close
(BCglue.glue) past \(\Pi\)-flat. On the tame-prime locus
\(v\notin\{2,3\}\) the closure of (BCglue.local) is unconditional; on
the wild locus \(v\in\{2,3\}\) it is conditional on a
Pa{\v s}k{\=u}nas-type block-tangent match from route~10 B
(\Cref{v4-arith:w10-b:thm:class-match}) and on \(\Pi\)-flat. Net
residual: \(\sim 8\)--\(12\)~pp, concentrated on the wild locus and the
prismatic-flat input, down from \(45\)--\(70\)~pp.}
...
The seven primary catalogs deployed by route~10 E are:
\begin{description}
...
\item[Dotto--Emerton--Gee 2023, arXiv:2603.26887]% RECTIFICATION-FLAG [Gate 1]: year tag 2023 conflicts with arXiv prefix 2603 (March 2026); inherits the verification note at v4-arith:w1:open-frontiers (one fix point across the manuscript)
 Fully faithful
...
block-\(\mathrm{Ext}^{1}\) argument; route~10 E bypasses this constraint by using
...
2014 (block structure at \(p=3\)) + route~10 B class-match (pseudo-character tangent). Disjointness: the
three verification catalogs each use a distinct primary-source chain
(PT: Cohen--Macaulay block centralisers; CDP: supercuspidal and
principal-series block structure; route~10 B: Kisin pseudo-character
deformation).
\end{proof}
...
\begin{remark}[residuals not closed by route~10 E]
...
route~10 B have addressed this via the Pa{\v s}k{\=u}nas-block
...
\section{Seven Adversarial Obstructions and Their Resolution}
\label{v4-arith:w10-e:seven-attack}

Seven independent obstruction-checks were applied to the route~10 E
argument, each identifying a potential gap in the chain from local
...
assembly of route~10 E, which treats the cross-prime glue as a
...
\(45\)--\(70\)~pp residual reduces to \(\sim 8\)--\(12\)~pp, with the
...
Primary sources for route~10 E:
...
(BCglue.glue) (\(20\)--\(30\)~pp and \(25\)--\(40\)~pp respectively;
see \Cref{v4-arith:w6-g:rem:scope-delta}).
