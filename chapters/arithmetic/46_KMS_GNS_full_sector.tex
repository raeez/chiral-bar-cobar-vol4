...
\emph{Chapter~\ref{v4-arith:kp-compat} scope-restricted
...
Eisenstein sector is not automatically closed and requires either a
convention absorption or a scope exclusion. The Eisenstein residual is
...
\subsection{Residuals from the Koszul--Petersson Scope Restriction}
...
\subsection{Verification Plan for the Three Residuals}
\label{v4-arith:kms-gns:ledger}

Four verifications structure the chapter. Each runs the seven-attack
protocol of \Cref{v4-arith:cl:protocol}.

\begin{description}
\item[\textbf{KMS\({}_{1}\)/GNS identification of the BC involution.}] \emph{Attack.} The Bost--Connes involution
...
\item[\textbf{Full-sector finite-place Rankin--Selberg.}] \emph{Attack.}
...
The four verifications draw on disjoint primary-source catalogs:

\begin{itemize}
\item \emph{KMS/GNS identification}: Bost--Connes 1995 Selecta~1
(construction); Takesaki 1970 Lecture Notes (modular theory); Takesaki
1979 \emph{Theory of Operator Algebras~I} (GNS verification). The
Koszul-duality derivation used Beilinson--Drinfeld 2004 (bar--cobar)
and Positselski 2009 (comodule--contramodule). The catalogs are
HZ-IV disjoint.

\item \emph{Full-sector finite-place}: Jacquet 1972 SLN~260,
Jacquet--Piatetski-Shapiro--Shalika 1983, and Godement--Jacquet 1972.
Verification against the Koszul side uses Beilinson--Drinfeld plus the
...
\item \emph{Eisenstein continuum}: same catalog as the Langlands
...
\begin{proof}
The Tomita--Takesaki identity
(Takesaki 1979 Ch.~VI Thm~1.19) asserts \(J\pi(x)J=J_{\varphi_{1}}\pi_{\varphi_{1}}(x)J_{\varphi_{1}}\in\pi_{\varphi_{1}}(\mathbb{A}^{\mathrm{BC}}_{p})'\)
for all \(x\in\mathbb{A}^{\mathrm{BC}}_{p}\). Applying this to generators:
\begin{itemize}
\item \(J_{\varphi_{1}}\pi_{\varphi_{1}}(\mu_{m})J_{\varphi_{1}}=\pi_{\varphi_{1}}(\mu_{m})^{*}\) by
the commutant theorem (the commutant of \(\pi_{\varphi_{1}}(\mathbb{A}^{\mathrm{BC}}_{p})\)
in \(B(\mathcal{H}_{\varphi_{1}})\) is generated by the contragredient
representation, so \(\pi_{\varphi_{1}}(\mu_{m})^{*}=\pi_{\varphi_{1}}(\mu_{m}^{*})\)
in the type~III\({}_{1}\) factor; Bost--Connes~Thm~25).
\item \(J_{\varphi_{1}}\pi_{\varphi_{1}}(e(\gamma))J_{\varphi_{1}}=\pi_{\varphi_{1}}(e(\gamma)^{*})=\pi_{\varphi_{1}}(e(-\gamma))\).
\end{itemize}
On basis vectors: \(J_{\varphi_{1}}\) is antiunitary and satisfies
\(J_{\varphi_{1}}\Omega_{\varphi_{1}}=\Omega_{\varphi_{1}}\)
(Takesaki 1979 Ch.~VI Lem.~1.3). For \(\delta_{\gamma}=\delta_{0}\) the
zero-mode, \(J_{\varphi_{1}}(\delta_{m}\otimes\delta_{0})=\delta_{m}\otimes\delta_{0}\)
(since \(\mu_{m}^{*}\) contributes the same lattice index \(m\)
and \(e(0)^{*}=e(0)\)). For \(\gamma\neq 0\), antiunitarity together with
\(J\pi(e(\gamma))J=\pi(e(-\gamma))\) gives
\(J(\delta_{m}\otimes\delta_{\gamma})=\delta_{m}\otimes\delta_{-\gamma}\),
establishing (\ref{v4-arith:kms-gns:eq:J-basis}).

Comparing to \(\sigma^{\mathrm{BC}}_{p}\)
(\ref{v4-arith:kp:eq:BC-involution-local}):
\(\sigma^{\mathrm{BC}}_{p}(\mu_{m})=\mu_{m}^{*}\) and
\(\sigma^{\mathrm{BC}}_{p}(e(\gamma))=e(-\gamma)\). The actions of
\(J_{\varphi_{1}}\) and \(\sigma^{\mathrm{BC}}_{p}\) on generators agree;
both are antiunitary involutions; they extend identically to the
\(*\)-closure.
\end{proof}
...
\ClaimStatusConditional. Assuming the four-input conditional spine of
\Cref{v4-arith:w3-audit-B:thm:four-input-spine} is the minimal
closure path to RH within Vol~IV's framework, this chapter tightens
the second input (Koszul--Pet compatibility) from
...
\begin{proof}
The second input of the four-input spine is updated by
\Cref{v4-arith:kms-gns:thm:P3-tempered-cuspidal} and
\Cref{v4-arith:kms-gns:thm:KMS1-GNS-BC-identification}; the scope
of the update is exactly the tempered cuspidal sector. The
minimality of the four-input spine itself is the content of
\Cref{v4-arith:w3-audit-B:thm:four-input-spine} (outside this
chapter's scope).
\end{proof}
...
\section{Seven-Attack Verification}
\label{v4-arith:kms-gns:verification}

Each verification runs the seven-attack protocol
...
cuspidal P3 of route~3 to all tempered cuspidal representations
...
and explicit conditional passes (finite-place at non-tempered;
Eisenstein at distributional scope). No claim rises above its documented
status; no residual is left implicit.
\end{enumerate}

Of the three open sub-items isolated by
\Cref{v4-arith:w3-audit-B:repair-26} (I, II, III), one is closed
...
minimal path nor closed here. The main achievement is~(I): the
KMS\({}_{1}\)/GNS identification pins the Bost--Connes involution
down as the unique Tomita--Takesaki modular conjugation of the
KMS\({}_{1}\) state, a clean operator-algebra fact that
\Cref{v4-arith:w3-audit-B:repair-26} correctly flagged as open.
