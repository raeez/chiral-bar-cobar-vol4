...
\emph{Route~2 adversarial pressure (A2-BZN-01, Etingof + Hypotheses,
...
$\infty$-categories. Route~2 auditor A2-BZN-01 (\Cref{v4-arith:sw:wave2},
...
\section{Attack-and-Repair Ledger (Six Cycles)}
\label{v4-arith:bzn-sub:ledger}

The chapter threads six attack-and-repair cycles:
...
\item[\textbf{Cycle 6: Consistency with route~1 F3.BZN$^{\mathrm{Ar}}$.}]
\emph{Attack.} Does the present structural reduction conflict with
the route~1 closure
(\Cref{v4-arith:bzn:thm:HH-IndCoh-arith})? \emph{Heal.} The route~1
...
are therefore consistent: route~1 assumed the ingredient, route~2
...
\subsection{Attack-and-Repair Trace for R2}
...
Route~1 chapter~\ref{v4-arith:arith-bzn}
...
\begin{proof}
Route~2 split \Cref{v4-arith:sw:thm:BZN-Ar-split} expresses
...
\subsection{Attack-and-Repair Trace for R1.formal}
...
open content is now explicit: route~1 compressed the formal-loop and
flat-pullback assumptions into the structural input
\Cref{v4-arith:bzn:thm:HH-IndCoh-arith}; route~2 names them as
R1.formal.b and R1.formal.c, and the present FLE audit keeps R3 and
R4 residual as well.
\end{remark}

\begin{remark}[comparison with route~1 chapter 28]
...
in the pro-\'etale setting). Route~1 chapter~\ref{v4-arith:arith-bzn}
...
chapters are consistent: route~1 was correct to flag the
ingredient as formal; route~2 names it and inscribes it as a
...
R1.formal.a (HH colim cont.) & Implicit in route~1 & \textbf{Theorem} \\
R1.formal.b (loop formal comp.) & Implicit in route~1 & Conjectural with scope \\
R1.formal.c (flat locus) & Implicit in route~1 & Conjectural with scope \\
R3 (loop identif.) & Definitional & Definitional (under R2) \\
F3.BZN$^{\mathrm{Ar}}$ net residual & R1+R2+R3+R4 & R1.formal.b+R1.formal.c+R4 \\
\end{tabular}
\caption{Claim-status deltas from route~2 F3.BZN$^{\mathrm{Ar}}$.}
\label{v4-arith:bzn-sub:table:deltas}
\end{table}

\subsection{Next-Route Target Priorities}
...
force a restriction of route~1's structural theorem
...
\item \textbf{Consistency with route~1.} The present chapter
surfaces, as R1.formal.b, the implicit pro-\'etale extension of
Ben-Zvi--Francis--Nadler used in route~1
...
The route~2 attack on F3.BZN$^{\mathrm{Ar}}$ is now structurally
