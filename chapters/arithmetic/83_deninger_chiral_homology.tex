...
Deninger's programme demands a cohomology theory for
...
orbits recover the explicit formula. The bar-cobar programme does not
...
(the infinitesimal generator whose eigenvalues are the non-trivial
zeros), a regularised determinant equal to the completed zeta function,
a Lefschetz trace formula whose closed-orbit side is the prime side of
the explicit formula, Poincar\'{e} duality, and purity. For the Riemann
zeta function the middle degree should carry the non-trivial zeros.

% RECTIFICATION-FLAG: Gate 1 -- The notation \zeta(s) here follows
% Deninger's original convention in which \zeta(s) denotes the
% completed zeta function (including Gamma and archimedean factors),
% not the bare Riemann zeta. In standard analytic number theory,
% this object is typically called \xi(s). The equation is correct
% under Deninger's convention; a reader note may prevent confusion.
\begin{equation}
...
The bar-cobar programme produces four inputs with theorem-grade status;
...
\Cref{v4-arith:3d-acs:conj:spectral-flow}: the Hilbert--Polya
self-adjointness of the zero-side operator. This is the single
missing piece that the bar-cobar programme does not supply.
\end{enumerate}
...
the conjectural 3D ACS bulk. Their equality is a conditional package,
...
primes. Deninger's programme requires a global Frobenius/flow operator
...
(to be constructed) whose spectrum equals \(\{\gamma_\rho\}\), the
imaginary parts of the non-trivial zeros. The identification
\begin{equation}
...
The preceding sections address \(\overline{\mathrm{Spec}\,\mathbb{Z}}\),
which is the genus-zero case corresponding to the Riemann zeta
function. Higher-genus arithmetic data arise for arithmetic surfaces
and motives of higher weight; the formal extension records what the
candidate construction produces in those cases.
...
\(\mathcal{D}_g\) is a smooth projective arithmetic surface of
geometric genus \(g\) together with a choice of polarisation and
\(L\)-function data (see \Cref{v4-arith:ag:defn:datum}).
The corresponding \(\mathcal{V}^{\mathrm{prim},g}\) is the
Hochschild trace class of the identity on the genus-\(g\) arithmetic
Hecke category, the rank-\(2g\) extension of the rank-\(2\) primary
\(\mathcal{V}^{\mathrm{prim}}\) of \Cref{v4-arith:00:def:Vprim}. The
...
The preceding sections separate four unconditionally proved results
from six conditional or open items. The separation is the chapter's
main output.
...
\Cref{v4-arith:3d-acs:conj:spectral-flow}. All other components of
the construction are either established unconditionally or packaged
conditionally on that hypothesis.
