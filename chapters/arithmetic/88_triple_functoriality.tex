...
isolated the conditional triple comparison
\(H^{\bullet}_{\Delta}=H^{\bullet}_{\mathrm{ch}}=\partial\text{-chiral}\)
on the primary Heisenberg domain for the baseline genus \(g=0\) and the
fixed arithmetic Heisenberg \(\mathsf{H}^{\mathrm{Ar}}\). The
functoriality residuals of \Cref{v4-arith:w8-a:residual}(5,6) remain:
the three cohomology assignments must extend to functors on a category
\(\mathsf{ArChAlg}\) of arithmetic chiral algebras on
\(\overline{\mathrm{Spec}\,\mathbb{Z}}\), and the triple comparison
must become a natural isomorphism of these functors, uniformly
compatible with the genus-\(g\) restriction and induction maps of
Chapter~\ref{v4-arith:all-genus-koszul}. The conditional master theorem
is \Cref{v4-arith:w9-r5:thm:master-functoriality}; it rests on the
...
is the explicit non-conformal additivity test. The residuals are exactly
the hypotheses inherited from Chapters~\ref{v4-arith:3d-acs-E1bar} and
...
\begin{definition}[genus stabilization]
...
\(S_{g,g+1}\colon\mathsf{ArChAlg}_{g}\to\mathsf{ArChAlg}_{g+1}\)
tensors with the rank-\(2\) archimedean Heisenberg
(\Cref{v4-arith:ag:defn:datum}'s rank-\(2\) enhancement at each
place). A projection to genus \(g\) is defined only on split objects
with a specified decomposition into old and new oscillators.
\end{definition}

\begin{remark}[why the filtration]
\label{v4-arith:w9-r5:rem:filtration-reason}
The filtration records stabilization by a fresh rank-\(2\) factor at
every place. It is not a reflective inclusion without a chosen
splitting of oscillators.
\end{remark}
...
These two source families are disjoint. The morphism axioms
(1)--(6) each arise from exactly one of them; hence the
functoriality statements below have independent derivations on the
finite and archimedean sides.
\end{remark}
...
\ClaimStatusDefinitional.
The \emph{Deninger functor} is the conditional assignment
\begin{equation}
...
\end{equation}
defined on the residual domain where the Deninger Frobenius has been chosen. It
...
\begin{remark}[conditional form of the bulk-boundary lift]
...
inherited from Chapter~\ref{v4-arith:3d-acs-E1bar} and enter every
theorem below.
\end{remark}
...
paths are pairwise independent; the natural isomorphism theorem
below is a source-disjoint comparison on its conditional domain, not a
restatement of any single derivation.
\end{remark}
...
\subsection*{Independence and domain of the chain-level identification}
\label{v4-arith:w9-r5:ah:chain}

\emph{Independence.} The ordered-incidence bar rests on
...
\emph{Unconditional domain.} The equivalence between ordered-bar
and Platonic-bar homology on the primary Heisenberg domain is the
content of \Cref{v4-arith:platonic:prop:bar-comparison}, which
holds unconditionally within that domain. No numerical comparison backs
this identification.
...
\subsection*{Independence and domain of naturality in \(\mathsf{A}\)}
\label{v4-arith:w9-r5:ah:A}

\emph{Independence.} Beilinson--Drinfeld 2004 supplies the
...
\begin{theorem}[compatibility under genus stabilization]
...
the three genus-\(g\) functors commute with the stabilization functor
\(S_{g,g+1}\colon\mathsf{ArChAlg}_{g}\to\mathsf{ArChAlg}_{g+1}\)
...
\begin{tikzcd}[column sep=large,row sep=large]
\mathsf{ArChAlg}_{g}\arrow[rr,"S_{g,g+1}"]\arrow[dr,"H^{\bullet}_{\star,g}\otimes V_{2}"']
...
\begin{proof}
The functor \(S_{g,g+1}\) adjoins a rank-\(2\) archimedean Heisenberg
...
stabilization functor \(S_{g,g+1}\) of
\Cref{v4-arith:w9-r5:thm:genus-inclusion} and with the retraction
\(\pi_{g+1,g}\) on split stabilized objects: the three genus-\(g\)
identifications commute, up to \(\vartheta^{\star}_{g}\), with the
corresponding genus-\((g+1)\) identifications.
\end{theorem}
...
The compatibility with genus stabilization follows by applying
...
H^{\bullet}_{\Delta,g+1}(S_{g,g+1}\mathsf{A})\arrow[r,"="]
&H^{\bullet}_{\mathrm{ch},g+1}(S_{g,g+1}\mathsf{A})\arrow[r,"="]
&(\partial\circ\mathrm{BdryACS})_{g+1}(S_{g,g+1}\mathsf{A})
\end{tikzcd}
\]
commutes.
...
\subsection*{Independence and domain of genus functoriality}
...
cokernel: on the primary Heisenberg domain the freshly-added rank-\(2\)
...
\(\mathsf{H}^{\mathrm{Ar}}\to\mathsf{H}^{\mathrm{Ar}}\oplus\mathsf{H}^{\mathrm{Ar}}\)
tests the non-conformal additive structure; the conformal diagonal is
excluded by the central-charge obstruction.
...
Positselski, H-P\(^{\mathrm{Ar}}\), 3D quantum ACS domains)}. The
...
functors, compatible with the stabilization \(S_{g,g+1}\) and with
\(\pi_{g+1,g}\) on split stabilized objects;
...
\begin{proposition}[central-charge obstruction to the conformal diagonal]
\label{v4-arith:w9-r5:prop:diagonal-verification}
\ClaimStatusProvedHereConditional \emph{(on
\Cref{v4-arith:w9-r5:thm:master-functoriality})}. The diagonal
\(\Delta\) of \Cref{v4-arith:w9-r5:ex:diagonal} is not a morphism in
\(\mathsf{ArChAlg}\) with the coproduct conformal vector. After applying
the forgetful functor to pointed non-conformal \(E_{1}\)-coefficient
systems, it becomes a morphism preserving spherical units, Adams maps,
adjacent products, and Parshin--Gersten boundaries. On this
non-conformal underlying system the three additive maps
\[
\bigl(H^{\bullet}_{\Delta}(\Delta),\,H^{\bullet}_{\mathrm{ch}}(\Delta),\,\partial\circ\mathrm{BdryACS}(\Delta)\bigr)
\]
commute with \(\eta_{\Delta,\mathrm{ch}}\) and
...
\begin{proof}
A rank-\(2\) Heisenberg conformal vector has central charge \(2\);
the coproduct of two such Heisenberg factors has conformal vector
\(T\oplus T\) and central charge \(4\). If the diagonal were conformal,
the OPE of the image of \(T\) with itself would have to preserve the
central term. It instead doubles it. Hence no conformal
\(\mathsf{ArChAlg}\)-morphism has the displayed diagonal as its
underlying map.

After forgetting the conformal vector, \(\Delta^{(p)}\) commutes with
\(\psi_{p}\) because \(\psi_{p}(x,x)=(\psi_{p}(x),\psi_{p}(x))\).
The spherical unit is preserved:
\(\Delta^{(v)}(u^{(v)})=(u^{(v)},u^{(v)})\). The adjacent product
on the coproduct is the diagonal of the adjacent product on each
summand; \(\Delta\) commutes with this by construction. The
tame-symbol boundary \(\partial^{\mathrm{PG}}\) is linear, so
\(\partial^{\mathrm{PG}}(x,x)=(\partial^{\mathrm{PG}}(x),\partial^{\mathrm{PG}}(x))\).
Tate compatibility is preserved because the spherical units are
preserved and the restricted tensor factors through each summand.
...
domain (the bar is additive in its argument, a consequence of
...
\Cref{v4-arith:w9-r5:def:functor-ch}; after forgetting the conformal
vector, applying naturality to \(\Delta\) gives the chain map on bars.
Bar differential compatibility is the Parshin--Gersten boundary
compatibility proved above.

Claim (3). The bulk-boundary assignment
\(\mathrm{BdryACS}(\Delta)\) is by
\Cref{v4-arith:w9-r5:def:functor-bdryACS} the non-conformal 3D bulk interface
...
The diagonal \(\Delta\) is the simplest non-trivial morphism after the
conformal vector is forgotten: it is not an isomorphism, not a
projection, not a single-factor inclusion. It tests additivity of the
three comparison functors, but it does not test the conformal axiom of
\(\mathsf{ArChAlg}\). Because the coproduct
...
example also shows why genus stabilization is not an arbitrary
diagonal embedding: the conformal central charge records the added
rank.
\end{remark}

\subsection*{Independence and domain of the master theorem}
\label{v4-arith:w9-r5:ah:master}

\emph{Independence.} The master theorem combines three source
...
\emph{Spectral domain.} The master theorem claims the
...
\section{Domain of the Master Theorem}
\label{v4-arith:w9-r5:residual-summary}

\subsection{Functoriality residuals of Chapter~\ref{v4-arith:w8-a:deninger-chiral-homology}}
\label{v4-arith:w9-r5:residual-closed}

The following implications hold on the conditional domain of the master
theorem:
\begin{enumerate}
\item \Cref{v4-arith:w8-a:residual}(5): triple-identification
functoriality under pullback along number-field extensions
\(K/\mathbb{Q}\). Any such pullback is a morphism in
...
Frobenius identification. On the residual domain of
\Cref{v4-arith:w9-r5:thm:genus-uniformity}, the genus-\(g\)
restriction of \(\partial\circ\mathrm{BdryACS}\) is functorially
identified with the genus-\(g\) Deninger cohomology; the rank-\(2g\)
Wilson-loop observables then identify with the rank-\(2g\)
...
On the conditional domain, the triple comparison
\(H^{\bullet}_{\Delta}=H^{\bullet}_{\mathrm{ch}}=\partial\circ\mathrm{BdryACS}\),
previously isolated object-wise for \(\mathsf{H}^{\mathrm{Ar}}\)
at genus \(g=0\), is a natural isomorphism of genus-filtered functors
on the stated subcategory of \(\mathsf{ArChAlg}\): every morphism of arithmetic chiral
...
disjoint; the natural isomorphism is therefore a comparison of three
independent constructions on that domain. The remaining spectral content (that the Frobenius \(F\) on \(H^{1}\)
...
Ch.~\ref{v4-arith:3d-acs-E1bar}. Each theorem has two independent
paths: derivation through one family and comparison with another.
