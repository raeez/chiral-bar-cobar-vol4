...
\(N^{\mathrm{LS}}(s)\cdot N^{\mathrm{LS}}(1-s)=\tfrac{1}{2}\)
on the critical line. Explicitly:
\[
N^{\mathrm{LS}}(s)\cdot N^{\mathrm{LS}}(1-s)
=\frac{\xi(2s-1)}{\sqrt{2}\,\xi(2s)}\cdot\frac{\xi(1-2s)}{\sqrt{2}\,\xi(2-2s)}
=\frac{\xi(2s-1)\cdot\xi(2s)}{2\,\xi(2s)\cdot\xi(2s-1)}=\frac{1}{2},
\]
using the functional equation \(\xi(1-2s)=\xi(2s)\) and
\(\xi(2-2s)=\xi(2s-1)\) (Titchmarsh 1986~\S2.1). The Shahidi
global normalisation before the \(1/\sqrt{2}\) density prefactor
satisfies \(n(s)\cdot n(1-s)=1\) (Shahidi 2010~Thm~3.5.1); the
\(1/\sqrt{2}\) is introduced here to absorb the Maass--Selberg
coefficient~\(2\) into the pairing definition (see
\Cref{v4-arith:w6-c:thm:density-match}).
...
\end{equation*}
Taking squared modulus and using
\Cref{v4-arith:w6-c:lem:unit-modulus}
(\(|\xi(2s)/\xi(2s-1)|=1\) on \(\mathrm{Re}(s)=1/2\)):
\begin{equation*}
\bigl|\bigl\langle\phi_{s},\sigma^{\mathrm{BC}}(\phi_{s})\bigr\rangle^{\mathrm{Koszul},\mathrm{LS}}\bigr|^{2}\;=\;2\cdot\bigl|\langle\phi_{s},\phi_{s}\rangle^{\mathrm{loc}}\bigr|^{2}.
\end{equation*}
The spherical vector \(\phi_{s}\) is normalised so that the
local Petersson self-pairing satisfies
\(\langle\phi_{s},\phi_{s}\rangle^{\mathrm{loc}}=1\):
this is the standard spherical-vector normalisation, equivalent
(via the Casselman--Shalika formula, Thm~5.4) to
\(W^{\circ}(e)=1\) for the associated spherical Whittaker function,
and consistent with the Tate adelic restricted-tensor-product
measure (Tate 1950~\S3.3,~\S4.2; Weil 1967~Ch.~VII). Therefore
\begin{equation*}
...
\begin{equation*}
N^{\mathrm{LS}}(s)\cdot N^{\mathrm{LS}}(1-s)\;=\;\frac{\xi(2s-1)}{\sqrt{2}\,\xi(2s)}\cdot\frac{\xi(1-2s)}{\sqrt{2}\,\xi(2-2s)}\;=\;\frac{\xi(2s-1)\xi(1-2s)}{2\,\xi(2s)\xi(2-2s)}\;=\;\frac{\xi(2s-1)\xi(2s)}{2\,\xi(2s)\xi(2s-1)}\;=\;\frac{1}{2}
\end{equation*}
...
\section{Source Disjointness and Epistemic-Status Audit}
\label{v4-arith:w6-c:LS:disjointness-audit}

\begin{remark}[Source disjointness, conventions, and hypotheses for \Cref{v4-arith:w6-c:thm:main}]
\label{v4-arith:w6-c:rem:disjointness-audit}
\begin{description}
...
\item[Hypotheses.] The identity is unconditional on the Eisenstein
continuum; conditional on Ramanujan for Maass on the Maass
cuspidal discrete sector (inherited from
\Cref{v4-arith:w5-c:thm:discrete-core-identity}). The condition
is explicit in the theorem statement. \textbf{Pass with explicit condition.}
...
\item[Compute engine.] No compute engine is involved.
\textbf{Pass.}
\end{description}
Six independent checks yield six passes and one explicit conditional
(Ramanujan for Maass on the cuspidal discrete sector; external to the
Eisenstein closure).
\end{remark}

\begin{remark}[Scope and compatibility of the discrete-core and Eisenstein repairs]
\label{v4-arith:w6-c:rem:status}
Chapter~\ref{v4-arith:w5-c:full-vprim-P3} (Route~5) closes the
...
fully supplied by Repair~(a) alone. Route~6 Repair~(b) closes the
Eisenstein continuum in addition, at the
...
on both sides of any identification in this chapter. The Vol IV
source-disjointness requirement is satisfied.
