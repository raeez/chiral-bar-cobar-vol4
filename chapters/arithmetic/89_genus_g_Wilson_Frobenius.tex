...
surfaces. The genus-\(g\) extension replaces the gauge group
...
\ref{v4-arith:w9-r2:archimedean-wilson-surface}, together with
Chs.~\ref{v4-arith:rh-reformulation} and \ref{v4-arith:tfe-gutzwiller}.
The uniform \(g=0\) reduction is
...
and \(\rho_{0,p}\) is the unit representation.% RECTIFICATION-FLAG [Gate 1]: the g=0 Wilson-line Frobenius identity in Ch.v4-arith:w9-r1 uses an arbitrary GL_2 rep rho, not rho_{0,p}; the datum D_0 does not carry a rank-2g Galois rep -- the specialisation at g=0 is the degeneration of the datum, while the Wilson-line uses a separate rep rho into GL_2. These two uses of g=0 should be disentangled.
...
and inherits the conditional status of the \(g=0\) theory.
...
whether the expectation of the associated defect operator recovers the
arithmetic Frobenius \(\mathrm{Frob}_{p}\). The answer at rank \(2g\) is the
following identity.
...
\(\chi_{\rho_{g}}(\mathrm{Frob}_{p})\) in the standard representation.
\end{theorem}
...
The three inputs of \Cref{v4-arith:w9-r6:thm:wilson-frobenius-g} are source-disjoint.
...
\(\rho_{g}\) itself at \(\mathrm{Frob}_{p}\), not the character of
...
At the archimedean place, the 3-space \(\mathcal{B}_{g}^{(3)}\) has a
closed-surface boundary \(\Sigma_{\infty}=\mathcal{B}_{g}^{(3)}\mid_{\infty}\)
of dimension \(2g\) (a bundle of closed surfaces in Window~I), rather than a
knot. A defect operator on \(\Sigma_{\infty}\) must integrate an area form
...
\(\exp(-L^{(g)}_{0})\) where% RECTIFICATION-FLAG [Gate 1]: the Fock zero-mode L_{0,j}^{Ar} = x_j d/dx_j generates the scaling flow; under Mellin, this maps to s_j (multiplication operator), not s_j - w/2. The shift to s_j - w/2 arises from centering the Mellin transform on the critical line Re(s)=w/2, i.e. L_{0,j}^{Ar} is normalised so that its Mellin eigenvalue is s_j - w/2. Verify this normalisation against Ch.v4-arith:3d-acs:thm:km-lift definition of L_0^{Ar}.
...
The three primary inputs are source-disjoint: Faltings 1984 Arakelov
...
must assemble them into a single global operator on the Deninger cohomology
\(H^{\bullet}_{\Delta,g}\). The restricted tensor product over all places is
the global Frobenius.
...
The five primary inputs are pairwise source-disjoint: Deninger 1998
...
Hasse--Weil \(w=2\), Siegel spin \(w=2g+1\), per \Cref{v4-arith:ag:table:rows};
...
degenerate to the Riemann framework of Chs.~\ref{v4-arith:3d-acs-E1bar}
and \ref{v4-arith:w8-a:deninger-chiral-homology}.
...
conjugate to \(\Theta_{\xi}\) (the Deninger-flow generator of
\Cref{v4-arith:w8-a:def:F-infinity}); the rank-\(g\) Mellin unitary collapses to
...
rank-\(2g\) Fock, the Hodge decomposition \(\mathcal{E}_{0}\otimes\mathbb{C}=0\),
and the motivic weight \(w_{\mathcal{D}_{0}}=1\).
\end{proof}
...
\(\kappa^{\mathrm{Ar},g}+(\kappa^{!})^{\mathrm{Ar},g}=0\) is fixed at all genera
by \Cref{v4-arith:ag:thm:all-genus}; the bulk partition function is the
row-specific completed \(L\)-function \(\Lambda_{\mathcal{D}_{g}}\); and the
motivic Riemann Hypothesis for the row becomes the topological spectral
condition \(\mathrm{Re}(z)=w_{\mathcal{D}_{g}}/2\) on the non-trivial zeros
\(z\) of \(\Lambda_{\mathcal{D}_{g}}\), i.e.\ the \(F^{(g)}\)-weight purity on
\(H^{1}_{\Delta,g}\) (\Cref{v4-arith:w9-r6:thm:deninger-g-full}(4)).
\end{remark}
...
obligation list for a theorem-grade proof of the genus-\(g\) Wilson-line,
Wilson-surface, and Deninger identification. Four obligations remain open:
