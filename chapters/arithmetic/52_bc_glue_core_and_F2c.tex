...
the source-disjointness input for (Ch1).
...
deformation problem. Flatness follows from Chenevier 2014 Prop.~3.8 (the determinant-trace pseudocharacter
...
% RECTIFICATION-FLAG: [Gate 1] The pro-étale comparison at the level of
% the universal pseudocharacter ring R^Ch_v (not just the Emerton--Gee
% stack itself) requires an explicit citation beyond Bhatt--Scholze 2019.
% The identification of the GL_2-invariant trace with the determinantal
% cocharacter via the prismatic comparison is used here without a
% primary-source reference at the universal deformation ring level.
% Candidate: Emerton--Gee arXiv:1908.07185 combined with
% Bhatt--Scholze 2019 Thm.~1.8; verify that the GL_2-invariant trace
% argument is explicit there, or supply a direct argument.
...
\emph{Source disjointness.} Derivation source: Chenevier 2014 (categorical pseudocharacter axioms + representability).
...
\emph{Source disjointness.} Derivation source: Arinkin--Gaitsgory
...
\emph{Step 5: Pointed-unit reduction.}
...
\emph{Source disjointness.} Derivation source: Lurie HA 5.1 (pointed
...
The present theorem gives an
independent elementary proof for the \(\mathcal{O}=E_{2}\)
case, using only (i) pointed-unit centrality (HA 5.1.2.26), (ii)
filtered-colimit preservation (HA 3.2.3.1), and (iii) factorization
homology on discrete base spaces (Francis--Gaitsgory 2011). This
path avoids Dunn additivity
(HA 5.1.2.4), which was invoked in the route~3 proof for the general \(\mathcal{O}\)-version. The route~5 path is
...
The (AG.det.transition) conjecture is new: its closure requires a direct
AG 2012 + GKRW 2024 + BFN 2008 primary-source assembly (scope \(5\)--\(10\)~pp)
that has not been executed in the primary literature, but is
tractable and distinguished from the full arithmetic AG.
\end{proof}

\begin{corollary}[\(\mathrm{L}_{\mathrm{ACD}}\) residual catalogue after route~5]
...
\begin{proof}
Assemble
\Cref{v4-arith:bcglue-core:cor:LACD-tightened} (route~4 catalogue) with
...
\begin{remark}[source-disjoint realization decorators for route~5 closures]
\label{v4-arith:w5-h:rem:wave5-hziv-decorators}
The three route~5 closures carry source-disjoint derivation and verification catalogues:
\begin{itemize}
\item \Cref{v4-arith:w5-h:thm:ch1-closure} carries derived-from source Chenevier 2014 and verified-against sources Bellaiche--Chenevier
...
\item \Cref{v4-arith:w5-h:thm:agdet-reduction} is a scope-reduction
inscribed at \ClaimStatusProvedHere with derivation source
Arinkin--Gaitsgory arXiv:1201.6343 and verification sources
Galatius--Kupers--Randal-Williams arXiv:1805.07184 + BFN
arXiv:0805.0157. The residual (AG.det.transition) is
\ClaimStatusConjectured pending primary-source assembly.
\item \Cref{v4-arith:w5-h:thm:f2c-e2-transport} carries derived-from
source Lurie HA 5.1 (pointed \(E_{2}\) algebras and unit-centrality)
and verified-against sources Francis--Gaitsgory arXiv:1103.5803
+ Chapter~\ref{v4-arith:tate-tensor} Tate restricted tensor.
Disjointness: Lurie HA 5.1 (coherent \(\infty\)-operads) versus
Francis--Gaitsgory 2011 (factorization homology) versus
Chapter~\ref{v4-arith:tate-tensor} (Tate restricted tensor construction);
mutually disjoint infrastructures.
\end{itemize}
\end{remark}
...
identification (verification source disjoint from Chenevier 2014).
...
The ten primary sources are mutually source-disjoint across the
(Ch1), (AG.det), and F2.c derivation-verification pairs: no single
author appears in more than one derivation or verification role, and
no theorem of one catalogue is used as input to another catalogue's
proof.
