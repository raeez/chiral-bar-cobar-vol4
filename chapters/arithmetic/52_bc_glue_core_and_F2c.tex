\chapter{Route~5: BC-Glue-Core Residuals (Ch1, AG.det) and F2.c $E_{2}$ Transport}
\label{v4-arith:w5-h:bcglue-core-and-F2c}

\emph{Route~5 closure attempt on the three residuals inherited from
route~4 (Chapter~\ref{v4-arith:bcglue-core}) and from the F2 chapter
(Chapter~\ref{v4-arith:tate-tensor}).
The route~4 reduction split (BCglue.core) into two logically
...
Route~5 closes (Ch1) unconditionally via a Chenevier
...
\begin{corollary}[the genuine (AG.det) residual after route~5]
...
Before route~5, (AG.det) was ``the determinant-track projection of
...
\begin{remark}[what route~5 adds to F2.c]
...
(HA 5.1.2.4), which was invoked in the route~3 Theorem~\ref{v4-arith:tate:thm:F2c-main}
proof for its general $\mathcal{O}$-version. The route~5 path is
...
\section{Combined BC-Diamond-Glue Statement After Route~5}
\label{v4-arith:w5-h:combined}

\begin{theorem}[combined BC-diamond-glue after route~5 closures]
...
\begin{corollary}[$\mathrm{L}_{\mathrm{ACD}}$ residual catalog after route~5]
\label{v4-arith:w5-h:cor:LACD-wave5}
\ClaimStatusConjectured (conditional on the standing residuals listed).
After route~5, $\mathrm{L}_{\mathrm{ACD}}$ has the reduced residual
catalog:
\begin{enumerate}
\item (BCglue.local): \textbf{Theorem-waiting} (unchanged from
route~4);
\item (BCglue.glue): \textbf{Conjectural} (unchanged from route~4);
...
Chapter~\ref{v4-arith:tate-tensor}
\Cref{v4-arith:tate:thm:F2c-main} or the route~5 elementary path
...
\begin{proof}
Assemble
\Cref{v4-arith:bcglue-core:cor:LACD-tightened} (route~4 catalog) with
...
\section{Residual Obstruction After Route~5}
...
$5$--$10$ pp.
\end{itemize}
Total residual scope after route~5: $50$--$80$ pp, compared with the
before Route~1 estimate of $80$--$120$ pp for $\mathrm{L}_{\mathrm{ACD}}$.
Net route~5 delta: $30$--$40$ pp removed by (Ch1) closure +
(AG.det) tightening; (BCglue.local) and (BCglue.glue) remain at
their route~2 scope.
\end{remark}

\begin{remark}[what route~5 cannot touch]
\label{v4-arith:w5-h:rem:what-wave5-cannot-touch}
Route~5 cannot close (BCglue.local) or (BCglue.glue): the former
...
Route~5 also cannot close (AG.det.transition) unconditionally: it is a
...
route~5 is therefore the $50$--$80$ pp primary-source assembly,
...
\begin{remark}[route~5 HZ-IV decorators]
